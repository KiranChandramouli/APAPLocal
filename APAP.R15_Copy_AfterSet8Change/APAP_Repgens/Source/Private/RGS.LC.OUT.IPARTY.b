* @ValidationCode : MjoyMDI3MDEwODc6Q3AxMjUyOjE2ODg1NDU3NTIxMTA6SVRTUzotMTotMToxMDY2NzoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 13:59:12
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 10667
...
*-----------------------------------------------------------------------------------------------
SUBROUTINE RGS.LC.OUT.IPARTY
REM "RGS.LC.OUT.IPARTY",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.LC.OUT.IPARTY")
    END
*************************************************************************
...
*************************************************************************
    ...
    CLEARSELECT
    EXECUTE "HUSH ON"
    EXECUTE 'SSELECT ':FULL.FNAME:' WITH FLD.':TNO:' = "AC" OR WITH FLD2.':TNO:' = "DC"' ;* R22 Manual conversion - SELECT changed to SSELECT
    EXECUTE "HUSH OFF"
...
* Year 2000 compliance
            ...
            YKEYFD = FMT(YM.ISSUER.NO,"12L")


            IF LEN(YKEYFD) > 12 THEN YKEYFD = YKEYFD[1,11]:"|"
            GOSUB 8000000
            YM.ISSUE.NO.SORT = R.NEW(6)
            YKEYFD = YM.ISSUE.NO.SORT
...
* Year 2000 compliance
            ...
            YKEYFD = FMT(YM.ISSUE.NO.SORT,"12L")
...
* Year 2000 compliance
            ...
            YM.ISSUE.REF = R.NEW(3)
            YR.REC(8) = YM.ISSUE.REF
            YM.ISSUE.PARTY = R.NEW(6)
            IF YM.ISSUE.PARTY <> "" THEN
                YCOMP = "CUSTOMER_2_":YM.ISSUE.PARTY
                YFORFIL = YF.CUSTOMER
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
                YM.ISSUE.PARTY = YFOR.FD
            END
            YR.REC(9) = YM.ISSUE.PARTY
            YM.CURR = R.NEW(20)
            YR.REC(10) = YM.CURR
            YM.LC.AMT = R.NEW(21)
            YR.REC(11) = YM.LC.AMT
            YM.LIAB.AMT = R.NEW(117)
            YR.REC(12) = YM.LIAB.AMT
            YM.EXPIRY.DATE = R.NEW(28)
            YR.REC(13) = YM.EXPIRY.DATE
            YM.TOT.RISK = R.NEW(117)
            YR.REC(14) = YM.TOT.RISK
            YM.ID.DRAW.PRINT = ""
            YR.REC(15) = YM.ID.DRAW.PRINT
            YM.DR.ISSUE.NAME = ""
            YR.REC(16) = YM.DR.ISSUE.NAME
            YM.DRAW.AMT.LINE = ""
            YR.REC(17) = YM.DRAW.AMT.LINE
            YM.MAT.DATE = ""
            YR.REC(18) = YM.MAT.DATE
            YM.DRAW.AMT.TOT = ""
            YR.REC(19) = YM.DRAW.AMT.TOT
            YM.LIAB.TOT = R.NEW(117)
            YR.REC(20) = YM.LIAB.TOT
*------------------------------------------------------------------------
        CASE YFILE = "DRAWINGS"
            YKEY = ""; MAT YR.REC = ""
            YM.ISSUER.NO = FMT(ID.NEW,"12L"); YM.ISSUER.NO = YM.ISSUER.NO[1,12]
            IF YM.ISSUER.NO <> "" THEN
                YCOMP = "LETTER.OF.CREDIT_6_":YM.ISSUER.NO
                YFORFIL = YF.LETTER.OF.CREDIT
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
                YM.ISSUER.NO = YFOR.FD
            END
            YKEYFD = YM.ISSUER.NO
...
* Year 2000 compliance
            ...
            IF LEN(YKEYFD) > 12 THEN YKEYFD = YKEYFD[1,11]:"|"
            GOSUB 8000000
            YM.ISSUE.NO.SORT = FMT(ID.NEW,"12L"); YM.ISSUE.NO.SORT = YM.ISSUE.NO.SORT[1,12]
            IF YM.ISSUE.NO.SORT <> "" THEN
                YCOMP = "LETTER.OF.CREDIT_6_":YM.ISSUE.NO.SORT
                YFORFIL = YF.LETTER.OF.CREDIT
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
                YM.ISSUE.NO.SORT = YFOR.FD
            END
            YKEYFD = YM.ISSUE.NO.SORT
...
* Year 2000 compliance
            ...
            YR.REC(15) = YM.ID.DRAW.PRINT
            YM.GET.ISSUE.ID = FMT(ID.NEW,"12L"); YM.GET.ISSUE.ID = YM.GET.ISSUE.ID[1,12]
            IF YM.GET.ISSUE.ID <> "" THEN
                YCOMP = "LETTER.OF.CREDIT_6_":YM.GET.ISSUE.ID
                YFORFIL = YF.LETTER.OF.CREDIT
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
                YM.GET.ISSUE.ID = YFOR.FD
            END
            YM.DR.ISSUE.NAME = YM.GET.ISSUE.ID
            IF YM.DR.ISSUE.NAME <> "" THEN
                YCOMP = "CUSTOMER_2_":YM.DR.ISSUE.NAME
                YFORFIL = YF.CUSTOMER
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
                YM.DR.ISSUE.NAME = YFOR.FD
            END
            YR.REC(16) = YM.DR.ISSUE.NAME
