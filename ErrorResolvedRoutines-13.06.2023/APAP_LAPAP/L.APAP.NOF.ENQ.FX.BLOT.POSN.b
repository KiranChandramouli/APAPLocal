* @ValidationCode : MjotNzgyOTkyMTQ6Q3AxMjUyOjE2ODY1NzM3MjIwMjA6MzMzc3U6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 12 Jun 2023 18:12:02
...
*12/06/2023      CONVERSION TOOL            AUTO R22 CODE CONVERSION       INSERT FILE MODIFIED, FM TO @FM, VM TO @VM, ++ TO +=
*12/06/2023      SURESH                     MANUAL R22 CODE CONVERSION           NOCHANGE
*----------------------------------------------------------------------------------------
SUBROUTINE L.APAP.NOF.ENQ.FX.BLOT.POSN(Y.ARR)
*********************************************************************************************************
*Company   Name    : LAPAP.NOF.ENQ.FX.BLOT.POSN
*Developed By      : APAP
*Program   Name    : LAPAP.NOF.ENQ.FX.BLOT.POSN
*--------------------------------------------------------------------------------------------------------
*Description       : Basada en logica de la rutina de TEMENOS : LAPAP.NOF.ENQ.FX.BLOT.POSN DEL REPORTE de divisas blotler
*                     This is a nofile routine attached to the STANDARD.SELECTION:NOFILE.REDO.ENQ.FX.BLOT.POSN
*                    which is for ENQUIRY : REDO.ENQ.FX.BLOT.POSN.This to display position of given currency
*                    with their transaction details
*In Parameter      :N/A

*Out Parameter     :Y.ARR
*Files  Used       : LAPAP.NOF.ENQ.FX.BLOT.POSN
*--------------------------------------------------------------------------------------------------------
...
*  02/02/2023      APAP             MDP-1343                           Escluir las transaciones que no sean divisas que presenta el
*                                                                      el campo tasa pol en cero DEAL.RATE
*********************************************************************************************************
    ...
    $INSERT I_F.TELLER
    $INSERT I_F.CUSTOMER
    $INSERT I_F.REDO.FX.CCY.POSN ;*AUTO R22 CODE CONVERSION - END
...
*Y.TXN.TYPE = 'SELL'
    ...
    IF Y.TXN.TYPE EQ 'COMPRA' AND Y.FCY.AMT GT 0 THEN
        IF Y.FIRST.FLAG EQ 1 THEN
            ...
            Y.FIRST.FLAG += 1 ;*AUTO R22 CODE CONVERSION
        END ELSE
    ...
    IF Y.TXN.TYPE EQ 'VENTA' AND Y.FCY.AMT GT 0 THEN
        IF Y.FIRST.FLAG EQ 1 THEN
            ...
            Y.FIRST.FLAG += 1 ;*AUTO R22 CODE CONVERSION
        END ELSE
            Y.FCY.POSN -= Y.FCY.AMT ;*AUTO R22 CODE CONVERSION
        END
    END
...
ASSIGN.VAR:
***********
    IF NOT (Y.SPOT.RATE) OR Y.SPOT.RATE EQ 0 THEN
        RETURN;
    END
    Y.TXN = FIELD(Y.TXN,";",1)
