* @ValidationCode : MjotMTI0MTgyNDQ5MzpDcDEyNTI6MTY4NjY0Njg0OTg4MzpJVFNTMTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMl9TUDUuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 13 Jun 2023 14:30:49
...
$PACKAGE APAP.LAPAP
SUBROUTINE LAPAP.LY.POINTS.E.HIS(R.DATA)
*-----------------------------------------------------------------------------
*DESCRIPTION:
*------------
...
* Modificacion
* 13-06-2022
* basada en la misma logica de la rutina: REDO.LY.POINTS.E.HIS
*-----------------------------------------------------------------------------
*Modification History
*DATE                       WHO                         REFERENCE                                   DESCRIPTION
*12-06-2023            Conversion Tool             R22 Auto Code conversion                INSERT FILE MODIFIED,VM TO @VM,SM TO @SM,++ to +=1
...
********
    ...
    IF SEL.REC.LIST.TOT NE 0 THEN
        ...
        WHILE Y.SEL.REC.LIST.CNT LE SEL.REC.LIST.TOT
            ...
            GOSUB GET.ALL.DATA
            Y.SEL.REC.LIST.CNT += 1   ;*R22 AUTO CODE CONVERSION
        REPEAT
    END
...
**********
    ...
    Y.RET = 3
    Y.SPACE = DCOUNT(Y.FROM.TO.DATE,@SM)   ;*R22 AUTO CODE CONVERSION

    BEGIN CASE
        CASE Y.SPACE EQ 1
            Y.RET = 1
            Y.FIRST.DATE = FIELD(Y.FROM.TO.DATE,@SM,1)   ;*R22 AUTO CODE CONVERSION
            RETURN
        CASE Y.SPACE EQ 2
            Y.RET = 2
            Y.FIRST.DATE = FIELD(Y.FROM.TO.DATE,@SM,1)   ;*R22 AUTO CODE CONVERSION
            Y.SECOND.DATE = FIELD(Y.FROM.TO.DATE,@SM,2)   ;*R22 AUTO CODE CONVERSION
            RETURN
    END CASE
