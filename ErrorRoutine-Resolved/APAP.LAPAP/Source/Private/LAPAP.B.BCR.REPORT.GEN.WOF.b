* @ValidationCode : Mjo3Mjg0NzA3NDI6Q3AxMjUyOjE2ODU2MTc5NjYwMTI6SVRTUzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 01 Jun 2023 16:42:46
...
*21-04-2023      conversion tool     R22 Auto code conversion    VM TO @VM,FM TO @FM,SM TO @SM,++ TO +=1,I TO I.VAR
*21-04-2023      HARISHVIKRAM C      R22 Manual code conversion   CALL routine format modified, Y.CATEGORIA.WOF added in insert file  I_REDO.B.BCR.REPORT.GEN.COMMON
...
*-----------------------------------------------------------------------------
    ...
    ACCOUNT.PROPERTY = ''
    APAP.TAM.redoGetPropertyName(Y.AA.ID,'ACCOUNTWOF',R.OUT.AA.RECORD,ACCOUNT.PROPERTY,OUT.ERR)
...
GET.ALT.IDWOF:
    ...
    LOCATE R.ACCOUNT<AC.CATEGORY> IN Y.CATEGORIA.WOF<1,1> SETTING C.WOF.POS THEN
        ...
        FOR I.VAR = 1 TO Y.CNT.TYPE ;*R22 Auto code conversion
            ID.TYPE.ACTUAL = Y.ALT.TYPE<I.VAR>[1,7]

            IF ID.TYPE.ACTUAL NE 'ALTERNO' THEN
                CONTINUE
            END
            Y.ALT.ACCT.ID = ID.ALTENO<I.VAR>[3,LEN(ID.ALTENO)] ;*R22 Auto code conversion
            ...
            APAP.LAPAP.redoBBcrReportGenExt(Y.LEGACY.ARR,Y.TOTAL.CUOTAS.1,Y.LASTPAY.AMT,Y.LASTPAY.DAT, Y.MNTPAY,Y.PRODUCTO.GRUPO) ;*R22 Manual code conversion
