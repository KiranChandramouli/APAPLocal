* @ValidationCode : MjoyMDkyNzYxMjI6Q3AxMjUyOjE2ODU2MjEzMTMxMTI6SVRTUzotMTotMTo5MDM1OjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 01 Jun 2023 17:38:33
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 9035
...
* PACS00460183           Ashokkumar.V.P                  10/06/2015      new mapping changes.
* CN0080668              Ashokkumar                      21/01/2018      New REA changes
*-----------------------------------------------------------------------------------------------------------------
* Include files
*-----------------------------------------------------------------------------------------------------------------
*---------------------------------------------------------------------------------------
...
*30/05/2023      CONVERSION TOOL            AUTO R22 CODE CONVERSION      FM TO @FM, VM TO @VM, SM TO @SM, ++ to +=, -- to -=,I to I.VAR, J to J.VAR
*30/05/2023      HARISH VIKRAM              MANUAL R22 CODE CONVERSION    CALL routine format modified, POS.L.AA.COL,POS.L.AA.COL.VAL,Y.CARGO.CANCELACION   added in insert file  I_REDO.B.CON.LNS.BY.DEBTOR.COMMON
...
*
    ...
    ARR.ERR       = ""; R.ARRANGEMENT = "" ; Y.ACC.ACCRUAL.AMT = ''; YDE13.CLOSE.LN.FLG = 0; Y.PRINCIPAL.SALDO = 0 ; YACCT.GRP.IN = 0
    ...
    YCUSTOMER = R.ARRANGEMENT<AA.ARR.CUSTOMER>
    ...
    IF Y.MAIN.ARR.STATUS NE 'CURRENT' AND Y.MAIN.ARR.STATUS NE 'EXPIRED' THEN
        RETURN
    END
    Y.LOAN.STATUS = ''; YCLOSE.LN.FLG = 0 ; Y.NROPRESTAMO = ''
...
*------------------------------------------------------------------------
    ...
    YCONS.AMT = R.REDO.APAP.COMMER.DEBT.DET<2>
...
*-----------------------------
    ...
    END
    ERR.ACCOUNT = ''; R.ACCOUNT = ''; Y.PREV.ACCOUNT = ''; YREV.ACCTFLG = 0
...
**-----------------------------------------------------------
    ...
    CALL REDO.CON.LNS.CALC.DISB.DATE.AMT(IN.REC.VAL,OUT.REC.VAL)
*
...
*-----------
    ...
    IF Y.L.LOAN.STATUS.1 EQ "JudicialCollection" THEN
        ...
        IF YJC.L.STATUS.CHG THEN
            C$SPARE(491) = YJC.L.STATUS.CHG[7,2]:"/":YJC.L.STATUS.CHG[5,2]:"/":YJC.L.STATUS.CHG[1,4]
        END
    END ELSE
        Y.JUD.COLL = "N"
    END
    ...
    C$SPARE(490) = Y.ID.COMP[7,2]:"/":Y.ID.COMP[5,2]:"/":Y.ID.COMP[1,4]
...
*-----------
    ...
    Y.START.DATE = R.AA.PAYMENT.SCHEDULE<AA.PS.START.DATE>
    Y.LOAN.AMOUNT = R.AA.TERM.AMOUNT<AA.AMT.AMOUNT>
    Y.MAT.DATE = R.AA.TERM.AMOUNT<AA.AMT.MATURITY.DATE>
    YPS.PAYMENT.TYPE = ''; YPS.PAYMENT.TYPE = R.AA.PAYMENT.SCHEDULE<AA.PS.PAYMENT.TYPE>
...
*    LOCATE "PRINCIPALINT" IN Y.AA.PROPERTY<1,1> SETTING Y.AA.PROP.POS THEN
...
*Y.CHARGE.PROP = 'PRCANCANTIC'
    ...
    R.CONDITION = ''
    ERR.MSG = ''
    APAP.AA.redoCrrGetConditions(Y.AA.ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG);*MANUAL R22 CODE CONVERSION
    Y.CHG.RATE = R.CONDITION<AA.CHG.LOCAL.REF,Y.L.AA.CHG.RATE.POS>
    C$SPARE(476) = Y.CHG.RATE
RETURN
PROV.REQU:
*-------
...
*******************
    ...
    BEGIN CASE
        ...
        CASE NO.OF.DAYS NE 0 AND NO.OF.DAYS LT 365
            C$SPARE(495) = 'D1'
            YGRP =1
    END CASE
...
*------------------
    ...
    IF R.RETURN.MSG THEN
        CALL F.WRITE(FN.DR.REG.DE13.WORKFILE,Y.AA.ARR.ID,R.RETURN.MSG)
    END
RETURN
...
*********
    ...
    C$SPARE(481) = ''; C$SPARE(482) = ''; C$SPARE(483) = ''; C$SPARE(484) = ''; C$SPARE(485) = ''; C$SPARE(486) = ''; C$SPARE(487) = ''; C$SPARE(488) = ''; C$SPARE(489) = ''; C$SPARE(490) = ''
    C$SPARE(491) = ''; C$SPARE(492) = ''; C$SPARE(493) = ''; C$SPARE(494) = ''; C$SPARE(495) = ''
RETURN
