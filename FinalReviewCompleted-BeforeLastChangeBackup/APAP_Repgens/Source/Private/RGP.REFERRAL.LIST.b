* @ValidationCode : Mjo2MTM4MjcxNTI6Q3AxMjUyOjE2ODY4MjE3MjI1Mzc6SGFyaXNodmlrcmFtQzotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.REFERRAL.LIST
REM "RGP.REFERRAL.LIST",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    YHDR = "REFERRAL LIST"
    IF YPRINTING THEN
        YHDR = "RGP.REFERRAL.LIST ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "ACCOUNT NUMBER CUSTOMER                 CCY LEDGER BALANCE CCY MOVEMENT A       MOUNT REFERAL CONDITION OU              R REFERENCE"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.CCY2
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(7)  ;* YM.MOVEMENT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(8)  ;* YM.REFERRAL
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(9)  ;* YM.REFERENCE
        YFD = FMT(YFD,"12L")
