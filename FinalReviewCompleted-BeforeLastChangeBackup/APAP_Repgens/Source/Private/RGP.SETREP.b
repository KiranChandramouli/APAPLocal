* @ValidationCode : MjoxMDIxOTY5NTgzOkNwMTI1MjoxNjg2ODIxNzIyNzI2OkhhcmlzaHZpa3JhbUM6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.SETREP
REM "RGP.SETREP",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    YT.SMS.FILE = "SECURITY.TRANSFER"
    YT.SMS.FILE<-1> = "SECURITY.MASTER"
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.SETREP"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "SECURITIES TRANSFER LIST"
    IF YPRINTING THEN
        YHDR = "RGP.SETREP ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "SECURITY DESCRIPTION"
    YHDR2 = "              CUSTOMER                  QUANTITY          COST PRICE"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YFD = YR.REC(1)  ;* YM.SORT
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.SECNO
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        GOSUB 9000000
...
*
                ...
                YFD = YR.TOT1(YAV.TOT)<1>  ;* YM.SECNO
                YR.TOT1(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"10L")
                IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
                YTOTFD := YFD
...
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.TOTAL
                YR.TOT2(YAV.TOT)<1> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",11)
                END ELSE
...
*
    ...
    END
    MATREAD YR.REC FROM F.FILE, YKEY ELSE MAT YR.REC = "" ; GOSUB 1000000      ;*Manual R22 conversion GOSUB Changed to GOSUB
    YKEY = "C":YKEY
