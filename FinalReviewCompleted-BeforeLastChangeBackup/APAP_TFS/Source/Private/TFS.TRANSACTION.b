* @ValidationCode : MjotMzY1NTQyNjA2OkNwMTI1MjoxNjk4NzUwNjc0NTc0OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
...
* <Rating>501</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE TFS.TRANSACTION
*-----------------------------------------------------------------------------
*
* Table to define Alpha Numeric Transaction Codes that can be mapped to
* either FT or TT Transaction Codes
*
*-----------------------------------------------------------------------
...
*             Dont return E from TFS.TXN.CHECK.FIELDS during CHECK.RECORD stage.
*-----------------------------------------------------------------------------
*Modification History:
*DATE                 WHO                    REFERENCE                     DESCRIPTION
*26/10/2023         Ajithkumar             R22 Manual Conversion        USPLATFORM.BP File is Removed ,Call Rtn Format Can be Modified , VM to @VM
*
    $INSERT I_COMMON
    $INSERT I_EQUATE
    ...
    $INCLUDE  I_F.TFS.TRANSACTION ;*R22 Manual Code conversion _ End
*-----------------------------------------------------------------------------
...
* Validation and changes of the Record.  Set ERROR to 1 if in error.
    ...
    SAVE.COMI = COMI ; SAVE.ETEXT = ETEXT ; SAVE.AF = AF ; SAVE.MESSAGE = MESSAGE
    MESSAGE = 'DUMMY'

    CALL EB.READ.PARAMETER(FN.TFSP,"N",'',TFS$R.TFS.PAR,ID.TFSP,F.TFSP,ERR.TFSP)          ;* 04/12/05 - Sathish PS s/e
    IF TFS$R.TFS.PAR THEN
        IF R.NEW(TFS.TXN.INTERFACE.TO) THEN
            AF = TFS.TXN.INTERFACE.TO ; COMI = R.NEW(AF)
* CALL TFS.TXN.CHECK.FIELDS
            APAP.TFS.tfsTxnCheckFields() ;*r22 Manual Code conversion
            IF R.NEW(TFS.TXN.INTERFACE.AS) THEN
                AF = TFS.TXN.INTERFACE.AS ; COMI = R.NEW(AF)
*CALL TFS.TXN.CHECK.FIELDS
                APAP.TFS.tfsTxnCheckFields() ;*r22 Manual Code conversion
                E = ''        ;* 04 SEP 07 - Sathish PS s/e
        ...
        END
    END ELSE
        E = 'EB-TFS.PAR.RECORD.MISSING'
    END

    COMI = SAVE.COMI ; ETEXT = SAVE.ETEXT ; AF = SAVE.AF ; MESSAGE = SAVE.MESSAGE

    IF V$FUNCTION MATCHES 'I' :@VM: 'C' THEN ;*R22 Manual Code conversion
        IF E THEN
            CALL ERR
            V$ERROR = 1
            MESSAGE = 'REPEAT'
...
* CALL TFS.TXN.CROSSVAL
    APAP.TFS.tfsTxnCrossval() ;*R22 Manual Code Conversion
...
*CALL TFS.TXN.FIELD.DEFINITIONS
    APAP.TFS.tfsTxnFieldDefinitions();*R22 Manual Code conversion
