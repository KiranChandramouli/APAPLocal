* @ValidationCode : MjotOTI5MTA5NzAzOkNwMTI1MjoxNjgyNTk4MDE3NDg1OnNhbWFyOi0xOi0xOjA6MTpmYWxzZTpOL0E6REVWXzIwMjEwOC4wOi0xOi0x
...
*-----------------------------------------------------------------------
    Y.RESPONSE    = ''
*APAP.REDORETAIL.REDO.COL.R.CALL.JAVA.API(Y.EB.API.ID, Y.REQUEST, Y.RESPONSE)
    APAP.REDORETAIL.redoColRCallJavaApi(Y.EB.API.ID, Y.REQUEST, Y.RESPONSE);*MANUAL R22 CODE CONVERSION
    IF Y.RESPONSE NE '' THEN
        C.DESC = Y.RESPONSE
        GOSUB STORE.MSG.ON.QUEUE
        Y.ERROR = @TRUE
*APAP.TAM.REDO.R.COL.EXTRACT.ERROR(C.DESC, "REDO.COL.EXTRACT.SELECT","TMPCLIENTES")
        APAP.TAM.redoRColExtractError(C.DESC, "REDO.COL.EXTRACT.SELECT","TMPCLIENTES");* R22 Manual conversion
*APAP.TAM.REDO.R.COL.PROCESS.TRACE('EXTRACT', 20, 1, "TMPCLIENTES", '', C.DESC)
