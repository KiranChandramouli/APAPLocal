<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTA0NzcxOTMzMTpDcDEyNTI6MTY4ODUzNjg5MDIzMTpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjotMTA0NzcxOTMzMTpDcDEyNTI6MTY4NjgyMDU4NDUyOTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 14:46:24
>>>>>>> Stashed changes
=======
* @ValidationCode : MjotMTA0NzcxOTMzMTpDcDEyNTI6MTY4NjgyMDU4NDUyOTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 14:46:24
...
SUBROUTINE RGP.LD0110
REM "RGP.LD0110",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(40); MAT YR.REC = ""
    DIM YR.REC.OLD(38); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "MATURING EVENTS REPORT"
    IF YPRINTING THEN
        YHDR = "RGP.LD0110 ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "REFERENCE NO. CUSTOMER/NUMBER/          CCY LIQ. PRINC AMT/ACT. INT AMT/TAX     /ACT. COM AMT/TAX/ACT. FEE AM           T/ACT. TYPE"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                                            ":YFD
        YFD = YR.REC(33)  ;* YM.NO.PAYMENT.PRINT
        YFD = FMT(YFD,"23L")
        IF LEN(YFD) > 23 THEN YFD = YFD[1,22]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(34)  ;* YM.BLANK.LINE
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(35)  ;* YM.END.OF.REC.LIT1
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(36)  ;* YM.END.OF.REC.LIT2
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        YFD = YR.REC(37)  ;* YM.END.OF.REC.LIT3
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        YFD = YR.REC(38)  ;* YM.END.OF.REC.LIT4
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        GOSUB 9000000
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(39)
        C$RPT.ACCOUNT.NO = YR.REC(40)
