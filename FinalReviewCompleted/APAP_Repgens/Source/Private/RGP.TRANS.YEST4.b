<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTA3NzA0NTA3NTpDcDEyNTI6MTY4ODUzNjg5MzY1NzpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:33
=======
* @ValidationCode : MjotMTA3NzA0NTA3NTpDcDEyNTI6MTY4NjgyMjc3MzAzNTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:22:53
>>>>>>> Stashed changes
=======
* @ValidationCode : MjotMTA3NzA0NTA3NTpDcDEyNTI6MTY4NjgyMjc3MzAzNTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:22:53
...
SUBROUTINE RGP.TRANS.YEST4
REM "RGP.TRANS.YEST4",230614-4
*************************************************************************
*------------------------------------------------------------------------
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.TRANS.YEST4"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "LISTING OF YESTERDAYS ENTRIES"
    IF YPRINTING THEN
        YHDR = "RGP.TRANS.YEST4 ":YHDR
    ...
    YHDR1 = "TRANS.REFERENCE ACCT/CATEG/CRF L        CY AMOUNT EXCH.RATE CCY FCY A           MOUNT VAL.DATE T/C A/O. PRODUCT DESCRI  PTION"
    YHDR2 = "=============== ============== =        ========= ========= === =====           ===== ======== === ==== ======= ======  ====="
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "1" DO
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.UNDER
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"11L")
        ...
        IF Y1ST.LIN THEN
            ...
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD = YR.REC(3)  ;* YM.TRANS.REF
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := YFD
        YFD = YR.REC(4)  ;* YM.ACCOUNT
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(5)  ;* YM.AMOUNTLCY
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YFD = FMT(YFD,"19R2,")
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            NEXT YNO
        END
        YFD = YR.REC(9)  ;* YM.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(10)  ;* YM.CCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.FCYAMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
