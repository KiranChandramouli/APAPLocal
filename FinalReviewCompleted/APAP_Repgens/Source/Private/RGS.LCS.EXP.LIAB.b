* @ValidationCode : MjotODc1MTYzNjE4OkNwMTI1MjoxNjg4NTQ1NzUzNDk3OklUU1M6LTE6LTE6NTEzNjoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 13:59:13
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 5136
...
*-----------------------------------------------------------------------------------------------
SUBROUTINE RGS.LCS.EXP.LIAB
REM "RGS.LCS.EXP.LIAB",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.LCS.EXP.LIAB")
    END
*************************************************************************
...
*************************************************************************
    ...
    YR.VOC = "D"; YR.VOC<2> = 117
    YR.VOC<5> = "16R"
    YR.VOC<6> = "M"
    WRITE YR.VOC TO YF.VOC, "FLD.":TNO
    CLEARSELECT
    EXECUTE "HUSH ON"
    EXECUTE 'SSELECT ':FULL.FNAME:' WITH FLD.':TNO:' > "0.00"';* R22 Manual conversion - SELECT changed to SSELECT
    EXECUTE "HUSH OFF"
...
* Handle Decision Table
            YM.TODAY = R.NEW(137)
            IF YM.TODAY <> "" THEN
                YCOMP = "DATES_14_":YM.TODAY
                YFORFIL = YF.DATES
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
                YM.TODAY = YFOR.FD
            END
            YM.JUL.EXPDATE = R.NEW(28)
            YM12.GOSUB = YM.JUL.EXPDATE
            YM.EXP.JUL = ""
            YM13.GOSUB = YM.EXP.JUL
            CALL @JULDATE (YM12.GOSUB, YM13.GOSUB)
            YM.JUL.EXPDATE = YM12.GOSUB
            YM.EXP.JUL = YM13.GOSUB
            YM.MODIFIED.TODAY = YM.TODAY
            YM1.MODIFIED.TODAY = YM.MODIFIED.TODAY
            YM.MODIFIED.TODAY = "3"
            IF NUM(YM1.MODIFIED.TODAY) = NUMERIC THEN IF NUM(YM.MODIFIED.TODAY) = NUMERIC THEN
                YM1.MODIFIED.TODAY = YM1.MODIFIED.TODAY + YM.MODIFIED.TODAY
                IF YM1.MODIFIED.TODAY = 0 THEN YM1.MODIFIED.TODAY = ""
            END
            YM.MODIFIED.TODAY = YM1.MODIFIED.TODAY
            IF YM.EXP.JUL > YM.TODAY AND YM.EXP.JUL < YM.MODIFIED.TODAY THEN
                GOSUB 2000000
            END
1000:
        END
...
*
    ...
    YKEYFD = FMT(YM.EXPIRY.DATE,"11L")
    IF LEN(YKEYFD) > 11 THEN YKEYFD = YKEYFD[1,10]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.EXPIRY.DATE
    YM.LC.NUM = ID.NEW
    YR.REC(2) = YM.LC.NUM
    YM.LC.TYPE = R.NEW(2)
    YKEYFD = YM.LC.TYPE
