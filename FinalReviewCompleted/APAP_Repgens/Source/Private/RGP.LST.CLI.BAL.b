<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTU3MjU3NDY2MjpDcDEyNTI6MTY4ODUzNjg5MTM0NTpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjotMTU3MjU3NDY2MjpDcDEyNTI6MTY4NjgyNDg2MDQxNTpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:57:40
* @ValidationInfo : Encoding          : Cp1252
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LST.CLI.BAL"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "ACCOUNT       CATEGORY  CCY             DT BAL CCY     RATE              DT BAL CHF          CR BAL CCY     RATE          CR BAL CHF"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            END
        END
        YFD = YR.REC(3)  ;* YM.ACCTNO
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := YFD
        YFD = YR.REC(4)  ;* YM.CATEG
        YFD = FMT(YFD,"6L")
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            IF YT.DEC2(2)<7> = "" THEN
                YT.DEC2(2)<7> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<7> THEN
                    YT.DEC2(2)<7> = YDEC
                END
            END
        END
        YFD = YR.REC(16)  ;* YM.CRLCYBAL
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
...
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.TOT1
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"6L")
                IF LEN(YFD) > 6 THEN YFD = YFD[1,5]:"|"
                YFD = "TOTAL ":YFD
                YTOTFD := "          ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.TOT2
                YR.TOT2(YAV.TOT)<2> = ""
                YFD = FMT(YFD,"3L")
                IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
                YTOTFD := "  ":YFD
                YFD = YR.TOT2(YAV.TOT)<3>  ;* YM.DRTOTCCY
                YR.TOT2(YAV.TOT)<3> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := "    ":YFD
                YFD = YR.TOT2(YAV.TOT)<4>  ;* YM.DRTOTRATE
                YR.TOT2(YAV.TOT)<4> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",9)
                END ELSE
                ...
                IF LEN(YFD) > 9 THEN YFD = YFD[1,8]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.DRLCYTOT
                YR.TOT2(YAV.TOT)<5> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := "    ":YFD
                YFD = YR.TOT2(YAV.TOT)<6>  ;* YM.CRCCYTOT
                YR.TOT2(YAV.TOT)<6> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<7>  ;* YM.CRTOTRATE
                YR.TOT2(YAV.TOT)<7> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",9)
                END ELSE
                ...
                IF LEN(YFD) > 9 THEN YFD = YFD[1,8]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<8>  ;* YM.CRLCYTOT
                YR.TOT2(YAV.TOT)<8> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<8>:",")
                    YT.DEC2(YAV.TOT)<8> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
