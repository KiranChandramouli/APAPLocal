<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotNTE1MDIwMTE2OkNwMTI1MjoxNjg4NTM2ODkxNTcwOklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjotNTE1MDIwMTE2OkNwMTI1MjoxNjg2ODIyOTkxOTIzOmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:26:31
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGP.LST.FX.FW.PURCH
REM "RGP.LST.FX.FW.PURCH",230614-4
*************************************************************************
*----------------------------------------------------------------------------
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LST.FX.FW.PURCH"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "MATURITY        CUSTOMER     CCY              AMOUNT       REFERENCE"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        BEGIN CASE
            CASE YR.REC.OLD(3) <> YR.REC(3); YHEADER.STATUS = 1
        END CASE
        IF YHEADER.STATUS THEN
            GOSUB 9000010; Y1ST.LIN = 1
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD.OLD = YR.REC.OLD(3); YFD = YR.REC(3)  ;* YM.CCY
        ...
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.CUSTOMER
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := "     ":YFD
        YFD = YR.REC(4)  ;* YM.CCY1
        YR.TOT2(2)<1> = YFD
        IF NUM(YFD) = NUMERIC THEN
            ...
            END
        END
        YFD = YR.REC(5)  ;* YM.CCY2
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YFD = "  ":YFD
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.AMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
...
*
                ...
                YFD = "TOTAL IN ":YFD
                YTOTFD := "                    ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.AMOUNT1
                YR.TOT2(YAV.TOT)<2> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
