* @ValidationCode : Mjo1NjI1MjY1ODQ6Q3AxMjUyOjE2ODg1NTY2Mzc1MjI6SVRTUzotMTotMTo3OTU2OjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 05 Jul 2023 17:00:37
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 7956
...
*-----------------------------------------------------------------------------
SUBROUTINE RGP.LCS.EXP.LIAB
REM "RGP.LCS.EXP.LIAB",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LCS.EXP.LIAB"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "LC'S EXP WITH LIABILITY"
    IF YPRINTING THEN
        YHDR = "RGP.LCS.EXP.LIAB ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "                                           EXPIRYING LETTER OF CREDITS"
    YHDR2 = " EXPIRY        LC NUMBER     TYPE    EXTERNAL          CURRENCY    LC AMOUNT        ISSUE           CUSTOMER  / ISSUING SHORT NAME"
    YHDR3 = "  DATE                                                                              DATE"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(4),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(4)<1,1>  ;* YM.OLD.REF
        YFD = FMT(YFD,"14L")
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "   ":YFD
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YFD = YR.REC(4)<1,YAV>  ;* YM.OLD.REF
            YFD = FMT(YFD,"14L")
            IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
            YTOTFD := "                                   ":YFD
            YFD = YR.REC(9)<1,YAV>  ;* YM.LIAB
            IF YFD = "" THEN
                YFD = STR(" ",14)
            END ELSE
                YFD = FMT(YFD,"14R2,")
