<<<<<<< Updated upstream
* @ValidationCode : Mjo0NjEzNjQ4MDI6Q3AxMjUyOjE2ODg1MzY4OTEwMzA6SVRTUzotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:31
=======
* @ValidationCode : Mjo0NjEzNjQ4MDI6Q3AxMjUyOjE2ODY4MjM5MTE1ODU6SVRTUzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:41:51
>>>>>>> Stashed changes
...
SUBROUTINE RGP.LI.COLLAT.DET
REM "RGP.LI.COLLAT.DET",230614-4
*************************************************************************
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LI.COLLAT.DET"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(33); MAT YR.REC = ""
    DIM YR.REC.OLD(31); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "LIMIT/COLLATERAL DETAIL"
    IF YPRINTING THEN
        YHDR = "RGP.LI.COLLAT.DET ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "                                         LIMIT                                   O/L TOTAL                               AVAILABLE"
    YHDR2 = " LIMIT ID CUSTOMER                       TYPE CCY TOTAL SECUR                   ED UNSECURED LIMIT O/S                   /EXCESS"
    YHDR3 = " COLLAT CODE MAX SECURED RIGHT           ID SECURED PRODUCT CUSTOMER            LIMIT TYPE/CCY"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := YFD
        YFD = YR.REC(7)  ;* YM.LIMIT.REFERENCE
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(8)  ;* YM.SERIAL.NO
        YFD = FMT(YFD,"2L")
        IF LEN(YFD) > 2 THEN YFD = YFD[1,1]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(9)  ;* YM.SHORT.NAME
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(10)  ;* YM.LIMIT.PRODUCT
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.LIMIT.CURRENCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.MAXIMUM.TOTAL
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(16)  ;* YM.FIXED.VARIABLE
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(17)  ;* YM.TOTAL.OS
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(19)  ;* YM.COLLATERAL.CODE
        IF YFD = "" THEN
            YFD = STR(" ",3)
        END ELSE
            YFD = FMT(YFD,"R###")
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "    ":YFD
        YFD = YR.REC(20)  ;* YM.DESCRIPTION
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(21)  ;* YM.MAXIMUM.SECURED
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(23)  ;* YM.SECURED.AMT.DETAIL
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(24)  ;* YM.LIMIT.SUB.PRODUCT
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        ...
        YFD = YR.REC(25)<1,1,1>  ;* YM.LIMIT.SUB.CUSTOMER
        YFD = FMT(YFD,"6L")
        IF LEN(YFD) > 6 THEN YFD = YFD[1,5]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(26),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(26)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(26)<1,1,1>  ;* YM.SHORT.NAME2
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(27),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(27)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(27)<1,1,1>  ;* YM.DESCRIPTION3
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(28),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(28)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(28)<1,1,1>  ;* YM.LIMIT.SUB.CURRENCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(29),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(29)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(29)<1,1,1>  ;* YM.ONLINE.SUB.LIMIT
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(30),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(30)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(30)<1,1,1>  ;* YM.TOTAL.SUB.OS.AC
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := YFD
        YCNT = COUNT(YR.REC(31),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(31)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(31)<1,1,1>  ;* YM.AVAIL.SUB.AMT.AC
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
...
*
        FOR YAS = 2 TO YCOUNT.AS.LIN
            YFD = YR.REC(25)<1,YAV,YAS>  ;* YM.LIMIT.SUB.CUSTOMER
            YFD = FMT(YFD,"6L")
            IF LEN(YFD) > 6 THEN YFD = YFD[1,5]:"|"
            YTOTFD := "                                                                ":YFD
            YFD = YR.REC(26)<1,YAV,YAS>  ;* YM.SHORT.NAME2
            YFD = FMT(YFD,"11L")
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := " ":YFD
            YFD = YR.REC(27)<1,YAV,YAS>  ;* YM.DESCRIPTION3
            YFD = FMT(YFD,"10L")
            IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
            YTOTFD := " ":YFD
            YFD = YR.REC(28)<1,YAV,YAS>  ;* YM.LIMIT.SUB.CURRENCY
            YFD = FMT(YFD,"3L")
            IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
            YTOTFD := " ":YFD
            YFD = YR.REC(29)<1,YAV,YAS>  ;* YM.ONLINE.SUB.LIMIT
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
            ...
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := " ":YFD
            YFD = YR.REC(30)<1,YAV,YAS>  ;* YM.TOTAL.SUB.OS.AC
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
            ...
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := YFD
            YFD = YR.REC(31)<1,YAV,YAS>  ;* YM.AVAIL.SUB.AMT.AC
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            ...
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(25)<1,YAV,1>  ;* YM.LIMIT.SUB.CUSTOMER
            YFD = FMT(YFD,"6L")
            IF LEN(YFD) > 6 THEN YFD = YFD[1,5]:"|"
            YTOTFD := "                                                                ":YFD
            YCNT = COUNT(YR.REC(26)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(26)<1,YAV,1>  ;* YM.SHORT.NAME2
            YFD = FMT(YFD,"11L")
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := " ":YFD
            YCNT = COUNT(YR.REC(27)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(27)<1,YAV,1>  ;* YM.DESCRIPTION3
            YFD = FMT(YFD,"10L")
            IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
            YTOTFD := " ":YFD
            YCNT = COUNT(YR.REC(28)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(28)<1,YAV,1>  ;* YM.LIMIT.SUB.CURRENCY
            YFD = FMT(YFD,"3L")
            IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
            YTOTFD := " ":YFD
            YCNT = COUNT(YR.REC(29)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(29)<1,YAV,1>  ;* YM.ONLINE.SUB.LIMIT
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
            ...
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := " ":YFD
            YCNT = COUNT(YR.REC(30)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(30)<1,YAV,1>  ;* YM.TOTAL.SUB.OS.AC
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
            ...
            IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
            YTOTFD := YFD
            YCNT = COUNT(YR.REC(31)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(31)<1,YAV,1>  ;* YM.AVAIL.SUB.AMT.AC
            IF YFD = "" THEN
                YFD = STR(" ",11)
            END ELSE
                YFD = FMT(YFD,"11R0,")
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(32)
        C$RPT.ACCOUNT.NO = YR.REC(33)
