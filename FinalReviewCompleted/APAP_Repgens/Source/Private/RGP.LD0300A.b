<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotNjE3MTIzNTg5OkNwMTI1MjoxNjg4NTM2ODkwNTE0OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjotNjE3MTIzNTg5OkNwMTI1MjoxNjg2ODIxOTYwMTQ4OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:09:20
>>>>>>> Stashed changes
=======
* @ValidationCode : MjotNjE3MTIzNTg5OkNwMTI1MjoxNjg2ODIxOTYwMTQ4OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:09:20
...
SUBROUTINE RGP.LD0300A
REM "RGP.LD0300A",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(56); MAT YR.REC = ""
    DIM YR.REC.OLD(54); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "RATE REVISIONS REPORT"
    IF YPRINTING THEN
        YHDR = "RGP.LD0300A ":YHDR
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := "  ":YFD
        YFD = YR.REC(6)  ;* YM.PRINC.AMT.LIT
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "      ":YFD
        YFD = YR.REC(7)  ;* YM.PRINC.AMT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(8)  ;* YM.CUSTOMER.NO
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                ":YFD
        YFD = YR.REC(9)  ;* YM.LIT.1
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "                          ":YFD
        YFD = YR.REC(10)  ;* YM.INT.RATE.TYPE.PRINT
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := " ":YFD
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := YFD
        YFD = YR.REC(14)  ;* YM.LIT.2
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "             ":YFD
        YFD = YR.REC(15)  ;* YM.INT.BASIS.PRINT
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(16)  ;* YM.LIT.3
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "            ":YFD
        YFD = YR.REC(17)  ;* YM.COM.BASIS.PRINT
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(18)  ;* YM.LIT.4
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(19)  ;* YM.INT.PAY.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(20)  ;* YM.LIT.5
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(21)  ;* YM.COM.PAY.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(22)  ;* YM.LIT.6
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(23)  ;* YM.INT.RATE
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(24)  ;* YM.LIT.7
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "       ":YFD
        YFD = YR.REC(25)  ;* YM.NEW.INT.RATE
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(26)  ;* YM.LIT.8
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(27)  ;* YM.INT.KEY
        YFD = FMT(YFD,"2L")
        IF LEN(YFD) > 2 THEN YFD = YFD[1,1]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(28)  ;* YM.LIT.9
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(29)  ;* YM.INT.SPREAD
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(30)  ;* YM.LIT.10
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "        ":YFD
        YFD = YR.REC(31)  ;* YM.NEW.SPREAD
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(32)  ;* YM.LIT.11
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(33)  ;* YM.FIRST.DAY.ACCR
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(34)  ;* YM.LIT.13
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(35)  ;* YM.MIS.INT.KEY
        YFD = FMT(YFD,"2L")
        IF LEN(YFD) > 2 THEN YFD = YFD[1,1]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(36)  ;* YM.LIT.14
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(37)  ;* YM.MIS.INT.RATE
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(38)  ;* YM.LIT.16
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(39)  ;* YM.CAPITALISATION
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(40)  ;* YM.LIT.19
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "                ":YFD
        YFD = YR.REC(41)  ;* YM.LIQ.CD.PRINT
        YFD = FMT(YFD,"14L")
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(42)  ;* YM.LIT.17
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(43)  ;* YM.COMM.RATE
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(44)  ;* YM.LIT.18
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "       ":YFD
        YFD = YR.REC(45)  ;* YM.NEW.COMM.RATE
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(46)  ;* YM.LIT.12
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                    ":YFD
        YFD = YR.REC(47)  ;* YM.INT.REF.NO
        YFD = FMT(YFD,"5L")
        IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(48)  ;* YM.LIT.20
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "              ":YFD
        YCNT = COUNT(YR.REC(49),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(49)<1,1>  ;* YM.COM.DATE.REQD
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(50)  ;* YM.BLANK.LINE
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                           ":YFD
        YFD = YR.REC(51)  ;* YM.END.REC.LIT1
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(52)  ;* YM.END.REC.LIT2
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        YFD = YR.REC(53)  ;* YM.END.REC.LIT3
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        YFD = YR.REC(54)  ;* YM.END.REC.LIT4
        YFD = FMT(YFD,"33L")
        IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
        YTOTFD := YFD
        GOSUB 9000000
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YFD = YR.REC(49)<1,YAV>  ;* YM.COM.DATE.REQD
            BEGIN CASE
                CASE YFD MATCHES "8N"
                    YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
                CASE YFD MATCHES "6N"
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(55)
        C$RPT.ACCOUNT.NO = YR.REC(56)
