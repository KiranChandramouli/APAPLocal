...
*************************************************************************
    ...
    CALL FIELD.NUMBERS.TO.NAMES('7',SS.REC,fldName,dataType,errMsg)
    CLEARSELECT
    YSEL.A = ""
    ...
    END
    EXECUTE "HUSH ON"
    EXECUTE 'SELECT ':FULL.FNAME:' WITH ':fldName:' < "21040" OR ':fldName:' > "21044"':YSEL.A
    EXECUTE "HUSH OFF"
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.CUSTOMER,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.CUSTOMER
    ...
    IF NOT(YTRUE.1) THEN
        ...
        IF YM.RATE < 0 AND YM.RATE THEN
            YTRUE.1 = 1
            YM.RATE.TEXT = R.NEW(11)
            IF YM.RATE.TEXT <> "" THEN
                YCOMP = "BASIC.RATE.TEXT_1_":YM.RATE.TEXT
                YFORFIL = YF.BASIC.RATE.TEXT
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
                YM.RATE.TEXT = YFOR.FD
            END
            YM.PRINT.RATE = YM.RATE.TEXT
        END
    END
