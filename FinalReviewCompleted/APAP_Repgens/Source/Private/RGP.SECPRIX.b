* @ValidationCode : MjoyMjIwMzg2MzU6Q3AxMjUyOjE2ODY4MjE3MjI2MDA6SGFyaXNodmlrcmFtQzotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.SECPRIX
REM "RGP.SECPRIX",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.SECPRIX"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "NO             DESCRIPTION                          PRIX            DATE        CODE"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.DESCRIPTION
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(3)  ;* YM.PRIX
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(4)  ;* YM.DATE
        BEGIN CASE
            CASE YFD MATCHES "8N"
