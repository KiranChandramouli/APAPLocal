<<<<<<< Updated upstream
* @ValidationCode : MjotMTc3MTIxMjI2NDpDcDEyNTI6MTY4ODUzNjg5MTk0MjpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:31
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
=======
* @ValidationCode : MjotMTc3MTIxMjI2NDpDcDEyNTI6MTY4NjgyNTI2NTk5MzpoYWk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 16:04:25
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGP.OVERDRAFT.LIST
REM "RGP.OVERDRAFT.LIST",230614-4
*************************************************************************
*----------------------------------------------------------------------------
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(14); MAT YR.REC = ""
    DIM YR.REC.OLD(12); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "OVERDRAFT AND EXCESS REPORT."
    IF YPRINTING THEN
        YHDR = "RGP.OVERDRAFT.LIST ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "ACCOUNT NO/CREDIT LINE   DESCR.             CCY     LIMIT AMOUNT OR         OUTSTANDING           OVERDRAFT   OVERDRAWN STATUS TODAY"
    YHDR2 = "        CUSTOMER SHORT NAME                         CLEARED BALANCE             BALANCE              EXCESS       SINCE"
    YHDR3 = "------------------------------------------------------------------------------------------------------------------------------------"
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.ACCOUNT.OFFICER
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"35L")
            IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
            YFD = "ACCOUNT OFFICER NAME :    ":YFD
            YTOTFD := YFD
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.ACCT.OFF.NUM
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"4L")
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(3); YFD = YR.REC(3)  ;* YM.UNDERLINE
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"22L")
        ...
        IF Y1ST.LIN THEN
            ...
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD = YR.REC(4)  ;* YM.ACCOUNT.NUMBER
        YFD = FMT(YFD,"24L")
        IF LEN(YFD) > 24 THEN YFD = YFD[1,23]:"|"
        YTOTFD := YFD
        YFD = YR.REC(5)  ;* YM.LIMIT.DESC.
        YFD = FMT(YFD,"15L")
        IF LEN(YFD) > 15 THEN YFD = YFD[1,14]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.CUSTOMER
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(13)
        C$RPT.ACCOUNT.NO = YR.REC(14)
