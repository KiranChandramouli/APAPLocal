* @ValidationCode : MjoxMjU5NjQ4Mjc5OkNwMTI1MjoxNjg4NTU2NjM5NDMzOklUU1M6LTE6LTE6NTkzNjoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 17:00:39
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 5936
...
*04-07-2023 Narmadha V  Manual R22 Conversion  FM to @FM, VM to @VM , SM to @SM, GOTO to GOSUB
*-----------------------------------------------------------------------------
SUBROUTINE RGP.MG.RATE.EX
REM "RGP.MG.RATE.EX",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.MG.RATE.EX"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(5); MAT YR.REC = ""
    GOSUB 1000000
    YHDR = "MORTGAGE RATE EXCEPTION REPORT"
    IF YPRINTING THEN
        YHDR = "RGP.MG.RATE.EX ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "CONTRACT NO       MESSAGE                             INTEREST KEY"
    YHDR2 = "-----------       -------                             ------------"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YFD = YR.REC(1)  ;* YM.CONTRACT.NO
        YFD = FMT(YFD,"17L")
        IF LEN(YFD) > 17 THEN YFD = YFD[1,16]:"|"
        YTOTFD := YFD
        YCNT = COUNT(YR.REC(2),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YCNT = COUNT(YR.REC(2)<1,1>,@SM)
        IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
        YFD = YR.REC(2)<1,1,1>  ;* YM.MESSAGE
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(3),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(3)<1,1>  ;* YM.INTEREST.KEY
        YFD = FMT(YFD,"3L")
...
*
        FOR YAS = 2 TO YCOUNT.AS.LIN
            YFD = YR.REC(2)<1,YAV,YAS>  ;* YM.MESSAGE
            YFD = FMT(YFD,"35L")
            IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
            YTOTFD := "                  ":YFD
            GOSUB 9000000
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YCOUNT.AS.LIN = 1
            YCNT = COUNT(YR.REC(2)<1,YAV>,@SM)
            IF YCNT >= YCOUNT.AS.LIN THEN YCOUNT.AS.LIN = YCNT+1
            YFD = YR.REC(2)<1,YAV,1>  ;* YM.MESSAGE
            YFD = FMT(YFD,"35L")
            IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
            YTOTFD := "                  ":YFD
            YFD = YR.REC(3)<1,YAV>  ;* YM.INTEREST.KEY
            YFD = FMT(YFD,"3L")
            IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
            YTOTFD := " ":YFD
            GOSUB 9000000
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(4)
        C$RPT.ACCOUNT.NO = YR.REC(5)
