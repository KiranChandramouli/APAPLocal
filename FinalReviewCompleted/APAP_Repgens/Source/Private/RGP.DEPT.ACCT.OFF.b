...
SUBROUTINE RGP.DEPT.ACCT.OFF
REM "RGP.DEPT.ACCT.OFF",230614-4
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "DEPARTMENT ACCOUNT OFFICER LIST"
    IF YPRINTING THEN
        YHDR = "RGP.DEPT.ACCT.OFF ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "DEPT          AREA                                           NAME"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.AREA
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := "          ":YFD
        YFD = YR.REC(3)  ;* YM.NAME
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := "          ":YFD
        GOSUB 9000000
