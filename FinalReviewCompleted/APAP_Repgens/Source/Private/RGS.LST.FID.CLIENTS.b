<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTA4MTUwMzQ0OkNwMTI1MjoxNjg4NTM2OTAxNzQ1OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjJfU1A1LjA6LTE6LTE=
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjotMTA4MTUwMzQ0OkNwMTI1MjoxNjg2OTE4MjUxOTE5OklUU1MxOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
* @ValidationInfo : Timestamp         : 16 Jun 2023 17:54:11
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGS.LST.FID.CLIENTS
REM "RGS.LST.FID.CLIENTS",230614-4
*************************************************************************
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.LST.FID.CLIENTS")
    END
*************************************************************************
...
*************************************************************************
    ...
    DIM YR.REC(15)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LST.FID.CLIENTS"
    YOLDFILE = 1
...
* Year 2000 compliance
    ...
    END
    YR.REC(7) = YM.INT.AMOUNT
    YM.COMM.RATE = R.NEW(52)
    YR.REC(8) = YM.COMM.RATE
    YM.COM.AMT = R.NEW(53)
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY, YDEC)
    IF YM.COM.AMT <> "" THEN
    ...
    END
    YR.REC(9) = YM.COM.AMT
    YM.NET1 = YM.PRINCIPAL
    YM1.NET1 = YM.NET1
    YM.NET1 = YM.INT.AMOUNT
    IF NUM(YM1.NET1) = NUMERIC THEN IF NUM(YM.NET1) = NUMERIC THEN
        YM1.NET1 = YM1.NET1 + YM.NET1
        IF YM1.NET1 = 0 THEN YM1.NET1 = ""
    END
    YM.NET1 = YM1.NET1
    YKEYFD = YM.NET1
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.NET1,"3L")


    IF LEN(YKEYFD) > 3 THEN YKEYFD = YKEYFD[1,2]:"|"
    GOSUB 8000000
    YM.NET.AMOUNT = YM.NET1
    YM1.NET.AMOUNT = YM.NET.AMOUNT
    YM.NET.AMOUNT = YM.COM.AMT
    IF NUM(YM1.NET.AMOUNT) = NUMERIC THEN IF NUM(YM.NET.AMOUNT) = NUMERIC THEN
        YM1.NET.AMOUNT = YM1.NET.AMOUNT - YM.NET.AMOUNT
        IF YM1.NET.AMOUNT = 0 THEN YM1.NET.AMOUNT = ""
    END
    YM.NET.AMOUNT = YM1.NET.AMOUNT
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY, YDEC)
    IF YM.NET.AMOUNT <> "" THEN
        YM.NET.AMOUNT = TRIM(FMT(YM.NET.AMOUNT,"19R":YDEC))
    END
    YR.REC(10) = YM.NET.AMOUNT
    YM.FEES = R.NEW(42)
    YR.REC(11) = YM.FEES
    YM.START.DATE = R.NEW(5)
    YR.REC(12) = YM.START.DATE
    YM.MATURITY = R.NEW(6)
    YKEYFD = YM.MATURITY
    YKEYFD = FMT(YM.MATURITY,"9L")
    IF LEN(YKEYFD) > 9 THEN YKEYFD = YKEYFD[1,8]:"|"
    GOSUB 8000000
    YR.REC(13) = YM.MATURITY
    YM.PLAC.BANK = R.NEW(54)
    IF YM.PLAC.BANK <> "" THEN
        YCOMP = "CUSTOMER_1_":YM.PLAC.BANK
        YFORFIL = YF.CUSTOMER
        YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
        YM.PLAC.BANK = YFOR.FD
    END
    YR.REC(14) = YM.PLAC.BANK
    YM.CONTR.NO = ID.NEW
    YR.REC(15) = YM.CONTR.NO
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
