...
SUBROUTINE RGS.LD0110
REM "RGS.LD0110",230614-4
*------------------------------------------------------------------------------
...
*************************************************************************
    ...
    DIM YR.REC(38)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LD0110"
    YOLDFILE = 1
...
* Handle Decision Table
            ...
            FOR XXX = 1 TO DCOUNT( YM3.GOSUB,@VM )
                YM.EVENT.DATE = YM3.GOSUB<1,XXX>
                ...
                END
                IF (YM.EVENT.DATE <> "") AND ((YM.TYPE.P.SHED <> "") OR (YM.TYPE.I.SHED <> "") OR (YM.TYPE.C.SHED <> "") OR (YM.TYPE.F.SHED <> "")) THEN
                    GOSUB 2000000
                END
            NEXT XXX
        END
...
* Year 2000 compliance
    ...
    YM25.GOSUB = YM.FILE.NAME
    YM.SHED.KEY.PARAM = YM.SHED.KEY
    YM26.GOSUB = YM.SHED.KEY.PARAM
    YM.FIELD.NO = "14"
    YM27.GOSUB = YM.FIELD.NO
    YM.OP.CODE = "SUM"
    YM28.GOSUB = YM.OP.CODE
    YM.FEE.AMT = ""
    YM29.GOSUB = YM.FEE.AMT
    CALL @LD.EXTRCT.VAL (YM25.GOSUB, YM26.GOSUB, YM27.GOSUB, YM28.GOSUB, YM29.GOSUB)
    YM.FILE.NAME = YM25.GOSUB
    YM.SHED.KEY.PARAM = YM26.GOSUB
    YM.FIELD.NO = YM27.GOSUB
    YM.OP.CODE = YM28.GOSUB
    YM.FEE.AMT = YM29.GOSUB
    ...
    YTRUE.1 = 0
    YM.CUST.REM.TEMP = R.NEW(142)<1,1>
    ...
    IF YM.CUST.REM.TEMP <> "" THEN
        YTRUE.1 = 1
        YM.CUST.REM2 = R.NEW(142)<1,2>
    END
    IF NOT(YTRUE.1) THEN YM.CUST.REM2 = ""
    YR.REC(29) = YM.CUST.REM2
    YTRUE.1 = 0
    YM.OUR.REM.TEMP = R.NEW(144)<1,1>
    ...
    YR.REC(33) = YM.NO.PAYMENT.PRINT
    YM.BLANK.LINE = "."
    YR.REC(34) = YM.BLANK.LINE
    YM.END.OF.REC.LIT1 = "*********************************"
    YR.REC(35) = YM.END.OF.REC.LIT1
    YM.END.OF.REC.LIT2 = "*********************************"
    YR.REC(36) = YM.END.OF.REC.LIT2
    YM.END.OF.REC.LIT3 = "*********************************"
    YR.REC(37) = YM.END.OF.REC.LIT3
    YM.END.OF.REC.LIT4 = "*********************************"
    YR.REC(38) = YM.END.OF.REC.LIT4
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
    ...
    END
    MATBUILD VKT.REC FROM YR.REC
    CALL RGS.LD0110.POPULATE(YM.EVENT.DATE, ID.NEW, VKT.REC)
    MATPARSE YR.REC FROM VKT.REC
    MATWRITE YR.REC TO F.FILE, YKEY
*
    IF NOT(PHNO) THEN
        IF YWRITNO < 9 THEN
