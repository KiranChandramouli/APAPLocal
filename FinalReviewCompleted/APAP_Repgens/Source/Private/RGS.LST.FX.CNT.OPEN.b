<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTYzODM2NTA4OTpDcDEyNTI6MTY4ODUzNjkwMTg1NzpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjotMTYzODM2NTA4OTpDcDEyNTI6MTY4NjkxODI5MDk4NDpJVFNTMTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMl9TUDUuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Jun 2023 17:54:50
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGS.LST.FX.CNT.OPEN
REM "RGS.LST.FX.CNT.OPEN",230614-4
*************************************************************************
...
*************************************************************************
    ...
    DIM YR.REC(10)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LST.FX.CNT.OPEN"
    YOLDFILE = 1
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.LST.FX.CNT.OPEN")
    END
*
    YCOM = ID.COMPANY
*
...
*************************************************************************
    ...
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.FOREX, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
...
* Year 2000 compliance
    ...
    YR.REC(3) = YM.CCY1
    YM.AMOUNT1 = R.NEW(7)
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY1, YDEC)
    IF YM.AMOUNT1 <> "" THEN
        YM.AMOUNT1 = TRIM(FMT(YM.AMOUNT1,"19R":YDEC))
    END
    YR.REC(4) = YM.AMOUNT1
    YM.FORWARD.RATE = R.NEW(13)
    YR.REC(5) = YM.FORWARD.RATE
    YM.SPOT.RATE = R.NEW(12)
    YR.REC(6) = YM.SPOT.RATE
    YM.CCY2 = R.NEW(9)
    YR.REC(7) = YM.CCY2
    YM.AMOUNT2 = R.NEW(10)
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY2, YDEC)
    IF YM.AMOUNT2 <> "" THEN
        YM.AMOUNT2 = TRIM(FMT(YM.AMOUNT2,"19R":YDEC))
    END
    YR.REC(8) = YM.AMOUNT2
    YM.COUNTERPARTY = R.NEW(3)
    IF YM.COUNTERPARTY <> "" THEN
        YCOMP = "CUSTOMER_1_":YM.COUNTERPARTY
        YFORFIL = YF.CUSTOMER
        YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
        YM.COUNTERPARTY = YFOR.FD
    END
    YR.REC(9) = YM.COUNTERPARTY
    YM.REFERENCE = ID.NEW
