<<<<<<< Updated upstream
* @ValidationCode : MjoxNzEwNDY2Mzk4OkNwMTI1MjoxNjg4NTM2ODkwMTc5OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjoxNzEwNDY2Mzk4OkNwMTI1MjoxNjg2ODIwNjA2NTM3OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 14:46:46
>>>>>>> Stashed changes
...
SUBROUTINE RGP.LD0100
REM "RGP.LD0100",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(34); MAT YR.REC = ""
    DIM YR.REC.OLD(32); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "IMMINENT MATURITIES REPORT"
    IF YPRINTING THEN
        YHDR = "RGP.LD0100 ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "REFERENCE NO. CUSTOMER/NUMBER/           LIQ. PRINC AMT/ACT. INT AMT/TAX        /ACT. COM AMT/TAX/ACT. FEE AMT          /ACT. TYPE"
    YHDR2 = "MATURITY DATE START DATE                 CODE"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.CCY
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"20L")
        ...
        YTOTFD := YFD
        YFD = YR.REC(5)  ;* YM.CUSTOMER
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(6)  ;* YM.LIQ.CD.MOVE
        YFD = FMT(YFD,"2L")
        IF LEN(YFD) > 2 THEN YFD = YFD[1,1]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(7)  ;* YM.AMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            NEXT YNO
        END
        YFD = YR.REC(13)  ;* YM.FEE.AMT.SCHED
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            NEXT YNO
        END
        YFD = YR.REC(15)  ;* YM.CATEGORY
        YFD = FMT(YFD,"4L")
        IF LEN(YFD) > 4 THEN YFD = YFD[1,3]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(16)  ;* YM.CUST.ID
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                 ":YFD
        YFD = YR.REC(17)  ;* YM.TAX.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(20)  ;* YM.COM.TAX.TOTAL
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 2
            ...
            NEXT YNO
        END
        YFD = YR.REC(21)  ;* YM.MAT.DATE.PRINT
        BEGIN CASE
            CASE YFD MATCHES "8N"
        ...
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(27)  ;* YM.NO.PAYMENT.PRINT
        YFD = FMT(YFD,"23L")
        IF LEN(YFD) > 23 THEN YFD = YFD[1,22]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(28)  ;* YM.BLANK.LINE
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(29)  ;* YM.SEPARATOR1
        YR.TOT2(2)<8> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<8> = "" THEN
                YT.DEC2(2)<8> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<8> THEN
                    YT.DEC2(2)<8> = YDEC
                END
            END
        END
        YFD = YR.REC(30)  ;* YM.SEPARATOR2
        YR.TOT2(2)<9> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<9> = "" THEN
                YT.DEC2(2)<9> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<9> THEN
                    YT.DEC2(2)<9> = YDEC
                END
            END
        END
        YFD = YR.REC(31)  ;* YM.SEPARATOR3
        YR.TOT2(2)<10> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<10> = "" THEN
                YT.DEC2(2)<10> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<10> THEN
                    YT.DEC2(2)<10> = YDEC
                END
            END
        END
        YFD = YR.REC(32)  ;* YM.SEPARATOR4
        YR.TOT2(2)<11> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<11> = "" THEN
                YT.DEC2(2)<11> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<11> THEN
                    YT.DEC2(2)<11> = YDEC
                END
            END
        END
        ...
        GOSUB 1000000
*
        IF YKEY.OLD[1,55] <> YKEY[1,55] THEN
            YTOTAL.STATUS = 2
            FOR YAV.TOT = 2 TO YTOTAL.STATUS STEP -1
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.TOT.LIT.PRINT
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"25L")
                IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
                YTOTFD := "                    ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.AMT.TOT
                YR.TOT2(YAV.TOT)<2> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<3>  ;* YM.INT.AMT.TOT
                YR.TOT2(YAV.TOT)<3> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<3>:",")
                    YT.DEC2(YAV.TOT)<3> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<4>  ;* YM.COM.AMT.TOT
                YR.TOT2(YAV.TOT)<4> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<4>:",")
                    YT.DEC2(YAV.TOT)<4> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.FEE.AMT.TOT
                YR.TOT2(YAV.TOT)<5> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<6>  ;* YM.TAX.TOTAL
                YR.TOT2(YAV.TOT)<6> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := "                                                                  ":YFD
                YFD = YR.TOT2(YAV.TOT)<7>  ;* YM.COM.TAX.TOTAL
                YR.TOT2(YAV.TOT)<7> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<8>  ;* YM.SEPARATOR1
                YR.TOT2(YAV.TOT)<8> = ""
                YFD = FMT(YFD,"33L")
                ...
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<9>  ;* YM.SEPARATOR2
                YR.TOT2(YAV.TOT)<9> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<10>  ;* YM.SEPARATOR3
                YR.TOT2(YAV.TOT)<10> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<11>  ;* YM.SEPARATOR4
                YR.TOT2(YAV.TOT)<11> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(33)
        C$RPT.ACCOUNT.NO = YR.REC(34)
