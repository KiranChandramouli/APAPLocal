...
SUBROUTINE RGP.CO.RIGHT.EXPIRY
REM "RGP.CO.RIGHT.EXPIRY",230614-4
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.CO.RIGHT.EXPIRY"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(16); MAT YR.REC = ""
    DIM YR.REC.OLD(14); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "COLLATERAL RIGHT EXPIRY"
    IF YPRINTING THEN
        YHDR = "RGP.CO.RIGHT.EXPIRY ":YHDR
    ...
    END
    YHDR1 = ""
    YHDR2 = " EXPIRY                                                                          VALIDITY"
    YHDR3 = " DATE CUSTOMER                           COLLATERAL CODE                         NO DATE                                 STATUS"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.VALIDITY.DATE
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[3,2]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 9 THEN YFD = YFD[1,8]:"|"
        YTOTFD := " ":YFD
        YCNT = COUNT(YR.REC(13),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(13)<1,1>  ;* YM.NOTES
        YFD = FMT(YFD,"22L")
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(15)
        C$RPT.ACCOUNT.NO = YR.REC(16)
