<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMjEzMDAzMzkwOTpDcDEyNTI6MTY4ODUzNjkwMjEyNTpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjotMjEzMDAzMzkwOTpDcDEyNTI6MTY4NjkxODQyNDEyMjpJVFNTMTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMl9TUDUuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Jun 2023 17:57:04
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGS.MM0003
REM "RGS.MM0003",230614-4
*************************************************************************
...
*************************************************************************
    ...
    DIM YR.REC(8)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.MM0003"
    YOLDFILE = 1
...
* Handle Decision Table
            ...
            YM.BROKER = R.NEW(17)
            IF YM.BROKER <> "" THEN
                YCOMP = "CUSTOMER_2_":YM.BROKER
                YFORFIL = YF.CUSTOMER
                YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 0; GOSUB 9000000
                YM.BROKER = YFOR.FD
            END
            IF (YM.CONF < 1 AND YM.CONF) AND ((YM.BROKER > 0)) THEN
                GOSUB 2000000
            END
        END
*
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.BROKER,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.BROKER
    YM.REFERENCE.NUMBER = ID.NEW
    YR.REC(2) = YM.REFERENCE.NUMBER
    YM.CCY = R.NEW(2)
    YR.REC(3) = YM.CCY
    YM.AMOUNT = R.NEW(3)
    ...
    IF YM.RATE >= 0 THEN
        YTRUE.1 = 1
        YM.PRINT.RATE = R.NEW(10)
    END
    IF NOT(YTRUE.1) THEN
        YM.RATE = R.NEW(10)
        IF YM.RATE < 0 AND YM.RATE THEN
            YTRUE.1 = 1
            YM.PRINT.RATE = "FLOATING"
        END
    END
    IF NOT(YTRUE.1) THEN YM.PRINT.RATE = ""
    YM.RATE1 = YM.PRINT.RATE
    YR.REC(5) = YM.RATE1
    YM.VALUE.DATE = R.NEW(5)
    YR.REC(6) = YM.VALUE.DATE
    YM.MAT.DATE = R.NEW(6)
    YR.REC(7) = YM.MAT.DATE
    YM.TYPE = R.NEW(7)
    IF YM.TYPE <> "" THEN
        YCOMP = "CATEGORY_1_":YM.TYPE
        YFORFIL = YF.CATEGORY
