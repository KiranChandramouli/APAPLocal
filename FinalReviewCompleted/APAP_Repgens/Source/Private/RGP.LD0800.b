<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjotMTE3NjM4MTk5NjpDcDEyNTI6MTY4ODUzNjg5MDgzMTpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjotMTE3NjM4MTk5NjpDcDEyNTI6MTY4NjgyMzExOTE4OTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:28:39
>>>>>>> Stashed changes
=======
* @ValidationCode : MjotMTE3NjM4MTk5NjpDcDEyNTI6MTY4NjgyMzExOTE4OTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:28:39
...
SUBROUTINE RGP.LD0800
REM "RGP.LD0800",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(21); MAT YR.REC = ""
    DIM YR.REC.OLD(19); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "FX REQUIRED IN NEXT SEVEN DAYS"
    IF YPRINTING THEN
        YHDR = "RGP.LD0800 ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "CONTRACT CONTRACT PRINCIPAL.RAT         E/VALUE DATE INTEREST RATE/VALUE DATE    COMMISSION RATE/VALUE DATE FEES RATE   /VALUE DATE"
    YHDR2 = "REFERENCE NO. CURRENCY ACCOUNT NO/CU    RRENCY ACCOUNT NO/CURRENCY               ACCOUNT NO/CURRENCY ACCOUNT N          O/CURRENCY"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.EVENT.DATE.PRINT
        ...
        YTOTFD := YFD
        YFD = YR.REC(3)  ;* YM.CONTRACT.CCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(4)  ;* YM.PRINC.RATE.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "        ":YFD
        YFD = YR.REC(5)  ;* YM.INT.RATE.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "            ":YFD
        YFD = YR.REC(6)  ;* YM.COM.RATE.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "           ":YFD
        YFD = YR.REC(7)  ;* YM.FEE.RATE.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := "             ":YFD
        YFD = YR.REC(8)  ;* YM.PRINC.DATE.PRINT
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
            CASE YFD MATCHES "6N"
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                           ":YFD
        YFD = YR.REC(9)  ;* YM.INT.DATE.PRINT
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "                 ":YFD
        YFD = YR.REC(10)  ;* YM.COM.DATE.PRINT
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "                ":YFD
        YFD = YR.REC(11)  ;* YM.FEE.DATE.PRINT
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[1,4]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "                  ":YFD
        YFD = YR.REC(12)  ;* YM.PRINC.ACCT.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"L##############")
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                           ":YFD
        YFD = YR.REC(13)  ;* YM.INT.ACCT.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"L##############")
        ...
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "              ":YFD
        YFD = YR.REC(14)  ;* YM.COM.ACCT.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"L##############")
        ...
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "             ":YFD
        YFD = YR.REC(15)  ;* YM.FEE.ACCT.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"L##############")
        ...
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "               ":YFD
        YFD = YR.REC(16)  ;* YM.PRINC.CCY.PRINT
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                           ":YFD
        YFD = YR.REC(17)  ;* YM.INT.CCY.PRINT
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "                         ":YFD
        YFD = YR.REC(18)  ;* YM.COM.CCY.PRINT
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "                        ":YFD
        YFD = YR.REC(19)  ;* YM.FEE.CCY.PRINT
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "                          ":YFD
        GOSUB 9000000
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(20)
        C$RPT.ACCOUNT.NO = YR.REC(21)
