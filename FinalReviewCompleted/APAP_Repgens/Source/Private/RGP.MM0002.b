<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjoxNDEzODk2NTIxOkNwMTI1MjoxNjg4NTM2ODkxNzA4OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjoxNDEzODk2NTIxOkNwMTI1MjoxNjg2ODIzODgyMjg2OmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:41:22
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGP.MM0002
REM "RGP.MM0002",230614-4
*************************************************************************
*----------------------------------------------------------------------------
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "Money Market payments made"
    IF YPRINTING THEN
        YHDR = "RGP.MM0002 ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "CCY PRINCIPAL AMOUNT BENEFICIARY         BENEFICIARY BANK 1                      BENEFICIARY BANK 2 REFE                RENCE NO."
    YHDR2 = "                                         BENEFICIARY ADDRESS                     BENEFICIARY ACCOUNT AND ADDRESS"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END CASE
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.MAT.DATE
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.REMITTANCE.CUST
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"35L")
            IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
            YFD = "REMITTANCE ACCOUNT NAME:       ":YFD
            YTOTFD := " ":YFD
        END
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(5)  ;* YM.BENEFICIARY
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(6)  ;* YM.PRINC.BEN.BANK1
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(7)  ;* YM.PRINC.BEN.BANK2
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "       ":YFD
        YFD = YR.REC(8)  ;* YM.PRINC.ADDRESS
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                                                   ":YFD
        YFD = YR.REC(9)  ;* YM.PRINC.BENEF.ACC.ADD.
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "       ":YFD
        YFD = YR.REC(10)  ;* YM.REFERENCE
        YFD = FMT(YFD,"14L")
