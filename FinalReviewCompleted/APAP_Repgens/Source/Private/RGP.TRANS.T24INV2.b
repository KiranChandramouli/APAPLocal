<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjoxNTI2NTA1NzY1OkNwMTI1MjoxNjg4NTM2ODkzMzY5OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:33
=======
* @ValidationCode : MjoxNTI2NTA1NzY1OkNwMTI1MjoxNjg2ODIyNDIxNzg3OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:17:01
>>>>>>> Stashed changes
=======
* @ValidationCode : MjoxNTI2NTA1NzY1OkNwMTI1MjoxNjg2ODIyNDIxNzg3OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:17:01
...
SUBROUTINE RGP.TRANS.T24INV2
REM "RGP.TRANS.T24INV2",230614-4
*************************************************************************
*------------------------------------------------------------------------
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.TRANS.T24INV2"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "INVESTIGATION LISTING OF TODAYS ENTRIES"
    IF YPRINTING THEN
        YHDR = "RGP.TRANS.T24INV2 ":YHDR
    ...
    YHDR1 = "TRANS.REFERENCE  ACCT/CATEG/CRF        LCY AMOUNT EXCH.RATE   CCY          FCY AMOUNT VAL.DATE  T/C DEPT A/O. PRODUCT  CUSTOMER"
    YHDR2 = "===============  ==============        ========== =========   ===          ========== ========  === ==== ==== =======  ========"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "1" DO
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(3); YFD = YR.REC(3)  ;* YM.SORT.REF
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"16L")
        ...
        IF Y1ST.LIN THEN
            ...
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD = YR.REC(4)  ;* YM.TRANS.REF
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        YTOTFD := YFD
        YFD = YR.REC(5)  ;* YM.ACCOUNT
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.AMOUNTLCY
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YFD = FMT(YFD,"19R2,")
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            NEXT YNO
        END
        YFD = YR.REC(10)  ;* YM.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.CCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.FCYAMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
