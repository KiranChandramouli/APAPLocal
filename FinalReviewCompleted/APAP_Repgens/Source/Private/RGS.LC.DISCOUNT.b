* @ValidationCode : MjotMTU0NTE2NTYwMjpDcDEyNTI6MTY4ODU0NTc1MTIyMDpJVFNTOi0xOi0xOjk3MjE6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 13:59:11
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 9721
...
*-----------------------------------------------------------------------------------------------
SUBROUTINE RGS.LC.DISCOUNT
REM "RGS.LC.DISCOUNT",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.LC.DISCOUNT")
    END
*************************************************************************
...
*************************************************************************
    ...
    CLEARSELECT
    EXECUTE "HUSH ON"
    EXECUTE 'SSELECT ':FULL.FNAME:' WITH FLD.':TNO:' LIKE "AC..." OR WITH FLD.':TNO:' LIKE "DP..." AND FLD2.':TNO:' <> ""';* R22 Manual conversion - SELECT changed to SSELECT
    EXECUTE "HUSH OFF"
...
* Handle Decision Table
            ...
            IF YM.SYS.DATE <> "" THEN
                ...
                YM.SYS.DATE = YFOR.FD
            END
            YM.TIME.DATE = FMT(R.NEW(101),"7L"); YM.TIME.DATE = YM.TIME.DATE[1,6]
...
* Year 2000 compliance
            ...
            YKEYFD = FMT(YM.DISP.ISSUE.BNK,"22L")
...
* Year 2000 compliance
            ...
            YM.INT.RATE = R.NEW(13)
            YR.REC(7) = YM.INT.RATE
            YM.DISC.AMT = R.NEW(14)
            YR.REC(8) = YM.DISC.AMT
            YM.SPREAD.RATE = R.NEW(15)
            YR.REC(9) = YM.SPREAD.RATE
            YM.SPREAD.AMT = R.NEW(16)
            YR.REC(10) = YM.SPREAD.AMT
            YM.TOT.DISC = YM.SPREAD.AMT
            YM1.TOT.DISC = YM.TOT.DISC
            YM.TOT.DISC = YM.DISC.AMT
            IF NUM(YM1.TOT.DISC) = NUMERIC THEN IF NUM(YM.TOT.DISC) = NUMERIC THEN
                YM1.TOT.DISC = YM1.TOT.DISC + YM.TOT.DISC
                IF YM1.TOT.DISC = 0 THEN YM1.TOT.DISC = ""
            END
            YM.TOT.DISC = YM1.TOT.DISC
            YR.REC(11) = YM.TOT.DISC
*
        CASE YGROUP = "2"
            YKEY = "2"; MAT YR.REC = ""
            YM.MAT.DATE = R.NEW(7)
