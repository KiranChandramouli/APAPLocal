<<<<<<< Updated upstream
* @ValidationCode : MjotMTg3NDkyNjM3NTpDcDEyNTI6MTY4ODUzNjg5MDk3NjpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjotMTg3NDkyNjM3NTpDcDEyNTI6MTY4NjgyMzc3MjQxNDpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:39:32
>>>>>>> Stashed changes
...
SUBROUTINE RGP.LI.CO.EXCESS
REM "RGP.LI.CO.EXCESS",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "LIMIT/COLLATERAL EXCESS"
    IF YPRINTING THEN
        YHDR = "RGP.LI.CO.EXCESS ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = " LIMIT ID CUSTOMER                       LIMIT TYPE                              CCY TOTAL MAX TOTAL O/S O/L LIMIT       EXCESS"
    YHDR2 = " DATE & AMT LAST EXCESS DATE & AMT LAS  T MOVED"
    YHDR3 = " COLLATERAL RIGHT COLLATERAL C          ODE SECURED AMOUNT"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF LEN(YFD) > 6 THEN YFD = YFD[1,5]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(10)  ;* YM.SHORT.NAME
        YFD = FMT(YFD,"30L")
        IF LEN(YFD) > 30 THEN YFD = YFD[1,29]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.LIMIT.PRODUCT
        YFD = FMT(YFD,"24L")
        IF LEN(YFD) > 24 THEN YFD = YFD[1,23]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.LIMIT.CURRENCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(13)  ;* YM.MAXIMUM.TOTAL
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(17)  ;* YM.DATE.OF.EXCESS
        BEGIN CASE
            CASE YFD MATCHES "8N"
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "   ":YFD
        YFD = YR.REC(18)  ;* YM.AMT.LAST.EXCESS
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(19)  ;* YM.DATE.LAST.MOVED
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[3,2]
            CASE YFD MATCHES "6N"
        ...
        IF LEN(YFD) > 9 THEN YFD = YFD[1,8]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(20)  ;* YM.AMT.LAST.MOVED
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "       ":YFD
        YFD = YR.REC(23)  ;* YM.COLLATERAL.CODE
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(24)  ;* YM.DESCRIPTION
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(25)  ;* YM.SECURED.AMT
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
