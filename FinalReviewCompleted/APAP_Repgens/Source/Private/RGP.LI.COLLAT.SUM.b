<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjoxNTU4MTQwNjQzOkNwMTI1MjoxNjg4NTM2ODkxMDc4OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjoxNTU4MTQwNjQzOkNwMTI1MjoxNjg2ODIyMTA5MzcwOmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:11:49
* @ValidationInfo : Encoding          : Cp1252
...
*----------------------------------------------------------------------------
REM "RGP.LI.COLLAT.SUM",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "LIMIT/COLLATERAL SUMMARY"
    IF YPRINTING THEN
        YHDR = "RGP.LI.COLLAT.SUM ":YHDR
    ...
    END
    YHDR1 = ""
    YHDR2 = "                                         LIMIT MAXIMUM                           O/L TOTAL                               AVAILABLE"
    YHDR3 = " LIMIT ID CUSTOMER                       TYPE CCY TOTAL SECUR                   ED UNSECURED LIMIT O/S                   /EXCESS"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF Y1ST.LIN THEN
            Y1ST.LIN = 0; GOSUB 9000010 ;* R22 Manual Conversion changed GOTO TO GOSUB
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(17)  ;* YM.FIXED.VARIABLE
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(18)  ;* YM.TOTAL.OS
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
