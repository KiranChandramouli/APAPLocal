<<<<<<< Updated upstream
* @ValidationCode : MjoxODQ3NTUwMzEwOkNwMTI1MjoxNjg4NTM2ODkzODk2OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:33
=======
* @ValidationCode : MjoxODQ3NTUwMzEwOkNwMTI1MjoxNjg2ODIzOTU4NDY1OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:42:38
>>>>>>> Stashed changes
...
SUBROUTINE RGS.BALANCE.LIST
REM "RGS.BALANCE.LIST",230614-4
*************************************************************************
*------------------------------------------------------------------------
...
*************************************************************************
    ...
    DIM YR.REC(10)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.BALANCE.LIST"
    YOLDFILE = 1
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.BALANCE.LIST")
    END
*
    YCOM = ID.COMPANY
*
...
*
    YKEY = ""; MAT YR.REC = ""
    YM.ACCT.OFF. = R.NEW(11)
    IF YM.ACCT.OFF. <> "" THEN
        YCOMP = "DEPT.ACCT.OFFICER_2_":YM.ACCT.OFF.
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.ACCT.OFF.,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.ACCT.OFF.
    YM.ACCOUNT = ID.NEW
    YR.REC(2) = YM.ACCOUNT
    YM.CUSTOMER = R.NEW(1)
    IF YM.CUSTOMER <> "" THEN
...
* Year 2000 compliance
    ...
    GOSUB 8000000
    YR.REC(5) = YM.CCY
    YM.CLEARED.BALANCE = R.NEW(24)
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY, YDEC)
    IF YM.CLEARED.BALANCE <> "" THEN
        YM.CLEARED.BALANCE = TRIM(FMT(YM.CLEARED.BALANCE,"19R":YDEC))
    END
    YR.REC(6) = YM.CLEARED.BALANCE
    YM.LEDGER.BALANCE = R.NEW(23)
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY, YDEC)
    IF YM.LEDGER.BALANCE <> "" THEN
        YM.LEDGER.BALANCE = TRIM(FMT(YM.LEDGER.BALANCE,"19R":YDEC))
    END
    YR.REC(7) = YM.LEDGER.BALANCE
    YTRUE.1 = 0
    YM.LAST.DEBIT = R.NEW(37)
    YM.LAST.CREDIT = R.NEW(28)
    IF YM.LAST.CREDIT >= YM.LAST.DEBIT THEN
        YTRUE.1 = 1
        YM.LAST.CREDIT = R.NEW(28)
        YM.PRINT.DATE = YM.LAST.CREDIT
    END
    IF NOT(YTRUE.1) THEN
        YM.LAST.CREDIT = R.NEW(28)
        YM.LAST.DEBIT = R.NEW(37)
        IF YM.LAST.DEBIT > YM.LAST.CREDIT THEN
            YTRUE.1 = 1
            YM.LAST.DEBIT = R.NEW(37)
            YM.PRINT.DATE = YM.LAST.DEBIT
        END
    END
    IF NOT(YTRUE.1) THEN YM.PRINT.DATE = ""
    YM.MVT.DATE = YM.PRINT.DATE
    YR.REC(8) = YM.MVT.DATE
    YM.CATEGORY = R.NEW(2)
    IF YM.CATEGORY <> "" THEN
        ...
        YM.CATEGORY = YFOR.FD
    END
    YR.REC(9) = YM.CATEGORY
    YTRUE.1 = 0
    YM.POST.RESTR. = R.NEW(13)
    IF YM.POST.RESTR. > 0 THEN
        YTRUE.1 = 1
        YM.PRINT.YES = "YES"
    END
    IF NOT(YTRUE.1) THEN YM.PRINT.YES = ""
    YR.REC(10) = YM.PRINT.YES
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
