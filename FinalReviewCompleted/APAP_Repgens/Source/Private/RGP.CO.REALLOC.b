...
SUBROUTINE RGP.CO.REALLOC
REM "RGP.CO.REALLOC",230614-4
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(7); MAT YR.REC = ""
    DIM YR.REC.OLD(5); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "COLLATERAL ALLOCATION"
    IF YPRINTING THEN
        YHDR = "RGP.CO.REALLOC ":YHDR
        YTYPE = "HEADER.80":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = ""
    YHDR2 = "--------------------------------------------------------------------------------"
    YHDR3 = " LINE.NO DESCRIPTION                     AMOUNT TO MOVE"
    YHDR4 = "--------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END CASE
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.REPORT.NAME
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.BLANK
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"1L")
        ...
        YFD = YR.REC(3)  ;* YM.LINE.NO
        YFD = FMT(YFD,"4L")
        IF LEN(YFD) > 4 THEN YFD = YFD[1,3]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(4)  ;* YM.LINE.DESC1
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := "      ":YFD
        YFD = YR.REC(5)  ;* YM.AMT.TO.MOVE
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
...
*
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(6)
        C$RPT.ACCOUNT.NO = YR.REC(7)
        CALL PRINTER.CLOSE(REPORT.ID,PRT.UNIT,"")
    END ELSE
        TEXT = "END OF REPORT"; YEND = 1; GOSUB 9100000 ;*R22 manual code conversion
    END
RETURN
*************************************************************************
1000000:
