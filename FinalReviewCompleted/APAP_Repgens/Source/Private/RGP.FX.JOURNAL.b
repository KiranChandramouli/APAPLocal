<<<<<<< Updated upstream
* @ValidationCode : MjotMTYwMjAzNTM3NDpDcDEyNTI6MTY4ODUzNjg5MDEyNTpJVFNTOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:30
=======
* @ValidationCode : MjotMTYwMjAzNTM3NDpDcDEyNTI6MTY4NjgyMDYzMjMxOTpJVFNTOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 14:47:12
>>>>>>> Stashed changes
...
SUBROUTINE RGP.FX.JOURNAL
REM "RGP.FX.JOURNAL",230614-4
*************************************************************************
...
* T.REMTEXT(23) = "PAGE"
    ...
    YHDR = "FOREX JOURNAL"
    IF YPRINTING THEN
        YHDR = "RGP.FX.JOURNAL ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "                                                  AMOUNT                  AMOU            MONTANT                 MONT            FWG                     SCHW                                      NTANTEIZER                            "
    YHDR2 = "  TRANS.REF     TRANSACTION          DEP  REF.TRANS     TRANSACTION          DEP  TRANS.REF     TRANSAKTION          ABT                                        CCY        FCY         RATE        LCY  DEV       DEVISE       COURS        CH. WHG      BETRAG        KURS        FRA                                                VALUE      CUSTOMER/BANKF         VALEUR     CLIENT/BANQUENKEN      VALUTA     KUNDE/BANK                   "
    YHDR3 = "                                                                                                                  "
    YHDR4 = "                                                                                                                  "
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YFD = YR.REC(1)  ;* YM.TRANSAKT.NR
        YFD = FMT(YFD,"15L")
        IF LEN(YFD) > 15 THEN YFD = YFD[1,14]:"|"
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.TRANSAKT.TEXT
        YFD = FMT(YFD,"20L")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(4)  ;* YM.FREMD
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(5)  ;* YM.FREMDWHRG.BETRAG
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        YTOTFD := "   ":YFD
        YFD = YR.REC(9)  ;* YM.KUNDE.BANK
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(10)  ;* YM.TOTAL.FCCY
...
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.TOTAL.FCCY
                YR.TOT2(YAV.TOT)<1> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                YFD = "TOTAL                      ":YFD
                YTOTFD := "              ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.TOTAL.LCY
                YR.TOT2(YAV.TOT)<2> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",25)
                END ELSE
