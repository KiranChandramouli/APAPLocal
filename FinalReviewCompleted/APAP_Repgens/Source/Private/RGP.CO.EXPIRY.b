<<<<<<< Updated upstream
* @ValidationCode : MjoxNDI3NzY3MDc2OkNwMTI1MjoxNjg4NTM2ODg4OTA3OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 11:31:28
=======
* @ValidationCode : MjoxNDI3NzY3MDc2OkNwMTI1MjoxNjg2ODI5NDc4NTg0OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 17:14:38
>>>>>>> Stashed changes
...
SUBROUTINE RGP.CO.EXPIRY
REM "RGP.CO.EXPIRY",230614-4
*MODIFICATION HISTORY:
*DATE               WHO                       REFERENCE                        DESCRIPTION
*15-06-2023       AJITHKUMAR                  MANUAL R22 CODE CONVERSION       GOSUB to GOSUB
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.CO.EXPIRY"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "COLLATERAL EXPIRY"
    IF YPRINTING THEN
        YHDR = "RGP.CO.EXPIRY ":YHDR
    ...
    END
    YHDR1 = ""
    YHDR2 = " EXPIRY                                  RIGHT                                   NOMINAL EXECUTION 3RD PARTY"
    YHDR3 = " DATE CUSTOMER                           COLLATERAL TYPE & NO CCY                VALUE VALUE VALUE DESCRIP              TION STS"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(9)  ;* YM.COLLATERAL.TYPE
        IF YFD = "" THEN
            YFD = STR(" ",3)
        END ELSE
            YFD = FMT(YFD,"R###")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.COLLATERAL.RIGHT.SEQ
        IF YFD = "" THEN
            YFD = STR(" ",2)
        END ELSE
            YFD = FMT(YFD,"R##")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.COLLATERAL.SEQ
        IF YFD = "" THEN
            YFD = STR(" ",4)
        END ELSE
            YFD = FMT(YFD,"R####")
        ...
        IF LEN(YFD) > 4 THEN YFD = YFD[1,3]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(13)  ;* YM.CURRENCY
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(14)  ;* YM.NOMINAL.VALUE
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(15)  ;* YM.EXECUTION.VALUE
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(16)  ;* YM.THIRD.PARTY.VALUE
        IF YFD = "" THEN
            YFD = STR(" ",11)
        END ELSE
            YFD = FMT(YFD,"11R0,")
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(17)  ;* YM.DESCRIPTION1
        YFD = FMT(YFD,"15L")
        IF LEN(YFD) > 15 THEN YFD = YFD[1,14]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(18)  ;* YM.STATUS
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        GOSUB 9000000
