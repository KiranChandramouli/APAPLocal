<<<<<<< Updated upstream
<<<<<<< Updated upstream
* @ValidationCode : MjoxOTg4OTcyNjU0OkNwMTI1MjoxNjg4NTM2ODkxNDM4OklUU1M6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
=======
>>>>>>> Stashed changes
* @ValidationCode : MjoxOTg4OTcyNjU0OkNwMTI1MjoxNjg2ODIxMjk3MzE4OmhhaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 14:58:17
* @ValidationInfo : Encoding          : Cp1252
...
SUBROUTINE RGP.LST.FID.CLIENTS
*----------------------------------------------------------------------------
...
*----------------------------------------------------------------------------
REM "RGP.LST.FID.CLIENTS",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "CLI   ACCOUNT          CCY       PRINCIPAL    INT-%        INT-AMT   COM-%     COM-AMT     NET-AMOUNT   FEES   START      MATURITY"
    YHDR2 = "BANK       CONTRACT-NO"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.CLIENT1
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"10L")
            IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
            YFD = "FIDUCIARY PLACEMENTS FOR CLIENT NO ":YFD
            YTOTFD := YFD
        END
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.RATE
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(7)  ;* YM.INT.AMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",10)
        END ELSE
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            YFD = FMT(YFD,"10R":YDEC:",")
            IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
        END
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(8)  ;* YM.COMM.RATE
        YFD = FMT(YFD,"5L")
        IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(9)  ;* YM.COM.AMT
        IF YFD = "" THEN
            YFD = STR(" ",8)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.FEES
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(12)  ;* YM.START.DATE
        BEGIN CASE
            CASE YFD MATCHES "8N"
        ...
        IF LEN(YFD) > 9 THEN YFD = YFD[1,8]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(14)  ;* YM.PLAC.BANK
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(15)  ;* YM.CONTR.NO
        IF YFD = "" THEN
            YFD = STR(" ",15)
        END ELSE
            YFD = FMT(YFD,"L##.##.###.#####")
