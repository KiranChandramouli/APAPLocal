* @ValidationCode : MjotMzcwNzA1MzAxOkNwMTI1MjoxNjgzODkyNjI1NDE4OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
...
*   CALL REDO.CONVERT.ACCOUNT(VAR.AC.ID,Y.ARR.ID,OUT.ID,ERR.TEXT) ;*MANUAL R22 CODE CONVERSION
    CALL APAP.TAM.redoConvertAccount(VAR.AC.ID,Y.ARR.ID,OUT.ID,ERR.TEXT) ;*MANUAL R22 CODE CONVERSION
    VAR.AA.ID = OUT.ID
...
*   CALL REDO.GET.NEXT.PAYMENT.AMOUNT(VAR.AA.ID,TODAY,Y.NEXT.PAY.AMT)
    CALL APAP.TAM.redoGetNextPaymentAmount(VAR.AA.ID,TODAY,Y.NEXT.PAY.AMT) ;*MANUAL R22 CODE CONVERSION
...
*    CALL REDO.FORMAT.MESSAGE(VAR.AC.ID,Y.ORD.MESSAGE,Y.ADD.VALUE,Y.FMT.MSG.ARCIB)
    CALL APAP.TAM.redoFormatMessage(VAR.AC.ID,Y.ORD.MESSAGE,Y.ADD.VALUE,Y.FMT.MSG.ARCIB) ;*MANUAL R22 CODE CONVERSION
