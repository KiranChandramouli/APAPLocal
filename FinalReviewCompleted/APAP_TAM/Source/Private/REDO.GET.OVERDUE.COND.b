...
*    CALL REDO.CONVERT.ACCOUNT(Y.ACC.NO,IN.ARR.ID,Y.AA.ID,ERR.TEXT)
    APAP.TAM.redoConvertAccount(Y.ACC.NO,IN.ARR.ID,Y.AA.ID,ERR.TEXT) ;* R22 Manual Conversion
    IF Y.AA.ID ELSE
        O.DATA = ''
        RETURN
    END
...
*    CALL REDO.CRR.GET.CONDITIONS(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG)
    APAP.AA.redoCrrGetConditions(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG);* R22 Manual Converison
