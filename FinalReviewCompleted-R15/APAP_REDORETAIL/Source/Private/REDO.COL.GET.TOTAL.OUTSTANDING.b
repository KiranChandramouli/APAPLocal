* @ValidationCode : MjotMTU3NDkwOTg3MDpDcDEyNTI6MTY4NDg0OTAxODY3NDpJVFNTOi0xOi0xOi01ODoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 23 May 2023 19:06:58
...
*-----------------------------------------------------------------
    ...
    Y.ARR.ACC.ID  = ''
    Y.TOTAL.AMT   = ''
*APAP.TAM.REDO.CONVERT.ACCOUNT(IN.ACC.ID,ARR.ID,Y.ARR.ACC.ID,ERR.TEXT)
    APAP.TAM.redoConvertAccount(IN.ACC.ID,ARR.ID,Y.ARR.ACC.ID,ERR.TEXT);* R22 Manual conversion
...
* Here Overdue property aging status are obtained
    ...
    R.CONDITION.OVERDUE = ''
    ERR.MSG = ''
*APAP.AA.REDO.CRR.GET.CONDITIONS(ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.OVERDUE,ERR.MSG)
...
* Here we will get the account property Balances
    ...
    Y.ACC.BAL = 0
    Y.ACCOUNT.PROPERTY = ''
    APAP.TAM.redoGetPropertyName(ARR.ID,'ACCOUNT',R.OUT.AA.RECORD,Y.ACCOUNT.PROPERTY,OUT.ERR)
...
* Here we will get the Interest property Balances
    ...
    Y.INT.BALANCE = 0
    Y.INT.PROPERTY = ''
    APAP.TAM.redoGetPropertyName(ARR.ID,'INTEREST',R.OUT.AA.RECORD,Y.INT.PROPERTY,OUT.ERR)
...
* Here we will get the Charge property Balances
    ...
    Y.CHRG.BALANCE = 0
    Y.CHRG.PROPERTY = ''
    APAP.TAM.redoGetPropertyName(ARR.ID,'CHARGE',R.OUT.AA.RECORD,Y.CHRG.PROPERTY,OUT.ERR)
...
*-----------------------------------------------------------------
    ...
    WHILE Y.LOOP1 LE Y.PROPERTY.CNT
        ...
        WHILE Y.LOOP2 LE Y.BALANCE.CNT
            ...
            BALANCE.AMOUNT=''
****PACS00523653****
*APAP.AA.REDO.AA.GET.ECB.BALANCE.AMOUNT(Y.ARR.ACC.ID,BALANCE.TO.CHECK,TODAY,BALANCE.AMOUNT,RET.ERROR)
            APAP.AA.redoAaGetEcbBalanceAmount(Y.ARR.ACC.ID,BALANCE.TO.CHECK,TODAY,BALANCE.AMOUNT,RET.ERROR);* R22 Manual conversion
****PACS00523653****
