* @ValidationCode : MjoxNjgyOTE5OTczOkNwMTI1MjoxNjg4NTU2NjQwNzEwOklUU1M6LTE6LTE6NjA1NjoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 17:00:40
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 6056
...
*04-07-2023 Narmadha V  Manual R22 Conversion  FM to @FM, VM to @VM, SM to @SM, SELECT to SSELECT
*-----------------------------------------------------------------------------
SUBROUTINE RGS.LC.COLL.DUE
REM "RGS.LC.COLL.DUE",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.LC.COLL.DUE")
    END
*************************************************************************
...
*************************************************************************
    ...
    DIM YR.REC(6)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LC.COLL.DUE"
    YOLDFILE = 1
...
*************************************************************************
    ...
    CLEARSELECT
    EXECUTE "HUSH ON"
    EXECUTE 'SSELECT ':FULL.FNAME:' WITH FLD.':TNO:' = "CO"' ;* R22 Manual conversion - SELECT changed to SSELECT
    EXECUTE "HUSH OFF"
...
*
    YKEY = ""; MAT YR.REC = ""
    YTRUE.1 = 0
    YM.CTRY.CODE = FMT(R.NEW(2),"2L"); YM.CTRY.CODE = YM.CTRY.CODE[1,2]
    YM.COUNTRY.CODE = YM.CTRY.CODE
    YM3.GOSUB = YM.COUNTRY.CODE
    YM.DIS = "SYSTEM"
    IF YM.DIS <> "" THEN
    ...
    YM4.GOSUB = YM.DISPLACEMENT
    YM.RETURN.DATE = ""
    YM5.GOSUB = YM.RETURN.DATE
    YM.ERROR.CODE = ""
    YM6.GOSUB = YM.ERROR.CODE
    YM.RE.DISPLACEMENT = ""
    YM7.GOSUB = YM.RE.DISPLACEMENT
    CALL @RP.NEXT.WORKING.DAY (YM3.GOSUB, YM4.GOSUB, YM5.GOSUB, YM6.GOSUB, YM7.GOSUB)
    YM.COUNTRY.CODE = YM3.GOSUB
    YM.DISPLACEMENT = YM4.GOSUB
    YM.RETURN.DATE = YM5.GOSUB
    YM.ERROR.CODE = YM6.GOSUB
    YM.RE.DISPLACEMENT = YM7.GOSUB
    YM.MAT = R.NEW(7)
    IF YM.MAT <= YM.RETURN.DATE THEN
        YTRUE.1 = 1
        YM.MAT.DATE = R.NEW(7)
    END
    IF NOT(YTRUE.1) THEN YM.MAT.DATE = ""
...
* Year 2000 compliance
    ...
    IF YM.MAT <= YM.RETURN.DATE THEN
        ...
        IF YM.CUST.LINK <> "" THEN
            YCOMP = "LETTER.OF.CREDIT_18_":YM.CUST.LINK
