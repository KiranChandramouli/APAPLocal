* @ValidationCode : MjozNDA5ODcyMTY6Q3AxMjUyOjE2ODY4MzM3OTU0NDg6SVRTUzotMTotMTo1NDk2OjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 15 Jun 2023 18:26:35
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 5496
...
*15-06-2023    VICTORIA S          R22 MANUAL CONVERSION   SELECT changed to SSELECT
*----------------------------------------------------------------------------------------
$PACKAGE APAP.Repgens
SUBROUTINE RGS.PL.JOURNAL
REM "RGS.PL.JOURNAL",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    OPEN "", "VOC" TO YF.VOC ELSE
        ...
        CALL FATAL.ERROR ("RGS.PL.JOURNAL")
    END
*************************************************************************
    YT.SMS.COMP = "DO0010001"
    CONVERT " " TO @FM IN YT.SMS.COMP
    YT.SMS.FILE = "PL.YR.END.ENTRY"
...
*************************************************************************
    ...
    DIM YR.REC(6)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.PL.JOURNAL"
    YOLDFILE = 1
...
*
    ...
    YCOMTBL<1> = "DO0010001"
    DIM R.COMPANY.SAVE(EB.COM.AUDIT.DATE.TIME)
    MAT R.COMPANY.SAVE = MAT R.COMPANY
    LOOP UNTIL YCOMTBL = "" DO
        YCOM = YCOMTBL<1>; DEL YCOMTBL<1>
        T.OPF = ""; OPF.NO = 0
* Deleting table of opened files is important
        MATREAD R.COMPANY FROM YF.COMPANY, YCOM ELSE
            TEXT = "MISSING FILE=F.COMPANY ID=":YCOM
            CALL FATAL.ERROR ("RGS.PL.JOURNAL"); RETURN
        END
        CALL LOAD.COMPANY(YCOM)
*
        YT.SMS = ""
        YT.SMS.FILE = "PL.YR.END.ENTRY"
        YT.SMS.FILE<-1> = "CATEGORY"
...
*************************************************************************
        YFILE = "PL.YR.END.ENTRY"
        FULL.FNAME = "F.PL.YR.END.ENTRY"; YF.PL.YR.END.ENTRY = ""
        ...
        CALL OPF (FULL.FNAME, YF.PL.YR.END.ENTRY)
        fldName = '' ; dataType = '' ; errMsg = ''
        CALL FIELD.NUMBERS.TO.NAMES('3',SS.REC,fldName,dataType,errMsg)
        CLEARSELECT
        YSEL.A = ""
        EXECUTE "HUSH ON"
        EXECUTE 'SSELECT ':FULL.FNAME:' WITH ':fldName:' <> "0"':YSEL.A;* R22 Manual conversion - SELECT changed to SSELECT
        EXECUTE "HUSH OFF"
        CALL EB.READLIST('', YID.LIST, '', '', '')
        LOOP
            REMOVE ID.NEW FROM YID.LIST SETTING YDELIM
        WHILE ID.NEW:YDELIM
            MATREAD R.NEW FROM YF.PL.YR.END.ENTRY, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
            IF T.PWD THEN
...
*
        REPEAT
    REPEAT
    MAT R.COMPANY = MAT R.COMPANY.SAVE
    T.OPF=""; OPF.NO=0
    IF YKEYNO THEN
        YR.REC(6)  := @FM
...
*
    YKEY = ""; MAT YR.REC = ""
    YM.BOOK.DATE = R.NEW(25)
    YKEYFD = YM.BOOK.DATE
    YKEYFD = FMT(YM.BOOK.DATE,"11L")
    IF LEN(YKEYFD) > 11 THEN YKEYFD = YKEYFD[1,10]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.BOOK.DATE
    YM.CATEGORY = FMT(R.NEW(6)<1,1>,"8L"); YM.CATEGORY = YM.CATEGORY[4,5]
    YKEYFD = YM.CATEGORY
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.CATEGORY,"7L")


    IF LEN(YKEYFD) > 7 THEN YKEYFD = YKEYFD[1,6]:"|"
    GOSUB 8000000
    YR.REC(2) = YM.CATEGORY
    YM.DESCRIPT = YM.CATEGORY
    IF YM.DESCRIPT <> "" THEN
        YCOMP = "CATEGORY_1_":YM.DESCRIPT
        YFORFIL = YF.CATEGORY
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.DESCRIPT,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(3) = YM.DESCRIPT
    YM.AMOUNT = R.NEW(3)
    YKEYFD = FMT(YM.AMOUNT,"19R2")
    IF YM.AMOUNT[1,1] = "-" THEN YKEYFD[1,1] = "A" ELSE YKEYFD[1,1] = "B"
