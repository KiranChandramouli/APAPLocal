...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "LST.FX.CNT.OPEN"
    IF YPRINTING THEN
        YHDR = "RGP.LST.FX.CNT.OPEN ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "MATURITY      CCY       AMOUNT BOUGHT    FW-RATE     SPOT-RATE   CCY         AMOUNT SOLD        COUNTERPARTY   REFERENCE"
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(5)  ;* YM.FORWARD.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(6)  ;* YM.SPOT.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(7)  ;* YM.CCY2
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(8)  ;* YM.AMOUNT2
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
