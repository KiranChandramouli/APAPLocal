* @ValidationCode : MjotMTI2MDQzNzk5NjpDcDEyNTI6MTY4NjgyMTcyMjIyOTpIYXJpc2h2aWtyYW1DOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:05:22
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : HarishvikramC
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : N/A
* @ValidationInfo : Coverage          : N/A
* @ValidationInfo : Strict flag       : true
* @ValidationInfo : Bypass GateKeeper : false
* @ValidationInfo : Compiler Version  : R21_AMR.0
* @ValidationInfo : Copyright Temenos Headquarters SA 1993-2021. All rights reserved.
$PACKAGE APAP.Repgens
*----------------------------------------------------------------------------------------
*MODIFICATION HISTORY:
*
* DATE              WHO                REFERENCE                 DESCRIPTION
* 15-JUNE-2023      Harishvikram C   Manual R22 conversion      GOTO Changed to GOSUB
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.POS.AUDIT.SUM
REM "RGP.POS.AUDIT.SUM",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
    $INSERT I_SCREEN.VARIABLES
    $INSERT I_F.COMPANY
    $INSERT I_F.LANGUAGE
    $INSERT I_F.USER
*************************************************************************
    REPORT.ID = "RG.POS.AUDIT.SUM"
    PRT.UNIT = 0
    IF V$DISPLAY = "D" THEN YPRINTING = 0 ELSE YPRINTING = 1
    IF NOT(YPRINTING) THEN IF NOT(S.COL132.ON) THEN
        TEXT = "TERMINAL CANNOT DISPLAY 132 COLUMS"
        CALL REM; RETURN  ;* end of pgm
    END
*************************************************************************
    YT.SMS.COMP = ID.COMPANY
    YT.SMS.FILE = "POSITION.LWORK.DAY"
    YT.SMS.FILE<-1> = "STMT.ENTRY"
    YT.SMS.FILE<-1> = "RE.CONSOL.SPEC.ENTRY"
    YT.SMS.FILE<-1> = "FOREX"
    YT.SMS.FILE<-1> = "DATES"
    YT.SMS.FILE<-1> = "DEALER.DESK"
    YT.SMS.FILE<-1> = "CURRENCY"
    YT.SMS.FILE<-1> = "ACCT.ENT.TODAY"
    YT.SMS.FILE<-1> = "RE.SPEC.ENT.TODAY"
    YCOUNT = COUNT(YT.SMS.FILE,@FM)+1
    LOOP
        YID.COMP = YT.SMS.COMP<1>; DEL YT.SMS.COMP<1>
        FOR YAF = 1 TO YCOUNT
            YSMS.FILE = YT.SMS.FILE<YAF>
            YPW.OK = 0; Y = 1; X = 1
            LOOP WHILE X DO
                LOCATE YSMS.FILE IN R.USER<EB.USE.APPLICATION,Y> SETTING X ELSE X = 0
                IF X THEN
                    GOSUB 9300000
                    IF X THEN
                        IF R.USER<EB.USE.FUNCTION,X> = "N" THEN
                            IF R.USER<EB.USE.VERSION,X> THEN
                                Y += 1; X = 1
                            END ELSE
                                X = 0; YPW.OK = "NO"
                            END
                        END ELSE
                            Y = X+1; YPW.OK = 1; X = 0
                        END
                    END ELSE
                        Y += 1; X = 1
                    END
                END
* update pointers to definitions in version record
            REPEAT
            IF NOT(YPW.OK) THEN
* when no password for selected pgm ask for a global password
                LOCATE "ALL.PG" IN R.USER<EB.USE.APPLICATION,1> SETTING X ELSE X = 0
                GOSUB 9300000
* ask for a password for all programs
                IF NOT(X) THEN
                    YPGM.TYPE = ""; CALL PGM.TABLE (YSMS.FILE, YPGM.TYPE)
                    Y = "ALL.PG.":YPGM.TYPE[1,1]
                    LOCATE Y IN R.USER<EB.USE.APPLICATION,1> SETTING X ELSE X = 0
                    GOSUB 9300000
* ask for a password for the type of the program
                END
                YPW.OK = 1
            END
            IF NOT(YPW.OK) OR YPW.OK = "NO" THEN
                MSG = "NO PASSWORD FOR FILE=":YSMS.FILE
                MSG<2> = "SECURITY"
                CALL PRO("NO PASSWORD FOR FILE=":YSMS.FILE)
                TEXT = "SECURITY VIOLATION"
                CALL REM  ; RETURN  ;* end of pgm
            END
        NEXT YAF
    WHILE YT.SMS.COMP REPEAT
*************************************************************************
    F.LANGUAGE=""; CALL OPF("F.LANGUAGE",F.LANGUAGE)
    READV AMOUNT.FORMAT FROM F.LANGUAGE,LNGG,EB.LAN.AMOUNT.FORMAT ELSE AMOUNT.FORMAT=""
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.POS.AUDIT.SUM"
    EXECUTE "HUSH ON"
    EXECUTE "SSELECT ":YFILE
    EXECUTE "HUSH OFF"
    CALL EB.READLIST('', YID.LIST, '', '', '')
    IF YPRINTING THEN
        CALL PRINTER.ON(REPORT.ID,PRT.UNIT); YBLOCKNO = 0; YWRITNO = 0; COMI = C.F
    END ELSE
        PRINT S.COL132.ON:
        PRINT @(0,L1ST-1):STR("-",132):
        YEND = 0; Y = 1; LASTP = 1; L = L1ST; PRINT @(0,L):
        FOR LL = L TO 19; PRINT S.CLEAR.EOL; NEXT LL
        YTEXT = T.REMTEXT(23)
* T.REMTEXT(23) = "PAGE"
        PRINT @(0,20):STR("-",132):@(63+LEN(YTEXT),21):S.HALF.INTENSITY.OFF:@(63,21):S.HALF.INTENSITY.ON:YTEXT:S.HALF.INTENSITY.OFF:
        YT.PAGE = ""; T.CONTROLWORD = C.U:@FM:C.B:@FM:C.F:@FM:C.E:@FM:C.V:@FM:C.W
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(18); MAT YR.REC = ""
    DIM YR.REC.OLD(16); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "SUMMARY REPORT TOTALS ONLY"
    IF YPRINTING THEN
        YHDR = "RGP.POS.AUDIT.SUM ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "SUMMARY REPORT TOTALS ONLY"
    YHDR2 = "                                         TODAYS MOVEMENTS                        END OF CURRENT DAY PRO                 FIT AND LOSS"
    YHDR3 = "                                         ----------------                        ------------------ ---                 ------------"
    YHDR4 = " CURRENC                                Y.AMOUNT LOCAL.EQUIVALENT CURRENC       Y.AMOUNT LOCAL.EQUIVALENT LOCA          L.EQUIVALENT"
    IF YPRINTING THEN YSTML = "'L'" ELSE YSTML = ""
    IF YHDR1 <> "" OR YHDR2 <> "" OR YHDR3 <> "" OR YHDR4 <> "" THEN
        YTYPE<3> = YTYPE<3>:YSTML
        IF YHDR1 <> "" THEN YTYPE<4> = YHDR1:YSTML
        IF YHDR2 <> "" THEN YTYPE<5> = YHDR2:YSTML
        IF YHDR3 <> "" THEN YTYPE<6> = YHDR3:YSTML
        IF YHDR4 <> "" THEN YTYPE<7> = YHDR4:YSTML
    END
    IF YPRINTING THEN
        HEAD.SETTING = YTYPE<1>:YTYPE<2>:YTYPE<3>:YTYPE<4>:YTYPE<5>:YTYPE<6>:YTYPE<7>:YTYPE<8>:YTYPE<9>:YTYPE<10>:YTYPE<11>:YTYPE<12>
        IF HEAD.SETTING = "" THEN
            HEAD.SETTING = " "
        END
        HEADING HEAD.SETTING
    END ELSE
        PRINT @(25+LEN(SCREEN.TITLE)+LEN(PGM.VERSION),L1ST-4):S.CLEAR.EOL:YTYPE<1>:
        IF YTYPE<5> = "" THEN YTYPE<5> = YTYPE<4>; YTYPE<4> = ""
        PRINT @(0,L1ST-3):S.CLEAR.EOL:YTYPE<4>:
        PRINT @(0,L1ST-2):S.CLEAR.EOL:YTYPE<5>:
        L = L1ST; PRINT @(0,L):
        IF YTYPE<6> <> "" THEN
            YT.PAGE<P,L> = YTYPE<6>; L += 2; PRINT YTYPE<6>:@(0,L):
        END
        IF YTYPE<7> <> "" THEN
            YT.PAGE<P,L> = YTYPE<7>; L += 2; PRINT YTYPE<7>:@(0,L):
        END
        IF YTYPE<8> <> "" THEN
            YT.PAGE<P,L> = YTYPE<8>; L += 2; PRINT YTYPE<6>:@(0,L):
        END
        IF YTYPE<9> <> "" OR YTYPE<10> <> "" OR YTYPE<11> <> "" OR YTYPE<12> <> "" THEN
            IF YTYPE<9> <> "" THEN
                YT.PAGE<P,L> = YTYPE<9>; L += 1; PRINT YTYPE<9>:@(0,L):
            END
            IF YTYPE<10> <> "" THEN
                YT.PAGE<P,L> = YTYPE<10>; L += 1; PRINT YTYPE<10>:
            END
            IF YTYPE<11> <> "" THEN
                YT.PAGE<P,L> = YTYPE<11>; L += 1; PRINT YTYPE<11>:
            END
            IF YTYPE<12> <> "" THEN
                YT.PAGE<P,L> = YTYPE<12>; L += 1; PRINT YTYPE<12>:
            END
            L += 1; PRINT @(0,L):
        END
    END
    YTYPE = ""
*------------------------------------------------------------------------
    MAT YR.REC.OLD = "_"
    DIM YR.TOT2(4); MAT YR.TOT2 = ""
    DIM YT.DEC2(4); MAT YT.DEC2 = ""
    YKEYFD = ""; Y1ST.LIN = 1
    LOOP WHILE YKEYFD = "" DO
        YHEADER.DISPLAY = 0; YHEADER.STATUS = ""
        BEGIN CASE
            CASE YR.REC.OLD(1) <> YR.REC(1); YHEADER.STATUS = 1
            CASE YR.REC.OLD(2) <> YR.REC(2); YHEADER.STATUS = 2
            CASE YR.REC.OLD(3) <> YR.REC(3); YHEADER.STATUS = 3
            CASE YR.REC.OLD(4) <> YR.REC(4); YHEADER.STATUS = 4
            CASE YR.REC.OLD(5) <> YR.REC(5); YHEADER.STATUS = 5
        END CASE
        IF YHEADER.STATUS THEN
            GOSUB 9000010; Y1ST.LIN = 1
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.CURRENCY
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"3L")
            IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
            YFD = "CURRENCY : ":YFD
            YTOTFD := YFD
        END ELSE
            IF YHEADER.STATUS > 1 THEN
                YFD = "              "
                YTOTFD := YFD
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.DEPT.ACCT
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            IF YFD = "" THEN
                YFD = STR(" ",4)
            END ELSE
                YFD = FMT(YFD,"R####")
            END
            IF LEN(YFD) > 4 THEN YFD = YFD[1,3]:"|"
            YFD = "DEPARTMENT : ":YFD
            YTOTFD := "          ":YFD
        END ELSE
            IF YHEADER.STATUS > 2 THEN
                YFD = "                 "
                YTOTFD := "          ":YFD
            END
        END
        YFD.OLD = YR.REC.OLD(3); YFD = YR.REC(3)  ;* YM.DEAL.DESK
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"2L")
            IF LEN(YFD) > 2 THEN YFD = YFD[1,1]:"|"
            YFD = "DEALER DESK : ":YFD
            YTOTFD := "        ":YFD
        END ELSE
            IF YHEADER.STATUS > 3 THEN
                YFD = "                "
                YTOTFD := "        ":YFD
            END
        END
        YFD.OLD = YR.REC.OLD(4); YFD = YR.REC(4)  ;* YM.REF3
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"8L")
            IF LEN(YFD) > 8 THEN YFD = YFD[1,7]:"|"
            YFD = "POSITION : ":YFD
            YTOTFD := "         ":YFD
        END ELSE
            IF YHEADER.STATUS > 4 THEN
                YFD = "                   "
                YTOTFD := "         ":YFD
            END
        END
        YFD.OLD = YR.REC.OLD(5); YFD = YR.REC(5)  ;* YM.MID.RATE
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"10L")
            IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
            YFD = "MID.RATE : ":YFD
            YTOTFD := "      ":YFD
        END
        IF YHEADER.DISPLAY THEN
            GOSUB 9000010
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YCOUNT.LIN = 1; YCOUNT.AS.LIN = 1
        YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
        YFD = YR.REC(6)  ;* YM.FOOT.ONE
        YR.TOT2(4)<1> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<1> = "" THEN
                YT.DEC2(2)<1> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<1> THEN
                    YT.DEC2(2)<1> = YDEC
                END
            END
        END
        YFD = YR.REC(7)  ;* YM.FOOT.TWO
        YR.TOT2(4)<2> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<2> = "" THEN
                YT.DEC2(2)<2> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<2> THEN
                    YT.DEC2(2)<2> = YDEC
                END
            END
        END
        YFD = YR.REC(8)  ;* YM.FOOT.THREE
        YR.TOT2(4)<3> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<3> = "" THEN
                YT.DEC2(2)<3> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<3> THEN
                    YT.DEC2(2)<3> = YDEC
                END
            END
        END
        YFD = YR.REC(9)  ;* YM.FOOT.FOUR
        YR.TOT2(4)<4> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<4> = "" THEN
                YT.DEC2(2)<4> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<4> THEN
                    YT.DEC2(2)<4> = YDEC
                END
            END
        END
        YFD = YR.REC(10)  ;* YM.FOOT.FIVE
        YR.TOT2(4)<5> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<5> = "" THEN
                YT.DEC2(2)<5> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<5> THEN
                    YT.DEC2(2)<5> = YDEC
                END
            END
        END
        YFD = YR.REC(11)  ;* YM.FOOT.TEXT
        YR.TOT2(4)<6> = YFD
        IF NUM(YFD) = NUMERIC THEN
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            IF YT.DEC2(2)<6> = "" THEN
                YT.DEC2(2)<6> = YDEC
            END ELSE
                IF YDEC > YT.DEC2(2)<6> THEN
                    YT.DEC2(2)<6> = YDEC
                END
            END
        END
        YFD = YR.REC(12)  ;* YM.CCY.ENTRY.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<7> = YR.TOT2(YNO)<7> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<7> = "" THEN
                    YT.DEC2(YNO)<7> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<7> THEN
                        YT.DEC2(YNO)<7> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(13)  ;* YM.LCY.ENTRY.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<8> = YR.TOT2(YNO)<8> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<8> = "" THEN
                    YT.DEC2(YNO)<8> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<8> THEN
                        YT.DEC2(YNO)<8> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(14)  ;* YM.CCY.CLOSE.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<9> = YR.TOT2(YNO)<9> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<9> = "" THEN
                    YT.DEC2(YNO)<9> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<9> THEN
                        YT.DEC2(YNO)<9> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(15)  ;* YM.LCY.CLOSE.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<10> = YR.TOT2(YNO)<10> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<10> = "" THEN
                    YT.DEC2(YNO)<10> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<10> THEN
                        YT.DEC2(YNO)<10> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(16)  ;* YM.LCY.PANDL.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<11> = YR.TOT2(YNO)<11> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<11> = "" THEN
                    YT.DEC2(YNO)<11> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<11> THEN
                        YT.DEC2(YNO)<11> = YDEC
                    END
                END
            NEXT YNO
        END
        GOSUB 1000000
*
        IF YKEY.OLD[1,24] <> YKEY[1,24] THEN
            YTOTAL.STATUS = 4
            IF YKEY.OLD[1,10] <> YKEY[1,10] THEN
                YTOTAL.STATUS = 3
                IF YKEY.OLD[1,6] <> YKEY[1,6] THEN
                    YTOTAL.STATUS = 2
                END
            END
            Y1ST.LIN = 1
            FOR YAV.TOT = 4 TO YTOTAL.STATUS STEP -1
                GOSUB 9000010
                IF COMI = C.U THEN RETURN  ;* end of pgm
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.FOOT.ONE
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := "                             ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.FOOT.TWO
                YR.TOT2(YAV.TOT)<2> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<3>  ;* YM.FOOT.THREE
                YR.TOT2(YAV.TOT)<3> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<4>  ;* YM.FOOT.FOUR
                YR.TOT2(YAV.TOT)<4> = ""
                YFD = FMT(YFD,"21L")
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.FOOT.FIVE
                YR.TOT2(YAV.TOT)<5> = ""
                YFD = FMT(YFD,"21L")
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<6>  ;* YM.FOOT.TEXT
                YR.TOT2(YAV.TOT)<6> = ""
                YFD = FMT(YFD,"8L")
                IF LEN(YFD) > 8 THEN YFD = YFD[1,7]:"|"
                GOSUB 9000000
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := "          ":YFD
                YFD = YR.TOT2(YAV.TOT)<7>  ;* YM.CCY.ENTRY.TOT
                YR.TOT2(YAV.TOT)<7> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<7>:",")
                    YT.DEC2(YAV.TOT)<7> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YFD = "TOTALS : ":YFD
                YTOTFD := "  ":YFD
                YFD = YR.TOT2(YAV.TOT)<8>  ;* YM.LCY.ENTRY.TOT
                YR.TOT2(YAV.TOT)<8> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R2,")
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<9>  ;* YM.CCY.CLOSE.TOT
                YR.TOT2(YAV.TOT)<9> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<9>:",")
                    YT.DEC2(YAV.TOT)<9> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<10>  ;* YM.LCY.CLOSE.TOT
                YR.TOT2(YAV.TOT)<10> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",21)
                END ELSE
                    YFD = FMT(YFD,"21R4,")
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<11>  ;* YM.LCY.PANDL.TOT
                YR.TOT2(YAV.TOT)<11> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",21)
                END ELSE
                    YFD = FMT(YFD,"21R4,")
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                GOSUB 9000000
                IF COMI = C.U THEN RETURN  ;* end of pgm
                GOSUB 9000010
                IF COMI = C.U THEN RETURN  ;* end of pgm
            NEXT YAV.TOT
        END
    REPEAT
    YTEXT = "*** END OF REPORT ***"
    IF LNGG <> 1 THEN CALL TXT ( YTEXT )
    IF YPRINTING THEN
        PRINT
    END ELSE
        IF L < 19 THEN
            GOSUB 9000000
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
    END
    PRINT YTEXT
    IF NOT(YPRINTING) THEN YT.PAGE<P,L> = YTEXT; L += 1; PRINT @(0,L):
    IF YPRINTING THEN
        CALL PRINTER.OFF
        IF NOT(PHNO) THEN PRINT @(41,L1ST-3):YBLOCKNO+YWRITNO:
        C$RPT.CUSTOMER.NO = YR.REC(17)
        C$RPT.ACCOUNT.NO = YR.REC(18)
        CALL PRINTER.CLOSE(REPORT.ID,PRT.UNIT,"")
    END ELSE
        TEXT = "END OF REPORT"; YEND = 1; GOSUB 9100000
    END
RETURN
*************************************************************************
1000000:
*
    YKEY.OLD = YKEY
    MAT YR.REC.OLD = MAT YR.REC
    REMOVE YKEY FROM YID.LIST SETTING YDELIM
    IF NOT(YKEY:YDELIM) THEN
        YKEYFD = "***"; YKEY = STR("*",188); RETURN
    END
    MATREAD YR.REC FROM F.FILE, YKEY ELSE MAT YR.REC = "" ; GOSUB 1000000           ;*Manual R22 conversion GOTO Changed to GOSUB
    YKEY = "C":YKEY
    IF NOT(PHNO) AND YPRINTING THEN
        IF YWRITNO < 9 THEN
            YWRITNO += 1
        END ELSE
            YWRITNO = 0; YBLOCKNO += 10
            CALL PRINTER.OFF; PRINT @(41,L1ST-3):YBLOCKNO+YWRITNO:; CALL PRINTER.ON(REPORT.ID,PRT.UNIT)
        END
    END
RETURN
*************************************************************************
9000000:
*
    YTOTFD = TRIMB(YTOTFD)
    IF YTOTFD = "" THEN RETURN
*
9000010:
IF YPRINTING THEN PRINT YTOTFD; YTOTFD = ""; RETURN
PRINT YTOTFD:; YT.PAGE<P,L> = YTOTFD; YTOTFD = ""
IF L < 19 THEN L += 1; PRINT @(0,L):; RETURN
*
9100000:
    PRINT @(13,21):TIMEDATE()[1,8]:@(68,21):S.CLEAR.EOL:P:
*
9100010:
    Y = T.REMTEXT(4)  ;* AWAITING PAGE INSTRUCTIONS
    CALL INP (Y,8,22,5.1,"A")
    BEGIN CASE
        CASE COMI = C.B; NEXTP = P-1
        CASE COMI = C.F
            NEXTP = P+1; IF NEXTP = LASTP+1 THEN GOSUB 9190000      ;*Manual R22 conversion GOTO Changed to GOSUB
        CASE COMI = C.E; NEXTP = LASTP
        CASE COMI = C.V OR COMI = C.W OR COMI = C.U
            PRINT S.COL132.OFF:
            CLEARSELECT; COMI = C.U; RETURN
        CASE COMI = "P"; NEXTP = 1
        CASE COMI[1,1] = "P" AND NUM(COMI[2,99]) = NUMERIC
            NEXTP = COMI[2,99]
            IF NEXTP = LASTP+1 THEN COMI = C.F; GOSUB 9190000    ;*Manual R22 conversion GOTO Changed to GOSUB
        CASE 1
            E = ""; L = 22; CALL ERR; GOSUB 9100010        ;*Manual R22 conversion GOTO Changed to GOSUB
    END CASE
*
    IF NEXTP < 1 THEN
        NEXTP = 1
    END ELSE
        IF NEXTP > LASTP THEN NEXTP = LASTP
    END
    IF NEXTP = P THEN GOSUB 9100000                   ;*Manual R22 conversion GOTO Changed to GOSUB
    P = NEXTP
*
    GOSUB 9200000
    FOR LL = L1ST TO 19
        X = YT.PAGE<P,LL>; IF X <> "" THEN PRINT @(0,LL):X:
    NEXT LL; GOSUB 9100000               ;*Manual R22 conversion GOTO Changed to GOSUB
*
9190000:
    IF YEND THEN GOSUB 9100000 ELSE P = NEXTP     ;*Manual R22 conversion GOTO Changed to GOSUB
*
9200000:
    L = L1ST; PRINT @(0,L):
    FOR LL = L TO 19; PRINT S.CLEAR.EOL; NEXT LL
    PRINT @(0,L):
    IF P > LASTP THEN LASTP = P
RETURN
*************************************************************************
*
* Ask for valid file password in connection with Company code
9300000:
*
    IF X THEN IF R.USER<EB.USE.COMPANY.RESTR,X> THEN
        IF R.USER<EB.USE.COMPANY.RESTR,X> <> YID.COMP THEN X = 0
    END
RETURN
*************************************************************************
END
