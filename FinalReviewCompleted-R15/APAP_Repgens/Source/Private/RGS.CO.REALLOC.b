* @ValidationCode : MjotNzg5ODA2NTM4OkNwMTI1MjoxNjg2ODM1Mzg0MTMzOklUU1M6LTE6LTE6MzYyNDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 18:53:04
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 3624
...
SUBROUTINE RGS.CO.REALLOC
REM "RGS.CO.REALLOC",230614-4
*************************************************************************
...
*************************************************************************
    ...
    YT.SMS.FILE = "RE.COLLATERAL"
    YT.SMS.FILE<-1> = "RE.STAT.REPORT.HEAD"
    YT.SMS.FILE<-1> = "RE.STAT.REP.LINE"
...
*************************************************************************
    ...
    DIM YR.REC(5)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.CO.REALLOC"
    YOLDFILE = 1
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.CO.REALLOC")
    END
*
    YCOM = ID.COMPANY
*
    ...
    NEXT YAV
*
    YFILE = "F.RE.STAT.REPORT.HEAD"; YF.RE.STAT.REPORT.HEAD = ""
    CALL OPF (YFILE, YF.RE.STAT.REPORT.HEAD)
    YFILE = "F.RE.STAT.REP.LINE"; YF.RE.STAT.REP.LINE = ""
    CALL OPF (YFILE, YF.RE.STAT.REP.LINE)
*************************************************************************
    YFILE = "RE.COLLATERAL"
    FULL.FNAME = "F.RE.COLLATERAL"; YF.RE.COLLATERAL = ""
    ...
    CALL OPF (FULL.FNAME, YF.RE.COLLATERAL)
    CLEARSELECT
    ...
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.RE.COLLATERAL, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
...
*
    YKEY = ""; MAT YR.REC = ""
    YM.REPORT.ID =  FIELD(ID.NEW,".",1)
    YM.REPORT.NAME = YM.REPORT.ID
    IF YM.REPORT.NAME <> "" THEN
        YCOMP = "RE.STAT.REPORT.HEAD_1_":YM.REPORT.NAME
        YFORFIL = YF.RE.STAT.REPORT.HEAD
        YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
        YM.REPORT.NAME = YFOR.FD
    END
    YKEYFD = YM.REPORT.NAME
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.REPORT.NAME,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(1) = YM.REPORT.NAME
    YM.BLANK = " "
    YR.REC(2) = YM.BLANK
    YM.LINE.NO =  FIELD(ID.NEW,".",2)
    YR.REC(3) = YM.LINE.NO
    YM.LINE.DESC1 = ID.NEW
    IF YM.LINE.DESC1 <> "" THEN
        YCOMP = "RE.STAT.REP.LINE_2.2.1_":YM.LINE.DESC1
        YFORFIL = YF.RE.STAT.REP.LINE
        YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
        YM.LINE.DESC1 = YFOR.FD
    END
    YKEYFD = YM.LINE.DESC1
...
* Year 2000 compliance
    ...
    YKEYFD = FMT(YM.LINE.DESC1,"37L")


    IF LEN(YKEYFD) > 37 THEN YKEYFD = YKEYFD[1,36]:"|"
    GOSUB 8000000
    YR.REC(4) = YM.LINE.DESC1
    YM.AMT.TO.MOVE = R.NEW(1)
    YM.LCCY = "CHF"
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.LCCY, YDEC)
    IF YM.AMT.TO.MOVE <> "" THEN
        YM.AMT.TO.MOVE = TRIM(FMT(YM.AMT.TO.MOVE,"19R":YDEC))
    END
    YR.REC(5) = YM.AMT.TO.MOVE
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
