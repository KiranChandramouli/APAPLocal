...
SUBROUTINE RGP.CO.REVIEW
REM "RGP.CO.REVIEW",230614-4
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "COLLATERAL REVIEW"
    IF YPRINTING THEN
        YHDR = "RGP.CO.REVIEW ":YHDR
    ...
    END
    YHDR1 = ""
    YHDR2 = " REVIEW                                  RIGHT                                   NOMINAL EXECUTION 3RD PARTY REV         EXPIRY"
    YHDR3 = " DATE CUSTOMER                           COLLATERAL TYPE & NO CCY                VALUE VALUE VALUE FQU                   DATE STS"
    YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(17)  ;* YM.REVIEW.FQU
        YFD = FMT(YFD,"5L")
        IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(18)  ;* YM.EXPIRY.DATE
        BEGIN CASE
            CASE YFD MATCHES "8N"
                YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[3,2]
            CASE YFD MATCHES "6N"
