* @ValidationCode : MjotMjU1MjE1MDU4OkNwMTI1MjoxNjg2ODMzMzIzOTkzOklUU1M6LTE6LTE6NzY0NjoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 18:18:43
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 7646
...
SUBROUTINE RGP.PL.JOURNAL
REM "RGP.PL.JOURNAL",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    END ELSE
        PRINT @(0,L1ST-1):STR("-",80):
        YEND = 0; Y = 1; LASTP = 1; L = L1ST; PRINT @(0,L):
        FOR LL = L TO 19; PRINT S.CLEAR.EOL; NEXT LL
        YTEXT = T.REMTEXT(23)
* T.REMTEXT(23) = "PAGE"
        PRINT @(0,20):STR("-",80):@(63+LEN(YTEXT),21):S.HALF.INTENSITY.OFF:@(63,21):S.HALF.INTENSITY.ON:YTEXT:S.HALF.INTENSITY.OFF:
        YT.PAGE = ""; T.CONTROLWORD = C.U:@FM:C.B:@FM:C.F:@FM:C.E:@FM:C.V:@FM:C.W
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(8); MAT YR.REC = ""
    GOSUB 1000000
    YHDR = "PL JOURNAL END OF YEARJOURNAL P/P REPORTE"
    IF YPRINTING THEN
        YTEXT = "Page"; CALL TXT ( YTEXT )
        YTYPE = FMT(YHDR,"71L"):FMT(YTEXT,"5L"):"'PL'"
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "VALUE DATE   CTG                    DESCDATE VALEUR  CTG                    DESC                                       RIPTION                    AMOUNTRIPTION                    MONTANT                                                   "
    YHDR2 = "                                                                                                                     "
    YHDR3 = "                                                                                                                     "
    YHDR4 = "                                                                                                                     "
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := YFD
        YFD = YR.REC(2)  ;* YM.CATEGORY
        YFD = FMT(YFD,"5L")
        IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(3)  ;* YM.DESCRIPT
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(4)  ;* YM.AMOUNT
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YFD = FMT(YFD,"19R2,")
...
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.TOTAL1
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := "                                                      ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.TOTAL
                YR.TOT2(YAV.TOT)<2> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YFD = "AMOUNT TRANSFERRED TO PL ACCOUNTMONTANT TRANSFERE PP REPORTE          ":YFD
                GOSUB 9000000
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := "                  ":YFD
                GOSUB 9000000
