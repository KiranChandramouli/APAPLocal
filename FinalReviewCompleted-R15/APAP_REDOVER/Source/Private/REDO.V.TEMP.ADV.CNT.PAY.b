* @ValidationCode : MjoxMjM3OTAyNzIwOkNwMTI1MjoxNzAyOTYzNjQ0NjU4OmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 19 Dec 2023 10:57:24
...
* CALL F.READ(FN.AA.ACCOUNT.DETAILS,VAR.AA.ID,R.AA.ACCOUNT.DETAILS,F.AA.ACCOUNT.DETAILS,AA.AC.ERR)
    R.AA.ACCOUNT.DETAILS = AA.PaymentSchedule.AccountDetails.Read(Y.BILL.ID, BILL.ERR)
* Y.TOT.BILS = R.AA.ACCOUNT.DETAILS<AA.AD.BILL.ID>
    Y.TOT.BILS = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillId>
...
*Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.AD.BILL.ID,FLG.VAL,1>
        Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillId,FLG.VAL,1>
...
* Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.BD.SETTLE.STATUS,1>
        Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdSettleStatus,1>
        IF Y.SETTLE.STATUS EQ 'UNPAID' THEN
            Y.DUP.NO.OF.INS -= 1
            IF Y.DUP.NO.OF.INS GE 0 THEN
*Y.FIN.AMT += SUM(R.AA.BILL.DETAILS<AA.BD.OS.PROP.AMOUNT>)
                Y.FIN.AMT += SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOrPropAmount>)
            END
            FLG += 1
        END ELSE
            FIN.CNT += 1
...
* Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.AD.BILL.ID,FLG.VAL,FG.LG>
            Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillId,FLG.VAL,FG.LG>
*CALL F.READ(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
            R.AA.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BILL.ERR)
* Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.BD.SETTLE.STATUS,1>
            Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdSettleStatus,1>
...
*Y.DUP.FIN.AMT = (R.AA.BILL.DETAILS<AA.BD.OR.TOTAL.AMOUNT>*Y.PARTIAL/100)
                Y.DUP.FIN.AMT = (R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOrTotalAmount>*Y.PARTIAL/100)
            END
...
* CALL AA.SCHEDULE.PROJECTOR(ARRANGEMENT.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS,DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(ARRANGEMENT.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS,DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
