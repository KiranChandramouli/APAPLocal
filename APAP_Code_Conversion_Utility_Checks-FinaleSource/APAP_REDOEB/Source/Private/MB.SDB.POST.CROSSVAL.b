* @ValidationCode : MjoxMzE5MDIxNzg5OlVURi04OjE3MDI5OTA2Mjg4Njk6SVRTUzE6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
* 15-12-2023      Narmadha V        Manual R22 Conversion   = to EQ
*-----------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
* CASE SDB.OPERATE = 'MODIFY'
        CASE SDB.OPERATE EQ 'MODIFY' ;*Manual R22 Conversion - = to EQ
...
* CASE SDB.OPERATE = 'VISIT'
        CASE SDB.OPERATE EQ 'VISIT'    ;*Manual R22 Conversion  = to EQ
...
*  CASE SDB.OPERATE = 'CLOSE'
        CASE SDB.OPERATE EQ 'CLOSE' ;*Manual R22 Conversion  = to EQ
            IF SDB.STATUS THEN
                IF R.MB.SDB.STATUS<SDB.STA.STATUS> EQ 'RESERVED' THEN
                    AF = SDB.POST.SDB.OPERATE
        ...
        IF INDEX(N(AF), "C", 1) THEN
            BEGIN CASE
                CASE F(AF)[4,2] EQ 'XX'      ;* Sv
                    ...
                    NEXT AV
* CASE F(AF)[1,2] = 'XX'      ;* Mv
                CASE F(AF)[1,2] EQ 'XX' ;*Manual R22 Conversion  = to EQ
                    AS = ''
                    NO.OF.AV = DCOUNT(R.NEW(AF), @VM)
                    IF NO.OF.AV EQ 0 THEN
                        NO.OF.AV = 1
