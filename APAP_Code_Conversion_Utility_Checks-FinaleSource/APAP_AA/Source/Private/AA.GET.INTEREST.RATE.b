* @ValidationCode : MjotNDcxNDg1NjQ6Q3AxMjUyOjE3MDI5OTI0NTA2NjE6SVRTUzE6LTE6LTE6MDoxOmZhbHNlOk4vQTpERVZfMjAyMjAyLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 19 Dec 2023 18:57:30
...
PROCESS:
*---------------------------------------------------------------------------------
    Y.INT.PAY.TYPE=R.NEW(AA.Interest.Interest.IntLocalRef)<1,POS.L.AA.REV.RT.TY>
...
*CALL F.READ(FN.GROUP.DATE,Y.COND.AND.CURR,R.GROUP.DATE,F.GROUP.DATE,GROUP.ERR)
    ...
    IF Y.TOTAL.INT.RATE THEN
        ...
        Y.FIXED.RATE.CNT = DCOUNT(R.NEW(AA.Interest.Interest.IntFixedRate),@VM) ;*AUTO R22 CODE CONVERSION
        Y.VAR2 = 0
        LOOP
        WHILE Y.VAR2 LE Y.FIXED.RATE.CNT
            IF Y.FIXED.RATE.CNT EQ 0 THEN
                R.NEW(AA.Interest.Interest.IntFixedRate)<1,Y.VAR2+1> = Y.FINAL.RATE
                R.NEW(AA.Interest.Interest.IntFloatingIndex)<1,Y.VAR2+1> = ''
            END ELSE
                IF Y.VAR2 NE 0 THEN
                    R.NEW(AA.Interest.Interest.IntFixedRate)<1,Y.VAR2> = Y.FINAL.RATE
                    R.NEW(AA.Interest.Interest.IntFloatingIndex)<1,Y.VAR2> = ''
                END
            END
...
UPDATE.NOMINAL:
*----------------------------------------------------------------------------
    TOT.FIXED = DCOUNT(R.NEW(AA.Interest.Interest.IntFixedRate),@VM) ;*AUTO R22 CODE CONVERSION
    TIER.I = 1

    LOOP
    WHILE TOT.FIXED GE TIER.I
        IF R.NEW(AA.Interest.Interest.IntFixedRate)<1,TIER.I> NE '' THEN
            CURRENT.RATE = R.NEW(AA.Interest.Interest.IntFixedRate)<1,TIER.I>
            MARGIN.RATES = R.NEW(AA.Interest.Interest.IntMarginRate)<1,TIER.I>
            MARGIN.OPERANDS = R.NEW(AA.Interest.Interest.IntMarginOper)<1,TIER.I>
            MAX.TIER.CAP = R.NEW(AA.Interest.Interest.IntTierMaxRate)<1,TIER.I>
            MIN.TIER.CAP = R.NEW(AA.Interest.Interest.IntTierMinRate)<1,TIER.I>
*TUS START
