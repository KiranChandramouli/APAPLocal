* @ValidationCode : MjoyMDg0ODY0MTg3OkNwMTI1MjoxNzAyOTkyNDUwNTUxOklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6REVWXzIwMjIwMi4wOi0xOi0x
...
* 19/12/2023         Arun               R22 Manual Conversion             Changed AA.BUILD.PAYMENT.SCHEDULE.SCHEDULES  to AA.PaymentSchedule.ScheduleProjector
*----------------------------------------------------------------------------
...
*    END.DATE = R.PAY.SCHED<1,AA.PS.PAYMENT.END.DATE>
    END.DATE = R.ACC.DET<AA.PaymentSchedule.AccountDetails.AdPaymentEndDate>      ;*Single Value in R15
*TUS END
    NO.CYCLES=''
    ADJUST.FINAL.AMOUNT=''
    REQD.END.DATE=''
...
*    CALL AA.BUILD.PAYMENT.SCHEDULE.SCHEDULES(SCHEDULE.INFO, REQD.END.DATE, NO.CYCLES, ADJUST.FINAL.AMOUNT, PAYMENT.DATES, PAYMENT.TYPES, PAYMENT.METHODS, PAYMENT.AMOUNTS, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, TAX.DETAILS, OUTSTANDING.AMOUNT, FINAL.PRINCIPAL.POS, PAYMENT.PERCENTAGES, PAYMENT.MIN.AMOUNTS, PAYMENT.DEFER.DATES, PAYMENT.BILL.TYPES, CHARGE.CALC.INFO, RET.ERROR)   ;*Manual R22 conversion - CHARGE.CALC.INFO argument added
    ...
    DueTypeAmts = ''
    DueOuts = ''
    AA.PaymentSchedule.ScheduleProjector(Y.ARRANGEMENT.ID, SimulationRef, NoReset, DateRange, TotPayment, PAYMENT.DATES, DueDeferDates, PAYMENT.TYPES, DueMethods, DueTypeAmts, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, DueOuts);*MANUAL R22 CODE CONVERSION
...
*    CALL F.READ(FN.AA.ACCOUNT.DETAILS,Y.ARRANGEMENT.ID,R.ACC.DET,F.AA.ACCOUNT.DETAILS,ACC.DET.ERR) ;*R22 Manual Code conversion
    R.ACC.DET = AA.PaymentSchedule.AccountDetails.Read(Y.ARRANGEMENT.ID, ACC.DET.ERR)
    Y.PAY.START.DATE= R.ACC.DET<AA.PaymentSchedule.AccountDetailsHist.AdPaymentStartDate>       ;* Payment start date
