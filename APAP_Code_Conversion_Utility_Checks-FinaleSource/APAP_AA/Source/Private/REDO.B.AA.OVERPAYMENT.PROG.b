* @ValidationCode : MjotMTk3MTY5MzUyMzpDcDEyNTI6MTcwMjk5MjQ1NTQyNDpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOkRFVl8yMDIyMDIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 19 Dec 2023 18:57:35
...
* CALL AA.SCHEDULE.PROJECTOR(Y.AA.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(Y.AA.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS);*MANUAL R22 CODE CONVERSION
...
*------------------------------------------------------------------
    ...
    Y.VAR2 = 1
    Y.PAYMENT.TYPES = R.CONDITION.PAYSCH<AA.PaymentSchedule.PaymentSchedule.PsPaymentType>
    Y.PAYMENT.TYPES.CNT = DCOUNT(Y.PAYMENT.TYPES,@VM)
    LOOP
    WHILE Y.VAR2 LE Y.PAYMENT.TYPES.CNT

        Y.END.DATE     = R.CONDITION.PAYSCH<AA.PaymentSchedule.PaymentSchedule.PsEndDate,Y.VAR2>
        Y.END.DATE.CNT = DCOUNT(Y.END.DATE,@SM)
        Y.VAR3 = 1
        LOOP
        WHILE Y.VAR3 LE Y.END.DATE.CNT
            IF R.CONDITION.PAYSCH<AA.PaymentSchedule.PaymentSchedule.PsEndDate,Y.VAR2,Y.VAR3> GT Y.LAST.CAPITAL.DATE THEN
