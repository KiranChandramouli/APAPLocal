* @ValidationCode : MjoxMDg5MzIxNTYxOkNwMTI1MjoxNzAyOTkyNDcyNDQ2OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6REVWXzIwMjIwMi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 19 Dec 2023 18:57:52
...
* PACS00142802 -S
    ...
    IF Y.PART EQ 'YES' OR Y.PART EQ 'SI' THEN
        ...
        IF R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOrTotalAmount> LT SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>) THEN
            Y.DUP.FIN.AMT = (SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>)*Y.PARTIAL/100)
        END ELSE
            Y.DUP.FIN.AMT = (R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetailsHist.BdOrTotalAmount>*Y.PARTIAL/100)
        END
        IF Y.DUP.FIN.AMT LT SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>) THEN
            Y.FIN.AMT = Y.FIN.AMT - SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>) + Y.DUP.FIN.AMT
        END ELSE
            Y.FIN.AMT = Y.FIN.AMT
        END
*  Y.FIN.AMT = FMT(Y.FIN.AMT,'R2')
...
*CALL F.READ(FN.AA.ACCOUNT.DETAILS,VAR.AA.ID,R.AA.ACCOUNT.DETAILS,F.AA.ACCOUNT.DETAILS,AA.AC.ERR)
    R.AA.ACCOUNT.DETAILS = AA.PaymentSchedule.AccountDetails.Read(VAR.AA.ID, AA.AC.ERR)
    Y.TOT.BILS = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillId>
    Y.TOT.BILS = CHANGE(Y.TOT.BILS,@SM,@VM) ;*AUTO R22 CODE CONVERSION
    Y.CN.VM = DCOUNT(Y.TOT.BILS,@VM) ;*AUTO R22 CODE CONVERSION

    YFIRST.BILLDTE = ''
    YFIRST.BILLDTE = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillPayDate,1>
    IF NOT(YFIRST.BILLDTE) THEN
        YFIRST.BILLDTE = TODAY
    END

    Y.CNT = DCOUNT(R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillPayDate>,@VM) ;*AUTO R22 CODE CONVERSION
    FLG.VAL = ''
    IF Y.PART.VAL EQ 'YES' OR Y.PART.VAL EQ 'SI' THEN
        LOOP
        WHILE Y.CNT GT 0 DO
            FLG.VAL += 1
            Y.TYPE = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL>
...
SUB.PROCESS.EACH.BIL:
    ...
    IF Y.CNT.TYPE EQ 1 THEN
        IF R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,1> EQ 'PAYMENT' THEN
            Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillId,FLG.VAL,1>
*   CALL F.READ(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
            R.AA.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BILL.ERR)
            Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetailsHist.BdSettleStatus,1>
            IF Y.SETTLE.STATUS EQ 'UNPAID' THEN
                Y.FIN.AMT += SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>)
                FLG += 1
            END ELSE
                FIN.CNT += 1
            END
        END ELSE
            IF R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,1> EQ 'ACT.CHARGE' THEN
                FIN.CNT += 1
            END
        END
    END ELSE
...
PROCESS.MANY.BILS:
    ...
    WHILE Y.CNT.TYPE GT 0 DO
        ...
        IF R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,FG.LG> EQ 'PAYMENT' THEN
            Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillId,FLG.VAL,FG.LG>
*CALL F.READ(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
            R.AA.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BILL.ERR)
            Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetailsHist.BdSettleStatus,1>
            IF Y.SETTLE.STATUS EQ 'UNPAID' THEN
        ...
        IF R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,FG.LG> EQ 'ACT.CHARGE' THEN
            Y.BILL.ID = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillId,FLG.VAL,FG.LG>
*CALL F.READ(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
            R.AA.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BILL.ERR)
            Y.PAY.DATE = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdPaymentDate>
...
*CALL AA.SCHEDULE.PROJECTOR(ARRANGEMENT.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(ARRANGEMENT.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS);*MANUAL R22 CODE CONVERSION
