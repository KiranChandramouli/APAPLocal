* @ValidationCode : MjotMTc4MTk3NDk1MTpDcDEyNTI6MTcwMjk4ODM0NTE0ODpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMl9TUDUuMDotMTotMQ==
...
*-------------------------------------------------------------------------------------
    ...
    $INSERT I_F.REDO.APAP.INSTIT.FINANC.PARAM ;*R22 Manual Code Conversion_Utility Check
...
*---------------
    Y.CODE = "DO0010001"
    CALL CACHE.READ(FN.DATE, Y.CODE, R.DATE, ERR.DATE) ;*R22 AUTO CONVERSTION F.READ TO CACHE.READ AND REMOVED FV.DATE
...
*       CALL GET.LOC.REF("CUSTOMER", "L.CU.RNC", L.CU.RNC.POS)
        EB.LocalReferences.GetLocRef("CUSTOMER", "L.CU.RNC", L.CU.RNC.POS) ;*R22 Manual Code Conversion_Utility Check
...
*CALL L.APAP.INT.RATE.ACC(Y.ACC.ID, R.ACC, Y.CR.INT.RATE) ;*R22 MANAUAL CODE CONVERSION
            APAP.LAPAP.lApapIntRateAcc(Y.ACC.ID, R.ACC, Y.CR.INT.RATE) ;*R22 MANAUAL CODE CONVERSION
...
*   CALL F.READ(FN.IF.PARAM, Y.CUS.RNC, R.IF.PARAM, FV.IF.PARAM, IF.PARAM.ERR)
    CALL CACHE.READ(FN.IF.PARAM, Y.CUS.RNC, R.IF.PARAM, IF.PARAM.ERR) ;*R22 Manual Code Conversion_Utility Check
...
*        Y.CONTRAPARTE = R.IF.PARAM<2>
        ...
        Y.CO.INST  = R.IF.PARAM<INST.FIN.TIPO.INSTITUTION> ;*R22 Manual Code Conversion_Utility Check
        Y.NO.INST = R.IF.PARAM<INST.FIN.NUMERO.INSTITUTION>
        Y.CONTRAPARTE = R.IF.PARAM<INST.FIN.CUSTOMER.DEXCRIP>
    END ELSE
*       CALL F.READ(FN.IF.PARAM, Y.CUS.ID, R.IF.PARAM, FV.IF.PARAM, IF.PARAM.ERR)
        CALL CACHE.READ(FN.IF.PARAM, Y.CUS.ID, R.IF.PARAM, IF.PARAM.ERR) ;*R22 Manual Code Conversion_Utility Check
        IF (R.IF.PARAM) THEN
*            Y.CO.INST  = R.IF.PARAM<3>
*            Y.NO.INST = R.IF.PARAM<4>
*            Y.CONTRAPARTE = R.IF.PARAM<2>
