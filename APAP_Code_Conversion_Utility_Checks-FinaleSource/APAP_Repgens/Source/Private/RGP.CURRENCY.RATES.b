...
SUBROUTINE RGP.CURRENCY.RATES
REM "RGP.CURRENCY.RATES",230614-4
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "ISO     TK   CURRENCY-NAME              PURCH.          SALE          REVAL."
    YHDR2 = ""
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(3)  ;* YM.CCY.NAME
        YFD = FMT(YFD,"20L")
        IF LEN(YFD) > 20 THEN YFD = YFD[1,19]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(4)  ;* YM.PURCHASE.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(5)  ;* YM.SALE.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "    ":YFD
        YFD = YR.REC(6)  ;* YM.REVAL.RATE
        YFD = FMT(YFD,"11L")
        IF LEN(YFD) > 11 THEN YFD = YFD[1,10]:"|"
        YTOTFD := "    ":YFD
        GOSUB 9000000
