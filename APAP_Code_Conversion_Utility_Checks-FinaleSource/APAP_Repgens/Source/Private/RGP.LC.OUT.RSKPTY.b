* @ValidationCode : MjotMjAzMTU0MzgxMjpDcDEyNTI6MTY4ODU1NjYzNjAyODpJVFNTOi0xOi0xOjE0ODIyOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 05 Jul 2023 17:00:36
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 14822
...
*-----------------------------------------------------------------------------
SUBROUTINE RGP.LC.OUT.RSKPTY
REM "RGP.LC.OUT.RSKPTY",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YHDR1 = "------------------------------------------------------------------------------------------------------------------------------------"
    YHDR2 = "OUR REF.NO   OLD LC.NO       ISSUE.DATE  CCY      L/C AMT      UN-UTILD BAL    EXP.DATE       DEF/ACCEPTED  MATURING   LOCAL OS RISK"
    YHDR3 = "------------------------------------------------------------------------------------------------------------------------------------"
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        BEGIN CASE
            CASE YR.REC.OLD(8) <> YR.REC(8); YHEADER.STATUS = 1
        END CASE
        IF YHEADER.STATUS THEN
            GOSUB 9000010; Y1ST.LIN = 1
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD.OLD = YR.REC.OLD(8); YFD = YR.REC(8)  ;* YM.ISSUE.REF
        ...
        YTOTFD := YFD
        YCNT = COUNT(YR.REC(6),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(6)<1,1>  ;* YM.OLD.LC.NUM
        YFD = FMT(YFD,"12L")
        ...
        YTOTFD := "   ":YFD
        YFD = YR.REC(9)  ;* YM.CURR
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := "   ":YFD
        YFD = YR.REC(10)  ;* YM.LC.AMT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YDEC = INDEX(YFD,".",1)
            IF YDEC THEN YDEC = LEN(YFD) - YDEC
            YFD = FMT(YFD,"14R":YDEC:",")
            IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
        END
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "  ":YFD
        YCNT = COUNT(YR.REC(11),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(11)<1,1>  ;* YM.LIAB.AMT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := "                                ":YFD
        YFD = YR.REC(14)  ;* YM.ID.DRAW.PRINT
        YFD = FMT(YFD,"16L")
        IF LEN(YFD) > 16 THEN YFD = YFD[1,15]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := YFD
        YFD = YR.REC(15)  ;* YM.DRAW.AMT.LINE
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YDEC = INDEX(YFD,".",1)
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YFD = YR.REC(6)<1,YAV>  ;* YM.OLD.LC.NUM
            YFD = FMT(YFD,"12L")
            IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
            YTOTFD := "              ":YFD
            YFD = YR.REC(11)<1,YAV>  ;* YM.LIAB.AMT
            IF YFD = "" THEN
                YFD = STR(" ",14)
            END ELSE
                YDEC = INDEX(YFD,".",1)
        ...
        GOSUB 1000000
*
        IF YKEY.OLD[1,31] <> YKEY[1,31] THEN
            YTOTAL.STATUS = 2
...
*
                ...
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.TOTAL.OS.AMT
                YR.TOT2(YAV.TOT)<5> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",16)
                END ELSE
