* @ValidationCode : MjotMTgzNjAwMTE3NjpDcDEyNTI6MTY4NjkxODkwNDY0ODpJVFNTOi0xOi0xOjEwNTYwOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Jun 2023 18:05:04
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 10560
...
SUBROUTINE RGP.BALANCE.LIST
REM "RGP.BALANCE.LIST",230614-4
*-----------------------------------------------------------------------------------------------------------------------
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "B A L A N C E L I S T * * * * * * * * * * * * * *#"
    IF YPRINTING THEN
        YHDR = "RGP.BALANCE.LIST ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "ACCOUNT NO. CUSTOMER NAME                LIMIT CCY CLEARED BALANCE               LEDGER BALANCE DATE LAST CATEG         ORY POST"
    YHDR2 = "                                         REFER.                                  MOVEMENT                                RESTR"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        END CASE
        YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.ACCT.OFF.
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"35L")
            IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
            YFD = "ACCT.OFFICER NAME ":YFD
            YTOTFD := YFD
        END
        ...
        YTOTFD := YFD
        YFD = YR.REC(3)  ;* YM.CUSTOMER
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(4)  ;* YM.LIMIT
        YFD = FMT(YFD,"7L")
        ...
        YTOTFD := "  ":YFD
        YFD = YR.REC(6)  ;* YM.CLEARED.BALANCE
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(7)  ;* YM.LEDGER.BALANCE
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        YTOTFD := "     ":YFD
        YFD = YR.REC(9)  ;* YM.CATEGORY
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(10)  ;* YM.PRINT.YES
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        GOSUB 9000000
