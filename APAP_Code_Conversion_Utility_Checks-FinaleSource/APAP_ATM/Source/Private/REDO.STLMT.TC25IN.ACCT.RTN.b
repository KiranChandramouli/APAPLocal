* @ValidationCode : MjotMjAxNTA5NTAwMDpDcDEyNTI6MTcwMjk4ODI1MDgxNzpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 19 Dec 2023 17:47:30
...
*$INSERT I_F.ATM.REVERSAL  ;*--CAMBIO JORGE
    $INSERT I_F.ATM.TRANSACTION  ;*--CAMBIO JORGE	;* ATM changes by Mario
    ...
    F.ACCOUNT = ''
    CALL OPF (FN.ACCOUNT,F.ACCOUNT) ;*R22 MANUAL CODE CONVERSION
RETURN
...
* Fix for 2795720 [BRD001 - FAST FUNDS SERVICES]
    ...
    IF SET.OCT.FLAG EQ 1 THEN
        ...
        CALL CACHE.READ(FN.REDO.APAP.H.PARAMETER,ID.VAR,R.REDO.APAP.H.PARAMETER,ERR.PARAM)
...
*IF R.ATM.REVERSAL NE '' THEN  --CAMBIO JORGE
        IF R.ATM.TRANSACTION NE '' THEN  ;*--CAMBIO JORGE	;* ATM changes by Mario
