* @ValidationCode : Mjo1ODA3Mzc0Mzc6Q3AxMjUyOjE2ODg1NTY2MzU2NDI6SVRTUzotMTotMToxNjUwNzoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jul 2023 17:00:35
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 16507
...
* <Rating>10141</Rating>
*04-07-2023 Narmadha V  Manual R22 Conversion  FM to @FM, VM to @VM, GOTO to GOSUB
*-----------------------------------------------------------------------------
SUBROUTINE RGP.LC.OUT.IPARTY
REM "RGP.LC.OUT.IPARTY",040129-3
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.LC.OUT.IPARTY"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(22); MAT YR.REC = ""
    GOSUB 1000000
    YHDR = "L/C OUTSTANDINGS PER ISSUEING PARTY"
    IF YPRINTING THEN
        YHDR = "RGP.LC.OUT.IPARTY ":YHDR
    ...
    END
    YHDR1 = "------------------------------------------------------------------------------------------------------------------------------------"
    YHDR2 = "OUR REF.NO L/C TYPE ISSUE.DATE ISSUING PARTY   L/C CURR         L/C  AMT UN-UTILD. BAL. EXP DATE DEF/ACCEPTED  MATURING TOT O/S RISK"
    YHDR3 = "------------------------------------------------------------------------------------------------------------------------------------"
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        IF NUM(YFD) = NUMERIC THEN
            ...
            END
        END
        YFD = YR.REC(5)  ;* YM.LC.ID.PRINT
        YFD = FMT(YFD,"14L")
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := YFD
        YFD = YR.REC(6)  ;* YM.LC.TYPE
        YFD = FMT(YFD,"4L")
        IF LEN(YFD) > 4 THEN YFD = YFD[1,3]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(7)  ;* YM.ISSUE.DATE
        BEGIN CASE
            CASE YFD MATCHES "8N"
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(8)  ;* YM.ISSUE.REF
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(9)  ;* YM.ISSUE.PARTY
        YFD = FMT(YFD,"10L")
        IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(10)  ;* YM.CURR
        YFD = FMT(YFD,"3L")
        IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(11)  ;* YM.LC.AMT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"14R2,")
        ...
        IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
        YTOTFD := "  ":YFD
        YCNT = COUNT(YR.REC(12),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(12)<1,1>  ;* YM.LIAB.AMT
        IF YFD = "" THEN
            YFD = STR(" ",14)
        END ELSE
            YFD = FMT(YFD,"14R2,")
        ...
        YTOTFD := "                        ":YFD
        YFD = YR.REC(15)  ;* YM.ID.DRAW.PRINT
        IF YFD = "" THEN
            YFD = STR(" ",16)
        END ELSE
            YFD = FMT(YFD,"16R2,")
        ...
        YTOTFD := YFD
        YFD = YR.REC(16)  ;* YM.DR.ISSUE.NAME
        IF YFD = "" THEN
            YFD = STR(" ",1)
        END ELSE
            YFD = FMT(YFD,"1R0,")
        ...
        YTOTFD := "                         ":YFD
        YFD = YR.REC(17)  ;* YM.DRAW.AMT.LINE
        IF YFD = "" THEN
            YFD = STR(" ",12)
        END ELSE
            YFD = FMT(YFD,"12R2,")
        ...
        IF YCNT >= YCOUNT.TOT2 THEN YCOUNT.TOT2 = YCNT+1
        YFD = YR.REC(20)<1,1>  ;* YM.LIAB.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 2
                YR.TOT2(YNO)<6,1> = YR.TOT2(YNO)<6,1> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<6,1> = "" THEN
                    YT.DEC2(YNO)<6,1> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<6,1> THEN
                        YT.DEC2(YNO)<6,1> = YDEC
                    END
                END
            NEXT YNO
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YFD = YR.REC(12)<1,YAV>  ;* YM.LIAB.AMT
            IF YFD = "" THEN
                YFD = STR(" ",14)
            END ELSE
                YFD = FMT(YFD,"14R2,")
            ...
            IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
            YTOTFD := "                                                                        ":YFD
            YFD = YR.REC(14)<1,YAV>  ;* YM.TOT.RISK
            IF YFD = "" THEN
                YFD = STR(" ",12)
            END ELSE
                YFD = FMT(YFD,"12R2,")
        ...
        NEXT YAV
*
        FOR YAV = 2 TO YCOUNT.TOT2
            YFD = YR.REC(20)<1,YAV>  ;* YM.LIAB.TOT
            IF NUM(YFD) = NUMERIC THEN
                FOR YNO = 1 TO 2
                    YR.TOT2(YNO)<1,YAV> = YR.TOT2(YNO)<1,YAV> + YFD
                    YDEC = INDEX(YFD,".",1)
                    IF YDEC THEN YDEC = LEN(YFD) - YDEC
                    IF YT.DEC2(YNO)<1,YAV> = "" THEN
                        YT.DEC2(YNO)<1,YAV> = YDEC
                    END ELSE
                        IF YDEC > YT.DEC2(YNO)<1,YAV> THEN
                            YT.DEC2(YNO)<1,YAV> = YDEC
                        END
                    END
                NEXT YNO
        ...
        GOSUB 1000000
*
        IF YKEY.OLD[1,25] <> YKEY[1,25] THEN
            YTOTAL.STATUS = 2
...
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.FOT.TEXT1
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.FOT.TEXT2
                YR.TOT2(YAV.TOT)<2> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<3>  ;* YM.FOT.TEXT3
                YR.TOT2(YAV.TOT)<3> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<4>  ;* YM.FOT.TEXT4
                YR.TOT2(YAV.TOT)<4> = ""
                YFD = FMT(YFD,"33L")
                IF LEN(YFD) > 33 THEN YFD = YFD[1,32]:"|"
                YTOTFD := YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.DRAW.AMT.TOT
                YR.TOT2(YAV.TOT)<5> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",12)
                END ELSE
                ...
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := "                                                                                                 ":YFD
                YCNT = COUNT(YR.TOT2(YAV.TOT)<6>,@VM)
                IF YCNT >= YCOUNT.TOT2 THEN YCOUNT.TOT2 = YCNT+1
                YFD = YR.TOT2(YAV.TOT)<6,1>  ;* YM.LIAB.TOT
                YR.TOT2(YAV.TOT)<6,1> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",12)
                END ELSE
...
*
                FOR YAV = 2 TO YCOUNT.TOT2
                    YCOUNT.AS.TOT2 = 1
                    YFD = YR.TOT2(YAV.TOT)<6,YAV>  ;* YM.LIAB.TOT
                    YR.TOT2(YAV.TOT)<6,YAV> = ""
                    IF YFD = "" THEN
                        YFD = STR(" ",12)
                    END ELSE
                        YFD = FMT(YFD,"12R2,")
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(21)
        C$RPT.ACCOUNT.NO = YR.REC(22)
...
*
    ...
    END
    MATREAD YR.REC FROM F.FILE, YKEY ELSE MAT YR.REC = "" ; GOSUB 1000000 ;* R22 Manual conversion - GOTO changed to GOSUB
    YKEY = "C":YKEY
...
9100010:
    ...
    BEGIN CASE
        ...
        CASE COMI = C.F
            NEXTP = P+1; IF NEXTP = LASTP+1 THEN GOSUB 9190000  ;* R22 Manual conversion - GOTO changed to GOSUB
        ...
        CASE COMI[1,1] = "P" AND NUM(COMI[2,99]) = NUMERIC
            NEXTP = COMI[2,99]
            IF NEXTP = LASTP+1 THEN COMI = C.F; GOSUB 9190000 ;* R22 Manual conversion - GOTO changed to GOSUB
        CASE OTHERWISE
            E = ""; L = 22; CALL ERR; GOSUB 9100010 ;* R22 Manual conversion - GOTO changed to GOSUB
    END CASE
*
    IF NEXTP < 1 THEN
        NEXTP = 1
    ...
    END
    IF NEXTP = P THEN GOSUB 9100000 ;* R22 Manual conversion - GOTO changed to GOSUB
    P = NEXTP
*
    GOSUB 9200000
    FOR LL = L1ST TO 19
        X = YT.PAGE<P,LL>; IF X <> "" THEN PRINT @(0,LL):X:
    NEXT LL; GOSUB 9100000 ;* R22 Manual conversion - GOTO changed to GOSUB
*
9190000:
    IF YEND THEN GOSUB 9100000 ELSE P = NEXTP ;* R22 Manual conversion - GOTO changed to GOSUB
*
9200000:
    L = L1ST; PRINT @(0,L):
    FOR LL = L TO 19; PRINT S.CLEAR.EOL; NEXT LL
