...
*----------------------------------------------------------------------------------------
SUBROUTINE RGS.REFERRAL.LIST
REM "RGS.REFERRAL.LIST",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    DIM YR.REC(9)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.REFERRAL.LIST"
    YOLDFILE = 1
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.REFERRAL.LIST")
    END
*
    YCOM = ID.COMPANY
*
...
*
    ...
    YFILE = "F.REFERAL"; YF.REFERAL = ""
    CALL OPF (YFILE, YF.REFERAL)
*************************************************************************
    YFILE = "ACCOUNTING.REFER"
    FULL.FNAME = "F.ACCOUNTING.REFER"; YF.ACCOUNTING.REFER = ""
    ...
    CALL OPF (FULL.FNAME, YF.ACCOUNTING.REFER)
    CLEARSELECT
    ...
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.ACCOUNTING.REFER, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
...
* Year 2000 compliance
    ...
    IF YM.BALANCE <> "" THEN
        YM.BALANCE = TRIM(FMT(YM.BALANCE,"19R":YDEC))
    END
    YR.REC(5) = YM.BALANCE
    YM.CCY2 = R.NEW(5)
    YR.REC(6) = YM.CCY2
    ...
    YDEC = "NO.OF.DECIMALS"; CALL UPD.CCY (YM.CCY, YDEC)
    IF YM.MOVEMENT <> "" THEN
        YM.MOVEMENT = TRIM(FMT(YM.MOVEMENT,"19R":YDEC))
    END
    YR.REC(7) = YM.MOVEMENT
    YM.REFERRAL = R.NEW(10)
    IF YM.REFERRAL <> "" THEN
        YCOMP = "REFERAL_1_":YM.REFERRAL
        YFORFIL = YF.REFERAL
        YPART.S = ""; YFD.LEN = ""; YHANDLE.LNGG = 1; GOSUB 9000000
        YM.REFERRAL = YFOR.FD
    END
    YR.REC(8) = YM.REFERRAL
    YM.REFERENCE = R.NEW(9)
    YR.REC(9) = YM.REFERENCE
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
