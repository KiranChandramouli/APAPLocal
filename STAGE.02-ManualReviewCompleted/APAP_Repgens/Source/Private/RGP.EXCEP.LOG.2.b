...
SUBROUTINE RGP.EXCEP.LOG.2
REM "RGP.EXCEP.LOG.2",230614-4
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "E X C E P T I O N   L O G   P R I N T"
    IF YPRINTING THEN
        YHDR = "RGP.EXCEP.LOG.2 ":YHDR
    ...
    END
    YHDR.GROUP = ""
    YHDR1 = "APPLIC RECORD KEY AND CURR NO   EOD ROUTINE NAME         ERROR CODE AND MESSAGE"
    YHDR2 = "                                MODULE IN ERROR          FILE NAME                       VALUE"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "1" DO
        ...
        YFD = FMT(YFD,"70L")
        IF LEN(YFD) > 70 THEN YFD = YFD[1,69]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(9)  ;* YM.MODULE
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        YTOTFD := "                               ":YFD
        YFD = YR.REC(10)  ;* YM.FILENAME
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := YFD
        YFD = YR.REC(11)  ;* YM.VALUE
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := "        ":YFD
        YFD = YR.REC(12)  ;* YM.BLANK
        YFD = FMT(YFD,"1L")
        IF LEN(YFD) > 1 THEN YFD = YFD[1,0]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
...
9100010:
    ...
    BEGIN CASE
        ...
        CASE COMI[1,1] = "P" AND NUM(COMI[2,99]) = NUMERIC
            NEXTP = COMI[2,99]
            IF NEXTP = LASTP+1 THEN COMI = C.F; GOSUB 9190000 ;*R22 manual code converison
        CASE 1
            E = ""; L = 22; CALL ERR; GOSUB 9100010 ;*R22 manual code converison
    END CASE
*
    IF NEXTP < 1 THEN
        NEXTP = 1
    ...
    END
    IF NEXTP = P THEN GOSUB 9100000 ;*R22 manual code cnverison
    P = NEXTP
*
    GOSUB 9200000
    FOR LL = L1ST TO 19
