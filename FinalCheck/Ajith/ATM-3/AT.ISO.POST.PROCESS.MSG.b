* @ValidationCode : MjoxNTE0ODIzNDYzOkNwMTI1MjoxNjg0NDkyMjM0NTQ1OklUU1M6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 19 May 2023 16:00:34
...
*
    ...
    END
    CALL APAP.ATM.radLogMsg("ATM","DEBUG",ISO.MESSAGE)
    GOSUB INITIALISE
    CALL APAP.ATM.radLogMsg("ATM","DEBUG",ISO.MESSAGE)
    GOSUB READ.MAPPING
    GOSUB FORM.RESPONSE
    ISO.MESSAGE = FMT(LEN(RESPONSE.MSG),'R%4'):RESPONSE.MSG
    CALL APAP.ATM.radLogMsg("ATM","INFO",ISO.MESSAGE)
...
*----------*
    ...
    CALL APAP.ATM.radLogMsg("ATM","DEBUG","Response mapping to use ":RES.MAP.ID)
    ...
    END ELSE
        CALL APAP.ATM.radLogMsg("ATM","INFO","Missing Response mapping":RES.MAP.ID)
        ...
        BEGIN CASE
            ...
            CASE R.MAPPING<INTRF.MAP.FIELD.SOURCE,II> EQ 'RTN'
                ...
                IF COMP.FLAG EQ '1' THEN
                    ...
                    CALL APAP.ATM.radLogMsg("ATM","DEBUG","Calling ":FLD.RTN :" ,":FLD.VALUE:" ,":RET.VALUE)
                END
