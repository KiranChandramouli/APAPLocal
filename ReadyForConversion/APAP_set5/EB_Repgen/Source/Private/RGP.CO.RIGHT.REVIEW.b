$PACKAGE EB.Repgen
  SUBROUTINE RGP.CO.RIGHT.REVIEW
REM "RGP.CO.RIGHT.REVIEW",230614-4
*************************************************************************
$INSERT I_COMMON
$INSERT I_EQUATE
$INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
  ...
  GOSUB 1000000
  YHDR = "COLLATERAL RIGHT REVIEW"
  IF YPRINTING THEN
    YHDR = "RGP.CO.RIGHT.REVIEW ":YHDR
  ...
  END
  YHDR1 = ""
  YHDR2 = " REVIEW                                                                          REVIEW VALIDITY EX                     PIRY"
  YHDR3 = " DATE CUSTOMER                           COLLATERAL CODE                         NO FQU DATE D                          ATE STATUS"
  YHDR4 = "------------------------------------------------------------------------------------------------------------------------------------"
...
*------------------------------------------------------------------------
  ...
  LOOP WHILE YKEYFD = "" DO
    ...
    YFD = YR.REC(12)<1,1>  ;* YM.REVIEW.FQU
    YFD = FMT(YFD,"5L")
    IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
    YTOTFD := "   ":YFD
    YFD = YR.REC(13)  ;* YM.VALIDITY.DATE
    BEGIN CASE
       CASE YFD MATCHES "8N"
          YFD=YFD[7,2]:" ":FIELD(T.REMTEXT(19)," ",YFD[5,2]):" ":YFD[3,2]
       CASE YFD MATCHES "6N"
...
*
    FOR YAV = 2 TO YCOUNT.LIN
      YFD = YR.REC(12)<1,YAV>  ;* YM.REVIEW.FQU
      YFD = FMT(YFD,"5L")
      IF LEN(YFD) > 5 THEN YFD = YFD[1,4]:"|"
      YTOTFD := "                                                                                                  ":YFD
      GOSUB 9000000
