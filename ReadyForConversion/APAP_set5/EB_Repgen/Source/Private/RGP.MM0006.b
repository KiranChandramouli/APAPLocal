...
* T.REMTEXT(23) = "PAGE"
  ...
  IF YPRINTING THEN
    YHDR = "RGP.MM0006 ":YHDR
    YTYPE = "HEADER":@FM:YHDR
    CALL PST ( YTYPE )
  END ELSE
    YTYPE = YHDR
  END
  YHDR1 = "CUST.ID CCY PRINCIPAL AMOUNT REFERENCE   NR. INSTRUCTION TEXT                    REMARKS BR                             OKER ID"
  YHDR2 = " INTEREST AMOUNT"
  YHDR3 = ""
  YHDR4 = ""
...
*------------------------------------------------------------------------
  ...
  LOOP WHILE YKEYFD = "" DO
    ...
    END
    YFD.OLD = YR.REC.OLD(1); YFD = YR.REC(1)  ;* YM.DISPLAY.DATE
    ...
    YTOTFD := " ":YFD
    YFD = YR.REC(5)  ;* YM.REFERENCE
    YFD = FMT(YFD,"14L")
    IF LEN(YFD) > 14 THEN YFD = YFD[1,13]:"|"
    YTOTFD := " ":YFD
    YFD = YR.REC(6)  ;* YM.TEXT
    YFD = FMT(YFD,"35L")
    IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
    YTOTFD := " ":YFD
    YFD = YR.REC(7)  ;* YM.OUR.REMARKS
    YFD = FMT(YFD,"3L")
    IF LEN(YFD) > 3 THEN YFD = YFD[1,2]:"|"
    YTOTFD := " ":YFD
    YFD = YR.REC(8)  ;* YM.BROKER
    YFD = FMT(YFD,"10L")
    IF LEN(YFD) > 10 THEN YFD = YFD[1,9]:"|"
    YTOTFD := " ":YFD
    YFD = YR.REC(9)  ;* YM.INTEREST.AMOUNT
    IF YFD = "" THEN
      YFD = STR(" ",18)
    END ELSE
      YDEC = INDEX(YFD,".",1)
