$PACKAGE EB.Repgen
  SUBROUTINE RGS.TRANS.JOURNAL
REM "RGS.TRANS.JOURNAL",230614-4
*************************************************************************
$INSERT I_COMMON
$INSERT I_EQUATE
$INSERT I_RC.COMMON
...
*************************************************************************
  ...
  OPEN "", YFILE TO F.FILE ELSE
    ...
    CALL FATAL.ERROR ("RGS.TRANS.JOURNAL")
  END
*
  YCOM = ID.COMPANY
*
...
* Handle Decision Table
      ...
      IF YM.TODAY <> "" THEN
        ...
        YM.TODAY = YFOR.FD
      END
      YM.BOOKING.DATE = R.NEW(25)
      YM.OUTPUT.ARG = "NC"
      IF YM.BOOKING.DATE = YM.TODAY AND YM.OUTPUT.ARG = "NC" THEN
        YGROUP = "1"; GOSUB 2000000
      END
      IF YM.BOOKING.DATE = YM.TODAY AND YM.OUTPUT.ARG = "C" THEN
...
* Handle Decision Table
      ...
      YM12.GOSUB = YM.INPUT.ARG
      YM.OUTPUT.ARG = ""
      YM13.GOSUB = YM.OUTPUT.ARG
      CALL @RE.AC.CONTINGENT (YM12.GOSUB, YM13.GOSUB)
      YM.INPUT.ARG = YM12.GOSUB
      YM.OUTPUT.ARG = YM13.GOSUB
      IF YM.BOOKING.DATE = YM.TODAY AND YM.OUTPUT.ARG = "NC" THEN
        YGROUP = "1"; GOSUB 2000000
      END
      IF YM.BOOKING.DATE = YM.TODAY AND YM.OUTPUT.ARG = "C" THEN
...
* Handle Decision Table
      ...
      YM.OUTPUT.ARG = ""
      YM13.GOSUB = YM.OUTPUT.ARG
      CALL @RG.GET.CONSOL.TYPE (YM12.GOSUB, YM13.GOSUB)
      YM.INPUT.ARG = YM12.GOSUB
      YM.OUTPUT.ARG = YM13.GOSUB
      IF YM.BOOKING.DATE = YM.TODAY AND YM.OUTPUT.ARG = "NC" THEN
        YGROUP = "1"; GOSUB 2000000
    ...
    CASE YFILE = "CATEG.ENTRY"
      BEGIN CASE
        ...
        * Year 2000 compliance
          ...
          GOSUB 8000000
          YR.REC(1) = YM.TRANS.REF
          YM.ACCOUNT = R.NEW(7)
          YR.REC(2) = YM.ACCOUNT
          YM.AMOUNTLCY = R.NEW(3)
          YR.REC(3) = YM.AMOUNTLCY
          YM.TOTALNET = R.NEW(3)
          YR.REC(4) = YM.TOTALNET
          YTRUE.1 = 0
          ...
          YR.REC(12) = YM.DEPT
          YM.ACCT.OFF = R.NEW(9)
          YR.REC(13) = YM.ACCT.OFF
          YM.PRODUCT = R.NEW(10)
          YR.REC(14) = YM.PRODUCT
          YM.CUSTOMER = R.NEW(8)
          YR.REC(15) = YM.CUSTOMER
*
        CASE YGROUP = "2"
          YKEY = "2"; MAT YR.REC = ""
          YM.TRANS.REF = R.NEW(23)
