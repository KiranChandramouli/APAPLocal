SUBROUTINE REDO.V.INP.FT.DR.ACC.CHECK
*-------------------------------------------------------------------------
*DESCRIPTION:
*~~~~~~~~~~~~
...
*~~~~~~~
    ...
    IF ('JudicialCollection' MATCHES Y.LOAN.STATUS) OR ('Write-off' MATCHES Y.LOAN.STATUS) OR ('Legal' MATCHES LOAN.COND) THEN
        TEXT = 'EB-REQ.COLL.AREA.AUTH1'
...
* PACS00085210 -S
*    IF ('ThreeReturnedChecks' MATCHES LOAN.COND) THEN
*        TEXT = 'EB-CHK.PAY.AUTH'
*        CURR.NO  = DCOUNT(R.NEW(FT.OVERRIDE),VM)
*        IF CURR.NO EQ 0 THEN
*            CURR.NO = 1
*        END
