SUBROUTINE REDO.AUTH.UPD.DEP.AMT
***********************************************************************
...
*----------------------------------------------------------------------
    ...
    IF VAR.CURRENCY NE LCCY THEN
        VAR.ORIG.LCY.AMT = R.NEW(AZ.LOCAL.REF)<1,POS.ORIG.LCY.AMT>
        VAR.INDEX.AMT  = R.NEW(AZ.PRINCIPAL)
        VAR.EXCHANGE.RATE = VAR.ORIG.LCY.AMT/VAR.INDEX.AMT
        VAR.EXCHANGE.RATE = DROUND(VAR.EXCHANGE.RATE,4)
    END
RETURN
*----------------------------------------------------------------------
PROCESS:
...
*----------------------------------------------------------------------
    ...
    WHILE DEB.CNT LE Y.DEB.ACC.CNT
        ...
        IF VAR.CURRENCY EQ LCCY THEN
            VAR.AMOUNT +=Y.AMOUNT
        END ELSE
            INDEX.AMOUNT = Y.AMOUNT/VAR.EXCHANGE.RATE
            INDEX.AMOUNT = DROUND(INDEX.AMOUNT,2)
...
*----------------------------
    IF VAR.CURRENCY EQ LCCY THEN
        ...
        R.FT<FT.CREDIT.VALUE.DATE> = R.NEW(AZ.VALUE.DATE)
        R.FT<FT.DEBIT.VALUE.DATE> = R.NEW(AZ.VALUE.DATE)
    END ELSE
        Y.CUR = R.ACC<AC.CURRENCY>
        R.FT<FT.DEBIT.ACCT.NO>    = VAR.DEB.ACC
        R.FT<FT.DEBIT.CURRENCY>   = Y.CUR
        R.FT<FT.CREDIT.ACCT.NO>   = VAR.ACCT.NUM
        R.FT<FT.CREDIT.CURRENCY>  = VAR.CURRENCY
        R.FT<FT.CREDIT.AMOUNT>    = INDEX.AMOUNT
        R.FT<FT.TREASURY.RATE>    = VAR.EXCHANGE.RATE
        R.FT<FT.CREDIT.VALUE.DATE> = R.NEW(AZ.VALUE.DATE)
        R.FT<FT.DEBIT.VALUE.DATE> = R.NEW(AZ.VALUE.DATE)
    END
RETURN
...
*--------------------------
    IF VAR.AMOUNT THEN
        ...
        R.TELL<TT.TE.VALUE.DATE.1> = R.NEW(AZ.VALUE.DATE)
        IF VAR.CURRENCY EQ LCCY THEN
            R.TELL<TT.TE.AMOUNT.LOCAL.1> = VAR.AMOUNT
        END ELSE
            R.TELL<TT.TE.AMOUNT.FCY.1> = VAR.INDEX.AMT
        END
        R.TELL<TT.TE.VALUE.DATE.2> = R.NEW(AZ.VALUE.DATE)
