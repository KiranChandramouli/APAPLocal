...
* Defaults the Charge Amount in from FT.COMMISSION.TYPE
    ...
    CALL CACHE.READ(FN.FT.COMMISSION.TYPE, Y.CHARGE.KEY, R.FT.COMMISSION.TYPE, COMM.TYPE.ERR)
...
*----------------------------------------------------------------------
    ...
    Y.NO.PRODUCTS=DCOUNT(Y.PRODUCTS,@VM)
...
*PACS00120767 -S
            IF R.Condition<AA.CUS.PRIMARY.OWNER> EQ Y.CUSTOMER.ID THEN
                R.NEW(REDO.LET.ISS.CUST.HOLD.STATUS)<1,VAR1>='MAIN HOLDER'
            END ELSE
                R.NEW(REDO.LET.ISS.CUST.HOLD.STATUS)<1,VAR1>='JOINT HOLDER'
            END
