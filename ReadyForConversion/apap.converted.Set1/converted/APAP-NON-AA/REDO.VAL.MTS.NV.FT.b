...
*While authorising no need to change NO.OF.AUTH
    ...
    IF E EQ "EB-UNKNOWN.VARIABLE" THEN
        WTM.AUTOR.PROCESS = ""
    END
    IF WTM.PROC.AUTOR EQ "A" OR WTM.PROC.AUTOR EQ "A" ELSE
...
*
    IF WTM.PROC.AUTOR EQ "A" OR V$FUNCTION EQ "D" OR V$FUNCTION EQ "A" OR MESSAGE EQ "VAL" THEN
        PROCESS.GOAHEAD = ""
    END
*
    LOOP
...
*IF R.NEW(FT.DEBIT.AMOUNT) EQ "" OR R.NEW(FT.DEBIT.AMOUNT) EQ 0 THEN
            ...
            CASE LOOP.CNT EQ 3
                ...
                CALL CACHE.READ(FN.FT.TXN.TYPE.CONDITION, WTRAN.TYPE, R.FT.TXN.TYPE.CONDITION, ERR.FTTC)
                IF ERR.FTTC THEN
