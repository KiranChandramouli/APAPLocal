...
* Selection: To sum up with charge of tax
    ...
    UNTIL Y.CHRG.AMT.LOOP GT Y.CHRG.AMT.COUNT
        ...
        CALL CACHE.READ(FN.FT.COMMISSION.TYPE, Y.CHARGE.CODE, R.FT.COMMISSION.TYPE, FT.COMMISSION.TYPE.ERR)
        IF R.FT.COMMISSION.TYPE THEN
            GOSUB TOT.CHARGE
        END

        CALL CACHE.READ(FN.FT.CHARGE.TYPE, Y.CHARGE.CODE, R.FT.CHARGE.TYPE, FT.CHARGE.TYPE.ERR)
        IF R.FT.CHARGE.TYPE THEN
            GOSUB TOT.CHARGE
        END

        CALL CACHE.READ(FN.TAX, Y.CHARGE.CODE, R.TAX, TAX.ERR)
        IF R.TAX THEN
            GOSUB TOT.TAX
        END
        Y.CHRG.AMT.LOOP+=1
