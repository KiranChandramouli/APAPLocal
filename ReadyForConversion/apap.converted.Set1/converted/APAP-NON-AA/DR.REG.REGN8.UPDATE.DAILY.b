...
AC.CHRG.PROCS:
**************
    ERR.AC.CHARGE.REQUEST = ''; R.AC.CHARGE.REQUEST = ''; AC.CHARGE.REQUEST.HIS.ERR = ''
    ...
    END
    NOT.USED.AMT += R.AC.CHARGE.REQUEST<CHG.TAX.AMT>
    NOT.USED.CNT +=1
    CTR.ENT += 1
RETURN
...
GET.FT.FIELD:
*-----------*
    FT.TXN.VAL = ''; FT.REC.STATUS = ''; DB.AC = ''; FT.COMM.TYPE = ''; FT.COMM.AMT = ''
    TRANS.REF = FIELD(TRANS.REF,'\',1)
    GOSUB READ.FUNDS.TRANS
    IF R.FUNDS.TRANSFER THEN
        FT.REC.STATUS = R.FUNDS.TRANSFER<FT.RECORD.STATUS>
        IF FT.REC.STATUS EQ 'REVE' THEN
            YFLG = 2
            RETURN
        END
        FT.TXN.VAL = R.FUNDS.TRANSFER<FT.TRANSACTION.TYPE>
...
*--------------*
    ...
    R.DR.REG.REGN8.CONCAT<DR.REGN8.CONCAT.FIELD29> = CASH.WTDW.CNT:'|':CASH.WTDW.AMT
RETURN
