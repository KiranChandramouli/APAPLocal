SUBROUTINE REDO.APAP.NOF.INTER.BNK.HIS.DET(Y.FINAL.ARRAY)
*********************************************************************************************
*  Company   Name    :Asociacion Popular de Ahorros y Prestamos
*  Developed By      :DHAMU.S
*  ODR Number        :ODR-2010-08-0394
*  Program   Name    :REDO.APAP.NOF.INTER.BNK.HIS.DET
*---------------------------------------------------------------------------------------------
...
*                    records with CATEGORY EQ 21077
*---------------------------------------------------------------------------------------------
* Modification History :
*-----------------------
*  DATE            WHO                       REFERENCE         DESCRIPTION
*  13-Sep-2010     Dhamu.S                ODR-2010-08-0394     INITIAL CREATION
*
*  17-Mar-2011     Krishna Murthy T.S     PACS00034159         Amended the logic for calculating
*                                                              difference between dates
*  18-Apr-2011     Pradeep S              PACS00054884         Logic changed for Interest Type
*  10-Apr-2012     Pradeep S              PACS00191158         Category validations
*  10-Oct-2012     Marimuthu S
*------------------------------------------------------------------------------------------------
...
*-------
    ...
    CALL CACHE.READ(FN.CATEG,Y.CAT.ID,R.CATEG,Y.CATEG.ERR)
    Y.DESC = R.CATEG<CALL.LIST.CATEG.DESCRIPTION>

    LOCATE 'TESN3' IN Y.DESC<1,1> SETTING Y.CAT.POS THEN
        Y.CATEGORY = R.CATEG<CALL.LIST.CATEG.CATEGORY,Y.CAT.POS>
        CHANGE @SM TO " " IN Y.CATEGORY      ;* PACS00191158 - S/E
    END
    ...
    IF Y.STAT.VAL EQ  'ALL' OR Y.STAT.VAL EQ '' OR Y.STAT.VAL EQ 'MATURED' THEN
...
*PACS00034159 - END
            ...
            GOSUB CALC.BASE
            Y.ACT.INT.RT = (Y.BDY.INTEREST.RATE/100)/Y.BASE

*Y.BDY.INT.ACCRUED=Y.BDY.FACE.VALUE * Y.ACT.INT.RT * Y.DATES
            GOSUB INT.RATE.CALC
            CALL EB.ROUND.AMOUNT(Y.CCY.VAL,Y.BDY.INT.ACCRUED,"","")

*IF Y.BDY.STATUS EQ 'LIVE' THEN        ;*PACS00054884 - S/E
*END
        ...
        END ELSE
            ...
            Y.ID = FIELD(Y.ID,";",1)          ;* PACS00054884 - S/E
...
*PACS00034159 - START
        Y.TOT.INT.RATE = Y.BDY.FACE.VALUE*Y.BDY.INTEREST.RATE
        Y.FINAL.ARRAY<-1>= Y.BDY.MM.ID:"*":Y.BDY.CUSTOMER:"*":Y.BDY.VAL.DATE:"*":Y.BDY.MAT.DATE:"*":Y.BDY.FACE.VALUE:"*":Y.BDY.INTEREST.TYPE:"*":Y.BDY.INTEREST.RATE:"*":Y.BDY.SPREAD:"*":Y.BDY.DAYS:"*":Y.BDY.STATUS:"*":Y.BDY.INT.ACCRUED:"*":Y.BDY.AMT.PAID:"*":Y.HDR.COUNTPTY:"*":Y.HDR.PRINCI:"*":Y.HDR.MAT:"*":Y.HDR.STATUS:"*":Y.TOT.INT.RATE
*PACS00034159 - END
...
*New Section for Flating rate Interest
    ...
    IF Y.BDY.DAYS LT 0 THEN
        Y.BDY.INT.ACCRUED = 0
    END
    Y.BDY.INT.ACCRUED = ABS(Y.BDY.INT.ACCRUED)
RETURN
*---------
CALC.BASE:
*----------
...
*PACS00054884 - S
    BEGIN CASE
        CASE Y.BDY.INTEREST.TYPE EQ "1" ;
            Y.BDY.INTEREST.TYPE = "FIXED"
        CASE Y.BDY.INTEREST.TYPE EQ "3" ;
            Y.BDY.INTEREST.TYPE = "FLOATING"
        CASE Y.BDY.INTEREST.TYPE EQ "9" ;
            Y.BDY.INTEREST.TYPE = "NONE"
    END CASE
*PACS00054884 - E
...
*PACS00054884 - S
    Y.REC.STATUS = R.REC<MM.RECORD.STATUS>
    ...
    BEGIN CASE
        ...
        CASE LEN(Y.BDY.MAT.DATE) NE 8
            ...
            Y.BDY.STATUS = "LIVE"
        CASE Y.BDY.MAT.DATE GE TODAY          ;* PACS00054884 - S/E
            Y.DATE1 = TODAY
            Y.DATE2 = Y.BDY.VAL.DATE
            Y.BDY.STATUS = "LIVE"
        CASE Y.BDY.MAT.DATE LT TODAY          ;* PACS00054884 - S/E
