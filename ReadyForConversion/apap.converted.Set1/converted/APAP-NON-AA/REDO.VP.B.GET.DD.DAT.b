...
* Log writing: process started
    CALL REDO.S.NOTIFY.INTERFACE.ACT ('VPL004', 'BATCH', '06', 'EMAIL ARCHIVO DEBITO DIRECTO', 'INICIO - DEBITO DIRECTO A LAS ' : TIMEDATE(), '', '', '', '', '', OPERATOR, '')
...
* Read the input file
    OPENSEQ DD.PATH, DD.FILE TO DD.FILE.POINTER THEN
        ...
        REPEAT
    END ELSE
        CALL REDO.S.NOTIFY.INTERFACE.ACT ('VPL004', 'BATCH', '06', 'EMAIL ARCHIVO DEBITO DIRECTO', 'DEBITO DIRECTO ARCHIVO NO ENCONTRADO EN ' : DD.PATH : '/' : DD.FILE, '', '', '', '', '', OPERATOR, '')
        CALL FATAL.ERROR('ERROR: Archivo no encontrado: ' : DD.FILE)
    END

    IF NOT(READ.FILE) THEN
        CALL REDO.S.NOTIFY.INTERFACE.ACT ('VPL004', 'BATCH', '06', 'EMAIL ARCHIVO DEBITO DIRECTO', 'DEBITO DIRECTO ARCHIVO NO SE PUDO LEER EN ' : DD.PATH : '/' : DD.FILE, '', '', '', '', '', OPERATOR, '')
        CALL FATAL.ERROR('ERROR: Archivo no encontrado: ' : DD.FILE)
    END
