...
*PACS00072193 - S
    ...
    IF D.FIELDS NE '' THEN
        ...
        CALL REDO.E.FORM.SEL.STMT(FILE.NAME, '', '', SEL.CUS.CMD)
...
******************
    ...
    CALL CACHE.READ(FN.SECTOR, VAR.SECTOR, R.SECTOR, SEC.ERR)
...
*       MAIN.ECONOMIC.ACTIVITY   = R.INDUSTRY<EB.IND.DESCRIPTION,1>
    ...
    CALL CACHE.READ(FN.COUNTRY, VAR.COUNTRY, R.COUNTRY, COUNT.ERR)
    IF R.COUNTRY THEN
    ...
    CALL REDO.DS.BASIC.INFO(CUST.ID,NAME.CORP,ID.CORP,ROLE.1)
    ...
    CALL REDO.DS.SHA.INFO(CUST.ID,SHA.NAME.CORP,SHA.ID.CORP,SHA.ROLE.1)
    ...
    CALL REDO.CUS.OTH.FININS.ONE(CUST.ID,OTH.FIN)
    FIN.INST.ONE               = OTH.FIN
    CALL REDO.CUS.OTH.FININS.TWO(CUST.ID,OTH.FIN2)
    FIN.INST.TWO               = OTH.FIN2
    CALL REDO.CUS.OTH.FININS.THREE(CUST.ID,OTH.FIN3)
    FIN.INST.THREE             = OTH.FIN3
    CALL REDO.CUS.OTH.FININS.FOUR(CUST.ID,OTH.FIN4)
...
CATEG.DESC:
*-----------------------------------------------------------------------------------------------------
    CALL CACHE.READ(FN.CATEGORY, PROD.CODE, R.CATEG, CATEG.ERR)
