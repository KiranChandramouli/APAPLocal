...
* Initailise variables
    ...
    ERR.MSG = ""
    IF COMPANY.IDS EQ "" AND FIELD(COND.ID,".",2) THEN       ;* check to see if there are any company specific records
        COND.REC = ""
...
*---------------
*
    CALL CACHE.READ(FN.CONSOLIDATE.COND, ID, R.CONSOLIDATE.COND, ERR.MSG)
*
...
*-------------------
    ...
    LOOP F.CNT += 1 WHILE F.CNT <= FILE.COUNT
        ...
        IF Y.FILE.NAME NE "LOCAL" AND Y.FILE.NAME NE "NO LOOKUP" THEN
...
*
    IF ERR.MSG THEN
        READ.ERR = ''
        CALL CACHE.READ(FN.LRT, Y.FILE.NAME, R.LRT, READ.ERR)
...
*----------------
    ...
    FOR Y.LF = 1 TO LF.COUNT
        ...
        CALL CACHE.READ(FN.LT, LT.KEY, R.LT, READ.ERR)
        IF R.LT<2,1> EQ Y.FIELD.NAME THEN
...
*
    IF NOT(YR.COND) THEN
        CALL CACHE.READ(FN.CONSOLIDATE.COND, "ASSET&LIAB", YR.COND, READ.ERR)
    END
