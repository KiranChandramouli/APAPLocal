...
MAIN.PROCESS:
    ...
    IF Y.AC.FLAG THEN
        ...
        CALL CACHE.READ(FN.CCY, R.ACCOUNT<AC.CURRENCY>, R.CCY, CCY.ERR)
...
*
EXPIRY.DATE.VAL:
    IF EXPIRY.DATE EQ '' THEN
        IF R.CARD.TYPE<CARD.TYPE.RENEWAL.DATE> EQ '' THEN
    ...
    IF ACCOUNT.ID THEN
        ...
        IF R.CARD.TYPE<CARD.TYPE.CATEGORY> THEN
            LOCATE R.ACCOUNT<AC.CATEGORY> IN R.CARD.TYPE<CARD.TYPE.CATEGORY,1> SETTING V$ ELSE
                ...
                CALL STORE.END.ERROR
            END
            IF R.ACCOUNT<AC.CURRENCY> NE LCCY AND ALLOW.FCY.ACCT[1,1] NE 'Y' THEN
                Y.AC.FLAG = 0
                AF = CARD.IS.ACCOUNT
                ETEXT='EB-AC.LCCY'
                CALL STORE.END.ERROR
