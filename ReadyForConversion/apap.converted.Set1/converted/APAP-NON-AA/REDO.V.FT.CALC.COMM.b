SUBROUTINE REDO.V.FT.CALC.COMM
*-----------------------------------------------------------------------------
*------------------------------------------------------------------------------------------
...
*
    IF Y.FT.COMM.CODE EQ "DEBIT PLUS CHARGES" OR Y.FT.COMM.CODE EQ "" THEN
        IF COMI THEN
            Y.CURRENCY.ID=LCCY
            Y.COMI.AMOUNT=COMI
            CALL EB.ROUND.AMOUNT(Y.CURRENCY.ID,Y.COMI.AMOUNT,CAL,CUSTOMER.ID)
            COMI=Y.COMI.AMOUNT
        END
        R.NEW(FT.LOCAL.REF)<1,L.TT.TRANS.AMT.POS>  = COMI + WCHRG.AMT
    END ELSE
        R.NEW(FT.LOCAL.REF)<1,L.TT.TRANS.AMT.POS>  = COMI
    END
*
RETURN
*
