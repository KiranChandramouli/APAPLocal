...
*-----------------------------------------------------------------------------------------------
*
    APPLN = "CARD.ISSUE":@FM:"STOCK.ENTRY"

    FLD.NAMES = 'L.CI.CUST.NO':@VM:'L.CI.TYP.OF.CARD':@VM:'L.CI.CARD.NO'
...
*
    LOCATE CARD.SERIES.ID IN SERIES.ID SETTING SER.POS THEN
        ...
        NO.OF.SERIES = DCOUNT(STOCK.SERIES,@SM)
...
*    END
    IF R.CARD.ORDER.HIS<CARD.IS.NAME.ON.PLASTIC,2> NE R.NEW(CARD.IS.LOCAL.REF)<1,NAME.ON.PLASTIC.POS,2> THEN
        CHANGE.FLAG = 1
    END
    OLD.ACCOUNTS = DCOUNT(R.CARD.ORDER.HIS<CARD.IS.ACCOUNT>,@VM)
    ...
    IF CHANGE.FLAG THEN
        NOF.CHANGES = DCOUNT(REG.DATES,@VM)
