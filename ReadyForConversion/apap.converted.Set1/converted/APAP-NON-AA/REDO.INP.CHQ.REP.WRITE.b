...
PROCESS:
    ...
    IF NOT(Y.CHEQ.NO) THEN
        RETURN
    END
    Y.CHEQ.NO = CHANGE(Y.CHEQ.NO,@SM,@VM)
    Y.CNT = DCOUNT(Y.CHEQ.NO,@VM)
    FLG = ''
    LOOP
    WHILE Y.CNT GT 0 DO
        FLG += 1
    ...
    R.REDO.LOAN.FT.TT.TXN<LN.FT.TT.VERSION.NAME> = APPLICATION:PGM.VERSION
    ...
    R.REDO.LOAN.FT.TT.TXN<LN.FT.TT.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR
    R.REDO.LOAN.FT.TT.TXN<LN.FT.TT.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR
