SUBROUTINE REDO.LY.POINTS.NFE.LIB.P(ENQ.DATA)
*-------------------------------------------------------------------------------------------
*DESCRIPTION:
...
*---------
    ...
    WHILE POS1 LE Y.TOT.PRG.PROD
        ...
        IF (Y.PROG EQ Y.PRGM.ID) AND (Y.STAT EQ 'Liberada' OR Y.STAT EQ 'Pendiente.Someter') THEN
            ...
            Y.REC.COM = Y.PROG.NAME:'*':Y.PTS.ID:'-':Y.PROD:'*':Y.PT:'*':Y.PT.VAL:'*':Y.AV.DATE:'*':Y.DUE.DATE:'*':Y.STAT
