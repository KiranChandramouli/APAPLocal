...
GET.USER.DETIALS:
*----------------------------------------------------------------------------------------
    LOC.REF.APPLICATION="AZ.ACCOUNT":@FM:"USER"
    LOC.REF.FIELDS='L.AZ.RECEIPT.NO':@FM:'L.US.IDC.BR':@VM:'L.US.IDC.CODE'
    LOC.REF.POS=''
...
*----------------------------------------------------------------------------------------
    ...
    LOCATE ID.NEW IN Y.ACCOUNT.ID.LIST SETTING POS.AC.LIS THEN
        ...
        R.PASS.INV<REDO.DEP.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR
...
***********CR.18-S*********
        ...
        Y.CATEG.ID.PARAM = CHANGE(Y.CATEG.ID.PARAM,@VM,@FM)
        Y.CATEG.ID.PARAM = CHANGE(Y.CATEG.ID.PARAM,@SM,@FM)

        Y.INV.MAIN = R.INV.PARAM<IN.PR.INV.MAINT.TYPE>
        Y.INV.MAIN = CHANGE(Y.INV.MAIN,@VM,@FM)
        Y.INV.MAIN = CHANGE(Y.INV.MAIN,@SM,@FM)
...
*    R.PASS.INV<REDO.DEP.CATEGORY> = R.NEW(AZ.CATEGORY)
    ...
    INPUTTER = C$T24.SESSION.NO:'_':OPERATOR
    AUTHORISER = C$T24.SESSION.NO:'_':OPERATOR
...
*-------------------------------------------------------------------------------------
    ...
    CHANGE @SM TO @FM IN VAR.CATEG.ID
