SUBROUTINE REDO.APAP.NOF.CASH.WINDOW.WIT.SHORTAGE.R32(Y.CCY.LIST,Y.FINAL.ARRAY,SET.CUR,Y.DUP.CUR)
*********************************************************************************************************
*Company   Name    : ASOCIACION POPULAR DE AHORROS Y PRESTAMOS
*Developed By      : Temenos Application Management
*Program   Name    : REDO.APAP.NOF.CASH.WINDOW.WIT.SHORTAGE.R32
*--------------------------------------------------------------------------------------------------------
...
*   ------             -----                       -------------             -------------
* 15 Jun 2011       Marimuthu S              ODR-2011-04-0007 35         changes made in REDO.APAP.NOF.CASH.WINDOW.WIT.SHORTAGE
*
*********************************************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
* In this para of the code, the SELECT command is formed to get the TELLER transactions IDs for processing
    SEL.CMD.SHORT = 'SELECT ':FN.ACCOUNT:' WITH @ID LIKE ...':Y.SHORT.CATEG:'...'
    CALL EB.READLIST(SEL.CMD.SHORT,SEL.LIST.SHORT,'',NO.OF.REC.SHORT,SEL.ERR.SHORT)
    Y.DUP.LIST = SEL.LIST.SHORT

    LOCATE 'TELLER.ID' IN D.FIELDS SETTING POS.D.F THEN
        Y.DUP.TELL.ID = D.RANGE.AND.VALUE<POS.D.F>
    END ELSE
        RETURN
    END
...
* GOSUB READ.TELLER.IDHIS
    IF NOT(R.TELLER.ID) THEN
        ...
        END
    END ELSE
        IF Y.DUP.TELL.ID NE TELLER.ID THEN
            RETURN
        END
    END
