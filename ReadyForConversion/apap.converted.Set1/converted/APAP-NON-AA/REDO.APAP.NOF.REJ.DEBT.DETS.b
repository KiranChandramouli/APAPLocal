...
*-------
    ...
    IF R.STANDING.ORDER THEN
        ...
        CALL CACHE.READ(FN.FT.TXN.TYPE.CONDITION, Y.PAY.MTHD, R.FT.TXN.TYPE.CONDITION, Y.ERR.FT.TXN.TYPE.CONDITION)
...
*---------------
    ...
    IF NOT(Y.BENEF.BNK) AND Y.BEN.ID THEN
        CALL CACHE.READ(FN.BENEFICIARY, Y.BEN.ID, R.BENEFICIARY, BENEFICIARY.ERR)
