...
***********************
    BEGIN CASE
        CASE YRETURN EQ "COMPANY.CODE" OR YRETURN EQ "CO.CODE"
            CALL CACHE.READ(FN.MNEMONIC.COMPANY, STO.APP.ID, R.MNEMONIC.COMPANY, Y.ERR.MC)
            MNE1 = R.MNEMONIC.COMPANY<AC.MCO.COMPANY>
*CALL DBR("MNEMONIC.COMPANY":FM:AC.MCO.COMPANY:FM:"L",STO.APP.ID,MNE1)

            CALL CACHE.READ(FN.COMPANY, MNE1, R.COMP, Y.ERR.COMP)
            COMI.ENRI =R.COMP<EB.COM.COMPANY.NAME>
*CALL DBR("COMPANY":FM:EB.COM.COMPANY.NAME:FM:"L",MNE1,COMI.ENRI)
            GOSUB CHECK.MNE1
        CASE YRETURN EQ "DEPARTMENT.CODE" OR YRETURN EQ "DEPT.CODE"

            CALL CACHE.READ(FN.MNEMONIC.DAO, STO.APP.ID, R.MNEM.DAO, Y.ERR.MNE.DAO)
            MNE2 = R.MNEM.DAO<MDAO.DEPT.ACCT.OFFICER>
*CALL DBR("MNEMONIC.DAO":FM:MDAO.DEPT.ACCT.OFFICER:FM:"L",STO.APP.ID,MNE2)

            CALL CACHE.READ(FN.DEPT.ACCT.OFFICER, STO.APP.ID, R.DEP.ACT.OFF1, Y.ERR.DAO)
...
*CALL DBR("DEPT.ACCT.OFFICER":FM:EB.DAO.AREA:FM:"L",STO.APP.ID,COMI.ENRI)
            ...
            IF COMI.ENRI EQ '' THEN
                CALL CACHE.READ(FN.DEPT.ACCT.OFFICER, MNE2, R.DEP.ACT.OFF2, Y.ERR.DAO1)
                COMI.ENRI = R.DEP.ACT.OFF2<EB.DAO.AREA>
*CALL DBR("DEPT.ACCT.OFFICER":FM:EB.DAO.AREA:FM:"L",MNE2,COMI.ENRI)
            END
            GOSUB CHECK.MNE2
        CASE YRETURN EQ "LOCAL.TABLE" OR YRETURN EQ "LOCAL.TAB"
            F.LOCAL.TABLE = ''
            ...
            CALL CACHE.READ("F.LOCAL.TABLE", MNE3, LOC.TAB.REC, LOC.ERR)
...
*CALL REBUILD.SCREEN
    END ELSE
        ...
        CALL CACHE.READ(FN.COMPANY, STO.APP.ID, R.COMP, Y.ERR.COMP)
...
CHECK.MNE3:
    IF MNE3 EQ STO.APP.ID THEN
        ...
        END ELSE
            IF STK.REG.ID2 EQ "LOCAL.TAB" THEN
                R.NEW(CARD.IS.STOCK.REG.ID) = FIELD(R.NEW(CARD.IS.STOCK.REG.ID),'-',1):'-':MNE3
            END ELSE
...
CHECK.MNE2:
    ...
    IF MNE2 THEN
        ...
        END ELSE
            IF STK.REG.ID2 EQ "DEPT.CODE" THEN
                R.NEW(CARD.IS.STOCK.REG.ID) = FIELD(R.NEW(CARD.IS.STOCK.REG.ID),'-',1):'-':MNE2
            END ELSE
...
*CALL REBUILD.SCREEN
    END ELSE
        CALL CACHE.READ(FN.DEPT.ACCT.OFFICER, STO.APP.ID, R.DEP.ACT.OFF3, Y.ERR.DAO)
