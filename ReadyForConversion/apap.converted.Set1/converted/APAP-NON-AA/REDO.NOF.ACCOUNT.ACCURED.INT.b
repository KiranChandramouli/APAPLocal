...
*Reading Category Application Using Y.CATEG
    R.CTG = '' ; CTG.ERR = '' ;
    CALL CACHE.READ(FN.CATEGORY, Y.CATEG, R.CTG, CTG.ERR)
...
*Reading the Group Credit Int Application  Using GP.CR.ID
    ...
    CALL CACHE.READ(FN.GROUP.DATE, Y.CDN.GROUP, R.GROUP.DATE, Y.DATE.ERR)
    ...
    WHILE GP.CR.ID:GP.CR.POS
        ...
        CALL CACHE.READ(FN.GROUP.CREDIT.INT, Y.GP.CR.ID, R.GROUP.CREDIT.INT, GP.CR.ERR)
        IF Y.INT.RATE  THEN
