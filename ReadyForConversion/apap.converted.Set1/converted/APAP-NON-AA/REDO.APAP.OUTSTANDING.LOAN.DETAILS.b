...
*---------------
    ...
    Y.OWNER.S = Y.CUSTOMER.CONDITION<AA.CUS.OWNER>
...
CALL.GET.CHARGE:
*---------------
    CALL REDO.APAP.GET.CHARGE.AMT(Y.ARR.ID,Y.LOAN.BK.TYP,Y.FIRST.DISB.DATE,Y.END.DATE,Y.BALANCE.CHARGE,Y.TOT.BAL.DUE)     ;*------ 31ST & 32ND FIELD VALUE
    Y.BALANCE.CHARGE -= Y.TOT.BAL.DUE
RETURN
*-----------------------------------------------------------------------------
CALL.GET.ACC.INT:
*---------------
    CALL REDO.APAP.GET.INT.ACC(Y.ARR.ID,Y.FIRST.DISB.DATE,Y.END.DATE,Y.UNPAID.BILL.CNT,Y.DELAYED.BILLS.VAL)     ;*----------------- 36TH & 37TH FIELD VALUE
RETURN
*-----------------------------------------------------------------------------
ACCT.INT.BAL:
*-----------------------------------------------------------------------------
