SUBROUTINE REDO.FC.S.COLL.DI.MONT.DISP

*    This Routine is on charge to calculate the available amount that can use in Depositos Internos
...
*Get the prymary key customer
            ...
            IF YERROR THEN
                ETEXT = "EB-FC-READ.ERROR" : @FM : FN.AZ
                CALL STORE.END.ERROR
            END ELSE

*Value of fix deposit
                VAR.VALOR    =  R.AZ<AZ.PRINCIPAL>
                SELECT.STATEMENT = 'SELECT ':FN.LOCK1 :' WITH ACCOUNT.NUMBER LIKE ':VAR.CUENTA
                LOCK.LIST = ''
                LIST.NAME = ''
                SELECTED = ''
                SYSTEM.RETURN.CODE = ''
