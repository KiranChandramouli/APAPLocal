...
*-----------------------------------------------------------------------------
    ...
    LOC.REF.FIELD = 'L.CU.NOM.AIR':@VM:'L.CU.NO.VIA.F'
...
*    CALL F.READ(FN.REDO.LY.PROGAERO,'SYSTEM',R.PROGAERO,F.REDO.LY.PROGAERO,PROGAERO.ERR) ;*Tus Start
        ...
        IF R.PROGAERO THEN
            ...
            CHANGE @VM TO @FM IN Y.PROG.REL
            CHANGE @VM TO @FM IN Y.AIRL.REL
    ...
    Y.TOT.CUS.WR = DCOUNT(NEW.FILE.CONTENT,@VM)
...
*-----------------------------------------------------------------------------
    ...
    Y.TOT.CUS.WR = 0
    Y.TOT.CUS.WR = DCOUNT(NEW.FILE.CONTENT,@VM) - 1
    Y.CUS.WR = 1
    LOOP
    WHILE Y.CUS.WR LE Y.TOT.CUS.WR
        Y.CUS.WR.LINE = FIELD(NEW.FILE.CONTENT,@VM,Y.CUS.WR)
        WRITESEQ Y.CUS.WR.LINE APPEND TO F.PATH ELSE
        END
        Y.CUS.WR += 1
    REPEAT
...
*  READU FILE.CONTENT FROM F.REDO.LY.CUST.NOTRAVREC,Y.PTS.ID THEN ;*Tus Start
    ...
    WHILE COUNTER1 LE Y.TOT.PROD
        Y.PROG.TO.CHK = FIELD(Y.PROGRAMS,@VM,COUNTER1)
        Y.TOT.PROG = DCOUNT(Y.PROG.TO.CHK,@SM)
        COUNTER2 = 1
        LOOP
        WHILE COUNTER2 LE Y.TOT.PROG
            Y.PROGRAM.ID = FIELD(Y.PROG.TO.CHK,@SM,COUNTER2)
...
*-------------
    ...
    VAR.MILES.TO.CHK = FIELD(VAR.MILES,@VM,COUNTER1)
    VAR.MILES.VAL.TO.CHK = FIELD(VAR.MILES.VAL,@VM,COUNTER1)
    VAR.GEN.DATE.TO.CHK = FIELD(VAR.GEN.DATE,@VM,COUNTER1)
    VAR.STATUS.TO.CHK = FIELD(VAR.STATUS,@VM,COUNTER1)
    Y.MILES = FIELD(VAR.MILES.TO.CHK,@SM,COUNTER2)
    Y.MILES.VAL = FIELD(VAR.MILES.VAL.TO.CHK,@SM,COUNTER2)
    Y.GEN.DATE = FIELD(VAR.GEN.DATE.TO.CHK,@SM,COUNTER2)
    Y.STATUS = FIELD(VAR.STATUS.TO.CHK,@SM,COUNTER2)
...
*----------
    ...
    IF R.CUSTOMER THEN
        ...
        CHANGE @VM TO @FM IN Y.AIR.NAMES
        CHANGE @VM TO @FM IN Y.AA.NOS
...
*----------------------------------------------------------
    ...
    END ELSE
        ...
        R.REC.POINTS<REDO.PT.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR
        R.REC.POINTS<REDO.PT.DATE.TIME> = G.DATE[3,6]:CUR.TIME
        R.REC.POINTS<REDO.PT.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR
...
*-----------------------------------------------------------------------------
    ...
    Y.GEN.DATE.SET := FIELD(Y.REC.COM,',',1):@VM
    Y.AA.NO.SET := FIELD(Y.REC.COM,',',2):@VM
    Y.TOT.MILES.SET := FIELD(Y.REC.COM,',',3):@VM
    Y.SEC.NAME.SET := FIELD(Y.REC.COM,',',4):@VM
    Y.FIRST.NAME.SET := FIELD(Y.REC.COM,',',5):@VM
...
*-------------------------------------------------------------------------------
    ...
    R.REDO.LY.MFILE.BKP<REDO.MFILE.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR
    R.REDO.LY.MFILE.BKP<REDO.MFILE.DATE.TIME> = G.DATE[3,6]:CUR.TIME
    R.REDO.LY.MFILE.BKP<REDO.MFILE.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR
