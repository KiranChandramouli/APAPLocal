...
* Obtain the local ref position
    ...
    LOC.REF.FIELDS = 'L.AA.REV.RT.TY' : @FM : 'L.COL.NUM.INSTR'
    LOC.REF.POS = ''
...
* is not used due to specific user's requirements
        ...
        IF Y.ACCOUNT.ID THEN
            ...
            IF Y.ACI.DATE EQ '' THEN
                Y.ACI.ID = ''
            END ELSE
                Y.ACI.ID = Y.ACCOUNT.ID:'-':Y.ACI.DATE<1,DCOUNT(Y.ACI.DATE,@VM)>
            END
            IF Y.APP.ID NE '' THEN
            ...
            ELSE
                ...
                ELSE
                    ...
                    CALL CACHE.READ(FN.GROUP.DATE, Y.COND.AND.CURR, R.GROUP.DATE, GROUP.ERR)
...
UPDATE.NOMINAL:
*----------------------------------------------------------------------------
    TOT.FIXED = DCOUNT(R.NEW(AA.INT.FIXED.RATE),@VM)
    TIER.I = 1
