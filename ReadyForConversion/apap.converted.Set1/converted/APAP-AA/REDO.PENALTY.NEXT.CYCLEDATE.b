...
*TUS AA Changes - 20161019
    ...
    R.AA.ACC  = ''
    ERR.MSG           = ''
    CALL REDO.CRR.GET.CONDITIONS(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.AA.ACC,ERR.MSG)
...
*----------------------------------------------------------
    ...
    IF Y.REPAY.REF EQ '' AND Y.REPAY.AMT EQ '' THEN
        ...
        RETURN
    END
    Y.REPAY.CNT = DCOUNT(Y.REPAY.REF,@SM)
    LOOP
...
*-----------------------------------------
    ...
    R.CONDITION.TERM  = ''
    ERR.MSG           = ''
    CALL REDO.CRR.GET.CONDITIONS(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.TERM,ERR.MSG)
