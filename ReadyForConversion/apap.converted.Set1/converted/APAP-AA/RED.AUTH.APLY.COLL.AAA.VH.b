...
OPEN.FILES:
    ...
    POS.CA = ''
    L.APPLN = 'COLLATERAL':@FM:'AA.PRD.DES.TERM.AMOUNT'
    L.FLDS = 'L.AC.LK.COL.ID':@VM:'L.COL.PRO.DESC2':@FM:'L.AA.COL':@VM:'L.AA.COL.VAL':@VM:'L.AA.AV.COL.BAL':@VM:'L.AA.COL.DESC'
...
*
    ...
    Y.AA.ID = CHANGE(Y.AA.ID,@SM,@VM)                  ; Y.CNT.AA = DCOUNT(Y.AA.ID,@VM)
...
* PACS00307565 - E
    ...
    Y.DESC.GAR = R.NEW(COLL.LOCAL.REF)<1,POS.DESC> ; Y.DESC.GAR = CHANGE(Y.DESC.GAR,@SM,@VM) ; Y.DESC.GAR = CHANGE(Y.DESC.GAR,@VM,' ')
    Y.OLD.DESC = R.OLD(COLL.LOCAL.REF)<1,POS.DESC> ; Y.OLD.DESC = CHANGE(Y.OLD.DESC,@SM,@VM) ; Y.OLD.DESC = CHANGE(Y.OLD.DESC,@VM,' ')
...
* ///////////////// R.NEW change processing
    ...
    Y.AA.ID.TMP     = Y.AA.ID
    Y.AA.ID         = ''
    W.VAR = 1 ; Y.NEW.CNT = DCOUNT(Y.AA.ID.TMP,@VM)
    LOOP
    WHILE W.VAR LE Y.NEW.CNT
        Y.AA.SD = '' ; Y.AA.SD = Y.AA.ID.TMP<1,W.VAR>
...
* ////////////////// R.OLD change processing
    ...
    Y.OLD.AA.ID     = ''
    X.VAR = 1 ; Y.OLD.CNT = DCOUNT(Y.OLD.AA.ID.TMP,@SM)
    LOOP
    WHILE X.VAR LE Y.OLD.CNT
        Y.AA.SD = '' ; Y.AA.SD = Y.OLD.AA.ID.TMP<1,1,X.VAR>
        IF Y.AA.SD[1,2] NE "AA" THEN
            GOSUB FIND.BY.ACCOUNT
        END
...
* PACS00307565 - E
        ...
        Y.AA.COLL.IDS = CHANGE(Y.AA.COLL.IDS,@SM,@VM) ; Y.CL.CNT = DCOUNT(Y.AA.COLL.IDS,@VM)

        Y.AA.COLS.VALS = returnConditions<AA.AMT.LOCAL.REF,Y.COL.VAL.POS> ;
        Y.AA.COLS.VALS = CHANGE(Y.AA.COLS.VALS,@SM,@VM)
* PACS00307565 - S
        Y.AA.BAL.VALS  = returnConditions<AA.AMT.LOCAL.REF,Y.COL.AV.POS>
        Y.AA.BAL.VALS  = CHANGE(Y.AA.BAL.VALS,@SM,@VM)
...
PROCESS.DEL:
    ...
    Y.OLD.AA.ID = CHANGE(Y.OLD.AA.ID,@SM,@VM)
    Y.OLD.VM = DCOUNT(Y.OLD.AA.ID,@VM)
    FLG.O = ''
    ...
    WHILE Y.OLD.VM GT 0 DO
        ...
        POS.OLD = '' ; Y.AA.ID.ARR = Y.AA.ID ; Y.AA.ID.ARR = CHANGE(Y.AA.ID.ARR,@VM,@FM)
        ...
        LOCATE Y.AA.SD IN Y.AA.ID.ARR SETTING POS.OLD ELSE
            ...
            Y.AA.COLL.ODS = CHANGE(Y.AA.COLL.ODS,@SM,@VM) ; Y.CL.CNT.OD = DCOUNT(Y.AA.COLL.ODS,@VM)

            Y.AA.COLS.VALS.OD = returnConditions<AA.AMT.LOCAL.REF,Y.COL.VAL.POS>
            Y.AA.COLS.VALS.OD = CHANGE(Y.AA.COLS.VALS.OD,@SM,@VM)
