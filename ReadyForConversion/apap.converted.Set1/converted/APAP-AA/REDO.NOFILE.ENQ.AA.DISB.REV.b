...
*----------------
    ...
    IF R.REDO.AA.DISB.REVERSE AND NOT(DISB.ERR) THEN
        ...
        Y.DISB.CNT = 0
        Y.DISB.CNT = DCOUNT(Y.DISB.REF.LIST,@VM)
        TEMP.REF.LIST = ''
        Y.VAR1 = 1
        LOOP
...
*------------------
    Y.TEMP.REF.CNT = 0
    Y.TEMP.REF.CNT = DCOUNT(Y.TEMP.REF.LIST<1,Y.VAR1>,@SM)
    Y.REF1 = 1
    LOOP
...
* This part is to fetch the FT records to authorize the reversed record.
    ...
    CHANGE @VM TO @FM IN Y.ACTIVITIES
    CHANGE @SM TO @FM IN Y.ACTIVITY.REF
    CHANGE @VM TO @FM IN Y.ACTIVITY.REF
    CHANGE @SM TO @FM IN Y.ACTIVITY.STATUS
    CHANGE @VM TO @FM IN Y.ACTIVITY.STATUS
