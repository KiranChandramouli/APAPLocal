...
*------------------------------------------------------------------------
    ...
    IF c_aalocActivityStatus MATCHES 'AUTH':@VM:'UNAUTH':@VM:'UNAUTH-CHG' THEN
        GOSUB OPEN.FILES
        GOSUB PROCESS
    END
    ...
    IF Y.PAYOFF.ACTIVITY THEN
        LOCATE AA$CURR.ACTIVITY IN Y.PAYOFF.ACTIVITY<1,1> SETTING POS.PA THEN
            IF c_aalocActivityStatus MATCHES 'UNAUTH':@VM:'UNAUTH-CHG' THEN
                CALL REDO.POST.PAYOFF.REM.PAYSCH  ;* This routine to remove the scheduled charge after payoff.
            END
...
*-----------------------------------------------------------
    ...
    ERR.MSG = ''
    VAR.ID = c_aalocArrId
    CALL REDO.CRR.GET.CONDITIONS(VAR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.TERM.AMOUNT,ERR.MSG)
