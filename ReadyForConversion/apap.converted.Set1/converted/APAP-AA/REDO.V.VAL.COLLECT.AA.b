...
GET.DESCRIPTIONS.ST:
    ...
    Y.ST.CNT = DCOUNT(LOAN.STATUS,@VM) ; Y.FL.ST = '' ; LOAN.STATUS.DUP = ''
    LOOP
    WHILE Y.ST.CNT GT 0 DO
        ...
        Y.ST.CNT -= 1
    REPEAT
    LOAN.STATUS.DUP = CHANGE(LOAN.STATUS.DUP,@FM,@VM)
    LOAN.STATUS = LOAN.STATUS.DUP


    Y.LOOKUP.ID   = "L.LOAN.COND"
    Y.CD.CNT = DCOUNT(LOAN.COND,@VM) ; Y.FL.CD = '' ; LOAN.COND.DUP = ''
    LOOP
    ...
    LOAN.COND.DUP = CHANGE(LOAN.COND.DUP,@FM,@VM)
...
* PACS00126000 -S
    Y.CNT.BL.PR = DCOUNT(Y.PS.PROPS,@VM)
    FLG.BL = ''
    LOOP
