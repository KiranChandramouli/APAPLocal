...
*
    ...
    Y.REL.CODE = CHANGE(R.CUSTOMER<EB.CUS.RELATION.CODE>,@VM,@SM)
...
* May be the contract reached its maturity date
        E = K.REC.NOT.FOUND
        E<2> = P.AA.ID : @VM : FN.AA.ARRANGEMENT
        RETURN
    END
...
*
    IF NOT(L.AA.CAMP.TY) THEN
        ...
        E<2> = "L.AA.CAMP.TY" : @VM : "AA.PRD.DES.CUSTOMER"
        RETURN
    END
...
*--------------------------------------------------------------------------------------------
    ...
    WHILE LOOP.CNT LE MAX.LOOPS AND PROCESS.GOAHEAD DO
        BEGIN CASE
            CASE LOOP.CNT EQ 1
                IF P.AA.ID EQ "" THEN
                    E = K.PARAMETER.IS.EMPTY : @FM : "P.AA.ID" : @VM : "S.REDO.CCRG.AA.EVALUATOR"
                    PROCESS.GOAHEAD = @FALSE
                END
            CASE LOOP.CNT EQ 2
                IF R.RCBTP EQ "" THEN
                    E = K.PARAMETER.IS.EMPTY : @FM : "R.RCBTP" : @VM : "S.REDO.CCRG.AA.EVALUATOR"
                    PROCESS.GOAHEAD = @FALSE
                END
            CASE LOOP.CNT EQ 3
                IF R.CUSTOMER EQ "" THEN
                    E = K.PARAMETER.IS.EMPTY : @FM : " R.CUSTOMER" : @VM : "S.REDO.CCRG.AA.EVALUATOR"
                    PROCESS.GOAHEAD = @FALSE
                END
        END CASE
