...
*
  ...
  Y=1 ; Y.CO.VMP = '' ; Y.TMP.CO.VMP = ''
  LOOP
  WHILE Y LE W.COUNT.C
*
    GOSUB GET.COID
    GOSUB CALC.INDV.UA
*
  ...
  REPEAT
*
  PAY.AMOUNT.BALANCE = PAY.AMOUNT.BALANCE - PAY.AMOUNT
  PAY.AMOUNT.OG = PAY.AMOUNT.OG - PAY.AMOUNT
*
  GOSUB REGISTRA
  GOSUB RETURN.SD
  RETURN
...
*
  IF Y.TMP.TOTAMT GT 0 AND TOTAL.AMT NE 0 THEN    ;* Pending coll. amount to support outstanding AA balance
    PAY.SD<1,Y> = Y.CO.VMP    ;* Using maximum amount of co. record
    Y.CO.VMP = ''
  END
*
...
*
  IF Y.TMP.TOTAMT LT 0 AND NOT(Y.FLG.COV) AND TOTAL.AMT NE 0 THEN     ;* Current collateral(s) amount(s) supporting outstanding AA balance
    PAY.SD<1,Y> = Y.BEF.DISCNT          ;* Using remaining amount (pending Outstanding bal) of curr. co. record
    Y.FLG.COV = 1
  END
  RETURN
...
* Get all collateral linked to AA - TERM.AMOUNT
  ...
  MMARK = CHAR(251) ; Y.COL.ID.MIG = CHANGE(COL.ID.LINKED, MMARK , VM )
  RETURN
...
* Updating available collateral balance for S-Multiple attached records
  Y = 1 ; W.CL.NUM = DCOUNT(PAY.COLL.ID,VM) ; Y.CO.VMP = ''
  LOOP
  WHILE Y LE W.CL.NUM
    Y.CL.ID = PAY.COLL.ID<1,Y>
    GOSUB UPD.CO.AVAMT
    Y++
  REPEAT
  RETURN
...
*
  ...
  IF Y.CO.CODE EQ '150' AND Y.NEW.F NE "" THEN    ;* New contract and DI product.
    Y.AVAIL.AMT.BAL = R.REDO.FC.CL.BALANCE<FC.CL.MG.ORIGINAL,Y>       ;* Summation of all Original amts (Guaranteed amts for NEW recs.) of CO records attached to Loan.
  END
...
*
  ERR.CO = '' ; R.COLLATERAL = ''
  CALL F.READ(FN.COLLATERAL,PAY.COLL.ID<1,Y>,R.COLLATERAL,F.COLLATERAL,ERR.CO)
