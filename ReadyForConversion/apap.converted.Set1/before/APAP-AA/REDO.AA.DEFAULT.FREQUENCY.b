...
FREQ.DATE.VALIDATION:
    ...
    START.DATE.CNT = DCOUNT(R.PAYMENT.SCHEDULE<AA.PS.START.DATE>,FM)
...
GET.PROP.COUNT:
    ...
    PROP.CNT = DCOUNT(R.PAYMENT.SCHEDULE<AA.PS.DUE.FREQ,PAY.TYPE>,SM)
...
*-----------------------------------------------------------------------------------------------------------
    ...
    Y.PAY.TYPES.CNT = DCOUNT(Y.PAY.TYPES,VM)
    Y.VAR1 = 1
    LOOP
    WHILE Y.VAR1 LE Y.PAY.TYPES.CNT
        ...
        CALL F.READ(FN.AA.PAYMENT.TYPE,Y.PAY.TYP,R.PAYMENT.TYPE,F.AA.PAYMENT.TYPE,PAY.ERR)
