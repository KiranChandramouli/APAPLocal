...
* ///////////////// R.NEW change processing
  ...
  Y.AA.ID.TMP     = Y.AA.ID
  Y.AA.ID         = ''
  W = 1 ; Y.NEW.CNT = DCOUNT(Y.AA.ID.TMP,VM)
  LOOP
  WHILE W LE Y.NEW.CNT
    Y.AA.SD = '' ; Y.AA.SD = Y.AA.ID.TMP<1,W>
...
* ////////////////// R.OLD change processing
  ...
  Y.OLD.AA.ID     = ''
  X = 1 ; Y.OLD.CNT = DCOUNT(Y.OLD.AA.ID.TMP,SM)
  LOOP
  WHILE X LE Y.OLD.CNT
    Y.AA.SD = '' ; Y.AA.SD = Y.OLD.AA.ID.TMP<1,1,X>
    IF Y.AA.SD[1,2] NE "AA" THEN
      GOSUB FIND.BY.ACCOUNT
    END
