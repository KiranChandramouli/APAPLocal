...
* PACS00313556 - 2014OCT10 - S
    CALL REDO.APAP.NOFILE.AA.LAON.STATUS.RPT.SPLIT.2(AA.ARRANGEMENT.ID,Y.AC.CONTRACT,Y.CAP.TOT.BAL,Y.INT.TOT.BAL,Y.INS.TOT.BAL,Y.CHG.TOT.BAL)
...
*************************
  ...
  CUSTOMER.ID = R.CONDITION<AA.CUS.PRIMARY.OWNER>
...
* PACS00313556 - S
  ...
  I=1
  LOOP
  WHILE I LE Y.CNT.COLS
    IF Y.GUARANT.NO<I> EQ "" THEN       ;* Dropping FM collateral id null values
      DEL Y.GUARANT.NO<I>
    END
    I ++
  REPEAT
