...
***-----------------------------------------------------------------------------
    ...
    BEGIN CASE
    CASE CONTROL.LIST<1,1> = 'Y.AP.LOANS'
        GOSUB PROCESS.AP.LOANS
    CASE CONTROL.LIST<1,1> = 'Y.AP.CARDS'
...
*BUCLE PARA LA LECTURA DE RELATION.CODE PARA SABER SU CONYUGE
    ...
    I = NRO.REL.CODE
    LOOP WHILE I GE 1
        IF Y.RELATION.CODE<1,I> MATCHES "7" : VM : "8" THEN
            Y.CUS.CODE.CONYUGE = R.CUSTOMER<EB.CUS.REL.CUSTOMER,I>
...
*ASIGNACION DE LA INFORMACION RESPECTO A EL CONYUGE
        ...
        IF Y.RELATION.CODE<1,I> EQ 104 THEN       ;*ASIGNACION DEL TIPO DE DOCUMENTO DEL REPRESENTANTE
            Y.CUS.CODE.REPRES = R.CUSTOMER<EB.CUS.REL.CUSTOMER,I>
...
* PP, take first position   for NAME.1 & NAME.2
    I = NRO.REL.CODE.PER
    LOOP WHILE I GE 1
        Y.TIPO.CLIENTE = Y.CLI.TIPO.PERSONA<1,1,I>
        GOSUB GET.TIPO.PERSONA
        Y.CLI.TIPO.PERSONA<1,1,I> = Y.TIPO.CLIENTE
        IF Y.CLI.TIPO.PERSONA<1,1,I> EQ 0 THEN    ;*ASIGNACION NOMBRE CLIENTE JURIDICA
            Y.CLI.RAZON.SOCIAL.NOM1 = R.CUSTOMER<EB.CUS.NAME.1, 1>
            Y.CLI.RAZON.SOCIAL.NOM2 = R.CUSTOMER<EB.CUS.NAME.2, 1>
        END  ELSE
            IF Y.CLI.TIPO.PERSONA<1,1,I> EQ 1 THEN          ;*ASIGNACION NOMBRE Y APELLIDO CLIENTE NATURAL
...
*    CALL CACHE.READ('F.REDO.COL.TRACE.PHONE',CUSTOMER.ID,R.COL.TRACE.PHONE,Y.ERR)
    ...
    Y.TEL.TYPE.SEQ  = ''
    I = 1
    LOOP WHILE I LE NRO.REL.CODE.TEL
*        Y.INSERT.TEL = C_DML_STMT_TELEFONOS
        Y.CLI.TELF.TYPE = R.CUSTOMER<EB.CUS.LOCAL.REF, Y.CLI.TELF.TYPE.POS, I>
...
*-------------------
    ...
    Y.CLI.TELF.NO = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
    Y.CLI.TELF.AREA.DC= R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
    Y.CLI.TELF.NO=Y.CLI.TELF.AREA.DC:Y.CLI.TELF.NO
    Y.CLI.TELF.EXT = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.EXT.POS,I>
...
*-------------------
    IF Y.TIPO.CLIENTE MATCHES "...FISICA" OR   Y.TIPO.CLIENTE  MATCHES "...MENOR" THEN
        Y.TIPO.CLIENTE = 1
    END
    IF Y.CLI.TIPO.PERSONA<1,1,I> MATCHES "...JURIDICA" THEN
        Y.TIPO.CLIENTE = 0
    END
...
*-------------------
    ...
    LOOP WHILE I GE 1
        BEGIN CASE
        CASE Y.CLI.TELF.TYPE<1,1,I> EQ "05"
            Y.CLI.TELF.AREA.10 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
            Y.CLI.TELF.NO.10   = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
            Y.CLI.TELF.EXT.10  = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.EXT.POS,I>
            Y.CLI.CONTACTO     =  R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TEL.P.CONT,I>
        CASE Y.CLI.TELF.TYPE<1,1,I> EQ "01"
            Y.CLI.TELF.AREA.11 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
            Y.CLI.TELF.NO.11 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
        CASE Y.CLI.TELF.TYPE<1,1,I> EQ "06"
            Y.CLI.TELF.AREA.13 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
            Y.CLI.TELF.NO.13 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
        END CASE
        I--
    REPEAT
    RETURN
...
*-----------------------------------------------------------------------------------
    ...
    Y.CREDIT = ""
    Y.CREDIT.TXN = ""
    CALL REDO.COL.FF.EXTRACT.CREDIT(CUSTOMER.ID, Y.CREDIT, Y.CREDIT.TXN)
*    IF E THEN
*        RETURN
*    END
