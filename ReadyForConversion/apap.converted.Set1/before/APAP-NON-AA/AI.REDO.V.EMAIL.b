...
*       ID Process
  ...
  PREV.SPC = 'N'
  FOR I = 1 TO LEN(EMAIL.ID)
    IF REGEXP(EMAIL.ID[I,1],ER.STR.SPC) NE 0 THEN
      IF PREV.SPC EQ 'Y' OR I EQ 1 OR I EQ LEN(EMAIL.ID) THEN
        ETEXT ='AI-INVALID.EMAIL'
        CALL STORE.END.ERROR
        RETURN
      END
  ...
  LOOP WHILE NOT.END EQ 0 DO
    ...
    PREV.SPC = 'N'
    FOR I = 1 TO LEN(PREV.DOM)
      IF REGEXP(PREV.DOM[I,1],ER.STR.SPC) NE 0 THEN
        IF PREV.SPC EQ 'Y' OR I EQ 1 OR I EQ LEN(PREV.DOM) THEN
          ETEXT ='AI-INVALID.EMAIL'
          CALL STORE.END.ERROR
          RETURN
        END
