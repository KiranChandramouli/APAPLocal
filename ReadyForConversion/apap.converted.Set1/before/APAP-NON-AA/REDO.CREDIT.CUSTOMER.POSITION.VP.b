...
*******************
  ...
  FOR I = 1 TO Y.CC.NUM
    CUST.DATA = ''
* Numero_Plastico
    CCARD.NUM = FIELD(WS.DATA<4>,VM,I)
    CCARD.NUM = CCARD.NUM[4,LEN(CCARD.NUM)]
    CUST.DATA<5> = CCARD.NUM
* Balance_total_RD
    CUST.DATA<10> = FIELD(WS.DATA<7>,VM,I)
* Tipo_producto
    CUST.DATA<11> = FIELD(WS.DATA<2>,VM,I)
* Balance_DisponibleRD
    CUST.DATA<12> = FIELD(WS.DATA<9>,VM,I)
* Balance_Total_US
    CUST.DATA<13> = FIELD(WS.DATA<8>,VM,I)
* Balance_DisponibleUS
    CUST.DATA<15> = FIELD(WS.DATA<10>,VM,I)

** Remaining fields from WS Invocation not used
* NUMERO_CUENTA
    CACCT.NUM = FIELD(WS.DATA<3>,VM,I)
