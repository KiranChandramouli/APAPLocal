...
* <Rating>-195</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE REDO.APAP.NOF.CASH.WINDOW.DEP.TEXT.R32(Y.CCY.LIST,Y.COMPANY.LIST,Y.FINAL.ARRAY)
*********************************************************************************************************
*Company   Name    : ASOCIACION POPULAR DE AHORROS Y PRESTAMOS
*Developed By      : Temenos Application Management
*Program   Name    : REDO.APAP.NOF.CASH.WINDOW.DEP.TEXT.R32
*--------------------------------------------------------------------------------------------------------
...
*   ------             -----                       -------------             -------------
* 15 Jun 2011       Marimuthu S              ODR-2011-04-0007 35         Modification made in REDO.APAP.NOF.CASH.WINDOW.DEP.TEXT
...
* In this para of the code, the static text is added in the report for display
    ...
    WHILE Y.REC.START LE Y.REC.COUNT
        Y.VALS = Y.FINAL.ARRAY<Y.REC.START>
        IF Y.VALS NE '' THEN
            GOSUB DEPOSITS
...
*PACS00106559 - S
    ...
    Y.OTR.LOANS = FIELD(Y.OTR.LOANS,VM,41,99)
...
* Fix for PACS00311265 [REPORT 32 SPANISH TRANSLATION #2]

*    Y.FINAL.ARRAY<Y.REC.START,18,2>  = 'OTHER'
    Y.FINAL.ARRAY<Y.REC.START,18,2>  = 'OTROS'
...
********************
    YCCY.VAL = ''
    YCCY.VAL = Y.CCY.LIST<Y.REC.START>
...
* PACS00708935 - S
    Y.I = 1
    LOOP
    WHILE Y.I LE Y.COMP.COUNT
        Y.FINAL.ARRAY<Y.REC.START,Y.TOT.POS-1> := VM
        Y.I += 1
    REPEAT
* PACS00708935 - E
*
...
*  Y.FINAL.ARRAY<Y.REC.START,Y.TOT.POS,2>  = 'TOTAL PAGOS TERCEROS'
    ...
    Y.FINAL.ARRAY<Y.REC.START,Y.TOT.POS,3> = 0
    Y.FINAL.ARRAY<Y.REC.START,Y.TOT.POS,4> = 0
...
CREDIT.ADJUSTMENT:
******************
    Y.CR.ADJ.POS = 20 + Y.COMP.COUNT + 9

    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,1>  = Y.CCY.LIST<Y.REC.START>
*    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,2>  = 'SOBRANTE'

* Fix for PACS00311265 [REPORT 32 SPANISH TRANSLATION #2]

*    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,8>  = 'CREDIT.ADJUSTMENT'
    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,8>  = 'AJUSTE DE CREDITO'

* End of Fix

    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,4>  = ''
    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,5>  = ''
    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,6>  = ''
    Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,3> = ''

* IF NOT(Y.FINAL.ARRAY<Y.REC.START,Y.OVER.POS,3>) THEN
*     Y.FINAL.ARRAY<Y.REC.START,Y.OVER.POS,3> = ''
* END

    IF NOT(Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,7>) THEN
        Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,7> = 0
    END
...
* PACS00702382 - S
    Y.FINAL.ARRAY<Y.REC.START,Y.TI.POS,7> = Y.FINAL.ARRAY<Y.REC.START,4,7> + Y.FINAL.ARRAY<Y.REC.START,9,7> + Y.FINAL.ARRAY<Y.REC.START,14,7> + Y.FINAL.ARRAY<Y.REC.START,19,7> + Y.FINAL.ARRAY<Y.REC.START,Y.TOT.POS,7> + Y.FINAL.ARRAY<Y.REC.START,Y.OIH.POS,7> + Y.FINAL.ARRAY<Y.REC.START,Y.CT.POS-1,7> + Y.FINAL.ARRAY<Y.REC.START,Y.CT.POS,7> + Y.FINAL.ARRAY<Y.REC.START,Y.BUY.POS-1,7> + Y.FINAL.ARRAY<Y.REC.START,Y.BUY.POS,7> + Y.FINAL.ARRAY<Y.REC.START,Y.CR.ADJ.POS,7> + Y.FINAL.ARRAY<Y.REC.START,Y.OVER.POS,7>
* PACS00702382 - E
    RETURN
*--------------------------------------------------------------------------------------------------------
********
