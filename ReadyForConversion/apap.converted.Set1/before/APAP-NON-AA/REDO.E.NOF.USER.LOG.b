...
*------------------------------------------------------
  FOR I=1 TO SER.CNT
    ...
    CASE Y.DATE.OPERAND EQ 1
      IF B.DATE<I> EQ Y.DATE THEN
        GOSUB ENQ.REC.UPDATE
      END
    CASE Y.DATE.OPERAND EQ 2
      DAT1 = FIELD(Y.DATE, SM, 1)
      DAT2 = FIELD(Y.DATE, SM, 2)
      IF B.DATE<I> GE DAT1 AND B.DATE<I> LE DAT2 THEN
        GOSUB ENQ.REC.UPDATE
      END
    CASE Y.DATE.OPERAND EQ 3
      IF B.DATE<I> LT Y.DATE THEN
        GOSUB ENQ.REC.UPDATE
      END
    CASE Y.DATE.OPERAND EQ 8
      IF B.DATE<I> LE Y.DATE THEN
        GOSUB ENQ.REC.UPDATE
      END
    CASE Y.DATE.OPERAND EQ 9
      IF B.DATE<I> GE Y.DATE THEN
        GOSUB ENQ.REC.UPDATE
      END
    CASE Y.DATE.OPERAND EQ 4
      IF B.DATE<I> GT Y.DATE THEN
        GOSUB ENQ.REC.UPDATE
      END
    END CASE
  NEXT I
...
ENQ.REC.UPDATE:
*-----------------------------------------------------
  LOCATE B.DATE<I> IN B.DATE SETTING B.DATE.POS THEN
    Y.DAT = B.DATE<I>
