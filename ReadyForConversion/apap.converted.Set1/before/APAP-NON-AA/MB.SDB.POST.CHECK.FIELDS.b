...
CHECK.FIELDS:
        ...
        IF R.NEW(SDB.POST.SDB.OPERATE) = 'OPEN' THEN
            ...
            IF DISCOUNT.FLAT OR DISCOUNT.PERCENT THEN
                IF DISCOUNT.PERCENT THEN
                    ...
                    PERIODIC.RENT = PERIODIC.RENT - CUST.DISC.AMT
                END ELSE
    ...
    CASE AF = SDB.POST.COLLECT.NEXT.P
        IF R.NEW(SDB.POST.SDB.OPERATE) = 'OPEN' THEN
    ...
    CASE AF = SDB.POST.INITIAL.OFFER.AMT
        IF R.NEW(SDB.POST.SDB.OPERATE) = 'OPEN' THEN
    ...
    CASE AF = SDB.POST.CUSTOMER.ACCT
        IF R.NEW(SDB.POST.SDB.OPERATE) = 'OPEN' THEN
            SDB.AVAIL.COMP = R.MB.SDB.TYPE<SDB.TYP.BRANCH.CODE>
            LOCATE R.NEW(SDB.POST.SDB.COMPANY) IN SDB.AVAIL.COMP<1,1> SETTING SDB.COMP.POS THEN
                PERIODIC.RENT = R.MB.SDB.TYPE<SDB.TYP.PERIODIC.RENT,SDB.COMP.POS>
...
GET.START.DATE:
    ...
    END ELSE
        ...
        IF Y.MONTH GT 1 THEN
            Y.MONTH -= 1
            IF LEN(Y.MONTH) EQ 1 THEN Y.MONTH = "0":Y.MONTH
    ...
    IF Y.DAY GT YLAST.DAY THEN Y.DAY = YLAST.DAY
...
DEFAULT.FIELDS:
    ...
    BEGIN CASE
    CASE AF = SDB.POST.ACCESS.DATE AND V$FUNCTION = 'I'
        IF COMI = '' THEN
            COMI = TODAY
        END
    CASE AF = SDB.POST.ACCESS.TIME AND V$FUNCTION = 'I'
        IF COMI = '' THEN
