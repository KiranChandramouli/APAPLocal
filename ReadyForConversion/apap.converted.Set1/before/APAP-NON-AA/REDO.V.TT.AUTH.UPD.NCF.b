...
*
  ...
  LOOP
    Y.WV.CM.YN = ''
  WHILE (I LE YNUM.COMM)
    Y.WV.CM.YN = Y.WV.COM<1,1,I>
    IF Y.WV.CM.YN EQ "NO" THEN
      WCOM.VAL += Y.COM.AMT<1,1,I>
      WARR.COM<-1> = Y.COM.COD<1,1,I>
    END ELSE
      WCOM.WAIVED += Y.COM.AMT<1,1,I>
    END
    I = I + 1
  REPEAT
*
...
*
  ...
  LOOP
    Y.WV.TX.YN = ''
  WHILE (J LE YNUM.TAX)
    Y.WV.TX.YN = Y.WV.TAX<1,1,J>
    IF Y.WV.TX.YN EQ 'NO' THEN
      WTAX.VAL += Y.TAX.AMT<1,1,J>
      WARR.TAX<-1> = Y.TAX.COD<1,1,J>
    END ELSE
      WTAX.WAIVED += Y.TAX.AMT<1,1,J>
    END
    J = J + 1
  REPEAT
*
...
* COMBINED BOTH COMMISSION AND TAX CALC TO MAKE SINGLE REQUEST TO THE NCF GENERATION
  ...
  IF Y.NCF.CNT NE 0 THEN
    CALL REDO.NCF.PERF.RTN(Y.NCF.CNT,Y.NCF.LIST)
  END
...
* Fix for PACS00265097[NCF ISSUE - ENRICHMENT FOR ID.NUMBER #4]
  ...
  CALL F.READ(FN.ACCOUNT.CLASS,'NOSTRO',R.ACCOUNT.CLASS,F.ACCOUNT.CLASS,ACCOUNT.CLASS.ERR)
