...
*
  ...
  Y.STT.REC        = ""
  Y.IN.MSG         = Y.NEW.TXT.MSG<I>
  Y.NEW.TXT.MSG<I> = Y.NEW.TXT.MSG<I>:",":W.ADDITIONAL.INFO
  CALL REDO.FI.MSG.FORMAT(FI.INTERFACE,Y.NEW.TXT.MSG<I>,DATO.OUT)
  R.PARAM<1>  = "NULL"
  R.PARAM<2>  = "N"
...
* Y.OUT.MSG = Y.OUT.MSG :"@":W.STATUS:"@":W.ERROR.MSG:"@":OUT.RESP<1>
  ...
  IF Y.ERR.MSG NE "" THEN
    CALL REDO.INTERFACE.REC.ACT(FI.INTERFACES,"BATCH",WRECORD.NUMBER,Y.TOT.REC,FI.W.REDO.FI.CONTROL.ID,FI.FILE.ID,"","04",Y.ERR.MSG,Y.OUT.MSG,"","")
    IF R.PARAM<10> EQ "N" THEN
      W.REC.FT.ERR += 1
      W.TOT.FT.ERR +=R.PARAM<7>
    END
  END ELSE
    CALL REDO.INTERFACE.REC.ACT(FI.INTERFACES,"BATCH",WRECORD.NUMBER,Y.TOT.REC,FI.W.REDO.FI.CONTROL.ID,FI.FILE.ID,WRECORD.NUMBER,"01","OK",Y.OUT.MSG,"","")
    IF R.PARAM<10> EQ "N" THEN
      W.REC.FT.OK += 1
...
*   General Variables
  ...
  MAX.LOOPS            = 1
*   MessageTXT
  Y.SEPARATOR.REC      = CHAR(13):CHAR(10)        ;*CHAR(13) is CR, CHAR(13) is LF
