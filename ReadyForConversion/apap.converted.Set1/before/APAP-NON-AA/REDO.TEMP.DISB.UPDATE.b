...
*
*Tus Start
    WVCR.TEMPLATE.ID  = System.getVariable("CURRENT.Template.ID")     ;*ID
    IF E<1,1> EQ "EB-UNKNOWN.VARIABLE" THEN
        WVCR.TEMPLATE.ID = ""
    END

    WRCA.NEXT.VERSION = System.getVariable("CURRENT.WRCA.NEXT.VERSION")         ;*POSIBLES VALORE SI/NO PARA SABER SI ES LA ULTIMA
    IF E<1,1> EQ "EB-UNKNOWN.VARIABLE" THEN
        WRCA.NEXT.VERSION = ""
    END

    WRCA.ACT.DISB     = System.getVariable("CURRENT.WRCA.ACT.DISB")   ;*POSICION
    IF E<1,1> EQ "EB-UNKNOWN.VARIABLE" THEN
        WRCA.ACT.DISB = ""
    END
