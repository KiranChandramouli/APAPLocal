...
* <Rating>-77</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.E.CONV.MAIL.URD
*--------------------------------------------------------------------------------------------------------
...
*   Y.DATA:=TRIM(Y.SEC):" ":TRIM(Y.COUNTRY):" ": TRIM(Y.TOWN.COUNTRY):" ":TRIM(Y.PAIS):" ":TRIM(Y.POST)
  Y.DATA = TRIM(Y.URD):" ":TRIM(Y.SEC)

*IF Y.DIR NE '' OR Y.URD NE '' OR Y.SEC NE '' OR Y.PAIS NE '' OR Y.POST NE '' OR Y.TOWN.COUNTRY NE '' OR Y.COUNTRY NE '' OR Y.STR.ADR NE '' THEN
  IF Y.URD NE '' OR Y.SEC NE '' THEN
    O.DATA=TRIM(Y.DATA)
  END ELSE
*        O.DATA = " "
*   END
