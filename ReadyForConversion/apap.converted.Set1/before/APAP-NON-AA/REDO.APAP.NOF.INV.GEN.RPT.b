...
* <Rating>-152</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.APAP.NOF.INV.GEN.RPT(Y.FINAL.ARRAY)
*********************************************************************************************************
...
*   ------         ------                         -------------                    -------------
*  14/09/2010    Jeyachandran S /Pradeep           ODR-2010-03-0105                   Initial Creation
...
*TUS Start
*  CALL OPF(FN.AC.COLLATERAL,F.AC.COLLATERAL)
...
**************
*TUS Start
*  CALL F.READ(FN.AC.COLLATERAL,Y.AZ.ID,R.AC.COLL,F.AC.COLLATERAL,AC.COL.ERR)
   CALL F.READ(FN.ECB,Y.AZ.ID,R.ECB,F.ECB,ECB.ERR)
...
*TUS Start
*    CALL F.READ(FN.AC.COLLATERAL,Y.ALT.ID,R.AC.COLL,F.AC.COLLATERAL,AC.COL.ERR)
     CALL F.READ(FN.ECB,Y.ALT.ID,R.ECB,F.ECB,ECB.ERR)
  END

*  Y.COL.ID = R.AC.COLL<1>
   Y.COL.ID = R.ECB<ECB.COLLATERAL>
   CONVERT VM TO FM IN Y.COL.ID
*TUS End
...
********************
  ...
  LOCATE 'INVEST.TYPE' IN D.FIELDS<1> SETTING Y.INVEST.POS THEN
    Y.INVEST.TYPE=D.RANGE.AND.VALUE<Y.INVEST.POS>
  END

  IF Y.INVEST.TYPE THEN
    SEL.CMD.AZ:= ' AND ALL.IN.ONE.PRODUCT EQ ':Y.INVEST.TYPE
  END

  LOCATE 'CLIENT.TYPE' IN D.FIELDS<1> SETTING Y.CLIENT.POS THEN
    Y.CLIENT.TYPE=D.RANGE.AND.VALUE<Y.CLIENT.POS>
    Y.CLIENT.TYPE = CHANGE(Y.CLIENT.TYPE,SM,' ')
  END
...
*TUS Start
    CALL EB.READ.HVT('EB.CONTRACT.BALANCES', Y.ACC.ID, R.ECB.REC, ECB.ERR)
*    Y.WORK.BAL = R.ACC1<AC.ONLINE.CLEARED.BAL>
     Y.WORK.BAL = R.ECB.REC<ECB.ONLINE.CLEARED.BAL>
*TUS End
...
*=============
  ...
  IF R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TIPO.CL.POS> = "PERSONA FISICA" OR R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TIPO.CL.POS> = "CLIENTE MENOR" THEN
    Y.CUS.NAMES = R.CUSTOMER<EB.CUS.GIVEN.NAMES>:" ":R.CUSTOMER<EB.CUS.FAMILY.NAME>
  END
  IF R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TIPO.CL.POS> = "PERSONA JURIDICA" THEN
