...
* In case of RISK.GROUP limits, we have to search related customers, according Risk Group
    ...
    FOR I = 1 TO Y.NO.RISK.GRP
*Get each Group Risk
      Y.RL.REL.CUS.ID = Y.CUSTOMER.ID:'-':Y.RL.ID:'-':R.RISK.GRP<1,I>
      Y.COND.DEF<3> =  R.RISK.GRP<1,I>
...
* The risk limtit is RISK.GROUP.TOTAL+R.RISK.GRP and we need the information of RISK.GROUP.SECURED+R.RISK.GRP and RISK.GROUP.UNSECURED+R.RISK.GRP
  ...
  Y.GROUPS.SEC = Y.CUSTOMER.ID:"-RISK.GROUP.SECURED-":R.RISK.GRP<1,I>
...
*
  ...
  CALL REDO.CCRG.B.TRACE.ERROR(R.SOURCE.INFO, Y.MSG.ERR, @FALSE, Y.CUSTOMER.ID, @TRUE)
