...
*------*
    ...
    IF STAR.DATE.VAL GE YTODAY THEN
        RETURN
    END
...
****************
    ...
    ERR.ACCOUNT = ''; R.ACCOUNT = ''; YACC.NABBAL = ''; YNAB.STATUS = ''; YOPEN.DATE = ''
    ...
    IF YOPEN.DATE EQ YTODAY THEN
        RETURN
    END
    YACC.NABBAL = ABS(YACC.NABBAL)
    ...
    IF R.EB.CONTRACT.BALANCES AND YNAB.STATUS EQ 'NAB' THEN
        ...
        END
        RETURN.MSG = STRT.DATE.PRINT:'|':NU.LOAN.PRINT:'|':TERM.PRINT:'|':INT.ACCRUAL.PRINT:'|':CUST.NAME.PRINT:'|':CUST.ID.PRINT:'|':YGL.CODE.TMP:'|':YACC.NABBAL:'|':SECT.PROD:'|':TOT.SEC.PROD
        CNT.LINE = CNT.LINE + 1
...
GET.LOAN.TERM:
**************
    R.PROD.GROUP = ''; PRODUCT.GROUP.ERR = ''
    CALL F.READ(FN.AA.PRODUCT.GROUP,YAA.PROD.GRP,R.PROD.GROUP,F.AA.PRODUCT.GROUP,PRODUCT.GROUP.ERR)
    IF NOT(R.PROD.GROUP) THEN
        RETURN
    END
    LOCATE 'TERM.AMOUNT' IN R.PROD.GROUP<AA.PG.PROPERTY.CLASS,1> SETTING PROP.POS THEN
        PROPERTY.VAL = R.PROD.GROUP<AA.PG.PROPERTY,PROP.POS>
        TERM.AMT.ID = REC.ID:'-':PROPERTY.VAL:'-':STAR.DATE.VAL:'.1'
