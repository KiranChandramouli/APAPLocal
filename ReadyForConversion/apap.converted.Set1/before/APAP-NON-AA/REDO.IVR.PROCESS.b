...
***********
    ...
    IF Y.STR EQ '' THEN
        ...
        CALL REDO.INTERFACE.REC.ACT(Y.INT.CODE,Y.INT.TYPE,Y.BAT.NO,Y.BAT.TOT,Y.INFO.OR,Y.INFO.DE,Y.ID.PROC,Y.MON.TP,Y.DESC,Y.REC.CON,Y.EX.USER,Y.EX.PC)
    END
...
**************
    ...
    WHILE Y.POS : Y.ROW
        Y.ROW = CHANGE(Y.ROW,CHAR(9),FM)
        Y.TOT.FLDS = DCOUNT(Y.ROW,FM)
        Y.CNT.X = 1
        FOR I = 1 TO Y.TOT.FLDS
            Y.OUT := TRIM(CHANGE(Y.ROW<I>,'"',""))
            IF I LT Y.TOT.FLDS THEN
                Y.OUT := "%"
            END
        NEXT I
        Y.OUT := ','
...
***********
    ...
    Y.CNT.VAL = DCOUNT(Y.FIELD.VALUE,CHAR(10))
    ...
    END ELSE
        ...
        WHILE Y.NEXT.VAL LE Y.CNT.VAL
            Y.TEMP.VAL = FIELD(Y.FIELD.VALUE,CHAR(10),Y.NEXT.VAL)
...
*    CALL OFS.GLOBUS.MANAGER(Y.OFS.SOURCE.ID,Y.MSG)
    ...
    R.REDO.IVR.RECEPTOR<REDO.IVR.REC.INPUTTER> = TNO:"_":OPERATOR
    R.REDO.IVR.RECEPTOR<REDO.IVR.REC.AUTHORISER> = TNO:"_":OPERATOR
