...
*-----------
  ...
  IF FM.POS EQ (V-4) THEN
    Y.NEW.VAL = TNO:"_":OPERATOR:"_":"OFS":"_":OFS$SOURCE.ID
  END
...
*--------------------
  ...
  END ELSE
    Y.DATE.TIME.1 = TODAY
    X = TIMEDATE()
    Y.DATE.TIME = TODAY[3,6]:X[1,2]:X[4,2]
  END
...
*--------------------------------------------------------------------------------------------
  ...
  CALL F.READ(FN.LOCAL.REF.TABLE,APPLICATION,R.LRT,F.LOCAL.REF.TABLE,LRT.ERR)
  LRF.FLD.ID = R.LRT<EB.LRT.LOCAL.TABLE.NO,VM.POS>
  CALL F.READ(FN.LOCAL.TABLE,LRF.FLD.ID,R.LT,F.LOCAL.TABLE,LT.ERR)
