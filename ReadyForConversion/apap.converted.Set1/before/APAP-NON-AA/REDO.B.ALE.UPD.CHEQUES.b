    ...
    IF R.REDO.CLEARING.OUTWARD<CLEAR.OUT.CHQ.STATUS> EQ 'CLEARED' THEN
      IF Y.USED.AMT THEN
        IF Y.CHEQUE.AMT GT Y.USED.AMT  THEN
          Y.CHEQUE.AMT=Y.CHEQUE.AMT-Y.USED.AMT
          Y.USED.AMT=0
        END
        ELSE
          Y.USED.AMT=Y.USED.AMT-Y.CHEQUE.AMT
          RETURN
        END
      END

      IF Y.FIT.AMOUNT.REQ GT Y.CHEQUE.AMT THEN
        Y.FIT.AMOUNT.REQ=Y.FIT.AMOUNT.REQ-Y.CHEQUE.AMT
        Y.CLEARED.AMT   =Y.CLEARED.AMT+Y.CHEQUE.AMT
      END
      ELSE

        Y.CLEARED.AMT   =Y.CLEARED.AMT+Y.FIT.AMOUNT.REQ
