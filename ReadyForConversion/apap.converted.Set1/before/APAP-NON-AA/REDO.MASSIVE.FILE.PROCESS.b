...
*    SUBROUTINE REDO.MASSIVE.FILE.PROCESS(Y.FILE.NAME)
    SUBROUTINE REDO.MASSIVE.FILE.PROCESS(Y.RECORD)
*------------------------------------------------------------
...
* 10-10-2011     JEEVA T          PACS00165067 - B.16  RATE review freq is changed
* 10-10-2017     D Edwin Charles  Reprice(Interest Rate change)
* ----------------------------------------------------------------------------
...
* PACS00761324 START
* -----------------------------------------------------------
*        IF Y.PROCESS.HEADER THEN
*            Y.REC.PRC.FILE<-1> = Y.PROCESS.HEADER
*            Y.REC.PRC.FILE<-1> = Y.RECORD
*            Y.PROCESS.HEADER = ''
*        END ELSE
*            READ Y.REC.PRC.FILE FROM F.PROCESS.PATH,Y.FILE.NAME ELSE
*                CALL OCOMO("File could not be read")
*            END
...
*        END
*        WRITE Y.REC.PRC.FILE TO F.PROCESS.PATH,Y.FILE.NAME
* ----------------------------------------------------------
* PACS00761324 END
    END ELSE
* PACS00761324 START
* -----------------------------------------------------------
*        IF Y.EXCEPTION.HEADER THEN
*            Y.REC.EXP.FILE<-1> = Y.EXCEPTION.HEADER
*            Y.REC.EXP.FILE<-1> = Y.RECORD
*            Y.EXCEPTION.HEADER = ''
*        END ELSE
*           READ Y.REC.EXP.FILE FROM F.EXCEPTION.PATH,Y.FILE.NAME ELSE
*               CALL OCOMO("File could not be read")
*           END
        J.ID = ACC.ID:"_":C$T24.SESSION.NO
...
*        END
*        WRITE Y.REC.EXP.FILE TO F.EXCEPTION.PATH,Y.FILE.NAME
* -----------------------------------------------------------
* PACS00761324 END
    END
