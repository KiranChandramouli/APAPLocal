...
* <Rating>-77</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.E.CONV.MAIL.COUNTRY
*--------------------------------------------------------------------------------------------------------
...
*   Y.DATA:=TRIM(Y.SEC):" ":TRIM(Y.COUNTRY):" ": TRIM(Y.TOWN.COUNTRY):" ":TRIM(Y.PAIS):" ":TRIM(Y.POST)
  Y.DATA = TRIM(Y.COUNTRY):" ": TRIM(Y.TOWN.COUNTRY)
*:FM:TRIM(Y.DIR):FM:TRIM(Y.URD):FM:TRIM(Y.SEC):FM:TRIM(Y.COUNTRY):FM:TRIM(Y.TOWN.COUNTRY):FM:TRIM(Y.PAIS):FM:TRIM(Y.POST)

  IF Y.COUNTRY NE '' OR Y.TOWN.COUNTRY NE '' THEN
    O.DATA=TRIM(Y.DATA)
*    END ELSE
