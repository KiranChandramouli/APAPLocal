...
*---------------
  ...
  CASE Y.APP.VER EQ "EB.SECURE.MESSAGE,AI.REDO.CLAIM.NEW.CONFIRM"
    CALL AI.REDO.PRINT.RECEIPTS.PROCESS.CLAIM(Y.APP.VER,Y.REC.ID,HEADER.FLD,DATA.FLD,FOOTER.FLD,Y.XSL)
...
*---------------
  ...
  IF Y.STO.BEN.ID THEN
    CALL F.READ(FN.BENEF,Y.STO.BEN.ID,R.BENEF,F.BENEF,BENEF.ERR)
...
*---------------
  ...
  END ELSE
    CALL F.READ(FN.FT.COMM,R.REC<REDO.H.SOL.COSTO.CK>,R.FT.COMM,F.FT.COMM,FT.COMM.ERR)
...
*---------------
  ...
  CALL REDO.CONVERT.ACCOUNT(LOAN.ACC.ID,Y.ARR.ID,ARR.ID,ERR.TEXT)
*    DATA.FLD = DESC.TXN.REFERENCE:SEP.DATA:Y.REC.ID:SEP.DATA:SEP.LINE
