...
*       RECORD
    Y.DATA = R.TXT.MSG<I>
    CHANGE CHAR(09) TO '' IN Y.DATA
    CHANGE CHAR(13) TO '' IN Y.DATA
    CHANGE CHAR(10) TO '' IN Y.DATA
    Y.DATA = TRIM(Y.DATA)
...
*           Get new TXT record
      OUT.TXT.MSG<I-1> = Y.ACCOUNT:",":Y.TOTAL.AMOUNT:",":Y.CURRENCY:",":Y.SING
    END
...
*    Tus End
    END ELSE
      ...
      IN.ACCT.TYPE = 'CREDIT'
      CALL REDO.NOMINA.ACCT.STATUS.CHECK(Y.ACCOUNT.ID,IN.ACCT.TYPE,OUT.ACCT.STATUS)
...
*   Validate DETAIL data
  ...
  WHILE LOOP.CNT LE MAX.LOOPS AND PROCESS.GOAHEAD DO
    ...
    CASE LOOP.CNT EQ 2        ;* Validate card type
      IF  Y.TYP.PRODUCT = C.TYP.D.CARD  THEN
        GOSUB GET.CARD.TYPE
      END
...
*   Get CardTypeId using the to get BIN number
  ...
  LOOP
  WHILE K.POS GT 0 DO
    LOCATE Y.BIN.NUMBER IN R.CARD.TYPE.BIN<1,K.POS> SETTING K THEN
      Y.CARD.TYPE     = R.CARD.TYPE.CT<K>
...
*
  ...
  IF PROCESS.GOAHEAD EQ 0 THEN RETURN
...
*   Get Card.Type
  ...
  F.CODE                 = ""
  Y.SEL.ID               = ""
  Y.EOL                  = CHAR(13):CHAR(10)      ;* Validate end of line
  Y.CH13                 = CHAR(13)
*
  RETURN
*
