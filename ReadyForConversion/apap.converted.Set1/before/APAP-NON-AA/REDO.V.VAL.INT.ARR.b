...
Y.FIXED.COUNT=DCOUNT(R.NEW(AA.INT.FIXED.RATE),VM)
...
!REMOVE Y.FIXED1 FROM Y.FIXEDRATE SETTING Y.POS1
...
!R.NEW(AA.INT.LOCAL.REF)<1,INTARR.POS>=Y.EFFECTIVERATE<1,INT.RATE.COUNT>
...
Y.MARGIN.COUNT=DCOUNT(Y.MARGINRATE,SM)
...
CALL F.READ(FN.BASICINTEREST,Y.BI.ID,R.BASICINTEREST,F.BASICINTEREST,Y.ERR)
