...
*
  NO.ORANGE = DCOUNT(WSORTED.DATA,FM)
...
*
  FOR FL.CONT = 1 TO NO.ORANGE
    ...
    IF W.DATE NE PREV.DATE THEN
      ...
      CHANGE ":" TO "" IN W.TIME
      REG.HEADER = "01FTDAPAP    " : FMT(DATE.COUNT,"R%7") : FMT(DATE.AMOUNT,"R2%13") : PREV.DATE : W.TIME : TODAY : FMT(" ",35L)
      WRITESEQ REG.HEADER TO YFILE.POINTER ELSE
        W.ERROR = "Error.writing.record.number.&.data-&":FM:"HEADER":VM:REG.HEADER
      END
    END
...
*
  ...
  DELETESEQ Y.FILE.TO.DELETE SETTING yDeleteRet ELSE
    ...
    W.ERROR = "File.&.could.not.be.deleted":FM:FI.FILE.ID
  END
*
  RETURN
