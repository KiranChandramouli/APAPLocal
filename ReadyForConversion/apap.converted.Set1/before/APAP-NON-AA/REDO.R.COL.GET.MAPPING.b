...
*
  ...
  FOR I = 1 TO Y.TOTAL
    Y.MAP.STATIC = P.OUT.R.STATIC.MAPPING<I,1>
    IF Y.MAP.STATIC  EQ Y.MAP.TYPE THEN
      R.MAP.TO.USE = P.OUT.R.STATIC.MAPPING<I>
      BREAK
    END
  NEXT I
...
* The first position is FIELD.ID
  FOR I = 2 TO Y.TOTAL
    IF R.MAP.TO.USE<1,I>[",",1,1] EQ Y.MAP.VALUE THEN
      Y.MAP.VALUE = R.MAP.TO.USE<1,I>[",",2,1]
      Y.FOUND = 1
      BREAK
    END
  NEXT I
