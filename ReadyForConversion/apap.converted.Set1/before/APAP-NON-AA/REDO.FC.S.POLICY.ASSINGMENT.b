...
COLL.CODE.TYPE:
  LOCATE Y.COLL.CODE IN R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.COLLATERAL.CODE,K.POS> SETTING K THEN
    K.POS = K
  END ELSE
    RETURN
  END
...
*PARA VER SI HUBO ERROR
    LOCATE Y.COLL.TYPE IN R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.COLLATERAL.TYPE,K.POS,1> SETTING K THEN
    ...
    LOCATE Y.COLL.TYPE IN R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.COLLATERAL.TYPE,K.POS,K.POS.I> SETTING K THEN
      Y.POL.TYPE<-1> = R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.POLICY.TYPE,K.POS,K>
      Y.POL.MAND<-1> = R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.POLI.MANDATORIO,K.POS,K>
    END
...
*Valida si ya existe el POlicy Number Aux
  ...
  LOCATE Y.POL.T IN Y.POL.TYPE<1> SETTING K THEN
    CALL REDO.FC.GEN.POL.ID(Y.ID)
    Y.ID.AUX = Y.ID
...
VALIDA.POLIZA.AUX:
  ...
  IF R.NEW(REDO.FC.POLICY.NUMBER.AUX)<1,K.POS.TYPE> THEN
    ...
    LOCATE Y.AUX IN R.COLLATERAL<1,1,1> SETTING K THEN
      Y.EXISTE = 1
    END
  END
  GOSUB THROW.ERROR
