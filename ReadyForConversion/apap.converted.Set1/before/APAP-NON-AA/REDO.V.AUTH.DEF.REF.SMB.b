...
* <Rating>-100</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.V.AUTH.DEF.REF.SMB
*-------------------------------------------------------------------------------------------
*DESCRIPTION:
...
***********
  ...
  IF VAR.REPAY.ACC THEN
    R.NEW(AZ.LOCAL.REF)<1,POS.L.AZ.REF.NO>="AZ-":VAR.ID.NEW
  END
...
**************
  ...
  CALL F.READ(FN.CATEGORY,Y.CATEGORY,R.CATEGORY,F.CATEGORY,CATEG.ERR)
  Y.CATEG.DESC = R.CATEGORY<EB.CAT.DESCRIPTION>
  Y.NAME = R.NEW(AZ.LOCAL.REF)<1,POS.L.AZ.DEP.NAME>
  Y.NAME = Y.NAME:'-':VAR.ID.NEW
  ...
  WHILE Y.COUNT LE Y.TOTAL.METHOD
    ...
    LOCATE 'FROM.CUST.ACC' IN Y.AZ.METHOD.PAY SETTING Y.METHOD.PAY.POS THEN
      IF Y.AZ.METHOD.PAY<Y.COUNT> EQ 'FROM.CUST.ACC' THEN
        ...
        Y.TOTAL.AMOUNT  = R.NEW(AZ.CURRENCY):Y.POS.L.AZ.AMOUNT
        Y.CATEGORY = R.NEW(AZ.CATEGORY)
        CALL F.READ(FN.CATEGORY,Y.CATEGORY,R.CATEGORY,F.CATEGORY,CATEG.ERR)
        Y.CATEG.DESC = R.CATEGORY<EB.CAT.DESCRIPTION>
        Y.NAME = R.NEW(AZ.LOCAL.REF)<1,POS.L.AZ.DEP.NAME>:'-':VAR.ID.NEW
        CALL F.READ(FN.ACCOUNT,VAR.ID.NEW,R.ACCOUNT,F.ACCOUNT,ACC.ERR)
        Y.TITLE2 = R.ACCOUNT<AC.ACCOUNT.TITLE.1>:R.ACCOUNT<AC.ACCOUNT.TITLE.2>
        CALL System.setVariable("CURRENT.ACCT.NO",Y.ACCT.NO)
        CALL System.setVariable("CURRENT.VALUE.DATE",SYS.DATE)
...
*******************
  ...
  ERR.OFS = ''
  OPTION = ''
  CALL OFS.BUILD.RECORD(ACTUAL.APP.NAME,OFS.FUNCTION,PROCESS,VERSION.ACCT,VAR.GTSMODE,NO.OF.AUTH,TRANSACTION.ID,Y.ACCT.ARRAY,OFS.ACCT.CAP)
