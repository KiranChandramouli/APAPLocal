...
*-------------------------------------------------------------------------------------------------------
  Y.APPLN='AZ.ACCOUNT'
  CALL F.READ(FN.SS,Y.APPLN,R.SS,F.SS,SS.ERR)
  IF R.SS NE '' THEN
...
*-------------------
  IF Y.PREV.VAL<xCtr> = Y.NEW.VAL<xCtr> THEN
...
*-------------------------------------------------------------------------------------------------------
  ...
  FOR I=1 TO TOT.SYS.FLD.NAME.CNT
    FLD.NAME=TOT.SYS.FLD.NAME<1,I>
    Y.AUDIT.FIELD.ARRAY = ''
...
CALL F.READ(FN.CUSTOMER,CUSTOMER.ID,R.CUSTOMER,F.CUSTOMER,R.CUSTOMER.ERR)
 IF R.CUSTOMER.ERR THEN  ;* Tus Start
    ...
    CASE R.CUSTOMER<EB.CUS.LOCAL.REF,LOC.L.CU.TIPO.CL.POS> = "PERSONA FISICA" OR R.CUSTOMER<EB.CUS.LOCAL.REF,LOC.L.CU.TIPO.CL.POS> = "CLIENTE MENOR"
      CusName = R.CUSTOMER<EB.CUS.GIVEN.NAMES>:" ":R.CUSTOMER<EB.CUS.FAMILY.NAME>

    CASE R.CUSTOMER<EB.CUS.LOCAL.REF,LOC.L.CU.TIPO.CL.POS> = "PERSONA JURIDICA"
