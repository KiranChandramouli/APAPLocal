...
* <Rating>0</Rating>
*-----------------------------------------------------------------------------------------------------
  SUBROUTINE REDO.B.SC.REV.DISC.ENTRY.LOAD
*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
* 18 Feb 2012      Arundev            RTC-553577          CR008 Effective Discount and Interest Accrual
*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  LOC.REF.FIELDS = 'L.DISC.ADJ.CAT'
  LOC.REF.POS=''
  CALL MULTI.GET.LOC.REF(LOC.REF.APPLICATION,LOC.REF.FIELDS,LOC.REF.POS)
  POS.L.DISC.ADJ.CAT = LOC.REF.POS
  ...
  IF R.SC.STD.TRADE THEN
    Y.SC.CR.TXN.CODE = R.SC.STD.TRADE<SC.SST.CU.DISC.CR.CODE>
    Y.SC.DR.TXN.CODE = R.SC.STD.TRADE<SC.SST.CU.DISC.DB.CODE>
  END ELSE
  ...
  CALL F.READ(FN.SC.PARAMETER,ID.COMPANY,R.SC.PARAM,F.SC.PARAMETER,ERR.SC.PARAM)

  IF R.SC.PARAM THEN
    Y.CCY.MARKET   = R.SC.PARAM<SC.PARAM.DEFAULT.CCY.MARKET>
    Y.DEFAULT.CATEG = R.SC.PARAM<SC.PARAM.DEFAULT.PRODUCT>
  END ELSE
    CALL F.READ(FN.SC.PARAMETER,R.COMPANY(EB.COM.FINANCIAL.COM),R.SC.PARAM,F.SC.PARAMETER,ERR.SC.PARAM)
    Y.CCY.MARKET = R.SC.PARAM<SC.PARAM.DEFAULT.CCY.MARKET>
    Y.DEFAULT.CATEG = R.SC.PARAM<SC.PARAM.DEFAULT.PRODUCT>
  END
