...
* Setea los valores de Comisiones al Areglo temporal, RCA
  ...
  END
  Y.CONT.PROPER = Y.CONT.PROPER + Y.CONT.INI
...
* AGREGARLO A LA SIGUIENTE POSICION
  IF K > 0 THEN
    Y.NEW.POS = DCOUNT(R.COM<K,COM.START.DATE>,SM) + 1
    R.COM<K,COM.START.DATE,Y.NEW.POS> = Y.DATE.BEG.CHARG<1,Y.I,Y.J>   ;*VALOR start date
    R.COM<K,COM.END.DATE,Y.NEW.POS> = Y.DATE.END.CHARG<1,Y.I,Y.J>
    R.COM<K,COM.AMOUNT,Y.NEW.POS> = Y.INS.SEC.COM.AMT<1,Y.I,Y.J>
  END ELSE
    IF NOT(R.COM) THEN
...
*            CONTINUE
  ...
  IF K > 0 THEN
* AGREGARLO A LA SIGUIENTE POSICION
    Y.NEW.POS = DCOUNT(R.PAYMENT.SHCEDULE<K,START.DATE>,SM) + 1
    R.PAYMENT.SHCEDULE<K,START.DATE,Y.NEW.POS> = Y.INS.DATE.BEG.CHARG<1,Y.I,Y.J>          ;*VALOR start date
    R.PAYMENT.SHCEDULE<K,END.DATE,Y.NEW.POS> = Y.INS.DATE.END.CHARG<1,Y.I,Y.J>
    R.PAYMENT.SHCEDULE<K,AMOUNT,Y.NEW.POS> = Y.INS.XMON.POL.AMT<1,Y.I,Y.J>
  END ELSE
...
* AGREGARLO A LA SIGUIENTE POSICION
  IF K > 0 THEN
    ...
    R.PAYMENT.SHCEDULE<K,START.DATE,Y.NEW.POS> = Y.INS.DATE.BEG.CHARG<1,Y.I,Y.J>          ;*VALOR start date
    R.PAYMENT.SHCEDULE<K,END.DATE,Y.NEW.POS> = Y.INS.DATE.END.CHARG<1,Y.I,Y.J>
    R.PAYMENT.SHCEDULE<K,AMOUNT,Y.NEW.POS> = Y.INS.MON.POL.AMT<1,Y.I,Y.J>
  END ELSE
    IF NOT(R.PAYMENT.SHCEDULE)  THEN
