...
************
  ...
  WHILE ACCOUNT.ID:Y.ACC.POS1
    ...
    * Tus end
    ...
    CALL F.READ(FN.COMPANY,Y.ACC.AGENCY,R.COMPANY.REC,F.COMPANY,ERR.CMP)
...
*       CALL EB.READLIST(SEL.CMD.GINT,SEL.LIST.GINT,'',NO.OF.REC.GINT,SEL.ERR.GINT)
    IF Y.COND.ID THEN
      CALL F.READ(FN.GROUP.DATE,Y.COND.ID,R.GROUP.DATE,F.GROUP.DATE,Y.ERR)
      Y.DATE = R.GROUP.DATE<AC.GRD.CREDIT.GROUP.DATE>
      SEL.LIST.GINT = Y.COND.ID:Y.DATE
      CALL F.READ(FN.GROUP.CREDIT.INT,SEL.LIST.GINT<1>,R.GROUP.CREDIT.INT,F.GROUP.CREDIT.INT,GINT.ERR)
