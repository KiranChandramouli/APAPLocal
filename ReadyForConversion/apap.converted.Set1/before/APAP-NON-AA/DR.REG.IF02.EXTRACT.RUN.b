...
RUN.CUSTOMER:
*-----------*
  CUSTOMER.TYPE = R.CUSTOMER; INT.NATION = ''; CUSTOMER.CODE = ''
  ...
  CASE R.CUSTOMER<EB.CUS.LOCAL.REF,FOREIGN.POS> NE ''
    CUSTOMER.CODE = INT.NATION:R.CUSTOMER<EB.CUS.LOCAL.REF,FOREIGN.POS>
  CASE 1
    CUSTOMER.CODE = INT.NATION:R.CUSTOMER<EB.CUS.LEGAL.ID,1>
  END CASE
...
*
  ...
  END ELSE
  CALL EB.READ.HVT ('EB.CONTRACT.BALANCES', ACCOUNT.VAL, R.ECB, ECB.ERR)
  LOCATE 'CUST-CR' IN R.ECB<ECB.INITIATOR.TYPE,1> SETTING CUST.CR.POS THEN
  CUT.DATE=R.ECB<ECB.DATE.LAST,CUST.CR.POS>
  END
...
*  CASE AAR.STATUS.VAL EQ 'MATURED' OR AAR.STATUS.VAL EQ 'EXPIRED' OR AAR.STATUS.VAL EQ 'REVERSED' OR AAR.STATUS.VAL EQ 'CANCELLED' OR AAR.STATUS.VAL EQ 'CLOSE'
  CASE AAR.STATUS.VAL EQ 'PENDING.CLOSURE' OR AAR.STATUS.VAL EQ 'EXPIRED' OR AAR.STATUS.VAL EQ 'REVERSED' OR AAR.STATUS.VAL EQ 'CANCELLED' OR AAR.STATUS.VAL EQ 'CLOSE'
*TUS END
    ACC.STATUS = 'C'
  END CASE
  ...
  ACC.OPEN.DATE = R.AA.ARRANGEMENT<AA.ARR.START.DATE>
  ACC.OPEN.DATE = ACC.OPEN.DATE[7,2]:'/':ACC.OPEN.DATE[5,2]:'/':ACC.OPEN.DATE[1,4]
  RETURN
...
*        BALANCE = R.ACCOUNT<AC.WORKING.BALANCE>
...
*****************
  START.DATE=TODAY
  END.DATE=START.DATE
  BALANCES.TO.CHECK='CURACCOUNT':FM:'DE1ACCOUNT':FM:'DE2ACCOUNT':FM:'DELACCOUNT':FM:'NABACCOUNT'
...
*
  ...
  IF R.AA.ARRANGEMENT<AA.ARR.CURRENCY> EQ LCCY THEN
    BALANCE = ABS(BALANCE.AMT)
  END ELSE
    Y.FAMT = TOTAL.BAL
    Y.FCY = R.AA.ARRANGEMENT<AA.ARR.CURRENCY>
    Y.MARKET = '' ; Y.LAMT = '' ; Y.DIF.AMT = '' ; Y.DIF.PCT = '' ; Y.RATE = ''
    CALL MIDDLE.RATE.CONV.CHECK(Y.FAMT,Y.FCY,Y.RATE,Y.MARKET,Y.LAMT,Y.DIF.AMT,Y.DIF.PCT)
    BALANCE = ABS(Y.LAMT)
  END
...
GET.LOAN.TYPE:
**************
  CALL F.READ(FN.AA.PRODUCT.GROUP,PROD.GROUP.ID,R.PROD.GROUP,F.AA.PRODUCT.GROUP,PRODUCT.GROUP.ERR)
  IF R.PROD.GROUP THEN
    GOSUB LOCAT.TERM.AMT
  END
  RETURN
...
**************
  LOCATE 'TERM.AMOUNT' IN R.PROD.GROUP<AA.PG.PROPERTY.CLASS,1> SETTING PROP.POS THEN
    ...
    TERM.AMT.ID = ARR.ID:'-':PROPERTY.VAL:'-':STAR.DATE.VAL:'.1'
    GOSUB READ.ARR.TERM.AMT
  END
  RETURN
...
******************
  ...
  IF R.AA.ARR.TERM.AMOUNT THEN
    ACC.TYPE = R.AA.ARR.TERM.AMOUNT<AA.AMT.LOCAL.REF,AA.INV.FACILITY>
  END
  RETURN
...
*-------------*
  ...
  IF R.LMM.CUSTOMER THEN
    ...
    LMM.CUSTOMER.CTR = 1
    ACC.TYPE = ''; ACC.STATUS = ''; BALANCE = ''
    CCY.TYPE = ''; ACC.OPEN.DATE = ''; CUT.DATE = ''; CLOSE.DATE = ''
    GOSUB MM.PROCESS
  END
  RETURN
...
***********
  ...
  WHILE LMM.CUSTOMER.CTR LE LMM.CUSTOMER.CNT
    ...
    ACC.TYPE = R.MM.MONEY.MARKET<MM.LOCAL.REF,MM.INV.FACILITY>
    GOSUB MM.SUB.PROCESS
    GOSUB FMT.FLDS
    OUTPUT.ARR<-1> = SEQ.NUMB:'*':CUST.TYPE:'*':CUST.ID:'*':NAMES:'*':NAME.INITIALS:'*':ACC.TYPE:'*':ACC.NUMBER:'*':ACC.STATUS:'*':BALANCE:'*':CCY.TYPE:'*':ACC.OPEN.DATE:'*':CUT.DATE:'*':CLOSE.DATE:'*':COMMENTS:'*':YEAR.OF.CIRCULAR:'*':T24.CLIENT
    LMM.CUSTOMER.CTR += 1
  REPEAT
...
***************
  GOSUB MM.AC.STATUS
  ACC.NUMBER = MM.ID
  CCY.TYPE = R.MM.MONEY.MARKET<MM.CURRENCY>
  IF CCY.TYPE EQ LCCY THEN
    BALANCE = R.MM.MONEY.MARKET<MM.PRINCIPAL>
  END ELSE
    Y.FAMT = R.MM.MONEY.MARKET<MM.PRINCIPAL>
    Y.FCY = CCY.TYPE
    Y.MARKET = '' ; Y.LAMT = '' ; Y.DIF.AMT = '' ; Y.DIF.PCT = '' ; Y.RATE = ''
    CALL MIDDLE.RATE.CONV.CHECK(Y.FAMT,Y.FCY,Y.RATE,Y.MARKET,Y.LAMT,Y.DIF.AMT,Y.DIF.PCT)
    BALANCE = Y.LAMT
  END
  ACC.OPEN.DATE = R.MM.MONEY.MARKET<MM.VALUE.DATE>
  IF ACC.OPEN.DATE THEN
    FORM.DATE = ACC.OPEN.DATE[7,2]:"/":ACC.OPEN.DATE[5,2]:"/":ACC.OPEN.DATE[1,4]
  END
  ACC.OPEN.DATE = FORM.DATE

  CUT.DATE = R.MM.MONEY.MARKET<MM.VALUE.DATE>
  IF CUT.DATE THEN
    CUT.DATE = CUT.DATE[7,2]:"/":CUT.DATE[5,2]:"/":CUT.DATE[1,4]
  END
  T24.CLIENT = 'S'
  CLOSE.DATE = R.MM.MONEY.MARKET<MM.MATURITY.DATE>
  IF CLOSE.DATE THEN
    CLOSE.DATE = CLOSE.DATE[7,2]:"/":CLOSE.DATE[5,2]:"/":CLOSE.DATE[1,4]
  END
  RETURN
...
MM.AC.STATUS:
*************
  ACC.STATUS = R.MM.MONEY.MARKET<MM.STATUS>
  BEGIN CASE
  CASE ACC.STATUS EQ 'FWD' OR ACC.STATUS EQ 'CUR'
    ACC.STATUS = 'A'
  CASE ACC.STATUS EQ 'REV' OR ACC.STATUS EQ 'LIQ'
    ACC.STATUS = 'C'
  END CASE
  RETURN
...
***********
  ...
  IF R.FOREX THEN
    ACC.STATUS = R.FOREX<FX.STATUS>
    BEGIN CASE
    CASE ACC.STATUS EQ ''
      ACC.STATUS = 'A'
    CASE ACC.STATUS EQ 'MAT'
      ACC.STATUS = 'C'
    END CASE
    ACC.NUMBER = REC.ID
    YACC.CCATEG = 'FX.CAT1'; TXNACE.POS = ''
    LOCATE YACC.CCATEG IN Y.TXNACC.VAL.ARR<1,1> SETTING TXNACE.POS THEN
      ACC.TYPE= Y.TXNACC.DIS.ARR<1,TXNACE.POS>
    END

    BALANCE = R.FOREX<FX.BUY.LCY.EQUIV>
    FX.SEL = R.FOREX<FX.CURRENCY.SOLD>
    FX.BUY = R.FOREX<FX.CURRENCY.BOUGHT>
    IF FX.SEL EQ LCCY THEN
      CCY.TYPE = FX.BUY
    END
    IF FX.BUY EQ LCCY THEN
      CCY.TYPE = FX.SEL
    END
    ACC.OPEN.DATE = R.FOREX<FX.DEAL.DATE>
    IF ACC.OPEN.DATE THEN
      FORM.DATE = ACC.OPEN.DATE[7,2]:"/":ACC.OPEN.DATE[5,2]:"/":ACC.OPEN.DATE[1,4]
    END
    ACC.OPEN.DATE = FORM.DATE

    CUT.DATE = R.FOREX<FX.VALUE.DATE.BUY>
    IF CUT.DATE THEN
      FORM.DATE = CUT.DATE[7,2]:"/":CUT.DATE[5,2]:"/":CUT.DATE[1,4]
    END
    CUT.DATE = FORM.DATE
    CLOSE.DATE = FORM.DATE
...
INIT.FX:
********
  SEL.CMD1 = ''; ID.LIST1 = ''; ID.CNT1 = ''
  ERR.SEL1 = ''; REC.ID = ''; STMT.NO = ''
  RETURN
...
*
  FN.LMM.CUSTOMER = 'F.LMM.CUSTOMER'; F.LMM.CUSTOMER = ''
...
GET.LR.POSN:
************
  APPL.NAME = 'CUSTOMER':FM:'ACCOUNT':FM:'MM.MONEY.MARKET':FM:'AA.ARR.TERM.AMOUNT':FM:'AZ.ACCOUNT'
