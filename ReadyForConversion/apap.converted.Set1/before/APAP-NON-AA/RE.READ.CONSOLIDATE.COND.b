...
* Initailise variables
    ...
    ERR.MSG = ""
    IF COMPANY.IDS = "" AND FIELD(COND.ID,".",2) THEN       ;* check to see if there are any company specific records
        COND.REC = ""
...
*---------------
*
    CALL F.READ(FN.CONSOLIDATE.COND, ID, R.CONSOLIDATE.COND, F.CONSOLIDATE.COND, ERR.MSG)
*
...
*
    IF ERR.MSG THEN
        READ.ERR = ''
        CALL F.READ(FN.LRT, Y.FILE.NAME, R.LRT, F.LRT, READ.ERR)
...
*----------------
    ...
    FOR Y.LF = 1 TO LF.COUNT
        ...
        CALL F.READ(FN.LT, LT.KEY, R.LT, F.LT, READ.ERR)
        IF R.LT<2,1> = Y.FIELD.NAME THEN
...
*
    IF NOT(YR.COND) THEN
        CALL F.READ(FN.CONSOLIDATE.COND, "ASSET&LIAB", YR.COND, F.CONSOLIDATE.COND, READ.ERR)
    END
