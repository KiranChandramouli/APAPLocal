...
*Data by category code
    Y.CNT2 = K
    Y.CAT = R.RL.BAL.DET<Y.POS.CATEGORY><1,K>
    ...
    END ELSE
      IF Y.CAT THEN
        CALL F.READ(FN.CATEGORY,Y.CAT,R.CAT.DATA,F.CATEGORY,CATEGORY.ERR)
        Y.DESCRIPTION = R.CAT.DATA<EB.CAT.DESCRIPTION>
      END
    END
    Y.DIR.BAL     = R.RL.BAL.DET<Y.POS.BAL.DIR,K>
    Y.INT.REC     = R.RL.BAL.DET<Y.POS.INTEREST.RECEIVABLE,K>
    Y.CONTINGEN   = R.RL.BAL.DET<Y.POS.CONTINGENT.BALANCE,K>
    Y.TOT.USED    = R.RL.BAL.DET<Y.POS.TOTAL,K>
    GOSUB GET.DETAIL.BY.ENQUIRY
    GOSUB SUM.TO.TOTALS
  NEXT K
