...
*-----------------------------------------------------------------------------
* Read Main Files
  CALL REDO.COL.EXTRACT.CREDIT.READ(AA.ID,R.AA,R.ACT.HIST,R.PRINCIPALINT,R.PENALTYINT,R.AA.ACCOUNT.DETAILS)
  IF E NE "" THEN
    RETURN
  END
* << PACS00169639
...
* Due Days & Number of due payments
  CALL REDO.S.COL.GET.BILL.DETAILS(R.AA.ACCOUNT.DETAILS,  Y.PROCESS.DATE,  Y.MORA.CTA.VEN, Y.TMPCREDITODIASATRASO, Y.TMPCREDITOCUOTASVENCIDAS, Y.TMPCREDITOCUOTASPAGADAS, Y.TMPCREDITOMONTOMOROSO )
* Total
...
*
  CALL REDO.COL.EXTRACT.CREDIT.2("GET.AA.CUSTOMER", Y.ACCOUNT.ID, AA.ID, "", P.GET.AA.CUSTOMER, "")
  IF E THEN
    RETURN
  END
...
*
  CALL REDO.COL.EXTRACT.CREDIT.2("GET.CREDIT.GRANTED", Y.ACCOUNT.ID, AA.ID, P.GET.CREDIT.GRANTED, "", "")
  IF E THEN
    RETURN
  END
  ...
  GOSUB INSERT.STMT
  CALL REDO.COL.EXTRACT.TXN(Y.PROCESS.DATE, AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.GROUP, Y.AGENCY.CODE, Y.CREDIT.TXN)
...
*-----------------------------------------------------------------------------
  ...
  I = 1
  LOOP WHILE I LE Y.TOTAL.ACT
    Y.AH.ACTIVITY.TOTAL = DCOUNT(R.ACT.HIST<AA.AH.ACTIVITY,I>,SM)
    Y = 1
    LOOP WHILE Y LE Y.AH.ACTIVITY.TOTAL
      Y.IS.AUTH = R.ACT.HIST<AA.AH.ACT.STATUS,I,Y> EQ "AUTH"
      Y.IS.PAY  = R.ACT.HIST<AA.AH.ACTIVITY,I,Y>["-",1,2] EQ "LENDING-APPLYPAYMENT"
      Y.IS.PAY  = Y.IS.PAY OR R.ACT.HIST<AA.AH.ACTIVITY,I,Y>["-",1,3] EQ "LENDING-CREDIT-ARRANGEMENT"
      Y.IS.PAY  = Y.IS.PAY OR R.ACT.HIST<AA.AH.ACTIVITY,I,Y>["-",1,3] EQ "LENDING-SETTLE-PAYOFF"
...
*-----------------------------------------------------------------------------
  IF Y.IS.AUTH AND Y.IS.PAY THEN
    Y.SYSTEM.DATE = R.ACT.HIST<AA.AH.SYSTEM.DATE,I,Y>
    Y.AMT         = R.ACT.HIST<AA.AH.ACTIVITY.AMT,I,Y>
...
*-----------------------------------------------------------------------------
  ...
  CALL REDO.COL.EXTRACT.CREDIT.2("PROCESS.ITEMS.DETAIL", Y.ACCOUNT.ID, AA.ID, "", "", P.PROCESS.ITEMS.DETAILS)
