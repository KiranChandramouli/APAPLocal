...
* Here we gets the  outstanding principal from the customer
  ...
  CALL REDO.GET.PROPERTY.NAME(Y.ARRANGEMENT.ID,'TERM.AMOUNT',R.OUT.AA.RECORD,OUT.PROPERTY,OUT.ERR)
  CALL REDO.CONVERT.ACCOUNT(IN.ACC.ID,Y.ARRANGEMENT.ID,OUT.ID,ERR.TEXT)
  Y.PRIN.BAL = 0
