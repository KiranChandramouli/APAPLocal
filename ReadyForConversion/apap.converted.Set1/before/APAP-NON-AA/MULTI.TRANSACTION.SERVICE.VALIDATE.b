...
CHEQ.CONDITION:
  ...
  IF Y.NO.OF.CHQ EQ 'CHEQUE' THEN
    ...
    Y.TOT.CHQ.AMT = Y.TOT.CHQ.AMT+Y.CHQ.AMOUNT
  END
...
*        REPEAT
    ...
    WHILE Y.TRAN.VAL LE Y.TRANS.CNT
      ...
      IF Y.PYMT.MODE3 EQ 'CASH' THEN
        Y.FINAL.AMT = Y.FINAL.AMT + Y.TOT.AMT
      END
      Y.TRAN.VAL +=1
    REPEAT
...
REPAY.MUL.MODE.CHK:
  ...
  WHILE Y.START.VAL LE Y.BILL.CNT
    ...
    IF Y.PYMT.MODE3 EQ 'CASH' THEN
      Y.NET.FINAL.VAL = Y.NET.FINAL.VAL + Y.NET.VAL
    END
    Y.START.VAL + =1
  REPEAT
