*-----------------------------------------------------------------------------
* <Rating>-51</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.V.VAL.FR.REQ.DOC.NAME
*--------------------------------------------------------------------------------
* Company Name : ASOCIACION POPULAR DE AHORROS Y PRESTAMOS
* Program Name : REDO.V.VAL.FR.REQ.DOC.NAME
*--------------------------------------------------------------------------------
* Description: This Validation routine is to populate the document that
* required for the request from REDO.SLA.PARAM
*
*-----------------------------------------------------------------------------
* Modification History :
*-----------------------------------------------------------------------------
*
*  DATE         WHO         REFERENCE         DESCRIPTION
* 24-May-2011   Pradeep S   PACS00071066      INITIAL CREATION
*
* 31-Jul-2012   Nandhini M  PACS00210436       Updated
*
*----------------------------------------------------------------------------
$INSERT I_COMMON
$INSERT I_EQUATE
$INSERT I_GTS.COMMON
$INSERT I_EB.TRANS.COMMON
$INSERT I_F.REDO.FRONT.REQUESTS
$INSERT I_F.REDO.SLA.PARAM

  GOSUB PRE.PROCESS
  RETURN


PRE.PROCESS:
*************
  IF COMI THEN
    GOSUB OPEN.FILES
    GOSUB PROCESS
  END ELSE
    R.NEW(FR.CM.DOC.NAME) = ''
    R.NEW(FR.CM.DOC.REV) = ''
    ETEXT = 'EB-MAND.INP'
    CALL STORE.END.ERROR
  END

  RETURN

OPEN.FILES:
************

  FN.SLA = 'F.REDO.SLA.PARAM'
  F.SLA = ''
  CALL OPF(FN.SLA,F.SLA)

  Y.PRDT.TYPE = R.NEW(FR.CM.PRODUCT.TYPE)

  IF Y.PRDT.TYPE THEN
    BEGIN CASE

    CASE Y.PRDT.TYPE EQ 'TARJETA.DE.CREDITO'
      T(FR.CM.ACCOUNT.ID)<3> = 'NOINPUT'
      N(FR.CM.CARD.NO) := '.1'
      R.NEW(FR.CM.ACCOUNT.ID) = ''
    CASE Y.PRDT.TYPE EQ 'OTROS'
      R.NEW(FR.CM.ACCOUNT.ID) = ''
      R.NEW(FR.CM.CARD.NO) = ''
    CASE 1
      T(FR.CM.CARD.NO)<3> = 'NOINPUT'
      N(FR.CM.ACCOUNT.ID) := '.1'
      R.NEW(FR.CM.CARD.NO) = ''
    END CASE

  END

  RETURN

PROCESS:
*********

  Y.TYPE = R.NEW(FR.CM.TYPE)
  Y.PDT.TYPE = R.NEW(FR.CM.PRODUCT.TYPE)
  Y.SLA.ID = Y.TYPE:'-':Y.PDT.TYPE
  R.NEW(FR.CM.SLA.ID) = Y.SLA.ID
  R.SLA = ''
  CALL CACHE.READ(FN.SLA,Y.SLA.ID,R.SLA,SLA.ERR)

  IF R.SLA THEN
    Y.CLAIM.TYPES = R.SLA<SLA.DESCRIPTION>
    LOCATE COMI IN Y.CLAIM.TYPES<1,1> SETTING POS THEN
      R.NEW(FR.CM.RISK.LEVEL) = R.SLA<SLA.RISK.LEVEL,POS>
      R.NEW(FR.CM.SUPPORT.GROUP) = R.SLA<SLA.SUPPORT.GROUP,POS>
      GOSUB CHECK.CHANNEL
      GOSUB CHK.DOC
    END
  END

  RETURN

CHECK.CHANNEL:
**************

  Y.SLA.CHANNEL = R.SLA<SLA.START.CHANNEL,POS>
  CHANGE SM TO FM IN Y.SLA.CHANNEL
  Y.CHANNEL = R.NEW(FR.CM.OPENING.CHANNEL)
  Y.SLA.CHANNEL = FIELDS(Y.SLA.CHANNEL,'-',1,1)
  Y.ERR.FLAG = @FALSE

  LOCATE Y.CHANNEL IN Y.SLA.CHANNEL SETTING CHANNEL.POS ELSE          ;* PACS00210436 S/E
    Y.ERR.FLAG = @TRUE
    ETEXT = 'EB-REQ.SLA.CHANNEL'
    CALL STORE.END.ERROR
  END

  RETURN
CHK.DOC:
*********
  IF NOT(Y.ERR.FLAG) THEN
    Y.DOC.REQ = R.SLA<SLA.DOC.REQUIRE,POS>
    CONVERT SM TO VM IN Y.DOC.REQ
    Y.DOC.REV.BK = R.NEW(FR.CM.DOC.REV)
    R.NEW(FR.CM.DOC.REV) = ''
    R.NEW(FR.CM.DOC.NAME) = Y.DOC.REQ
    Y.DOC.CNT = DCOUNT(Y.DOC.REQ,VM)

    Y.DOC.REV.INC = 1
    LOOP
    WHILE Y.DOC.REV.INC LE Y.DOC.CNT
      R.NEW(FR.CM.DOC.REV)<1,Y.DOC.REV.INC>  = Y.DOC.REV.BK<1,Y.DOC.REV.INC>
      Y.DOC.REV.INC += 1
    REPEAT

  END

  RETURN

END
