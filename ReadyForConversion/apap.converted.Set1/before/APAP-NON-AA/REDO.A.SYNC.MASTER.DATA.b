...
* Log writing: abnormal error that must be notified
        CALL REDO.S.NOTIFY.INTERFACE.ACT('VPL002', 'ONLINE', '04', 'EMAIL ERROR ARCHIVO NO MONETARIO [' : WS.DATA<1> : '] - ' : WS.DATA<2>, 'REVISAR EN LOG $JBOSS_HOME\server\default\log\server.log ERROR EN ARCHIVO NO MONETARIO A LAS ' : TIMEDATE(), '', '', '', '', '', OPERATOR, '')
...
* telefonos
    ...
    FOR I = 1 TO TELS
        NUM.TEL = OCONV(FIELD(Y.TEL.AREA,@SM,I) : ' ' : FIELD(Y.TEL.NUM,@SM,I),'MCN')
        IF FIELD(Y.TEL.TYPE,@SM,I) EQ '01' AND TEL.CASA EQ '' THEN    ;* telefono casa
            WS.DATA<16> = NUM.TEL
        END
        IF FIELD(Y.TEL.TYPE,@SM,I) EQ '05' AND TEL.TRAB EQ '' THEN    ;* telefono trabajo + extension
            WS.DATA<17> = NUM.TEL
            WS.DATA<18> = FIELD(Y.TEL.EXT,@SM,I)[1,4]
        END
        IF FIELD(Y.TEL.TYPE,@SM,I) EQ '06' AND TEL.CEL EQ '' THEN     ;* telefono celular
            WS.DATA<19> = NUM.TEL
        END
    NEXT I
* email
...
* estado_civil
    ...
    BEGIN CASE
    CASE Y.ESTADO.CIVIL = 'SOLTERO'
        WS.DATA<24> = 'S'
    CASE Y.ESTADO.CIVIL = 'CASADO'
        WS.DATA<24> = 'M'
    CASE Y.ESTADO.CIVIL[1,5] = 'UNION'
        WS.DATA<24> = 'U'
    CASE Y.ESTADO.CIVIL = 'DIVORCIADO'
        WS.DATA<24> = 'D'
    CASE Y.ESTADO.CIVIL = 'VIUDO'
        WS.DATA<24> = 'S'
    CASE 1
        WS.DATA<24> = ' '
    END CASE
...
************************************************
    ...
    NAME.POST = ''
    FOR I = 1 TO STRING.LEN STEP 1
        CHAR.VAL = NAME.PRE[I,1]
