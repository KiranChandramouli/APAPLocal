...
*-----------------------------------------------------------------------------
    ...
    CALL F.READ(FN.APPL.GEN.COND, APPL.ID, APPL.REC, F.APPL.GEN.COND, APPL.ERR)

    BEGIN CASE
    CASE AF = SDB.PAR.VAT.PERCENT
        GOSUB VAT.PERCENT.CHECK

    CASE AF = SDB.PAR.STAFF.GROUP
        GOSUB STAFF.GROUP.CHECK

    CASE AF = SDB.PAR.DISC.GROUP
        GOSUB DISC.GROUP.CHECK

    CASE AF = SDB.PAR.DISC.FLAT.AMT
        GOSUB DISC.FLAT.AMT.CHECK

    CASE AF = SDB.PAR.DISC.PERCENT
        GOSUB DISC.PERCENT.CHECK

    CASE AF = SDB.PAR.RENEW.FREQUENCY
        GOSUB RENEW.FREQUENCY.CHECK

    CASE AF = SDB.PAR.RENEW.NOTICE.FREQ OR AF = SDB.PAR.2ND.REMINDER.FREQ
        GOSUB CHECK.FREQ
...
RENEW.FREQUENCY.CHECK:
    ...
    IF R.NEW(SDB.PAR.FREQ.TYPE) = 'YEARLY' AND COMI AND COMI[9,3] NE 'M12' THEN
        E = 'Frequency Type and Renewal frequency do not match'
    END

    IF R.NEW(SDB.PAR.FREQ.TYPE) = 'MONTHLY' AND COMI AND COMI[9,3] NE 'M01' THEN
        E = 'Frequency Type and Renewal frequency do not match'
    END
    ...
    IF COMI THEN
        ...
        IF LAST.CHAR MATCHES 'W':VM:'C' THEN
            ...
            IF NUM(REST.OF.CHAR) THEN
                IF REST.OF.CHAR GT 99 THEN SET.ERR.FLG = 1
            END ELSE
                SET.ERR.FLG = 1
            END
        END ELSE
        ...
        IF SET.ERR.FLG = 1 THEN E = 'Invalid Frequency. E.g. 15C, 10W, etc'
    END
