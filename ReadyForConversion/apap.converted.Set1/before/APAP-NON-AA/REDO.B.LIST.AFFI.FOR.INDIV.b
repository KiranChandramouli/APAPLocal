...
*-----------------------------------------------------------------------------------------------------------------
    ...
    $INSERT I_F.CUSTOMER
    $INSERT I_F.COMPANY
    $INSERT TAM.BP I_REDO.B.LIST.AFFI.FOR.INDIV.COMMON
...
***********************
* In this para of the program, the customer is checked if reported in other development reports
**
    CUSTOMER.REPORTED     = ''
*    RELATION.CUSTOMER.ID  = FL.CUSTOMER.ID
*    GOSUB READ.RELATION.CUSTOMER
*    IF NOT(R.RELATION.CUSTOMER) THEN
*        GOSUB RAISE.FATAL.ERROR
*    END

    MV.FL.REL.CODES       = R.CUSTOMER.AP.MT<EB.CUS.RELATION.CODE>
    MV.FL.REL.CUSTOMER    = R.CUSTOMER.AP.MT<EB.CUS.REL.CUSTOMER>
    MV.FL.REL.CODES.COUNT = DCOUNT(MV.FL.REL.CODES,VM)
    MV.FL.REL.CODES.INIT  = 1

    LOOP
    WHILE MV.FL.REL.CODES.INIT LE MV.FL.REL.CODES.COUNT
        LOCATE MV.FL.REL.CODES<1,MV.FL.REL.CODES.INIT> IN MV.FL.PARAM.SEL.CODES<1,1,1> SETTING MV.FL.FOUND.POS THEN
...
**
*    MV.FL.CUSTOMER.ID = MV.FL.REL.CUSTOMER<1,MV.FL.REL.CODES.INIT>

*    RELATION.CUSTOMER.ID = MV.FL.CUSTOMER.ID
*    GOSUB READ.RELATION.CUSTOMER
...
*    END
    ...
    MV.SL.REL.CODES    = R.CUSTOMER.AP.MT<EB.CUS.RELATION.CODE>
    MV.SL.REL.CUSTOMER = R.CUSTOMER.AP.MT<EB.CUS.REL.CUSTOMER>

    MV.SL.REL.CODES.COUNT = DCOUNT(MV.SL.REL.CODES,VM)
    MV.SL.REL.CODES.INIT  = 1

    LOOP
    WHILE MV.SL.REL.CODES.INIT LE MV.SL.REL.CODES.COUNT
        LOCATE MV.SL.REL.CODES<1,MV.SL.REL.CODES.INIT> IN MV.SL.PARAM.SEL.CODES<1,1,1> SETTING MV.SL.FOUND.POS THEN
            Y.ROLE.MORE.INFO=R.CUSTOMER.AP.MT<EB.CUS.ROLE.MORE.INFO><1,MV.SL.REL.CODES.INIT,1>
            CUSTOMER.REPORTED=1
        END
        MV.SL.REL.CODES.INIT += 1
    REPEAT
...
CUSTOMER.REPORTED:
******************
* In this para of the program, the development name in which the customer reported is displayed in the report
**
*    C$SPARE(452) = 'MV31'
*    GOSUB MAP.RCL.RECORD
