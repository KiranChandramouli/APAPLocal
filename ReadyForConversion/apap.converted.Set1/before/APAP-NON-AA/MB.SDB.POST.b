...
BEFORE.UNAU.WRITE:
    ...
    IF R.NEW(SDB.POST.SDB.OPERATE) = 'VISIT' AND V$FUNCTION EQ 'I' THEN
        IF NOT(R.NEW(SDB.POST.ACCESS.TIME)) THEN R.NEW(SDB.POST.ACCESS.TIME) = OCONV(TIME(),'MTS')[1,2]:OCONV(TIME(),'MTS')[4,2]  ;* Don't store the ':' between the HH and MM. Just store the no.
        IF NOT(R.NEW(SDB.POST.ACCESS.DATE)) THEN R.NEW(SDB.POST.ACCESS.DATE) = TODAY
    END
...
* NSCU20090929 - E
    ...
    CASE R.NEW(V-8)[1,3] = "INA"
        ...
        BEGIN CASE
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'OPEN'
...
*   R.MB.SDB.CHARGES<SDB.CHG.PAY.REASON,-1> = 'Opening Charges, VAT and deposit'
        ...
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'RESERVE'
        ...
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'MAINTAIN'
            GOSUB READU.SDB.STATUS
****  R.MB.SDB.STATUS<SDB.STA.STATUS> = 'REPAIR'
        ...
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'VISIT'
            GOSUB READ.SDB.ACCESS
            R.MB.SDB.ACCESS<SDB.ACS.TXN.REF,-1> = ID.NEW
        ...
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'MODIFY'
...
*CAPL20090831 - E
                IF R.NEW(SDB.POST.CUSTOMER.ACCT) AND (R.MB.SDB.STATUS<SDB.STA.RENEW.METHOD> NE R.NEW(SDB.POST.RENEW.METHOD)) THEN
                    IF R.NEW(SDB.POST.RENEW.METHOD) EQ 'MANUAL' THEN GOSUB CANCEL.STO
                    IF R.NEW(SDB.POST.RENEW.METHOD) EQ 'AUTO' THEN
                        IF R.NEW(SDB.POST.PERIODIC.RENT) GT 0 THEN
                            GOSUB CREATE.STO
                            R.MB.SDB.STATUS<SDB.STA.STO.REF> = OFS.TXN.ID
...
***            GOSUB UPDATE.SDB.STATUS
        ...
        CASE R.NEW(SDB.POST.SDB.OPERATE) = 'CLOSE'
...
* GOTO BEF.AUT.EXIT
            ...
            IF R.NEW(SDB.POST.REFUND.METHOD) = 'AUTO' AND R.NEW(SDB.POST.REFUND.AMT) GT 0 THEN
...
*  GOTO BEF.AUT.EXIT
            ...
            IF ID.CNT GT 0 THEN MB.SDB.CLOSED.ID = NEW.ID
...
BEF.AUT.EXIT:
    ...
    IF V$ERROR EQ 'ERROR' THEN RELEASE
...
READ.SDB.COMPANY:
    ...
    CALL F.READ(FN.COMPANY, COMP.ID, R.COMP, FP.COMPANY, COMP.ERR)
...
CREATE.STO:
    ...
    IF ACCT.COM.CODE THEN YOFS.COMPANY = ACCT.COM.CODE
    IF R.NEW(SDB.POST.SDB.OPERATE) = 'MODIFY' THEN
        STO.END.DATE = ''
...
READ.SDB.PARAM:
    ...
    CALL F.READ(FN.COMPANY,MB.SDB.PARAM.ID,R.COMPANY.REC,F.COMPANY,COMPANY.REC.ERR)
...
READ.FT.COMM:
    ...
    CALL F.READ(FN.FT.COMMISSION.TYPE,FT.COMMISSION.TYPE.ID,R.FT.COMMISSION.TYPE,F.FT.COMMISSION.TYPE,YERR)
