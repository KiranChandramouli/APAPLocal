...
* <Rating>-46</Rating>
*-----------------------------------------------------------------------------
  SUBROUTINE REDO.DS.AZ.MAIL.ADD(Y.ACCT.NO,Y.ADDRESS.DET)
*--------------------------------------------------------------------------------------------------------
...
* -----------    -------------             ----------------        ----------------
* 18 MAR 2011    SABARIKUMAR A             ODR-2010-08-0181        Initial Creation
*--------------------------------------------------------------------------------------------------------
...
* Forms the value of O.DATA by fetching necessary values from DE.ADDRESS
  ...
  CALL F.READ(FN.DE.PRODUCT,Y.DE.ID,R.DE.PRODUCT.REC,F.DE.PRODUCT,DE.PRODUCT.ERR)
  ...
  IF Y.PAIS NE '' THEN
    CALL F.READ(FN.COUNTRY,Y.PAIS,R.COUNTRY,F.COUNTRY,CTRY.ERR)
...
*   Y.DATA:=TRIM(Y.SEC):" ":TRIM(Y.COUNTRY):" ": TRIM(Y.TOWN.COUNTRY):" ":TRIM(Y.PAIS):" ":TRIM(Y.POST)
  Y.DATA = TRIM(Y.STR.ADR):' ':TRIM(Y.DIR):FM:TRIM(Y.URD):" ":TRIM(Y.SEC):FM:TRIM(Y.COUNTRY):" ":TRIM(Y.TOWN.COUNTRY):FM:TRIM(Y.PAIS):FM:TRIM(Y.POST)

  IF Y.STR.ADR NE '' OR Y.DIR NE '' THEN
    Y.ADDRESS.DET=TRIM(Y.DATA)
*PACS00072406 -E
*END ELSE
*   O.DATA = "Not Applicable"
  END
  RETURN
