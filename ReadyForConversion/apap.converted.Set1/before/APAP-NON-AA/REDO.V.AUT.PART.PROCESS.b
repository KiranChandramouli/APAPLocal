...
* PACS00126000 -S
    IF APPLICATION EQ 'FUNDS.TRANSFER' AND (V$FUNCTION = "I" OR V$FUNCTION = "A") THEN
      TT.ID =R.NEW(FT.LOCAL.REF)<1,VAR.FT.ID>
      CALL F.DELETE(FN.REDO.PART.TT.PROCESS,TT.ID)
* PACS00126000 -E
    END
    IF APPLICATION EQ 'REDO.PART.TT.REJECT' AND V$FUNCTION = "I" AND PGM.VERSION EQ ",CREATE" THEN
      CALL F.DELETE(FN.REDO.PART.TT.PROCESS,ID.NEW)
    END
  END ELSE
* PACS00126000 -S
    IF APPLICATION EQ 'FUNDS.TRANSFER' AND V$FUNCTION = "A" THEN
      TT.ID = R.NEW(FT.LOCAL.REF)<1,VAR.FT.ID>
* PACS00126000 -E
      CALL F.DELETE(FN.REDO.PART.TT.PROCESS,TT.ID)
    END
    IF APPLICATION EQ 'REDO.PART.TT.REJECT' AND V$FUNCTION = "A" AND PGM.VERSION EQ ",CREATE" THEN
      CALL F.DELETE(FN.REDO.PART.TT.PROCESS,ID.NEW)
    END
  END
  ...
  IF APPLICATION EQ 'REDO.MULTI.TXN.REJECT' AND V$FUNCTION = "I" AND PGM.VERSION EQ ",CREATE" THEN
