...
************
    ...
    WHILE Y.CNT.CUR.TYPE LE Y.DCNT.CUR.ASS
        ...
        IF SYS.DATE LE YLST.TODAY THEN
...
PRICNI.MVMT.APPL:
*---------------
    YARR.CNT = 0; X.VAR = 1; TOTCOMMITMENT.VAL = 0
    YARR.CNT = DCOUNT(YARR.ASST,@FM)
    LOOP
    UNTIL X.VAR GT YARR.CNT
        ECB.AMT = 0; ORG.ECB.AMT = 0; ASSET.VAL = ''; TEMP.FLG = 0
        ASSET.VAL = YARR.ASST<X.VAR>
        ECB.AMT = Y.BAL.PRIC.GP<X.VAR>
        ORG.ECB.AMT = Y.BAL.PRIC.ORG<X.VAR>
