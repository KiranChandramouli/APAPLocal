...
* fix for PACS00151814
                ...
                BEGIN CASE
                    CASE Y.FIELD.TO EQ '@ID'
                        ...
                        Y.RECORD.ID = FIELD(R.NEW(Y.FIELD.FROM),@VM,K.VAR)
                        CONTINUE
...
* fix for PACS00151814
                    ...
                    CASE Y.FIELD.FROM.T EQ 'ADDRESS.BR'
                        Y.ADDRES.BR = FIELD(R.NEW(Y.FIELD.FROM),@VM,K.VAR)
...
*PACS00151814
    Y.CUSTOMER.ID = FIELD(R.NEW(Y.CUSTOMER.FLD.POS),@VM,K.VAR)   ;* Customer
...
* Seteo en R.NEW el valor del ID
    R.NEW(Y.CL.FIELD.ID)<1,K.VAR> = Y.CL.ID
...
* Obtiene cada COLLATERAL.CODE que el usuario debio haber ingresado
    Y.COLLATERAL.CODE = FIELD(R.REDO.FC.PROD.COLL.POLICY<REDO.CPL.COLLATERAL.CODE>,@VM,I.VAR,1)
