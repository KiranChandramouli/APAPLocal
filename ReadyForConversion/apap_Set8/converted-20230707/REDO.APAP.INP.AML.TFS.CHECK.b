...
CONFIRM.AML.CHECK:
*-----------------------------------------------------------------------------------------------------------
    CALL CACHE.READ(FN.TRANSACTION, Y.TT.TRANS.ID.1, R.TRANSACTION, TRANS.ERR)
    WTR.AML.CHECK = R.TRANSACTION<AC.TRA.LOCAL.REF><1,LOC.L.TR.AML.CHECK.POS>
    IF WTR.AML.CHECK EQ 'Y' THEN
        PROCESS.GOAHEAD = "1"
    END ELSE
        CALL CACHE.READ(FN.TRANSACTION, Y.TT.TRANS.ID.2, R.TRANSACTION, TRANS.ERR)
        WTR.AML.CHECK = R.TRANSACTION<AC.TRA.LOCAL.REF><1,LOC.L.TR.AML.CHECK.POS>
        IF WTR.AML.CHECK EQ 'Y' THEN
            PROCESS.GOAHEAD = "1"
        END
...
*****************
    ...
    CALL CACHE.READ(FN.TRANSACTION, Y.STMT.TXN.CODE, R.TRANSACTION, TRANS.ERR)
    WTR.AML.CHECK = R.TRANSACTION<AC.TRA.LOCAL.REF><1,LOC.L.TR.AML.CHECK.POS>
    IF WTR.AML.CHECK EQ 'Y' THEN
...
GET.AMLBUY.RATE:
*-------------------------------------------------------------------------------------------------------------
    CALL CACHE.READ(FN.CURR, WACCT.CCY, R.CURR, CURR.ERR)
...
* In this para of the code, file FT.TXN.TYPE.CONDITION is read
    ...
    CALL CACHE.READ(FN.FT.TXN.TYPE.CONDITION, FT.TXN.TYPE.CONDITION.ID, R.FT.TXN.TYPE.CONDITION, FT.TXN.TYPE.CONDITION.ER)
...
* In this para of the code, file OVERRIDE is read
    R.OVERRIDE  = ''
    OVERRIDE.ER = ''
    CALL CACHE.READ(FN.OVERRIDE, OVERRIDE.ID, R.OVERRIDE, OVERRIDE.ER)
