...
*-----------------------------------------------------------------------------------
    ...
    R.REDO.CLEARING.OUTWARD<CLEAR.OUT.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR
    R.REDO.CLEARING.OUTWARD<CLEAR.OUT.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR
...
*-----------------------------------------------------------------------------
    ...
    CHANGE @VM TO @FM IN BANK.CATEGORY
...
*-----------------------------------------------------------------------------
    ...
    FETCH.CUST.TYPE = ''
    CHANGE @VM TO @FM IN LIST.CURRENCY
    ...
    LOCATE VAR.CURRENCY IN LIST.CURRENCY SETTING CURR.POS THEN
        ...
        CHANGE @SM TO @FM IN FETCH.CUST.TYPE
        CHANGE @SM TO @FM IN FETCCH.TRANSIT.DAYS
...
* This has been added since we had problem in Processing OFS. since we posted diff ofs message for same ALE
    Y.LOOP.CNT = 1
    Y.TOTAL.CNT = DCOUNT(Y.FINAL.ALE.ID,@FM)
    LOOP
