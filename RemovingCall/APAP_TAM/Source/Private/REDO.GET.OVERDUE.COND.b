* @ValidationCode : MjoxNzkzOTAzODk5OkNwMTI1MjoxNjgzODkyNjI2ODg3OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 12 May 2023 17:27:06
...
*    CALL REDO.CONVERT.ACCOUNT(Y.ACC.NO,IN.ARR.ID,Y.AA.ID,ERR.TEXT)
    CALL APAP.TAM.redoConvertAccount(Y.ACC.NO,IN.ARR.ID,Y.AA.ID,ERR.TEXT) ;* R22 Manual Conversion
    IF Y.AA.ID ELSE
        O.DATA = ''
        RETURN
    END
...
*    CALL REDO.CRR.GET.CONDITIONS(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG)
    CALL APAP.AA.redoCrrGetConditions(Y.AA.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG);* R22 Manual Converison
