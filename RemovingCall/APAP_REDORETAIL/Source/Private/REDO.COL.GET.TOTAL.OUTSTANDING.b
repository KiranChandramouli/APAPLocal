...
*CALL APAP.TAM.REDO.CONVERT.ACCOUNT(IN.ACC.ID,ARR.ID,Y.ARR.ACC.ID,ERR.TEXT)
    CALL APAP.TAM.redoConvertAccount(IN.ACC.ID,ARR.ID,Y.ARR.ACC.ID,ERR.TEXT);* R22 Manual conversion
...
*CALL APAP.AA.REDO.AA.GET.ECB.BALANCE.AMOUNT(Y.ARR.ACC.ID,BALANCE.TO.CHECK,TODAY,BALANCE.AMOUNT,RET.ERROR)
            CALL APAP.AA.redoAaGetEcbBalanceAmount(Y.ARR.ACC.ID,BALANCE.TO.CHECK,TODAY,BALANCE.AMOUNT,RET.ERROR);* R22 Manual conversion
****PACS00523653****
