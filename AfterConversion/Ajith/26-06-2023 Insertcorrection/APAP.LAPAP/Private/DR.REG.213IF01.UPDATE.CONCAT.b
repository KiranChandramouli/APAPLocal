...
*26-06-2023       S.AJITHKUMAR               R22 Manual Code Conversion     FMto@FM,VMto@VM,Command this Insert file I_F.T24.FUND.SERVICES
*-----------------------------------------------------------------------------
...
*------------------*
    ...
    YSTMT.VAL = STMT.ID:'*':YACCT.ID:'*':Y.T24.FS.REF
    CALL F.READU(FN.DR.REG.213IF01.CONCAT,CONCAT.ID,R.DR.REG.213IF01.CONCAT,F.DR.REG.213IF01.CONCAT,DR.REG.213IF01.CONCAT.ERR,'')  ;*PACS00677208
...
CUST.REL.CHK:
*************
    YPEP.CHK = ''; CUSTOMER.ERR = ''; R.CUSTOMER = ''; YREL.CODE = ''; CU.PEPS.VAL = ''
    ...
    IF CU.PEPS.VAL EQ 'SI' OR YREL.CODE NE '' THEN
        YPEP.CHK = 'PEP'
    END
RETURN
...
*  YTEMP.AMT = R.T24.FUND.SERVICES<TFS.AMOUNT,YVM> ;*R22 MANUAL CODE CONVERSION
    END
RETURN
...
***************
    ...
    IF Y.T24.FS.REF THEN
        GOSUB READ.TFS
        GOSUB TFS.RTE.CHK
*        YTRANSACT = R.T24.FUND.SERVICES<TFS.TRANSACTION> ;*R22 MANUAL CODE CONVERSION
        GOSUB GET.TFS.VAL
    END
RETURN
...
READ.ACCOUNT:
*************
    R.ACCOUNT = ''; ERR.ACCOUNT = ''; Y.ACCT.HST = ''
...
*********
    ...
    END
* Y.RTE.FORM.VAL = R.T24.FUND.SERVICES<TFS.LOCAL.REF,TFS.L.RTE.FORM.POS> ;*R22 MANUAL CODE CONVERSION
*    Y.L.FT.ADD.INFO.VAL = R.T24.FUND.SERVICES<TFS.LOCAL.REF,TFS.L.FT.ADD.INFO.POS> ;*R22 MANUAL CODE CONVERSION
    RTE.FORM.VAL = Y.RTE.FORM.VAL
RETURN
