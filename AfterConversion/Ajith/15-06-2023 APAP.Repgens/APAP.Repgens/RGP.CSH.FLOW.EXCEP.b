...
SUBROUTINE RGP.CSH.FLOW.EXCEP
REM "RGP.CSH.FLOW.EXCEP",230614-4
...
*15-06-2023       AJITHKUMAR                  MANUAL R22 CODE CONVERSION > to > 0, GOSUB to GOSUB
    REPORT.ID = "RG.CSH.FLOW.EXCEP"
    PRT.UNIT = 0
...
*************************************************************************
    ...
    YT.SMS.FILE = "CASH.FLOW.EXCEPTION"
...
*************************************************************************
    ...
    CLEARSELECT
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.CSH.FLOW.EXCEP"
    EXECUTE "HUSH ON"
...
* T.REMTEXT(23) = "PAGE"
    ...
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = " ACCOUNT NO. ACCOU                      NT TITLE DATE                            BALANCE NARRATIVE"
    YHDR2 = " ---------------- --------------        --------------------- -----------       -------------------- ---------------    ------------"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YFD = YR.REC(1)  ;* YM.ACC.ID
        YFD = FMT(YFD,"L")
        IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|" ;*R22 manual code conversion
        YTOTFD := "     ":YFD
        YFD = YR.REC(2)  ;* YM.ACC.TITLE
        YFD = FMT(YFD,"L")
        IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|" ;*R22 manual code conversion
        YTOTFD := "                     ":YFD
        YCNT = COUNT(YR.REC(3),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(3)<1,1>  ;* YM.DATE
        YFD = FMT(YFD,"L")
        IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|" ;*R22 manual code conversion
        YTOTFD := "                                        ":YFD
        YCNT = COUNT(YR.REC(4),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(4)<1,1>  ;* YM.BALANCE
        YFD = FMT(YFD,"L")
        IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|" ;*R22 manual code conversion
        YTOTFD := "               ":YFD
        YCNT = COUNT(YR.REC(5),@VM)
        IF YCNT >= YCOUNT.LIN THEN YCOUNT.LIN = YCNT+1
        YFD = YR.REC(5)<1,1>  ;* YM.LIMIT.OD
        YFD = FMT(YFD,"L")
        IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|" ;*R22 manual code conversion
        YTOTFD := "                        ":YFD
        GOSUB 9000000
...
*
        FOR YAV = 2 TO YCOUNT.LIN
            YFD = YR.REC(3)<1,YAV>  ;* YM.DATE
            YFD = FMT(YFD,"L")
            IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|"
            YTOTFD := "                                                                  ":YFD
            YFD = YR.REC(4)<1,YAV>  ;* YM.BALANCE
            YFD = FMT(YFD,"L")
            IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|"
            YTOTFD := "               ":YFD
            YFD = YR.REC(5)<1,YAV>  ;* YM.LIMIT.OD
            YFD = FMT(YFD,"L")
            IF LEN(YFD) > 0 THEN YFD = YFD[1,-1]:"|"
            YTOTFD := "                        ":YFD
            GOSUB 9000000
