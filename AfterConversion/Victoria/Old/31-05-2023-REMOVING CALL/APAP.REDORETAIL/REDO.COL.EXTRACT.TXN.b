* @ValidationCode : MjoxNjcxMTI1NTY1OkNwMTI1MjoxNjg1NTI2MzM0ODc3OnZpY3RvOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 31 May 2023 15:15:34
...
*12-04-2023          jayasurya H                       MANUAL R22 CODE CONVERSION            CALL RTN ADDED
SUBROUTINE REDO.COL.EXTRACT.TXN(Y.PROCESS.DATE, Y.AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.ID, Y.AGENCY.CODE, Y.CREDIT.TXN)
******************************************************************************
*
...
*   $INSERT I_F.AA.ACTIVITY.HISTORY ;* R22 Auto converison
    $USING APAP.TAM
*
*************************************************************************
*
...
*         Y.MVMT.COUNTER++
    ...
    IF R.AA.ACT<AA.ARR.ACT.EFFECTIVE.DATE> EQ '' THEN
        ...
        RETURN
    END
    Y.TMPMOVFECHA = redoOracleDate(R.AA.ACT<AA.ARR.ACT.EFFECTIVE.DATE>,'yyyyMMdd')
...
*         END
    ...
    Y.TMPMOVHORA = redoOracleDate(R.AA.ACT<AA.ARR.ACT.DATE.TIME,1>,'yyMMddhh24mi')
...
* ---------
    ...
    Y.TMPMOVSECUENCIAL = redoOracleNull("'" : Y.TMPMOVSECUENCIAL : "'")
*     Y.TMPMOVFECHA = redoOracleNull("'" : Y.TMPMOVFECHA : "'")
    ...
    Y.TMPMOVPRODUCTO = redoOracleNull(Y.TMPMOVPRODUCTO)
    Y.TMPMOVMONTO = redoOracleNull(Y.TMPMOVMONTO)
    Y.TMPMOVMONTOPAGOCAPITAL = redoOracleNull(Y.TMPMOVMONTOPAGOCAPITAL)
    Y.TMPMOVMONTOPAGOINTERES = redoOracleNull(Y.TMPMOVMONTOPAGOINTERES)
    Y.TMPMOVMONTOPAGOOTROS = redoOracleNull(Y.TMPMOVMONTOPAGOOTROS)
    Y.TMPMOVMONTOMONEDAORIGEN = redoOracleNull(Y.TMPMOVMONTOMONEDAORIGEN)
    Y.TMPMOVUSUARIO = redoOracleNull("'" : Y.TMPMOVUSUARIO : "'")
*     Y.TMPMOVHORA = redoOracleNull("'" : Y.TMPMOVHORA : "'")
*     Y.TMPMOVCODIGOGESTOR = redoOracleNull("'" : Y.TMPMOVCODIGOGESTOR : "'")
    Y.TMPMOVTASACAMBIO = redoOracleNull(Y.TMPMOVTASACAMBIO)
    Y.TMPMOVMONEDACODIGO = redoOracleNull(Y.TMPMOVMONEDACODIGO)
    Y.TMPMOVTIPOTRNCODIGO = redoOracleNull("'" : Y.TMPMOVTIPOTRNCODIGO : "'")
    Y.TMPMOVTIPOTRNDESCRIPCION = redoOracleNull("'" : Y.TMPMOVTIPOTRNDESCRIPCION : "'")
    Y.TMPAGENCIACODIGO = redoOracleNull(Y.TMPAGENCIACODIGO)
...
*     Y.TMPMOVCARTERACLIENTECODIGO = redoOracleNull("'" : Y.TMPMOVCARTERACLIENTECODIGO : "'")
    ...
    Y.INS.VALUES = ""
    Y.INS.VALUES := Y.PARCOMPANIACODIGO: "," : Y.TMPMOVSECUENCIAL: "," : Y.TMPMOVFECHA: ","
    Y.INS.VALUES := Y.TMPCREDITONUMEROCONTRATO: "," : Y.TMPMOVSISTEMACODIGO: "," : Y.TMPMOVPRODUCTO: ","
    Y.INS.VALUES := Y.TMPMOVMONTO: "," : Y.TMPMOVMONTOPAGOCAPITAL: "," : Y.TMPMOVMONTOPAGOINTERES: ","
    Y.INS.VALUES := Y.TMPMOVMONTOPAGOOTROS: "," : Y.TMPMOVMONTOMONEDAORIGEN: "," : Y.TMPMOVUSUARIO: ","
*     Y.INS.VALUES := Y.TMPMOVMONTOPAGOOTROS: "," : Y.TMPMOVUSUARIO: ","
*     Y.INS.VALUE := Y.TMPMOVHORA: "," : Y.TMPMOVCODIGOGESTOR: "," : Y.TMPMOVTASACAMBIO: "," :
    Y.INS.VALUES := Y.TMPMOVHORA: "," : Y.TMPMOVTASACAMBIO: "," :
    Y.INS.VALUES := Y.TMPMOVMONEDACODIGO: "," : Y.TMPMOVTIPOTRNCODIGO: "," : Y.TMPMOVTIPOTRNDESCRIPCION: ","
...
*-----------------------------------------------------------------------------
    ...
    E = ""
*CALL APAP.TAM.REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, Y.MAP.TYPE, Y.MAP.VALUE)
    APAP.TAM.redoRColGetMapping(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, Y.MAP.TYPE, Y.MAP.VALUE);* R22 Manual conversion - CALL method format changed
    IF E THEN
        GOSUB TRACE.ERROR
    END
...
*-----------------------------------------------------------------------------
    ...
    Y.STMT.ID = R.AA.ACT.CHILD<AA.ARR.ACT.STMT.NOS,1>
...
*-----------------------------------------------------------------------------
    ...
    Y.STMT.ID = Y.STMT.ID : Y.STMT.POS
    YERR = ""
...
*CALL APAP.TAM.REDO.R.COL.PROCESS.TRACE(P.TYPE_PROC, P.STATUS.ERR, Y.CONTADOR, P.TABLE, Y.TYPE.TXN, C.DESC)
    APAP.TAM.redoRColProcessTrace(P.TYPE_PROC, P.STATUS.ERR, Y.CONTADOR, P.TABLE, Y.TYPE.TXN, C.DESC);* R22 Manual conversion - CALL method format changed
*CALL APAP.TAM.REDO.R.COL.EXTRACT.ERROR(C.DESC, "REDO.COL.EXTRACT.TXN",P.TABLE)
    APAP.TAM.redoRColExtractError(C.DESC, "REDO.COL.EXTRACT.TXN",P.TABLE);* R22 Manual conversion - CALL method format changed
*     C.MON.TP='08'
*     C.ID.PROC = REDO.INTERFACE.PARAM.ID
    Y.PROCESS.FLAG.TABLE<1,4> = ""
    GOSUB STORE.MSG.ON.QUEUE
*CALL APAP.REDOVER.REDO.INTERFACE.REC.ACT(C.INT.CODE,C.INT.TYPE,Y.CONTADOR,C.BAT.TOT,C.INFO.OR,C.INFO.DE,C.ID.PROC,C.MON.TP,C.DESC,C.REC.CON,C.EX.USER,C.EX.PC)
    PROCESS.GOAHEAD = 0
RETURN
