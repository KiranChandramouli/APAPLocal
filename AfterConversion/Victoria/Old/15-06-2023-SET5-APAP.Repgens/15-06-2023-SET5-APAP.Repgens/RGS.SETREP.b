...
*----------------------------------------------------------------------------------------
SUBROUTINE RGS.SETREP
REM "RGS.SETREP",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
*************************************************************************
    ...
    FULL.FNAME = "F.SECURITY.TRANSFER"; YF.SECURITY.TRANSFER = ""
    ...
    CALL OPF (FULL.FNAME, YF.SECURITY.TRANSFER)
    CLEARSELECT
    ...
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.SECURITY.TRANSFER, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
...
*
    REPEAT
*************************************************************************
    YFILE = "SECURITY.MASTER"
    FULL.FNAME = "F.SECURITY.MASTER"; YF.SECURITY.MASTER = ""
    LOCATE YFILE IN YT.SMS<1,1> SETTING X ELSE
        X = 0; T.PWD = ""
    END
    IF X THEN
    ...
    CALL GET.STANDARD.SELECTION.DETS(YFILE,SS.REC)
    LOCATE "CO.CODE" IN SS.REC<SSL.SYS.FIELD.NAME,1> SETTING COMP.FOUND ELSE COMP.FOUND = ""
    CALL OPF (FULL.FNAME, YF.SECURITY.MASTER)
    CLEARSELECT
    IF C$MULTI.BOOK AND COMP.FOUND THEN
        SEL.ARGS = " WITH CO.CODE EQUAL ":ID.COMPANY
    END ELSE
    ...
    LOOP
        REMOVE ID.NEW FROM YID.LIST SETTING YDELIM
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.SECURITY.MASTER, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
            CALL CONTROL.USER.PROFILE ("RECORD")
            IF ETEXT THEN ID.NEW = ""
        END
