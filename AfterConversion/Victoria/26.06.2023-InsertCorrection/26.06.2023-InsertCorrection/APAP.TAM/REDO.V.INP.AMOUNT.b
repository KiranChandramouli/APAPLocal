* @ValidationCode : Mjo2MjgwMzM5NzM6Q3AxMjUyOjE2ODc3Njc1MjkzOTk6dmljdG86LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 26 Jun 2023 13:48:49
...
*----------------------------------------------------------------------
    ...
    LOC.REF.APPLICATION="T24.FUND.SERVICES":@FM:"TELLER" ;*R22 MANUAL CONVERSION
    LOC.REF.FIELDS='L.TT.PROCESS':@VM:'L.TT.NO.OF.CHQ':@FM:'L.COMMENTS' ;*R22 MANUAL CONVERSION
    LOC.REF.POS=''
...
*Checking TFS amount value equal with the local table amount value
    ...
    Y.COUNT=DCOUNT(R.REDO.TFS.PROCESS<TFS.PRO.AMOUNT>,@VM) ;*R22 MANUAL CONVERSION
...
*IF Y.COUNT EQ Y.TFS.COUNT THEN
    ...
    WHILE Y.CNT LE Y.COUNT
        ...
        IF Y.TFS.AMT NE Y.AMT THEN
            AF=TFS.AMOUNT
            AV=Y.CNT
            ETEXT='TT-TFS.AMT.CHECK-&':@FM:Y.AMT ;*R22 MANUAL CONVERSION
            CALL STORE.END.ERROR
            RETURN
        END
        Y.CNT++
