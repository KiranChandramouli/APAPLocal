* @ValidationCode : MjotOTcwMDAyNDI5OkNwMTI1MjoxNjk3NzE3MDc1NjM0OnZpY3RvOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
...
OPENFILE:
    ...
    LF.APP = 'RE.STAT.REP.LINE':@FM:'ACCOUNT'  ;*R22 MANUAL CONVERSION
    LF.FLD = 'L.BALEM':@VM:'L.BALPG':@VM:'L.BALFA':@VM:'L.ACCEM':@VM:'L.ACCPG':@VM:'L.ACCFA':@FM:'L.AC.AV.BAL'  ;*R22 MANUAL CONVERSION
    LF.POS = ''
...
*-------Modification ends--------------------------------
    ...
    LOCK.TOT = DCOUNT(REC.ACCOUNT<AC.LOCKED.AMOUNT>,@VM)  ;*R22 MANUAL CONVERSION
...
*R22 MANUAL CONVERSION
    APAP.REDOAPAP.redoConvMnemToStatus(ACCT.ID,Y.STATUS)
    BEGIN CASE
...
* Get balances and splitted balances for entries
        ...
        END
    END ELSE
        IF ACCT.STATUS MATCHES 'AC':@VM:'IL':@VM:'IM':@VM:'LE':@VM:'ME':@VM:'FA':@VM:'PG':@VM:'EM' THEN  ;*R22 MANUAL CONVERSION
            BEGIN CASE
*            CASE Y.REV.AMOUNT
*                BAL.DR.AMT = Y.REV.AMOUNT
...
* Get accounting account for entries
    ...
    IF BAL.CR.AMT OR INT.CR.AMT OR BAL.DR.AMT OR INT.DR.AMT THEN
        ...
        END ELSE
            BEGIN CASE
                CASE ACCT.STATUS MATCHES 'AC':@VM:'IL':@VM:'IM':@VM:'LE':@VM:'ME':@VM:'EM'  ;*R22 MANUAL CONVERSION
                    Y.BAL.DET = R.STAT.REP.LINE.CAP<RE.SRL.LOCAL.REF,L.BALEM.POS>:'*':Y.NAME:'*':R.STAT.REP.LINE.CAP<RE.SRL.DESC,3,1>:'*':Y.SIB.ACC.NO:'*':RE.STAT.REP.ID.BAL:'*':Y.REV.FLAG
                    Y.INT.DET = R.STAT.REP.LINE.INT<RE.SRL.LOCAL.REF,L.ACCEM.POS>:'*':Y.NAME:'*':R.STAT.REP.LINE.INT<RE.SRL.DESC,3,1>:'*':Y.SIB.ACC.NO:'*':RE.STAT.REP.ID.INT
...
***********************
    ...
    Y.VARIABLE.7.LIST = CHANGE(Y.VARIABLE.7.LIST,@VM,@FM)  ;*R22 MANUAL CONVERSION START
    ...
    TOT.LINES = DCOUNT(Y.VARIABLE.7.LIST,@FM)  ;*R22 MANUAL CONVERSION END
    CNT = 1
    LOOP
...
**********************
    ...
    STAT.FM.CNTR = DCOUNT(PARAM.STATUS.VAL,@VM)  ;*R22 MANUAL CONVERSION END
    LOOP.FM.CNTR = 1
