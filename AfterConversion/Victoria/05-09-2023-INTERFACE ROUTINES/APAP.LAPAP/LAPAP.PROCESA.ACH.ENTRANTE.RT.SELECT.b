* @ValidationCode : MjotMTM2NzQ1NDIzMDpDcDEyNTI6MTY5MzkwMDUxOTI1ODp2aWN0bzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 05 Sep 2023 13:25:19
...
* 05-09-2023    VICTORIA S             R22 Manual Conversion     call routine format modified
*-----------------------------------------------------------------------------
...
*Procedemos a limpiar la linea entrante, ya que algunos bancos no la envian correctamente segun la espera T24.
            APAP.LAPAP.lapapCorrigeTramaEntAchRt(Y.INW.LINE, Y.DELIMITER) ;*R22 Manual Conversion
...
*Y.ID.DETAIL = Y.ID.MASTER:Y.SEQUENCE
    MAP.FMT = 'I'; ID.RCON.L = 'REDO.ACH.INWARD'; ID.APP = ''; R.APP = ''
...
*AUDIT FIELDS
    ...
    R.REDO.ACH.PROCESS.DET<REDO.ACH.PROCESS.DET.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR
...
*CALL REDO.CONDUIT.LINEAR.TRANSLATION(MAP.FMT,ID.RCON.L,APP,ID.APP,R.APP,R.RETURN.MSG,ERR.MSG) ;*R22 MANUAL CONVERSION
        APAP.LAPAP.redoConduitLinearTranslation(MAP.FMT,ID.RCON.L,APP,ID.APP,R.APP,R.RETURN.MSG,ERR.MSG)
        IF ERR.MSG NE '' THEN
