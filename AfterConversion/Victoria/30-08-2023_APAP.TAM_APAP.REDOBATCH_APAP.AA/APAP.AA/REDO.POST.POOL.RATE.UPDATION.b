* @ValidationCode : Mjo5OTUzNTU2NjA6Q3AxMjUyOjE2OTMzNzM1Nzg0Njc6dmljdG86LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 30 Aug 2023 11:02:58
...
*-----------------------------------------------------------------------------
    ...
    IF (OFS$BROWSER OR RUNNING.UNDER.BATCH) AND (c_aalocArrActivityRec<AA.ARR.ACT.ACTIVITY.CLASS> NE 'LENDING-NEW-ARRANGEMENT' AND c_aalocArrActivityRec<AA.ARR.ACT.ACTIVITY.CLASS> NE 'LENDING-TAKEOVER-ARRANGEMENT') AND Y.FREQ.REVIEW NE '' THEN
        FREQ = Y.FREQ.REVIEW
        IF R.NEW(AA.INT.LOCAL.REF)<1,Y.FIRS.REV.POS> AND R.NEW(AA.INT.LOCAL.REF)<1,Y.FIRS.REV.POS> GT TODAY THEN ; * TSR-482018
            Y.DATE = R.NEW(AA.INT.LOCAL.REF)<1,Y.FIRS.REV.POS>
        END ELSE
...
*
    ...
    Y.FIRS.REV.POS       = LOC.REF.POS<1,7> ; * TSR-482018
