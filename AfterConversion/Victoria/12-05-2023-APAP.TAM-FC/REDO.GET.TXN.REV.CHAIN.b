* @ValidationCode : MjotMzY1MTg4OTQ4OkNwMTI1MjoxNjgxMTI0OTYwNzcxOjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 10 Apr 2023 16:39:20
...
*-----------------------------------------------------------------------------------
SUBROUTINE REDO.GET.TXN.REV.CHAIN(Y.FINAL.ARRAY)
*-----------------------------------------------------
...
*-----------------------------------------------------
    ...
    Y.FINAL.ARRAY = ''
*CALL APAP.REDOENQ.REDO.E.FORM.SEL.STMT(FN.REDO.TRANSACTION.CHAIN, '', '', SEL.CMD)
*MANUAL R22 CODE CONVERSION
    CALL APAP.REDOENQ.redoEFormSelStmt(FN.REDO.TRANSACTION.CHAIN, '', '', SEL.CMD)
    IF D.RANGE.AND.VALUE THEN
        SEL.CMD := " AND TRANS.AUTH EQ 'A' AND "
        SEL.CMD := " BRANCH.CODE EQ " : ID.COMPANY : " AND TRANS.DATE EQ ":TODAY:" BY.DSND TRANS.DATE"
    END ELSE
        SEL.CMD := " WITH TRANS.AUTH EQ 'A' AND "
        SEL.CMD := " BRANCH.CODE EQ " : ID.COMPANY : " AND TRANS.DATE EQ ":TODAY:" BY.DSND TRANS.DATE"
    END
