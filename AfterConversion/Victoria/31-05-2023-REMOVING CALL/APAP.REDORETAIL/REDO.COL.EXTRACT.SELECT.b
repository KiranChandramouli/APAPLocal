* @ValidationCode : Mjo3NzI0ODk5MzpDcDEyNTI6MTY4NTUyNjI5MTMzNjp2aWN0bzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 31 May 2023 15:14:51
...
*-----------------------------------------------------------------------
    Y.RESPONSE    = ''
*CALL APAP.REDORETAIL.REDO.COL.R.CALL.JAVA.API(Y.EB.API.ID, Y.REQUEST, Y.RESPONSE)
    APAP.REDORETAIL.redoColRCallJavaApi(Y.EB.API.ID, Y.REQUEST, Y.RESPONSE);*MANUAL R22 CODE CONVERSION
    IF Y.RESPONSE NE '' THEN
        C.DESC = Y.RESPONSE
        GOSUB STORE.MSG.ON.QUEUE
        Y.ERROR = @TRUE
*CALL APAP.TAM.REDO.R.COL.EXTRACT.ERROR(C.DESC, "REDO.COL.EXTRACT.SELECT","TMPCLIENTES")
        APAP.TAM.redoRColExtractError(C.DESC, "REDO.COL.EXTRACT.SELECT","TMPCLIENTES");* R22 Manual conversion
*CALL APAP.TAM.REDO.R.COL.PROCESS.TRACE('EXTRACT', 20, 1, "TMPCLIENTES", '', C.DESC)
        APAP.TAM.redoRColProcessTrace('EXTRACT', 20, 1, "TMPCLIENTES", '', C.DESC);* R22 Manual conversion
    END
RETURN
*-----------------------------------------------------------------------
END
