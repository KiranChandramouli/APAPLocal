* @ValidationCode : MjoyMzEyNjQwMDc6Q3AxMjUyOjE2ODU1MzU4OTIwMDQ6dmljdG86LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 31 May 2023 17:54:52
...
*-----------------------------------------------------------------
    ...
    Y.ARR.ACC.ID  = ''
    Y.TOTAL.AMT   = ''
*CALL APAP.TAM.REDO.CONVERT.ACCOUNT(IN.ACC.ID,ARR.ID,Y.ARR.ACC.ID,ERR.TEXT)
...
* Here Overdue property aging status are obtained
    ...
    R.CONDITION.OVERDUE = ''
    ERR.MSG = ''
*CALL APAP.AA.REDO.CRR.GET.CONDITIONS(ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.OVERDUE,ERR.MSG)
...
*CALL REDO.GET.PROPERTY.NAME(ARR.ID,'INTEREST',R.OUT.AA.RECORD,Y.INT.PROPERTY,OUT.ERR)
    APAP.TAM.redoGetPropertyName(ARR.ID,'INTEREST',R.OUT.AA.RECORD,Y.INT.PROPERTY,OUT.ERR) ;* R22 Manual conversion
...
*-----------------------------------------------------------------
    ...
    WHILE Y.LOOP1 LE Y.PROPERTY.CNT
        ...
        WHILE Y.LOOP2 LE Y.BALANCE.CNT
            ...
            BALANCE.AMOUNT=''
****PACS00523653****
*CALL APAP.AA.REDO.AA.GET.ECB.BALANCE.AMOUNT(Y.ARR.ACC.ID,BALANCE.TO.CHECK,TODAY,BALANCE.AMOUNT,RET.ERROR)
