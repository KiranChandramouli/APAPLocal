* @ValidationCode : MjotMTAyNjA2MTc1MjpDcDEyNTI6MTY4NTAxNjY3MTYwNjpIYXJpc2h2aWtyYW1DOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 25 May 2023 17:41:11
...
*----------------------------------------------------------------------------------------
    ...
    $INSERT I_F.REDO.INTRF.REP.LINE  ;*MANUAL R22 CODE CONVERSION
    $INSERT I_F.REDO.CAPL.L.RE.STAT.LINE.CONT ;*MANUAL R22 CODE CONVERSION
...
*------------------------
    ...
    Y.FILE.ID=Y.LINE.1:'.':Y.RE.STAT.LINE.BAL.CUR:'.':SESSION.NO ;*MANUAL R22 CODE CONVERSION
...
****************
    ...
    Y.FT.OUT.LIST=''
    CALL APAP.REDOAPAP.redoApapSapGlDetailRptSplit1(Y.STMT.ENTRY.ID,Y.ENTRY.INDICATOR,Y.PARAM.DETAILS.LIST,Y.FT.OUT.LIST) ;* MANUAL R22 CODE CONVERSION
...
****************
    ...
    Y.FT.OUT.LIST=''
    CALL APAP.REDOAPAP.redoApapSapGlDetailRptSplit1(Y.RE.CONSOL.SPEC.ENTRY.ID,Y.ENTRY.INDICATOR,Y.PARAM.DETAILS.LIST,Y.FT.OUT.LIST) ;* MANUAL R22 CODE CONVERSION
    GOSUB WRITE.DATA.TO.SESSION.FILES
...
*****************
    ...
    Y.FT.OUT.LIST=''
    CALL APAP.REDOAPAP.redoApapSapGlDetailRptSplit1(Y.CATEG.ENTRY.ID,Y.ENTRY.INDICATOR,Y.PARAM.DETAILS.LIST,Y.FT.OUT.LIST) ;* MANUAL R22 CODE CONVERSION
    GOSUB WRITE.DATA.TO.SESSION.FILES
...
*-------------------------------------------------Accoiunting CR changes updated------------------------
    ...
    BAL.CR.AMT = ''; BAL.DR.AMT = '' ; INT.CR.AMT = '' ; INT.DR.AMT = '' ; Y.BAL.DET = '' ; Y.INT.DET = ''
    ...
    IF R.EB.CONTRACT.BALANCES THEN
        ...
        CALL APAP.REDOAPAP.redoVCheckBalIntEntries(Y.ACT.ACCOUNT,BAL.CR.AMT,BAL.DR.AMT,INT.CR.AMT,INT.DR.AMT,Y.BAL.DET,Y.INT.DET) ;* MANUAL R22 CODE CONVERSION
...
*------------------- This is added for accounting CR
    Y.BAL.ACC.NO = '' ; Y.INT.ACC.NO = '' ; Y.FT.OUT.LIST = ''
    ...
    IF BAL.CR.AMT AND Y.BAL.ACC.NO THEN
        Y.CR.TYPE.OF.TXN = '50'
        Y.CR.MOVEMENT = 0
        Y.CR.MVT.LCY  = BAL.CR.AMT
        Y.FT.OUT.LIST := Y.CR.TYPE.OF.TXN:'*':Y.BAL.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.CR.MVT.LCY:'*':Y.CR.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER
        GOSUB WRITE.NORMAL.PROCESS
    END
    ...
    IF BAL.DR.AMT AND Y.BAL.ACC.NO THEN
        Y.DB.TYPE.OF.TXN = '40'
        Y.DB.MOVEMENT = 0
        Y.DB.MVT.LCY  = BAL.DR.AMT
        Y.FT.OUT.LIST := Y.DB.TYPE.OF.TXN:'*':Y.BAL.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.DB.MVT.LCY:'*':Y.DB.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER
        GOSUB WRITE.NORMAL.PROCESS
    END

    IF INT.CR.AMT AND Y.INT.ACC.NO THEN
        INT.CR.AMT = TRIM(INT.CR.AMT, "", "D")
        Y.CR.TYPE.OF.TXN = '50'
        Y.CR.MOVEMENT = 0
        Y.CR.MVT.LCY = INT.CR.AMT
        Y.FT.OUT.LIST := Y.CR.TYPE.OF.TXN:'*':Y.INT.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.CR.MVT.LCY:'*':Y.CR.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER
        GOSUB WRITE.NORMAL.PROCESS
    END
    ...
    IF INT.DR.AMT AND Y.INT.ACC.NO THEN
        ...
        Y.DB.TYPE.OF.TXN = '40'
        Y.DB.MOVEMENT = 0
        Y.DB.MVT.LCY = INT.DR.AMT
        Y.FT.OUT.LIST := Y.DB.TYPE.OF.TXN:'*':Y.INT.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.DB.MVT.LCY:'*':Y.DB.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER
        GOSUB WRITE.NORMAL.PROCESS
    END
