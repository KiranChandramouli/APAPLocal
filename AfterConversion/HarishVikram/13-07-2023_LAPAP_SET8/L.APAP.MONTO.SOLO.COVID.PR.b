* @ValidationCode : MjoyMDAwMTU3NTI5OkNwMTI1MjoxNjg5MjQ1NzYzNDEzOkhhcmlzaHZpa3JhbUM6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 13 Jul 2023 16:26:03
...
* 13-07-2023     Conversion tool    R22 Auto conversion       FM TO @FM, VM to @VM, SM to @SM, I to I.VAR, ++ to +=, BP Removed
...
*-----------------------------------------------------------------------------
    ...
    $INSERT I_F.AA.ACTIVITY                           ;*R22 Auto conversion - End
    GOSUB APLICACIONES
    GOSUB PROCESS
RETURN
    ...
    IF R.AA.ACCOUNT.DETAILS THEN
        ...
        LOCATE Y.ID.REAY IN Y.REPAY.REFERENCE<1,1> SETTING PRO.POS THEN
            ...
            FOR I.VAR = 1 TO Y.CNT
                Y.BIIL.ID = Y.ARREGLO.BILL<I.VAR>
                Y.VALOR = ''
                GOSUB READ.AA.BILL.DETAILS
                Y.TOTAL += Y.MONTO.PRINCIPAL;
            NEXT I.VAR
...
GET.AA.ACT.BAL:
    ...
    FOR I.VAR = 1 TO Y.CNT.1
        Y.HIJA =  Y.CHILD.ACTIVITY<I.VAR>
