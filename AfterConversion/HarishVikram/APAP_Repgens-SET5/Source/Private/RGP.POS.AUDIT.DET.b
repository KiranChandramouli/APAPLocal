* @ValidationCode : MjoxNTU1Njk3ODAxOkNwMTI1MjoxNjg2ODIxNzIyMTM0OkhhcmlzaHZpa3JhbUM6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
...
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.POS.AUDIT.DET
REM "RGP.POS.AUDIT.DET",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    END
    YKEY = ""; YTOTFD = ""
    DIM YR.REC(25); MAT YR.REC = ""
    DIM YR.REC.OLD(23); MAT YR.REC.OLD = "_"
    GOSUB 1000000
    YHDR = "POSITION AUDIT REPORT"
    IF YPRINTING THEN
        YHDR = "RGP.POS.AUDIT.DET ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "DETAILED REPORT"
    YHDR2 = " PREVIO                                 US DAY AND TODAYS MOVEMENTS              END OF CURRENT DAY PRO                 FIT AND LOSS"
    YHDR3 = " ------                                 ---------------------------              ------------------ ---                 ------------"
    YHDR4 = "POSITION/REFERENCE CURRENC              Y.AMOUNT LOCAL.EQUIVALENT CURRENC       Y.AMOUNT LOCAL.EQUIVALENT LOCA          L.EQUIVALENT"
...
*------------------------------------------------------------------------
    MAT YR.REC.OLD = "_"
    DIM YR.TOT2(4); MAT YR.TOT2 = ""
    DIM YT.DEC2(4); MAT YT.DEC2 = ""
    YKEYFD = ""; Y1ST.LIN = 1
    LOOP WHILE YKEYFD = "" DO
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(2); YFD = YR.REC(2)  ;* YM.DEPT.ACCT
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            IF YFD = "" THEN
                YFD = STR(" ",4)
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(3); YFD = YR.REC(3)  ;* YM.DEAL.DESK
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"2L")
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(4); YFD = YR.REC(4)  ;* YM.REF3
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"8L")
        ...
        END ELSE
            ...
            END
        END
        YFD.OLD = YR.REC.OLD(5); YFD = YR.REC(5)  ;* YM.MID.RATE
        IF YHEADER.DISPLAY OR YFD.OLD <> YFD THEN
            YHEADER.DISPLAY = 1
            YFD = FMT(YFD,"10L")
        ...
        IF Y1ST.LIN THEN
            ...
            IF COMI = C.U THEN RETURN  ;* end of pgm
        END
        YFD = YR.REC(12)  ;* YM.REFERENCE
        YFD = FMT(YFD,"19L")
        IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
        YTOTFD := YFD
        YFD = YR.REC(13)  ;* YM.OPEN.COM
        YFD = FMT(YFD,"7L")
        IF LEN(YFD) > 7 THEN YFD = YFD[1,6]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(14)  ;* YM.CCY.OPEN.POS
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(16)  ;* YM.CCY.ENTRY.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
            ...
            NEXT YNO
        END
        YFD = YR.REC(17)  ;* YM.LCY.ENTRY.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<8> = YR.TOT2(YNO)<8> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<8> = "" THEN
                    YT.DEC2(YNO)<8> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<8> THEN
                        YT.DEC2(YNO)<8> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(18)  ;* YM.CCY.CLOSE.POS
        IF YFD = "" THEN
            YFD = STR(" ",19)
        END ELSE
            YDEC = INDEX(YFD,".",1)
        ...
        IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(19)  ;* YM.LCY.CLOSE.POS
        IF YFD = "" THEN
            YFD = STR(" ",21)
        END ELSE
            YFD = FMT(YFD,"21R4,")
        ...
        IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(21)  ;* YM.CCY.CLOSE.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<9> = YR.TOT2(YNO)<9> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<9> = "" THEN
                    YT.DEC2(YNO)<9> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<9> THEN
                        YT.DEC2(YNO)<9> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(22)  ;* YM.LCY.CLOSE.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<10> = YR.TOT2(YNO)<10> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<10> = "" THEN
                    YT.DEC2(YNO)<10> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<10> THEN
                        YT.DEC2(YNO)<10> = YDEC
                    END
                END
            NEXT YNO
        END
        YFD = YR.REC(23)  ;* YM.LCY.PANDL.TOT
        IF NUM(YFD) = NUMERIC THEN
            FOR YNO = 1 TO 4
                YR.TOT2(YNO)<11> = YR.TOT2(YNO)<11> + YFD
                YDEC = INDEX(YFD,".",1)
                IF YDEC THEN YDEC = LEN(YFD) - YDEC
                IF YT.DEC2(YNO)<11> = "" THEN
                    YT.DEC2(YNO)<11> = YDEC
                END ELSE
                    IF YDEC > YT.DEC2(YNO)<11> THEN
                        YT.DEC2(YNO)<11> = YDEC
                    END
                END
            NEXT YNO
...
*
        IF YKEY.OLD[1,24] <> YKEY[1,24] THEN
            ...
            END
            Y1ST.LIN = 1
            FOR YAV.TOT = 4 TO YTOTAL.STATUS STEP -1
                GOSUB 9000010
                IF COMI = C.U THEN RETURN  ;* end of pgm
*
                YCOUNT.TOT2 = 1; YCOUNT.AS.TOT2 = 1
                YFD = YR.TOT2(YAV.TOT)<1>  ;* YM.FOOT.ONE
                YR.TOT2(YAV.TOT)<1> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := "                             ":YFD
                YFD = YR.TOT2(YAV.TOT)<2>  ;* YM.FOOT.TWO
                YR.TOT2(YAV.TOT)<2> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<3>  ;* YM.FOOT.THREE
                YR.TOT2(YAV.TOT)<3> = ""
                YFD = FMT(YFD,"19L")
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<4>  ;* YM.FOOT.FOUR
                YR.TOT2(YAV.TOT)<4> = ""
                YFD = FMT(YFD,"21L")
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<5>  ;* YM.FOOT.FIVE
                YR.TOT2(YAV.TOT)<5> = ""
                YFD = FMT(YFD,"21L")
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<6>  ;* YM.FOOT.TEXT
                YR.TOT2(YAV.TOT)<6> = ""
                YFD = FMT(YFD,"8L")
                IF LEN(YFD) > 8 THEN YFD = YFD[1,7]:"|"
                GOSUB 9000000
                IF COMI = C.U THEN RETURN  ;* end of pgm
                YTOTFD := "          ":YFD
                YFD = YR.TOT2(YAV.TOT)<7>  ;* YM.CCY.ENTRY.TOT
                YR.TOT2(YAV.TOT)<7> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<7>:",")
                    YT.DEC2(YAV.TOT)<7> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YFD = "TOTALS : ":YFD
                YTOTFD := "  ":YFD
                YFD = YR.TOT2(YAV.TOT)<8>  ;* YM.LCY.ENTRY.TOT
                YR.TOT2(YAV.TOT)<8> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                ...
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<9>  ;* YM.CCY.CLOSE.TOT
                YR.TOT2(YAV.TOT)<9> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",19)
                END ELSE
                    YFD = FMT(YFD,"19R":YT.DEC2(YAV.TOT)<9>:",")
                    YT.DEC2(YAV.TOT)<9> = ""
                    IF AMOUNT.FORMAT#"" THEN CONVERT ",." TO AMOUNT.FORMAT IN YFD
                END
                IF LEN(YFD) > 19 THEN YFD = YFD[1,18]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<10>  ;* YM.LCY.CLOSE.TOT
                YR.TOT2(YAV.TOT)<10> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",21)
                END ELSE
                ...
                IF LEN(YFD) > 21 THEN YFD = YFD[1,20]:"|"
                YTOTFD := " ":YFD
                YFD = YR.TOT2(YAV.TOT)<11>  ;* YM.LCY.PANDL.TOT
                YR.TOT2(YAV.TOT)<11> = ""
                IF YFD = "" THEN
                    YFD = STR(" ",21)
                END ELSE
    ...
    IF YPRINTING THEN
        ...
        C$RPT.CUSTOMER.NO = YR.REC(24)
        C$RPT.ACCOUNT.NO = YR.REC(25)
