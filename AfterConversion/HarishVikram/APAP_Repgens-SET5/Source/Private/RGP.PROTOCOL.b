* @ValidationCode : Mjo5Mzk1ODc4MjA6Q3AxMjUyOjE2ODY4MjE3MjIzOTU6SGFyaXNodmlrcmFtQzotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
*----------------------------------------------------------------------------------------
SUBROUTINE RGP.PROTOCOL
REM "RGP.PROTOCOL",230614-4
*************************************************************************
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_RC.COMMON
...
* T.REMTEXT(23) = "PAGE"
    ...
    GOSUB 1000000
    YHDR = "P R O T O C O L   R E P O R T"
    IF YPRINTING THEN
        YHDR = "RGP.PROTOCOL ":YHDR
        YTYPE = "HEADER":@FM:YHDR
        CALL PST ( YTYPE )
    END ELSE
        YTYPE = YHDR
    END
    YHDR1 = "REFERENCE NO    TIME      TERM USER       APPLICATION                FUNCT.KEY                 MESSAGE"
    YHDR2 = "                          ID"
    YHDR3 = ""
    YHDR4 = ""
...
*------------------------------------------------------------------------
    ...
    LOOP WHILE YKEYFD = "" DO
        ...
        YTOTFD := "  ":YFD
        YFD = YR.REC(5)  ;* YM.USER
        YFD = FMT(YFD,"12L")
        IF LEN(YFD) > 12 THEN YFD = YFD[1,11]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(6)  ;* YM.APPLICATION
        YFD = FMT(YFD,"25L")
        IF LEN(YFD) > 25 THEN YFD = YFD[1,24]:"|"
        YTOTFD := " ":YFD
        YFD = YR.REC(7)  ;* YM.FUNCTION
        YFD = FMT(YFD,"6L")
        ...
        YTOTFD := " ":YFD
        YFD = YR.REC(9)  ;* YM.MESSAGE
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        YTOTFD := "  ":YFD
        YFD = YR.REC(10)  ;* YM.MESSAGE.2
        YFD = FMT(YFD,"35L")
        IF LEN(YFD) > 35 THEN YFD = YFD[1,34]:"|"
        GOSUB 9000000
        IF COMI = C.U THEN RETURN  ;* end of pgm
        ...
        GOSUB 1000000
*
        IF YKEY.OLD[1,14] <> YKEY[1,14] THEN
