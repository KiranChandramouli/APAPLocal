* @ValidationCode : Mjo3ODYxMTQ0Mzk6Q3AxMjUyOjE2OTY4Mjc3Nzg5ODk6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 09 Oct 2023 10:32:58
...
*To capture the ACCOUNT ids into savedlists
    IF RUNNING.UNDER.BATCH THEN
        ...
        F.SL = ''
        CALL OPF(FN.SL,F.SL)
        CALL F.READ(FN.SL,'TEMP.ACCOUNTS',REC.SL,F.SL,SL.ERROR)
        IF SL.ERROR NE '' THEN
            Y.MK.CMD = "mkdir ../bnk.run/&SAVEDLISTS&/TEMP.ACCOUNTS"
            EXECUTE Y.MK.CMD
        END
        FN.EXP.FILE.PATH = '&SAVEDLISTS&/TEMP.ACCOUNTS'
...
*CURR.DATE = R.DATE<EB.DAT.LAST.WORKING.DAY>
    ...
    IF STATUS.SEQ MATCHES 'DECEASED':@VM:'GARNISHMENT':@VM:'GARNISHMENT.REV':@VM:'GUARANTEE.REV' THEN  ;*TSR637100
    END ELSE
...
********************
    Y.FINAL.MNEM = ''
    Y.FINAL.MNEM = CHANGE(R.OLD.COMB.STATUS.FINAL,'-',@FM)
    Y.FINAL.STATUS = CHANGE(STATUS.SEQ,':',@FM)

****THIS WILL EXECUTE ONLY FOR STATSEQ HAS CONTAIN STATUS1 VALUE*****
    TOT.STATUS = 0
    TOT.STATUS = DCOUNT(Y.FINAL.STATUS,@FM)
    ...
    COMB.STATUS = CHANGE(Y.FINAL.MNEM,@FM,'-')
...
AC.STS1.MNEM:
*************
    TOT.STAT = DCOUNT(Y.FINAL.MNEM, @FM)
    BEGIN CASE
