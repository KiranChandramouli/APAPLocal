* @ValidationCode : MjoyMDQ5MDE4MTI6Q3AxMjUyOjE2OTUxMDg4MTA4MTg6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 19 Sep 2023 13:03:30
...
*19/09/2023    CONVERSION TOOL     AUTO R22 CODE CONVERSION @VM CHANGED TO @FM, LINES IS ADDED
*19/09/2023    VIGNESWARI          MANUAL R22 CODE CONVERSION NOCHANGE
*-------------------------------------------------------------------------------------------------------------------------------	-
*
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
* -----------------------------------------------------------------------------
    ...
    Y.CREDIT.TXN<-1> = Y.INS.VALUES : @FM : Y.TYPE.TXN  ;*AUTO R22 CODE CONVERSION- @VM changed to @FM
RETURN
...
*JV
    ...
    ;*GOSUB GET.STATIC.MAPPING   ;*AJUSTE ;*AUTO R22 CODE CONVERSION
...
*   CALL REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, Y.MAP.TYPE, Y.MAP.VALUE)
	...
	TEST.MAP = Y.MAP.VALUE ;*AUTO R22 CODE CONVERSION
RETURN
