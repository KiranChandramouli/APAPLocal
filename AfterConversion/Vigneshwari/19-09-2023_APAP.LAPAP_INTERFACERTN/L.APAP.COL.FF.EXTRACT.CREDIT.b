* @ValidationCode : MjoyMTM3MDA2NzEyOkNwMTI1MjoxNjk1MTI4NDI4MjAyOnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 19 Sep 2023 18:30:28
...
*13-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION                      TAM.BP is Removed, VM,FM to @VM,@FM,++ to +=,= to EQ
*13-07-2023    AJITHKUMAR S        R22 MANUAL CONVERSION                    CALL RTN FORMt can be modified
...
*---------------------------------------------------------------------------------------	-
    ...
    $INSERT I_REDO.COL.EXTRACT.CREDIT.COMMON ;*R22 Auto code conversion
*
    $INSERT I_F.REDO.CUSTOMER.ARRANGEMENT ;*R22 Auto code conversion
...
*-----------------------------------------------------------------------------
	...
	INFO.1 = PR.GESCREDITOSGARANTIAS  ;*ADDED COMMENT FOR INTERFACE CHANGES-START-Interface Change by Santiago
	INFO.2 = PR.GESTIPOGARANTIAS
	INFO.3 = P.GESCREDITOINTEGRACION  ;*ADDED COMMENT FOR INTERFACE CHANGES-END
...
* CALL L.APAP.COL.EXTRACT.CREDIT.READ(AA.ID,R.AA,R.ACT.HIST,R.PRINCIPALINT,R.MORA.CHARGE,R.AA.ACCOUNT.DETAILS) 
   ...
   APAP.LAPAP.lApapColExtractCreditRead(AA.ID,R.AA,R.ACT.HIST,R.PRINCIPALINT,R.MORA.CHARGE,R.AA.ACCOUNT.DETAILS)
    IF E NE "" THEN
        RETURN
    END
    ...
    IF Y.CONTINUE.AA EQ @FALSE THEN ;*R22 Auto code conversion
        RETURN
    END
...
* CALL REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, "PRODUCT.GROUP", Y.TMPCREDITOCODIGOPRODUCTO)
    ...
    ;*APAP.TAM.redoRColGetMapping(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, "PRODUCT.GROUP", ;*Y.TMPCREDITOCODIGOPRODUCTO) ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
...
* CALL REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, "TIPO.INV", Y.TMPCREDITOCODIGOTIPOINVERSION)
    ...
    ;*APAP.TAM.redoRColGetMapping(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, "TIPO.INV", ;*Y.TMPCREDITOCODIGOTIPOINVERSION) ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
...
* Agency Code
    ...
    IF YERR NE '' THEN
        E = yRecordNotFound : @FM : Y.CO.CODE : @VM : 'F.COMPANY' ;*R22 Auto code conversion
        RETURN
    END
    ...
    IF Y.TMPCREDITOCODIGOAGENCIA EQ '' THEN
        E = yValueMantatory : @FM : "COMPANY>SUB.DIVISION.CODE" : @VM : Y.CO.CODE ;*R22 Auto code conversion
        RETURN
    END
* Contract Number
    Y.ACCOUNT.ID = ""
...
*
    ...
    IF YERR NE '' THEN
        E = yRecordNotFound : @FM : Y.ACCOUNT.ID : @VM : 'F.ACCOUNT' ;*R22 Auto code conversion
        RETURN
    END
...
* CALL REDO.S.COL.GET.PAY.FREQ(AA.ID,  R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOTIPOCUOTA)
    ...
    APAP.REDOSRTN.redoSColGetPayFreq(AA.ID,  R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOTIPOCUOTA)
    IF E THEN
        RETURN
    END	
* Contract Status
* CALL REDO.S.COL.GET.AA.STATUS(AA.ID, Y.PROCESS.DATE, Y.TMPCREDITOESTADOCONTRATO, Y.TMPCREDITOCOBROJUDICIAL)
    APAP.AA.redoSColGetAaStatus(AA.ID, Y.PROCESS.DATE, Y.TMPCREDITOESTADOCONTRATO, Y.TMPCREDITOCOBROJUDICIAL)
    IF E THEN
        RETURN
    END
...
*CALL REDO.S.COL.GET.PAY.TYPE(AA.ID, R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOFORMAPAGO)
    APAP.REDOSRTN.redoSColGetPayType(AA.ID, R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOFORMAPAGO)
...
*CALL L.APAP.COL.FF.EXTRACT.CREDIT.2("GET.CREDIT.GRANTED", Y.ACCOUNT.ID, AA.ID, PGET.CREDIT.GRANTED, "", "")
    ...
    APAP.LAPAP.lApapColFfExtractCredit2("GET.CREDIT.GRANTED", Y.ACCOUNT.ID, AA.ID, PGET.CREDIT.GRANTED, "", "") ;*R22 MANUAL CODE CONVERSION
    IF E THEN
        RETURN
    END
...
*CALL REDO.COL.GET.TOTAL.OUTSTANDING(AA.ID,Y.PROP.AMT,Y.TMPCREDITOMONTODEUDATOTAL)
    ...
    APAP.REDORETAIL.redoColGetTotalOutstanding(AA.ID,Y.PROP.AMT,Y.TMPCREDITOMONTODEUDATOTAL)
  
    GOSUB GESTIPOGARANT.SUB
	
 
 TEST.F = P.GESCREDITOINTEGRACION ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
...
* CALL L.APAP.COL.FF.EXTRACT.TXN(Y.LWD, AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.GROUP, Y.AGENCY.CODE, Y.CREDIT.TXN)
	...
	TES.T = Y.CREDIT.TXN  ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
...
*-----------------------------------------------------------------------------
    ...
    ;*R.STATIC.MAPPING    =  C.STATIC.MAPPING       ;* Static Mapping  ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
	
	
*CALL REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING.OUT, "", "")
 
    APAP.TAM.redoRColGetMapping(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING.OUT, "", "")  ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
 
    
    IF E THEN
        RETURN
    END
    ;*R.STATIC.MAPPING = R.STATIC.MAPPING.OUT  ;*ADDED COMMENT FOR INTERFACE CHANGES-Interface Change by Santiago
...
* CALL REDO.S.GET.PERIOD.AMTS(Y.ACCOUNT.ID,Y.PROCESS.DATE, Y.PROCESS.DATE, Y.AA.BALANCE.LIST, Y.OUT.AA.AMOUNT.LIST)
    ...
    APAP.REDOSRTN.redoSGetPeriodAmts(Y.ACCOUNT.ID,Y.PROCESS.DATE, Y.PROCESS.DATE, Y.AA.BALANCE.LIST, Y.OUT.AA.AMOUNT.LIST)
...
* CALL L.APAP.B.COL.FF.EXT.LAST.PAY(AA.ID,Y.TOTAL.CUOTAS,Y.LASTPAY.AMT, Y.LASTPAY.DAT,Y.NEXTPAY.AMT,Y.NEXTPAY.DATE,Y.AL.PAID.BILLS.CNT)
    APAP.LAPAP.lApapBColFfExtLastPay(AA.ID,Y.TOTAL.CUOTAS,Y.LASTPAY.AMT, Y.LASTPAY.DAT,Y.NEXTPAY.AMT,Y.NEXTPAY.DATE,Y.AL.PAID.BILLS.CNT)
