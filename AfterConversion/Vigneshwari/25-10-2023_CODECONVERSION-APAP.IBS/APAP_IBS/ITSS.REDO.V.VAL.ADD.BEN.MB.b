* @ValidationCode : MjoyMzU0OTU4NTI6Q3AxMjUyOjE2OTgyMzE5MzQ4MTk6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 25 Oct 2023 16:35:34
...
* <Rating>-106</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE ITSS.REDO.V.VAL.ADD.BEN.MB
*--------------------------------------------------------------------------------
...
*------------
    ...
    Y.TEST = System.getVariable('EXT.SMS.CUSTOMERS')
...
! AF=ARC.BEN.BEN.ACCT.NO
    ...
    IF NOT(CUS.ERR) AND Y.TYPE NE "CARDS" THEN
...
*------------------
    ...
    IF Y.TYPE EQ 'CARDS' THEN
        ...
        R.NEW(ARC.BEN.BEN.ACCT.NO) = 'DOP1280400010017'     ;* TRIM(Y.ACCOUNT,'0','L')

        OTH.CUS = FIELD(WS.DATA<12>,'/',1)
        OTH.CUS = TRIM(OTH.CUS,'0','L')

        IF WS.DATA<1> NE 'OK' THEN
            ETEXT ='INVALID APAP CARD '
            CALL STORE.END.ERROR
            RETURN
        END
