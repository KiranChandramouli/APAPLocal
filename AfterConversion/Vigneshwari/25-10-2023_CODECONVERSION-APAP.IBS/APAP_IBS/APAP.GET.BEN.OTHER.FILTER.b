* @ValidationCode : Mjo2NzIyMzg4NTpDcDEyNTI6MTY5ODIyOTgwNDIxMjp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 25 Oct 2023 16:00:04
...
* <Rating>-20</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE APAP.GET.BEN.OTHER.FILTER(ID.LIST)
        
*-----------------------------------------------------------------------------------------------------------------------------------
*Modification HISTORY:
*DATE                          AUTHOR                   Modification                            DESCRIPTION
*25/10/2023                VIGNESHWARI        MANUAL R23 CODE CONVERSION                Remove APAP.BP IN INSERT FILE
*-----------------------------------------------------------------------------------------------------------------------------------
*-----------------------------------------------------------------------------
* Description :
*-----------------------------------------------------------------------------
    ...
    $INSERT I_EB.MOB.FRMWRK.COMMON ;*MANUAL R23 CODE CONVERSION-Remove APAP.BP IN INSERT FILE
...
*    END
    ...
    IF NOT(CUS.BEN.LIST.ER) THEN
        ...
        WHILE BEN.EACH.ID:BEN.LIST.POS
            ...
            IF NOT(BENEFICIARY.ER) AND R.BENEFICIARY<ARC.BEN.TRANSACTION.TYPE> EQ 'AC25' THEN
                ID.LIST<-1> = BEN.ID
            END
...
*    SEL.CMD = 'SELECT ':FN.BENEFICIARY:' WITH OWNING.CUSTOMER EQ ':SQUOTE(CUST.ID):' AND WITH TRANSACTION.TYPE EQ ':SQUOTE('AC25')
