* @ValidationCode : MjoxMTI1NTYwNDM4OkNwMTI1MjoxNzA1OTA4MTQ1NTcwOnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 22 Jan 2024 12:52:25
...
*22-01-2024	      VIGNESHWARI       ADDED COMMENT FOR INTERFACE CHANGES- SQA-12534 - By Santiago
*-------------------------------------------------------------------------------------
*
    $INSERT I_COMMON
    $INSERT I_EQUATE
    ...
    $INSERT I_TSS.COMMON
    $INSERT I_F.TELLER	;*Fix SQA-12534- By Santiago-INSERT FILE IS ADDED NEW
...
*    MAP.FMT = 'MAP'  ;*SJ
    MAP.FMT  = 'O'
    ID.RCON.L = Y.MAPPING.ID
    APP = 'F.':Y.APPLICATION	;*Fix SQA-12534- By Santiago - CHANGED "'F.':FIELD(Y.MAPPING.ID,'/',1)" TO "'F.':Y.APPLICATION"
    ID.APP = Y.RECORD.ID
    
    R.RETURN.MSG = ''
    ERR.MAPPING = ''
    
    Y.FN.APP = 'F.':Y.APPLICATION	;*Fix SQA-12534- By Santiago -CHANGED "'F.':FIELD(Y.MAPPING.ID,'/',1)" TO "'F.':Y.APPLICATION"
    Y.F.APP  = ''
 ...
 ;*Fix SQA-12534- By Santiago-NEW LINES ADDED-START   
    IF ID.RCON.L EQ 'TELLER/TC/0001' THEN
        IF R.APP<TT.TE.CUSTOMER.2> EQ '' THEN
            ID.RCON.L = 'TELLER/TC/0001.LTCUS'
        END
    END
;*Fix SQA-12534- By Santiago-END
