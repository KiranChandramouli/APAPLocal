* @ValidationCode : MjoxMTg4NDc1MzczOkNwMTI1MjoxNzAxMDg3NTk5NjMyOnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 27 Nov 2023 17:49:59
...
*
    BEGIN CASE
        CASE R.NEW(REDO.CUS.PRF.IDENTITY.TYPE) EQ "CEDULA"
            APAP.REDOVER.redoValCidentCust(Y.APP.VERSION) ;*MANUAL R22 CODE CONVERSION
        CASE R.NEW(REDO.CUS.PRF.IDENTITY.TYPE) EQ "RNC"
            RNC.NUMBER = COMI
            GOSUB RNC.PROOF.CHECK
        CASE R.NEW(REDO.CUS.PRF.IDENTITY.TYPE) EQ "PASAPORTE"
            APAP.TAM.redoValPassportCust(Y.APP.VERSION) ;*MANUAL R22 CODE CONVERSION
    END CASE
*
...
*Check the given identity number is valid or not
    ...
    IF LEN(RNC.NUMBER) EQ 9 THEN
        ...
        APAP.TAM.redoRncCheckDigit(RNC.CHK.RESULT) ;*MANUAL R22 CODE CONVERSION
    END ELSE
...
* APAP Customer RNC check to get customer name
*
    Cedule      = TRIM(RNC.NUMBER)          ;* adding TRIM for all padron ws ;*Fix Padron � By Santiago- changed "RNC.NUMBER" to "TRIM(RNC.NUMBER)"
