* @ValidationCode : MjoxNDM0NjUwNzcwOkNwMTI1MjoxNjk4MTUzMDY2MzcxOnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 24 Oct 2023 18:41:06
...
INITIALISE:
    ...
    LOCATE 'SAP.NORMAL.EXTRACT' IN Y.GIT.ROUTINE<1,1> SETTING NORMAL.REPORT.VM.POS THEN
		...
		F.EXTRACT.OUT.FILE.PATH.NOR = Y.EXTRACT.OUT.PATH.NOR
    END
...
;*CALL EB.READLIST(SEL.SESSION.FILES,SELECTED.SESSION.FILES,'',NO.OF.FILES.SELECTED,ERR.SESSION.FILES)
	...
	CALL OCOMO("READ.GENERATED.FILES Y.EXTRACT.OUT.PATH": Y.EXTRACT.OUT.PATH)    ;*BORRAR	;*Interface Change by Santiago-start
	
	CALL OCOMO("NOMBRE Y.REVAL.ID ": Y.REVAL.ID)    ;*BORRAR	
	CALL OCOMO("NOMBRE Y.NORMAL.ID ": Y.NORMAL.ID)    ;*BORRAR
	CALL OCOMO("SELECT Y.EXTRACT.OUT.PATH ": Y.EXTRACT.OUT.PATH)  ;*BORRAR	;*Interface Change by Santiago-end
...
*--------------------
    ...
    CALL OCOMO("PROCESS.NORMAL.FILES": Y.EXTRACT.OUT.PATH.NOR)	;*Interface Change by Santiago
...
;*cambio
    ...
    Y.REVAL.IND=''
	 CALL OCOMO("redoApapSapGlNrHdrExpLog IN ")	;*Interface Change by Santiago
    APAP.REDOAPAP.redoApapSapGlNrHdrExpLog(NORMAL.FILE.DATA,Y.REVAL.IND) ;*Manual R22 conversion	;*Interface Change by Santiago
	CALL OCOMO("redoApapSapGlNrHdrExpLog OUT ")	;*Interface Change by Santiago
    LOOP
...
*-------------------
	...
	Y.SEL.ID.R = Y.REVAL.ID	;*Interface Change by Santiago
	
	CALL OCOMO("PROCESS.REVAL.FILES ": Y.EXTRACT.OUT.PATH)		;*Interface Change by Santiago
    ...
    ;*READ REVAL.FILE.DATA FROM F.EXTRACT.OUT.FILE.PATH.REV,Y.REVAL.ID ELSE	;*Interface Change by Santiago-start
    ;*    CALL OCOMO("CANNOT READ FROM ":F.EXTRACT.OUT.FILE.PATH)
