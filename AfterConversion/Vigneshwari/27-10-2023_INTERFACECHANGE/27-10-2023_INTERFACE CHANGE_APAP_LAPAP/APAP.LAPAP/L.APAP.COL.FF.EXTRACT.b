* @ValidationCode : MjotMTAyNzE4ODAxNzpDcDEyNTI6MTY5ODYzODE3OTcyMjp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 30 Oct 2023 09:26:19
...
*27/10/2023    VIGNESHWARI         ADDED COMMENT FOR INTERFACE CHANGES      Interface Change by Santiago
*---------------------------------------------------------------------------------------	-
...
CALL OCOMO("CUSTOMER.ID1":CUSTOMER.ID )	;*Interface Change by Santiago-new line added
    BEGIN CASE
        CASE CONTROL.LIST<1,1> EQ 'Y.AP.LOANS' ;*R22 Auto Code conversion
             CALL OCOMO("CONTROL.LIST":CONTROL.LIST )	;*Interface Change by Santiago-new line added
            GOSUB PROCESS.AP.LOANS
        CASE CONTROL.LIST<1,1> EQ 'Y.AP.CARDS'
            CALL OCOMO("CONTROL.LIST":CONTROL.LIST )	;*Interface Change by Santiago-new line added
            GOSUB PROCESS.AP.CARDS	;*Interface Change by Santiago-uncommented
    END CASE
RETURN
...
*****************
    GOSUB INITIALISE
    IF CUSTOMER.ID[1,2] EQ 'AA' THEN
        Y.AA.ARR.ID = CUSTOMER.ID; R.ARRANGEMENT = ''; ARR.ERR = ''
        CALL AA.GET.ARRANGEMENT(Y.AA.ARR.ID,R.ARRANGEMENT,ARR.ERR)
        CUSTOMER.ID = R.ARRANGEMENT<AA.ARR.CUSTOMER>
    END
    GOSUB PROCESS
RETURN
...
*------------------------------
    ...
    IF Y.CREDIT THEN
        ...
        END
        GOSUB  FINAL.WRITE.RECORD	;*Interface Change by Santiago-UNCOMMENTED
    END
...
*****************
    ...
    IF NOT(R.CUSTOMER.HST) THEN ;*Interface Change by Santiago
        ...
        R.CUSTOMER.HST = R.CUSTOMER ;*Interface Change by Santiago
    END
    IF R.CUSTOMER THEN
        Y.L.AUDIT.POS = DCOUNT(R.CUSTOMER<EB.CUS.INPUTTER>, @VM)
        Y.CLI.FECHA.MODIF.TP = R.CUSTOMER<EB.CUS.DATE.TIME,Y.L.AUDIT.POS> ;*Interface Change by Santiago
        Y.CLI.FECHA.MODIF = ICONV(Y.CLI.FECHA.MODIF.TP,'DI') ;*Interface Change by Santiago-start-uncommented
        Y.CLI.FECHA.MODIF = OCONV(Y.CLI.FECHA.MODIF,'D4')
        Y.CLI.FECHA.MODIF = Y.CLI.FECHA.MODIF[7,4]:Y.CLI.FECHA.MODIF[4,2]:Y.CLI.FECHA.MODIF[1,2] ;*Interface Change by Santiago-end
    END
RETURN
...
* LECTURA DE LOS CAMPOS DE CORE DEL CONYUGE
            ...
            IF YERR NE '' THEN
                ...
                RETURN
            END
            Y.L.AUDIT.POSCC = DCOUNT(R.CUSTOMER.CONYUGE<EB.CUS.INPUTTER>, @VM)
            Y.CLI.FECHA.MODIF.TPCC = R.CUSTOMER.CONYUGE<EB.CUS.DATE.TIME,Y.L.AUDIT.POSCC>
            Y.CLI.FECHA.MODIF.CC = ICONV(Y.CLI.FECHA.MODIF.TPCC,'DI')
            Y.CLI.FECHA.MODIF.CC = OCONV(Y.CLI.FECHA.MODIF,'D4')
            Y.CLI.FECHA.MODIF.CC = Y.CLI.FECHA.MODIF.CC[7,4]:Y.CLI.FECHA.MODIF.CC[4,2]:Y.CLI.FECHA.MODIF.CC[1,2]
*ASIGNACION DE LA INFORMACION RESPECTO A EL CONYUGE
        ...
        IF Y.RELATION.CODE<1,I.VAR> EQ 104 THEN       ;*ASIGNACION DEL TIPO DE DOCUMENTO DEL REPRESENTANTE
            ...
            IF YERR NE '' THEN
                ...
                RETURN
            END
            Y.L.AUDIT.POSCR = DCOUNT(R.CUSTOMER.REPRES<EB.CUS.INPUTTER>, @VM)
            Y.CLI.FECHA.MODIF.TPCR = R.CUSTOMER.REPRES<EB.CUS.DATE.TIME,Y.L.AUDIT.POSCR>
            Y.CLI.FECHA.MODIF.CR = ICONV(Y.CLI.FECHA.MODIF.TPCR,'DI')
            Y.CLI.FECHA.MODIF.CR = OCONV(Y.CLI.FECHA.MODIF.CR,'D4')
            Y.CLI.FECHA.MODIF.CR = Y.CLI.FECHA.MODIF.CR[7,4]:Y.CLI.FECHA.MODIF.CR[4,2]:Y.CLI.FECHA.MODIF.CR[1,2]
...
* PP take first position for Employers.Name & Email, no definition available
    ...
    Y.CLI.ESTADO.CIVIL = R.CUSTOMER<EB.CUS.MARITAL.STATUS>[1,1]
    Y.CLI.APELLIDO = R.CUSTOMER<EB.CUS.GIVEN.NAMES>:' ':R.CUSTOMER<EB.CUS.FAMILY.NAME>
    Y.CALC.RISK.CLASS = R.CUSTOMER<EB.CUS.CALC.RISK.CLASS,1>

*    Y.CLI.CONTACTO =  R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TEL.P.CONT>

*CAMPOS DE AUDITORIA DEL REGISTRO DE CUSTOMER
    Y.CLI.ADD.POR = FIELD(R.CUSTOMER.HST<EB.CUS.INPUTTER, 1>,'_',2)   ;* Who created the row
    ...
    Y.CLI.MODIF.POR = FIELD(R.CUSTOMER<EB.CUS.INPUTTER, Y.L.AUDIT.POS>,'_',2)   ;* Last inputter & date
...
*** :Y.DELIM:"BPR"
    ...
    Y.LINEA=''
    Y.DELIM='~'
*    Y.LINEA := "000001":Y.DELIM:TRIM(Y.CLI.ADD.POR[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.PRIMER.APELLIDO[1,60],"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(Y.CLI.ID.REPRES,"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.APELLIDO[1,300],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.EMAIL[1,60],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.CLI.ESTADO.CIVIL[1,10],"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.ADICION,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.NACIMIENTO,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.INGRESO,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.MODIF,"","B"):Y.DELIM:TRIM(Y.CLI.NUM.REPRES[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.LUGAR.TRABAJO[1,250],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.CLI.MODIF.POR[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.NOM.CONYUGE[1,40],"","B"):Y.DELIM:TRIM(Y.NOM.APE.REPRES[1,250],"","B"):Y.DELIM:TRIM(Y.CLI.NUM.ID[1,20],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.PRIMER.APELLIDO[1,60],"","B"):Y.DELIM:TRIM(Y.CLI.PRIMER.NOMBRE[1,60],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.RAZON.SOCIAL[1,200],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.SEXO[1,1],"","B"):Y.DELIM:TRIM(Y.TEL.CONYUGE[1,10],"","B"):Y.DELIM:TRIM(Y.CALC.RISK.CLASS[1,40],"","B"):Y.DELIM:TRIM(Y.CLI.TIPO.ID,"","B"):Y.DELIM:TRIM(Y.TIPO.CLIENTE,"","B"):Y.DELIM:"BPR"
*    Y.LINEA := Y.DELIM:TRIM(Y.CAMPO.9[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.10[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.11[1,400],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CAMPO.13[1,400],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:""
*    Y.LINEA := Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:""
*    Y.LINEA := Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM
    ...
    IF Y.CLI.TIPO.ID THEN
        ...
        Y.LINEA := TRIM(Y.CLI.CONTACTO[1,300],"","B"):Y.DELIM:TRIM(Y.TEL.CONYUGE[1,10],"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.ID.REPRES,"","B"):Y.DELIM:TRIM(Y.CLI.NUM.REPRES[1,20],"","B"):Y.DELIM:TRIM(Y.NOM.APE.REPRES[1,250],"","B")
        ...
        Y.LINEA :="":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM

        R.COL.QUEUE = ''
        R.COL.QUEUE<1> = Y.TMPCLIENTES
        R.COL.QUEUE<2> = Y.LINEA

        GOSUB WRITE.RECORD ;*R22 interface Unit testing changes  ;*Interface Change by Santiago
    END
RETURN
*-------------------
PHONES.STMT:
...
*-------------------
    ...
    YSTATUS = ''; Y.STATUS = ''; Y.STATUS = R.CUSTOMER<EB.CUS.CUSTOMER.STATUS>
    ...
    Y.DELIM='~'
    Y.INSERT.TEL=''
    Y.INSERT.TEL := "000001":Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(Y.CLI.TELF.TYPE,"","B"):Y.DELIM:TRIM(Y.CURR.TEL.SEQ,"","B") :Y.DELIM:"1":Y.DELIM:TRIM(Y.CLI.TELF.NO[1,56],"","B"):Y.DELIM
...
* 20171020 /S
    Y.INSERT.TEL := TRIM(Y.TEL.DATE.TIME.UPD,"","B"):Y.DELIM:'T24':Y.DELIM:YSTATUS:Y.DELIM:'':Y.DELIM:'':Y.DELIM
* 20171020 /E
    R.COL.QUEUE = ''
...
LOOP WHILE I.VAR GE 1 ;*R22 Auto Code conversion
BEGIN CASE
    CASE Y.CLI.TELF.TYPE<1,1,I.VAR> EQ "05"
        Y.CLI.TELF.AREA.10 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I.VAR> ;*R22 Auto Code conversion
    ...
    CASE Y.CLI.TELF.TYPE<1,1,I.VAR> EQ "01"
        Y.CLI.TELF.AREA.11 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I.VAR> ;*R22 Auto Code conversion
...
* BUILD THE QUERY TO GET THE ADDRESS ID'S  19- inicio
    ...
    LOOP WHILE Y.SEQ > 0 DO
        ...
        IF R.DE.ADDRESS.OLD EQ '' THEN
            R.DE.ADDRESS.OLD = R.DE.ADDRESS
        END
        ...
        Y.CLI.DATE.TIME.LAST = ICONV(Y.CLI.DATE.TIME.LAST,'DI') ;*Interface Change by Santiago-uncommented-start
        ...
        Y.CLI.DATE.TIME.ADD = ICONV(Y.CLI.DATE.TIME.ADD,'DI')	;*Interface Change by Santiago-uncommented-start
...
Y.LINEA.DIR.CLI   = TRIM(Y.CLI.COD.EMPRESA,"","B"):Y.DELIM:TRIM(Y.CLI.COD.CLIENTE,"","B"):Y.DELIM:Y.TMPDIRCLIENTEBARRIO:Y.DELIM:"1":Y.DELIM:"T24"
Y.LINEA.DIR.CLI   := Y.DELIM:TRIM(Y.CLI.STREET.ADDR.DESC[1,200],"","B"):Y.DELIM:TRIM(Y.CLI.APR.POSTAL[1,100],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.INPUTTER.ADD[1,20],"","B")
Y.LINEA.DIR.CLI   := Y.DELIM:TRIM(Y.CLI.DATE.TIME.ADD,"","B"):Y.DELIM:TRIM(Y.CLI.INPUTTER.LAST[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.DATE.TIME.LAST,"","B")
Y.LINEA.DIR.CLI   := Y.DELIM:Y.TMPDIRCLIENTEPAIS:Y.DELIM:Y.TMPDIRCLIENTEESTADO:Y.DELIM:Y.TMPDIRCLIENTEMUNICIPIO:Y.DELIM:Y.TMPDIRCLIENTESECTOR
Y.LINEA.DIR.CLI   := Y.DELIM:Y.TMPDIRCLIENTEBARRIO:Y.DELIM:Y.TMPDIRCLIENTEBASEPRINCIPAL:Y.DELIM:"":Y.DELIM

R.COL.QUEUE = ''
R.COL.QUEUE<1> = Y.TMPDIRECCIONESCLIENTE
R.COL.QUEUE<2> = Y.LINEA.DIR.CLI
GOSUB WRITE.RECORD	;* Interface Change by Santiago
RETURN
*-----------------------------------------------------------------------------------
PROCESS.AA:         * Call to process each AA
*-----------------------------------------------------------------------------------
CALL OCOMO("CUSTOMER.ID2":CUSTOMER.ID )	;*Interface Change by Santiago-new line added
...
* CALL L.APAP.COL.FF.EXTRACT.CREDIT(CUSTOMER.ID, Y.CREDIT, Y.CREDIT.TXN,PR.GESGARANTIAS,PR.GESCREDITOSGARANTIAS,PR.GESTIPOGARANTIAS,P.GESCREDITOINTEGRACION)
	...
	CALL OCOMO("CUSTOMER.ID3":CUSTOMER.ID )	;*Interface Change by Santiago-new line added
*   IF E THEN
*       RETURN
*   END
    ...
    IF Y.PROCESS.FLAG.TABLE<1,7> EQ "gestipogarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESTIPOGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
        ...
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,6> EQ "gescreditointegracion" THEN
        Y.TOTAL.E = DCOUNT(P.GESCREDITOINTEGRACION,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
        ...
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,8> EQ "gesgarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
        ...
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,9> EQ "gescreditogarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESCREDITOSGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
...
*        Y.QUEUE.ID = DATE():Y.QUEUE.ID
        ...
        Y.FILE.ID='SESSION':R.REDO.COL.QUEUE<1>:AGENT.NUMBER	;*Interface Change by Santiago
        ...
        CLOSESEQ Y.FILE.PATH	;*Interface Change by Santiago
