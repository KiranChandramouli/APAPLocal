* @ValidationCode : MjoyMDQ2NTk1ODM6Q3AxMjUyOjE2OTg0MDA1ODA1NzM6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 27 Oct 2023 15:26:20
...
CALL OCOMO("CUSTOMER.ID1":CUSTOMER.ID )	;*Interface Change by Santiago
    BEGIN CASE
        CASE CONTROL.LIST<1,1> EQ 'Y.AP.LOANS' ;*R22 Auto Code conversion
             CALL OCOMO("CONTROL.LIST":CONTROL.LIST )	;*Interface Change by Santiago
...
*************
    ...
    END ELSE
        ...
        IF Y.PROCESS.FLAG.TABLE<1,3> EQ "tmpdireccionescliente" THEN
            GOSUB ADDRESS.STMT          ;*Ir al Parrafo de para extraer y crear los Inserts de Address
        END
        GOSUB  FINAL.WRITE.RECORD	;*R22 interface Unit testing changes	;*Interface Change by Santiago
    END
RETURN
...
*****************
    ...
    IF NOT(R.CUSTOMER.HST) THEN		;*R22 interface Unit testing changes	;*Interface Change by Santiago
        ...
        R.CUSTOMER.HST = R.CUSTOMER	;*R22 interface Unit testing changes	;*Interface Change by Santiago
    END
    IF R.CUSTOMER THEN
...
*-------------------
GET.CUSTOMER.PHONES:*BUCLE PARA LA LECTURA DEL CAMPO LOCAL DEL CLIENTE CORRESPONDIENTE A TELEFONOS	;*R22 interface Unit testing changes
*-------------------
IF Y.CLI.TELF.TYPE NE "" THEN
RETURN
END
...
RETURN
*-------------
ADDRESS.STMT:       *OBTENER TMPCLIENTEDIRECCIONES   18-inicio	;*R22 interface Unit testing changes
*-------------
...
RETURN
*-----------------------------------------------------------------------------------
PROCESS.CUS.ADDRESS:* Call to process each AA	;*R22 interface Unit testing changes
*-----------------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------------
CALL OCOMO("CUSTOMER.ID2":CUSTOMER.ID )	;*Interface Change by Santiago
...
* CALL L.APAP.COL.FF.EXTRACT.CREDIT(CUSTOMER.ID, Y.CREDIT, Y.CREDIT.TXN,PR.GESGARANTIAS,PR.GESCREDITOSGARANTIAS,PR.GESTIPOGARANTIAS,P.GESCREDITOINTEGRACION)
	...
	CALL OCOMO("CUSTOMER.ID3":CUSTOMER.ID )	;*Interface Change by Santiago
*   IF E THEN
*       RETURN
*   END
...
*R22 interface Unit testing changes-END
        ...
        Y.FILE.ID='SESSION':R.REDO.COL.QUEUE<1>:AGENT.NUMBER	;*R22 interface Unit testing changes	;*Interface Change by Santiago
