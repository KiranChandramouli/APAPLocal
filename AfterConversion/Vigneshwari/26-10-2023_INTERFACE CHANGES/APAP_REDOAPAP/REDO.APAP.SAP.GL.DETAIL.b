...
*-------------------------------------------------Accoiunting CR changes updated------------------------
    ...
    BAL.CR.AMT = ''; BAL.DR.AMT = '' ; INT.CR.AMT = '' ; INT.DR.AMT = '' ; Y.BAL.DET = '' ; Y.INT.DET = ''	;*Interface Change by Santiago
    ...
    IF R.EB.CONTRACT.BALANCES THEN
        ...
        APAP.REDOAPAP.redoVCheckBalIntEntries(Y.ACT.ACCOUNT,BAL.CR.AMT,BAL.DR.AMT,INT.CR.AMT,INT.DR.AMT,Y.BAL.DET,Y.INT.DET) ;* MANUAL R22 CODE CONVERSION	;*Interface Change by Santiago
...
*------------------- This is added for accounting CR
    ...
    IF BAL.DR.AMT AND Y.BAL.ACC.NO THEN
        Y.DB.TYPE.OF.TXN = '40'	;*Interface Change by Santiago
        Y.DB.MOVEMENT = 0
        Y.DB.MVT.LCY  = BAL.DR.AMT	;*Interface Change by Santiago
        Y.FT.OUT.LIST := Y.DB.TYPE.OF.TXN:'*':Y.BAL.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.DB.MVT.LCY:'*':Y.DB.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER	;*Interface Change by Santiago
        GOSUB WRITE.NORMAL.PROCESS
    END
    ...
    IF INT.DR.AMT AND Y.INT.ACC.NO THEN
        INT.DR.AMT = TRIM(INT.DR.AMT, "", "D")	;*Interface Change by Santiago
        Y.DB.TYPE.OF.TXN = '40'	;*Interface Change by Santiago
        Y.DB.MOVEMENT = 0
        Y.DB.MVT.LCY = INT.DR.AMT	;*Interface Change by Santiago
        Y.FT.OUT.LIST := Y.DB.TYPE.OF.TXN:'*':Y.INT.ACC.NO:'*':Y.SAP.TXN.COMP:'*':Y.RE.STAT.LINE.BAL.CUR:'*':Y.DB.MVT.LCY:'*':Y.DB.MOVEMENT:'*':Y.IS.CONTINGENT:'*':Y.SAP.COST.CENTER	;*Interface Change by Santiago
        GOSUB WRITE.NORMAL.PROCESS
    END
