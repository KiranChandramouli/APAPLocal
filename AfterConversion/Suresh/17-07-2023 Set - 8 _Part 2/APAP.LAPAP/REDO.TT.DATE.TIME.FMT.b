* @ValidationCode : MjotMzgzNDMzOTI2OkNwMTI1MjoxNjg5NTcwNTgwMDkyOjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 17 Jul 2023 10:39:40
...
*DATE                 WHO                    REFERENCE                     DESCRIPTION
*13/07/2023      Conversion tool            R22 Auto Conversion            FM TO @FM, VM TO @VM, SM TO @SM,++ TO +=, F.READ TO CACHE.READ, = TO EQ
...
*--------*
    ...
    BEGIN CASE
        ...
        CASE Y.FIELD.NAME EQ 'Y.CCY'
            ...
            IF  Y.CURRENCY EQ 'DOP' THEN ;*R22 Auto Conversion
        ...
        CASE Y.FIELD.NAME EQ 'AMT.FCY'
            APAP.REDORETAIL.redoDsAmountFcy(Y.FIELD.NAME)  ;*R22 Manual Conversion
        ...
        CASE Y.FIELD.NAME EQ 'Y.CONCEPT'
            ...
            CALL CACHE.READ(FN.TELLER.TRANSACTION, Y.TELL.TRANS, R.TELLER.TRANSACTION, TELLER.TRANSACTION.ERR) ;*R22 Auto Conversion
            ...
            CALL CACHE.READ(FN.TRANSACTION, Y.TELLER.TRANS.CODE, R.TRANSACTION, TRANSACTION.ERR) ;*R22 Auto Conversion
...
*----------------------------------------------------------------------------------------------------------------------
    ...
    BEGIN CASE
        ...
        CASE Y.FIELD.NAME EQ 'CLIENT.TYPE'
            APAP.REDORETAIL.redoDsClientType(Y.FIELD.NAME) ;*R22 Manual Conversion
            Y.FIELD.NAME = FMT(Y.FIELD.NAME,"27R")
            RETURN

        CASE Y.FIELD.NAME EQ 'SEL.DST'
            APAP.REDORETAIL.redoDsSelDst(Y.FIELD.NAME) ;*R22 Manual Conversion
            Y.FIELD.NAME = FMT(Y.FIELD.NAME,"18R")
            RETURN

        CASE Y.FIELD.NAME EQ 'CUSTOMER.NO'
            APAP.REDOSRTN.redoSGetCusNumber(Y.FIELD.NAME) ;*R22 Manual Conversion
            Y.FIELD.NAME = FMT(Y.FIELD.NAME,"12R")
            RETURN

        CASE Y.FIELD.NAME EQ 'RCEP.MTHD'
            APAP.REDORETAIL.redoDsRcepMthd(Y.FIELD.NAME) ;*R22 Manual Conversion
            Y.FIELD.NAME = FMT(Y.FIELD.NAME,"18R")
            RETURN

        CASE Y.FIELD.NAME EQ 'PAY.METHOD'
            APAP.REDORETAIL.redoDsPayMethod(Y.FIELD.NAME) ;*R22 Manual Conversion
...
*-------------------------------------------------------------------------
    ...
    WHILE Y.WVB LE Y.WV.COUNTING
        ...
        IF  Y.WV.COMM<1,Y.WVB> EQ 'NO' THEN ;*R22 Auto Conversion
            ...
            Y.TOTAL.COMM += Y.PLUS.COMM
        END

        Y.WVB += 1 ;*R22 Auto Conversion
    REPEAT
...
*----------------------------------------------------------------------------------------------------------------------
    ...
    WHILE Y.WV88 LE Y.WV.COUNT6
        ...
        IF  Y.WV.COMM88 EQ 'NO' THEN ;*R22 Auto Conversion
...
*----------------------------------------------------------------------------------------------------------------------
    ...
    WHILE Y.WV98 LE Y.WV.COUNT7
        ...
        IF  Y.WV.COMM98 EQ 'NO' THEN ;*R22 Auto Conversion
