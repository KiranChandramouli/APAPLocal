* @ValidationCode : MjotNzkyNjQ5OTIxOkNwMTI1MjoxNjk4MjM4NDgxOTM1OjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 25 Oct 2023 18:24:41
...
* <Rating>-20</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE APAP.GET.BEN.OTHER.LOAN.FILTER(ID.LIST)
*-----------------------------------------------------------------------------
* Description :
...
*    END
    ...
    IF NOT(CUS.BEN.LIST.ER) THEN
        ...
        WHILE BEN.EACH.ID:BEN.LIST.POS
            ...
            IF NOT(BENEFICIARY.ER) AND R.BENEFICIARY<ARC.BEN.TRANSACTION.TYPE> EQ 'AC08' THEN
                ID.LIST<-1> = BEN.ID
            END
...
*    SEL.CMD = 'SELECT ':FN.BENEFICIARY:' WITH OWNING.CUSTOMER EQ ':SQUOTE(CUST.ID):' AND WITH TRANSACTION.TYPE EQ ':SQUOTE('AC08')
