* @ValidationCode : MjotMTIxOTcyOTQ1ODpDcDEyNTI6MTY5NjU5ODUwMjI0MjozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 06 Oct 2023 18:51:42
...
*06/10/2023      Suresh                     R22 Manual Conversion           FM TO @FM, VM TO @VM
*-----------------------------------------------------------------------------
...
***GETTING LOCAL STATUS VALUES 1&2***
    APPL = 'ACCOUNT'
    F.FIELDS = 'L.AC.STATUS1':@VM:'L.AC.STATUS2':@VM:'L.AC.STATUS'
...
***LOCAL STATUS****
        ...
        CNT.STATUS = DCOUNT(AC.LOC.STATUS,@VM)
...
***LOCK DATE WITH IN RANGE OF START DATE***
        ...
        BOOKING.DATE = DAT.ID
        Y.BALANCE = '' ; Y.ACC.INT = ''
*  CALL REDO.V.CHECK.BAL.INT.ENTRIES(AC.ID,Y.BALANCE,BAL.CR.AMT,BAL.DR.AMT,Y.ACC.INT,INT.CR.AMT,INT.DR.AMT,Y.BAL.DET,Y.INT.DET)
        APAP.REDOAPAP.redoVCheckBalIntEntries(AC.ID,Y.BALANCE,BAL.CR.AMT,BAL.DR.AMT,Y.ACC.INT,INT.CR.AMT,INT.DR.AMT,Y.BAL.DET,Y.INT.DET)
...
*************
    ...
    TOT.CNT = DCOUNT(ACCT.ARR,@FM)
    FOR I = 1 TO TOT.CNT
        Y.CHECK = ACCT.ARR<I>
        CHANGE '*' TO @VM IN Y.CHECK
        Y.AC.ID = FIELD(Y.CHECK,@VM,1)
