$PACKAGE APAP.LAPAP
* @ValidationCode : MjotMTA4MzYwMzk1MjpDcDEyNTI6MTY5MTU2NzA0MzU0MTozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 09 Aug 2023 13:14:03
...
* <Rating>-240</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE LAPAP.NOF.AA.INFO.RT.CAN(Y.FINAL)
*---------------------------------------------------------------------------------------
*Modification History:
*DATE                 WHO                    REFERENCE                     DESCRIPTION
*09/08/2023      Suresh                     R22 Manual Conversion          T24.BP is removed,Variable is initialised, CALL routine format modified
*----------------------------------------------------------------------------------------
    ...
    $INSERT  I_GTS.COMMON ;*R22 Manual Conversion - End
...
DO.FORM.SELECT:
    ...
    Y.SEL.CMD = 'SELECT ' : FN.AA.ARR : ' WITH ARR.STATUS NE CURRENT AND ARR.STATUS NE AUTH '
...
*    CALL REDO.B.CON.LNS.BY.DEBTOR.AA.RECS(Y.AA.ARR.ID,OUT.RECORD)
    APAP.AA.redoBConLnsByDebtorAaRecs(Y.AA.ARR.ID,OUT.RECORD) ;*R22 Manual Conversion
...
*    CALL REDO.GET.TOTAL.OUTSTANDING.SIN.UNC.UND(Y.AA.ARR.ID,Y.BAL,Y.TOT.BAL)
    APAP.TAM.redoGetTotalOutstandingSinUncUnd(Y.AA.ARR.ID,Y.BAL,Y.TOT.BAL) ;*R22 Manual Conversion
    Y.BALANCE = Y.BAL<1>
    Y.OUT.BAL = Y.BALANCE
...
GO.GET.CUSTOMER:
*   Y.CUSTOMER.ID   = R.AA.CUSTOMER<AA..PRIMARY.OWNER>
    Y.CUSTOMER.ID   = R.AA.CUSTOMER<AA.CUS.CUSTOMER> ;*R22 Manual Conversion
