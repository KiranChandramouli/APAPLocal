* @ValidationCode : MjoxMDM4NjU4MTU1OkNwMTI1MjoxNjg3ODQzMDUzMzI2OjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 27 Jun 2023 10:47:33
...
*----------------------------------------------
    ...
    LOC.REF.FIELDS='L.INITIAL.ID':@VM:'CERT.CHEQUE.NO' ;*R22 AUTO CONVERSION
    LOC.REF.POS=''
...
*----------------------------------------------
    ...
    Y.TRANS.CNT = DCOUNT(Y.TRANS.IDS,@VM) ;*R22 AUTO CONVERSION
    Y.VAR1 = 1
    LOOP
...
*--------------------------------------------
    ...
    CHANGE @SM TO @FM IN Y.CHEQUE.NOS ;*R22 AUTO CONVERSION START
    CHANGE @VM TO @FM IN Y.CHEQUE.NOS
    Y.CNT  = DCOUNT(Y.CHEQUE.NOS,@FM) ;*R22 AUTO CONVERSION END
    Y.VAR2 = 1
    LOOP
