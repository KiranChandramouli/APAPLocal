* @ValidationCode : MjotMTk1NzIwODEyODpDcDEyNTI6MTY5NzcxNDQ1OTIwMDpJVFNTOi0xOi0xOjIzNjoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 19 Oct 2023 16:50:59
...
*04-04-2023      Samaran T           Manual R22 Code Conversion      VM TO @VM
*17/10/2023	 VIGNESHWARI      ADDED COMMENT FOR ATM CHANGES  Insert file modified
...
*----------------------------------------------------------------------
    ...
    $INSERT I_F.ATM.TRANSACTION   ;* R22 AUTO CONVERSION	;* ATM changes by Mario
...
*INICIAN CAMBIOS JORGE*	;* ATM changes by Mario-START
	LOCAL.APPLICATION = 'ATM.TRANSACTION'
    LOCAL.FIELD = 'L.WAIVE.FLAG':@VM:'L.FTTC.ID':@VM:'L.POS.COND'
    LOCAL.POSITION = ""
    CALL MULTI.GET.LOC.REF(LOCAL.APPLICATION, LOCAL.FIELD, LOCAL.POSITION)
    L.WAIVE.FLAG.POS = LOCAL.POSITION<1,1>
    L.FTTC.ID.POS = LOCAL.POSITION<1,2>
    L.POS.COND.POS = LOCAL.POSITION<1,3>
*TERMINAN CAMBIOS JORGE*	;* ATM changes by Mario-END

    *IF R.ATM.REVERSAL<AT.REV.WAIVE.FLAG> EQ 'Y' THEN --CAMBIO JORGE
	IF R.ATM.TRANSACTION<AT.REV.LOCAL.REF, L.WAIVE.FLAG.POS> EQ 'Y' THEN ;*--CAMBIO JORGE	;* ATM changes by Mario
...
* End of Fix
    ...
    *Y.ACC.NO= R.ATM.REVERSAL<AT.REV.ACCOUNT.NUMBER> --CAMBIO JORGE	;* ATM changes by Mario
	Y.ACC.NO= R.ATM.TRANSACTION<AT.REV.DEBIT.ACCT.NO>  ;*--CAMBIO JORGE	;* ATM changes by Mario
    IF DR.ACCT EQ '' THEN
        DR.ACCT=Y.ACC.NO
    END
    IF CR.ACCT EQ '' THEN
    ...
    *R.FUNDS.TRANSFER<FT.TRANSACTION.TYPE>=R.ATM.REVERSAL<AT.REV.FTTC.ID> --CAMBIO JORGE
	R.FUNDS.TRANSFER<FT.TRANSACTION.TYPE>=R.ATM.TRANSACTION<AT.REV.LOCAL.REF, L.FTTC.ID.POS>  ;*--CAMBIO JORGE	;* ATM changes by Mario
    ...
    R.FUNDS.TRANSFER<FT.LOCAL.REF,POS.AT.UNIQUE.ID>=ATM.TRANSACTION.ID	;* ATM changes by Mario-START
    *R.FUNDS.TRANSFER<FT.LOCAL.REF,POS.POS.COND>=R.ATM.REVERSAL<AT.REV.POS.COND> --CAMBIO JORGE
	R.FUNDS.TRANSFER<FT.LOCAL.REF,POS.POS.COND>=R.ATM.TRANSACTION<AT.REV.LOCAL.REF, L.POS.COND.POS>  ;*--CAMBIO JORGE	;* ATM changes by Mario-END
    R.FUNDS.TRANSFER<FT.LOCAL.REF,POS.BIN.NO>=CARD.NUMBER[1,6]
    R.FUNDS.TRANSFER<FT.LOCAL.REF,POS.AT.AUTH.CODE>=FIELD(ATM.TRANSACTION.ID,'.',2)	;* ATM changes by Mario
...
*----------------------------------------------------------------------
    ...
    OFSFUNCT='I'
    PROCESS  = ''
    OFSVERSION = FUNDS.TRANSFER:',':Y.FT.VERSION ;*R22 Manual Conversion
    GTSMODE = ''
    TRANSACTION.ID=''
    OFSRECORD = ''
    OFS.MSG.ID =''
    ...
    GTSMODE = ''
    *TRANSACTION.ID=R.ATM.TRANSACTION<AT.REV.TRANSACTION.ID> --CAMBIO JORGE
	TRANSACTION.ID=R.ATM.TRANSACTION<AT.REV.TRANS.REF>	;* ATM changes by Mario
    OFSRECORD = ''
    OFS.MSG.ID =''
    OFS.ERR = ''
    NO.OF.AUTH='0'
