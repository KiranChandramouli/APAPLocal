...
* Log writing: abnormal error that must be notified
        APAP.REDOSRTN.redoSNotifyInterfaceAct('VPL008', 'ONLINE', '04', 'Email ERROR WS EN LINEA - ' : WS.DATA<2>, 'TIMEOUT EN TRANSACCION EN LINEA ' : TIMEDATE(), '', '', '', '', '', OPERATOR, '') ;*MANUAL R22 CODE CONVERSION
    END
...
ONLINE.PAY.ERR.CHECK:
    BEGIN CASE
        ...
        CASE 1
            ...
            APAP.REDOSRTN.redoSNotifyInterfaceAct('VPL003', 'ONLINE', '04', 'Email RESPUESTA DE WEBSERVICE [' : WS.DATA<1> : '] - ' : WS.DATA<2>, ' ' : TIMEDATE() : ' - LOG EN Jboss : server.log', '', '', '', '', '', OPERATOR, '') ;*MANUAL R22 CODE CONVERSION
    END CASE
...
GENERAL.ERR.CHECK:
    BEGIN CASE
        CASE ERROR.CODE MATCHES 1 : @VM : 2 : @VM : 4 : @VM : 101 : @VM : 102
            ...
            WS.DATA<2> = ERR.ID : REDO.S.GET.USR.ERR.MSG('ST-VP-MSG.' : ERROR.CODE)
* Log writing: abnormal error that must be notified
            APAP.REDOSRTN.redoSNotifyInterfaceAct('VPL008', 'ONLINE', '04', 'Email ERROR WS EN LINEA [' : WS.DATA<1> : '] - ' : WS.DATA<2>, 'ERROR EN TRANSACCION EN LINEA ' : TIMEDATE() : ' - LOG EN Jboss : server.log', '', '', '', '', '', OPERATOR, '') ;*MANUAL R22 CODE CONVERSION
        CASE 1
