* @ValidationCode : MjoxODA0NTM4MjIyOlVURi04OjE2ODY4MjIxNjY3NTM6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 15 Jun 2023 15:12:46
...
SUBROUTINE RGS.DET.OVERRIDE
REM "RGS.DET.OVERRIDE",230614-4
*************************************************************************
...
*************************************************************************
    RG.OBTAIN.OVERRIDE = "RG.OBTAIN.OVERRIDE"
*************************************************************************
...
*************************************************************************
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.DET.OVERRIDE")
    END
*
    YCOM = ID.COMPANY
*
...
* Year 2000 compliance
    ...
    YM.KEY = YM1.KEY
    YKEYFD = FMT(YM.KEY,"L#######-#####:#######")
    IF LEN(YKEYFD) > 21 THEN YKEYFD = YKEYFD[1,20]:"|"
    GOSUB 8000000
    YR.REC(3) = YM.KEY
    YM.MODULE = R.NEW(4)
    YR.REC(4) = YM.MODULE
    YM.ROUTINE = R.NEW(3)
    YM6.GOSUB = YM.ROUTINE
    YM.FILE.NAME = R.NEW(7)
    YM7.GOSUB = YM.FILE.NAME
    YM.REC.KEY = R.NEW(8)
    YM8.GOSUB = YM.REC.KEY
    YM.CURR.NO = R.NEW(9)
    YM9.GOSUB = YM.CURR.NO
    YM.INPUTTER = ""
    YM10.GOSUB = YM.INPUTTER
    YM.AUTHORISER = ""
    YM11.GOSUB = YM.AUTHORISER
    YM.MESSAGE = ""
    YM12.GOSUB = YM.MESSAGE
    CALL @RG.OBTAIN.OVERRIDE (YM6.GOSUB, YM7.GOSUB, YM8.GOSUB, YM9.GOSUB, YM10.GOSUB, YM11.GOSUB, YM12.GOSUB)
    YM.ROUTINE = YM6.GOSUB
    YM.FILE.NAME = YM7.GOSUB
    YM.REC.KEY = YM8.GOSUB
    YM.CURR.NO = YM9.GOSUB
    YM.INPUTTER = YM10.GOSUB
    YM.AUTHORISER = YM11.GOSUB
    YM.MESSAGE = YM12.GOSUB
    YR.REC(5) = YM.INPUTTER
    YR.REC(6) = YM.AUTHORISER
    YR.REC(7) = YM.MESSAGE
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
