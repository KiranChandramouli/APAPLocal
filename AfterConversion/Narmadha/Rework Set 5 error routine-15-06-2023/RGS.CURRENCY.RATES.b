* @ValidationCode : MjotMTM4NTg5ODEyOTpVVEYtODoxNjg2ODM1NDQ4MDU5OkFkbWluOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 15 Jun 2023 18:54:08
...
SUBROUTINE RGS.CURRENCY.RATES
REM "RGS.CURRENCY.RATES",230614-4
*************************************************************************
...
*************************************************************************
    ...
    DIM YR.REC(6)
    YFILE = "F":R.COMPANY(EB.COM.MNEMONIC):".RGS.CURRENCY.RATES"
    YOLDFILE = 1
    ...
    OPEN "", YFILE TO F.FILE ELSE
        ...
        CALL FATAL.ERROR ("RGS.CURRENCY.RATES")
    END
*
    YCOM = ID.COMPANY
*
...
*************************************************************************
    YFILE = "CURRENCY"
    FULL.FNAME = "F.CURRENCY"; YF.CURRENCY = ""
    ...
    CALL OPF (FULL.FNAME, YF.CURRENCY)
    CLEARSELECT
    YSEL.A = ""
    ...
    END
    EXECUTE "HUSH ON"
    EXECUTE 'SSELECT ':FULL.FNAME:' WITH @ID LIKE "...USD" OR WITH @ID LIKE "...GBP" OR WITH @ID LIKE "...DEM" OR WITH @ID LIKE "...FFR" OR WITH @ID LIKE "...NLG" OR WITH @ID LIKE "...ITL" OR WITH @ID LIKE "...ATS" OR WITH @ID LIKE "...XEU" OR WITH @ID LIKE "...DKK" OR WITH @ID LIKE "...SEK" OR WITH @ID LIKE "...HKD" OR WITH @ID LIKE "...SGD" OR WITH @ID LIKE "...MYR" OR WITH @ID LIKE "...THB" OR WITH @ID LIKE "...ESP" OR WITH @ID LIKE "...CAD" OR WITH @ID LIKE "...JPY" OR WITH @ID LIKE "...AUD" OR WITH @ID LIKE "...NZD" OR WITH @ID LIKE "...KWD" OR WITH @ID LIKE "...XSO" OR WITH @ID LIKE "...XGO"':YSEL.A
    EXECUTE "HUSH OFF"
    CALL EB.READLIST('', YID.LIST, '', '', '')
    LOOP
        REMOVE ID.NEW FROM YID.LIST SETTING YDELIM
    WHILE ID.NEW:YDELIM
        MATREAD R.NEW FROM YF.CURRENCY, ID.NEW ELSE ID.NEW = "" ; MAT R.NEW = ""
        IF T.PWD THEN
...
* Year 2000 compliance
    ...
    YM.CCY.NO = R.NEW(2)
    YR.REC(2) = YM.CCY.NO
    YM.CCY.NAME = R.NEW(3)<1,1>
    YR.REC(3) = YM.CCY.NAME
    YM.PURCHASE.RATE = R.NEW(16)
    YR.REC(4) = YM.PURCHASE.RATE
    YM.SALE.RATE = R.NEW(17)
    YR.REC(5) = YM.SALE.RATE
    YM.REVAL.RATE = R.NEW(14)
    YR.REC(6) = YM.REVAL.RATE
*
    YKEYNO = YKEYNO + 1
    IF YKEYNO > 9999 THEN
        YKEY = YKEY:YKEYNO
