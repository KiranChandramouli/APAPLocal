* @ValidationCode : MjoxNjcxMTg1ODA6VVRGLTg6MTY4NTUyNzQxNzk2MzpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 31 May 2023 15:33:37
...
SUBROUTINE REDO.FI.EXT.PAYROLL.VALIDATE(IN.TXT.MSG,OUT.VAR.WORK,OUT.TXT.MSG,UPD.IN.TXT.MSG,OUT.ERR.MSG)
******************************************************************************************************
...
*  DATE             WHO                   REFERENCE
*05-APRIL-2023      Harsha                R22 Auto Conversion  - VM to @VM , FM to @FM , I to I.VAR , K to K.VAR , = to EQ and CHAR to CHARX
...
*   Take each element of the Head
    ...
    Y.ZEROS                = Y.DATA[1,12]
    Y.COMPANY.HEAD         = Y.DATA[13,4]
    Y.PAYMENT.DATE.HEAD    = Y.DATA[17,8]
    Y.SEQ.FILE.HEAD        = Y.DATA[25,1]
    Y.TOTAL.AMOUNT.HEAD    = Y.DATA[26,10]
    Y.TOTAL.RECORDS.HEAD   = TRIM(Y.DATA[36,6],"0","L")
    Y.TOTAL.AMOUNT.HEAD.INT = Y.TOTAL.AMOUNT.HEAD

    Y.TOTALHEAD.1 = Y.TOTAL.AMOUNT.HEAD[1,8]
    Y.TOTALHEAD.2 = Y.TOTAL.AMOUNT.HEAD[9,2]
    Y.TOTAL.AMOUNT.HEAD = TRIM(Y.TOTALHEAD.1,"0","L"):'.': Y.TOTALHEAD.2

    Y.TOTALHEAD = Y.TOTALHEAD.1 :'.': Y.TOTALHEAD.2
    UPD.IN.TXT.MSG<-1> = Y.COMPANY.HEAD:',':Y.PAYMENT.DATE.HEAD:',':Y.SEQ.FILE.HEAD:',':Y.TOTAL.AMOUNT.HEAD:',':Y.TOTAL.RECORDS.HEAD
...
* Validation to verify if the company is register
    ...
    Y.UNIQUE.PART.HEAD = Y.COMPANY.HEAD:Y.PAYMENT.DATE.HEAD:Y.SEQ.FILE.HEAD:Y.TOTAL.AMOUNT.HEAD.INT:Y.TOTAL.RECORDS.HEAD
...
*
    ...
    FI.PAYMENT.REF    = R.REDO.INTERFACE.PARAM<REDO.INT.PARAM.PAYMENT.REF>
...
*  Tus Start
    R.ECB = ''
    ECB.ERR = ''
    CALL EB.READ.HVT("EB.CONTRACT.BALANCES",Y.ACCOUNT.ID,R.ECB,ECB.ERR) ; * Tus End
    IF R.ACCOUNT THEN
*       Available Amount DebitAccount
        IF FI.INTERFACE EQ 'EXTNOMINANOTAX' THEN
*     Y.AVAILABLE.AMOUNT = R.ACCOUNT<AC.WORKING.BALANCE>
*    Tus Start
            Y.AVAILABLE.AMOUNT = R.ECB<ECB.WORKING.BALANCE>
*    Tus End
        END ELSE
            Y.AVAILABLE.AMOUNT  = R.ACCOUNT<Y.POS.LF.ACC,Y.POS.AV.BAL>
            IN.ACCT.TYPE = 'CREDIT'
*CALL APAP.REDOENQ.REDO.NOMINA.ACCT.STATUS.CHECK(Y.ACCOUNT.ID,IN.ACCT.TYPE,OUT.ACCT.STATUS)
            APAP.REDOENQ.redoNominaAcctStatusCheck(Y.ACCOUNT.ID,IN.ACCT.TYPE,OUT.ACCT.STATUS) ;*R22 Manual Conversion
            IF OUT.ACCT.STATUS THEN
                Y.ERR.MSG = OUT.ACCT.STATUS
                RETURN
            END
        END
...
*
    ...
    IF Y.ERR.MSG NE "" THEN
        Y.ERR.MSG = 'TRANSACTION REFERENCE NOT VALID'
        RETURN
    END
...
*VALIDATE THE AMOUNTS
    IF Y.TOTAL.AMOUNT.HEAD GT Y.AMOUNT.DEPOSITED THEN
*        WERROR.MSG = "EB-Not.enough.BALANCE.in.&.transaction":FM:WCONTROL.ID
        Y.ERR.MSG = 'INSUFFICIENT BALANCE'
        RETURN
    END
...
*   Sum amount from the detail
    ...
    Y.DET.AMT = ''
    Y.DET.AMT = TRIM(Y.DATA[13,6], "0","L"):'.':Y.DATA[19,2]
    Y.TOTAL.AMOUNT.DATA   += Y.DET.AMT
*   Amount by record
    Y.TOTAL.AMOUNT         = Y.DET.AMT
*   Type Producto
    Y.TYP.PRODUCT          = Y.DATA[1,12]
*   ID product
    Y.ID.PRODUCT           = Y.DATA[1,12]
*   BIN number
    Y.BIN.NUMBER           = SUBSTRINGS(Y.ID.PRODUCT,1,6)

    Y.ID.PRODUCT    =  TRIM(Y.ID.PRODUCT, "0", "L")

    IF LEN(Y.ID.PRODUCT) EQ 16 THEN
        Y.TYP.PRODUCT = 'T'
    END ELSE
        Y.TYP.PRODUCT = 'C'
    END

    UPD.IN.TXT.MSG<-1> = Y.TYP.PRODUCT:',':Y.ID.PRODUCT:',':Y.TOTAL.AMOUNT
...
*   Date parts - PAYMENT.DATE.HEAD's format ddMMyyyy buy TODAY's format is yyyyMMdd
    Y.DAY.HEAD             = Y.PAYMENT.DATE.HEAD[7,2]
    Y.MONTH.HEAD           = Y.PAYMENT.DATE.HEAD[5,2]
    Y.YEAR.HEAD            = Y.PAYMENT.DATE.HEAD[1,4]
