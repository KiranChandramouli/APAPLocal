* @ValidationCode : Mjo1NjQwMTI3Nzk6VVRGLTg6MTY4Nzc4MzE1MDA4NjpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 26 Jun 2023 18:09:10
...
SUBROUTINE REDO.E.CHECK.USER(ENQ.DATA)
*-----------------------------------------------------------------------------
*DESCRIPTION:
*------------
*This routine is used to fetch selection data for the enquiry REDO.TELLER.REJECT.LIST
*------------------------------------------------------------------------------------------
* Input/Output:
*--------------
* IN  : -NA-
...
* 26-06-2023        Narmadha V       Manual R22 Conversion     Change the variable name "Branch.code" to "Co.code"
*------------------------------------------------------------------------------------------
...
*      CALL F.READ(FN.USER.ACCESS,ID.COMPANY,R.REDO.USER.ACC,F.USER.ACCESS,ERR);*Tus Start
    ...
    LOCATE "@ID" IN ENQ.DATA<2,1> SETTING VAR.ID.POS THEN
        ...
        IF VAR.ID NE '' THEN
            CALL F.READ(FN.REDO.TELLER.REJECT,VAR.ID,R.REDO.TELLER.REJECT,F.REDO.TELLER.REJECT,ERR)
*VAR.BRANCH.CODE=R.REDO.TELLER.REJECT<TEL.REJ.BRANCH.CODE>
            VAR.BRANCH.CODE=R.REDO.TELLER.REJECT<TEL.REJ.CO.CODE> ;* Manual R22 Conversion
        END
        GOSUB PROCESS2
