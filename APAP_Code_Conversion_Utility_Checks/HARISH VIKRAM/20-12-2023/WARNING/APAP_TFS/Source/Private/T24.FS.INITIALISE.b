* @ValidationCode : MjoxODM2MjgzOTg3OkNwMTI1MjoxNjk4NzUwNjczMzA0OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
...
*26/10/2023         Suresh             R22 Manual Conversion             ID.VAR Initialised,GLOBUS.BP File Removed, USPLATFORM.BP File Removed, FM TO @FM, VM TO @VM
*-------------------------------------------------------------------------------
...
LOAD.TFS.PARAMETER:
    IF NOT(TFS$R.TFS.PAR) THEN          ;* 20100624 umar
        ...
        END ELSE
            E = 'EB-US.REC.MISS.FILE' :@FM: ID.TFSP :@VM: FN.TFSP ;*R22 Manual Conversion
            PROCESS.GOAHEAD = 0
        END
* 20100624 umar - start
    END ELSE
...
*        CALL CACHE.READ(FN.CCY,'SSelectIDs',TFS$CCY.LIST,'')
        ID.VAR = 'SSelectIDs'                                 ;*;*R22 Manual Conversion
        CALL CACHE.READ(FN.CCY,ID.VAR,TFS$CCY.LIST,'')        ;*;*R22 Manual Conversion
* 20100624 umar - end
...
*        CALL CACHE.READ(FN.TD,'SSelectIDs',TD.ID.LIST,ERR.TD)
        CALL CACHE.READ(FN.TD,ID.VAR,TD.ID.LIST,ERR.TD)
