* @ValidationCode : MjotMTE5MTU1NTA1NDpDcDEyNTI6MTY5ODc1MDY3NDUzMjpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
*26/10/2023         Ajithkumar             R22 Manual Conversion          USPLATFORM.BP file is Removed, VM TO @VM
*
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
PARSE.STMT.FOR.THIS.RECORD:
    ...
    NO.OF.VMS = DCOUNT(ALL.STMT.NOS,@VM) ;*R22 Manual Conversion
...
BUILD.UL.STMT.NOS:
    ...
    FOR ENTRY.NO = ENTRY.START TO ENTRY.END
        ...
        IF R.ENTRY THEN
            IF UL.STMT.NOS THEN
                UL.STMT.NOS := @VM: ENTRY.PREFIX :'-': ENTRY.ID :'\': ENTRY.COMP ;*R22 Manual Conversion
            END ELSE
...
*
            IF ENTRY.PREFIX EQ 'SE' AND NOT(CE.IDS) THEN
                ...
                IF R.ENTRY THEN
                    IF UL.STMT.NOS THEN
                        UL.STMT.NOS := @VM: 'CE-' : ENTRY.ID :'\': ENTRY.COMP ;*R22 Manual Conversion
                    END ELSE
