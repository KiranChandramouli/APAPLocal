...
*            CALL REFRESH.FIELD(DEFAULTED.FIELD,DEFAULTED.ENRI)
            EB.Display.RefreshField(DEFAULTED.FIELD,DEFAULTED.ENRI)  ;*R22 Manual Conversion
        END ELSE
            IF NOT(GTSACTIVE) AND TFS$MESSAGE EQ '' THEN
*                CALL REBUILD.SCREEN
                EB.Display.RebuildScreen()    ;*R22 Manual Conversion
            END
        END
    END
...
*            CASE CHG.METHOD = 'DEBIT.PLUS.CHARGES'
            CASE CHG.METHOD EQ 'DEBIT.PLUS.CHARGES'
                IF SIGN EQ '-1' THEN
                    AMOUNT += CHG.AMT
                END ELSE
                    CHG.PL.AMT = CHG.AMT
