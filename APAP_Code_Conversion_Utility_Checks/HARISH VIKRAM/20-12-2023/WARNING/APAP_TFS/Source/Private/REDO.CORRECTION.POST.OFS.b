* @ValidationCode : MjoxMjQ4NzM1OTQ0OkNwMTI1MjoxNjg0ODU1NjAzMjQ2OklUU1M6LTE6LTE6NDg0OjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
*19-04-2023       AJITHKUMAR                  MANUAL R22 CODE CONVERSION Y.ID INITIALISED
*----------------------------------------------------------------------------------------
...
*    CALL CACHE.READ(FN.OVERRIDE, "REDO.AA.OFS", R.OVERRIDE, OVER.ERR) ;*R22 AUTO CODE CONVERSION
    CALL CACHE.READ(FN.OVERRIDE, Y.ID, R.OVERRIDE, OVER.ERR) ;*MANUAL R22 CODE CONVERSION
...
*        CALL F.WRITE(FN.OVERRIDE,"REDO.AA.OFS",R.OVERRIDE)
        CALL F.WRITE(FN.OVERRIDE,Y.ID,R.OVERRIDE)          ;*MANUAL R22 CODE CONVERSION
        RETURN
    END
    ...
    IF R.OVERRIDE<EB.OR.PREV.MESSAGE,1,3> EQ "PROCESSED" THEN
        ...
        R.OVERRIDE<EB.OR.PREV.MESSAGE,1,5> = ERR.MSG
*        CALL F.WRITE(FN.OVERRIDE,"REDO.AA.OFS",R.OVERRIDE)
        CALL F.WRITE(FN.OVERRIDE,Y.ID,R.OVERRIDE)
        RETURN
    END
