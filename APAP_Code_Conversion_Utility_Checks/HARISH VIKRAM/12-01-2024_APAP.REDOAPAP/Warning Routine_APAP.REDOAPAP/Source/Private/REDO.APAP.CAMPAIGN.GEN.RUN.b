* @ValidationCode : Mjo0ODQ4OTAyNTQ6Q3AxMjUyOjE3MDM1ODU3NzUzNzE6SGFyaXNodmlrcmFtQzotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 26 Dec 2023 15:46:15
...
*22-12-2023      HARISHVIKRAM                 R22 Manual conversion        F.READ to F.READU, ID.VAR Initialised
*----------------------------------------------------------------------------------------
...
*    CALL F.READ(FN.REDO.APAP.CAM.GEN,Y.ID,R.CAM.GEN.REC,F.REDO.APAP.CAM.GEN,Y.CAM.ERR)
    CALL F.READU(FN.REDO.APAP.CAM.GEN,Y.ID,R.CAM.GEN.REC,F.REDO.APAP.CAM.GEN,Y.CAM.ERR,"")   ;*R22 Manual Conversion
...
*    CALL F.READ(FN.BATCH,Y.BATCH.ID,R.BATCH.REC,F.BATCH,Y.B.ERR)
    CALL F.READU(FN.BATCH,Y.BATCH.ID,R.BATCH.REC,F.BATCH,Y.B.ERR,"")    ;*R22 Manual Conversion
...
*    CALL F.READ(FN.TSA.SERVICE,"TSM",R.TSA.REC,F.TSA.SERVICE,Y.TSA.ERR)
    CALL F.READ(FN.TSA.SERVICE,ID.VAR,R.TSA.REC,F.TSA.SERVICE,Y.TSA.ERR)    ;*R22 Manual Conversion
    IF R.TSA.REC THEN
