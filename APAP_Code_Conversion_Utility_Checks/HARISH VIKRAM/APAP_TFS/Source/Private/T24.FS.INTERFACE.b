...
CALL.OFS:
    ...
    NO.OF.LEGS = DCOUNT(OFS.BODY,@FM) ;*R22 Manual Conversion
...
*       EB.Interface.OfsGlobusManager(OFSS.ID,OFS.MSG)     ;*R22 Manual Conversion
        EB.Interface.OfsGlobusManager(OFSS.ID,OFS.MSG)     ;*R22 Manual Conversion
* 20100624 umar - start
        DC.RECORD = ''
...
*                CALL TFS.ANALYSE.OFS.MSG(OFS.MSG,'',TXN.OVERRIDES,'')
                ...
                IF TXN.OVERRIDES THEN
                    IF UL.OVERRIDES<MV.NO> THEN UL.OVERRIDES<MV.NO> := @VM: TXN.OVERRIDES ELSE UL.OVERRIDES = TXN.OVERRIDES ;*R22 Manual Conversion
                END
            END
        END ELSE
