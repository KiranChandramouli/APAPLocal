* @ValidationCode : MjoxMTc2MDg0MTkwOkNwMTI1MjoxNzA0Njk3MjMyMTY2OkhhcmlzaHZpa3JhbUM6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 08 Jan 2024 12:30:32
...
* 29-12-2023      HARISHVIKRAM                 R22 Manual conversion
*----------------------------------------------------------------------------------------
...
*        CALL F.READ(FN.AA.ARRANGEMENT,Y.FLD.VALUE,R.ARR,F.AA.ARRANGEMENT,Y.AA.ERR)
        R.ARR = AA.Framework.Arrangement.Read(Y.FLD.VALUE, Y.AA.ERR)                            ;*R22 Manual Conversion
        
*IF R.ARR<AA.ARR.CUSTOMER> EQ '' AND Y.FLD.VALUE THEN
        IF R.ARR<AA.Framework.ArrangementSim.ArrCustomer> EQ '' AND Y.FLD.VALUE THEN           ;*R22 Manual Conversion
...
*    IF OPERAND EQ 'EQ' AND R.ARR<AA.ARR.CUSTOMER> NE Y.CUS.ID AND  R.ARR<AA.ARR.CUSTOMER> NE '' THEN
    IF OPERAND EQ 'EQ' AND R.ARR<AA.Framework.ArrangementSim.ArrCustomer> NE Y.CUS.ID AND  R.ARR<AA.Framework.ArrangementSim.ArrCustomer> NE '' THEN      ;*R22 Manual Conversion
        Y.ARR.FLAG = '1'
    END
...
*        CALL F.READ(FN.AA.ARRANGEMENT,Y.LOAN.ID,R.ARR.NEW,F.AA.ARRANGEMENT,Y.AA.ERR)
        R.ARR.NEW = AA.Framework.Arrangement.Read(Y.LOAN.ID, Y.AA.ERR)                   ;*R22 Manual Conversion
        
*    Y.STATUS = R.ARR.NEW<AA.ARR.ARR.STATUS>
        Y.STATUS = R.ARR.NEW<AA.Framework.ArrangementSim.ArrArrStatus>                ;*R22 Manual Conversion
...
*APAP.REDOAPAP.REDO.APAP.NOFILE.CUS.CO.DET.GET.DET(Y.LOAN.ID,Y.ALT.ID,Y.LOAN.STATUS,Y.AMOUNT,Y.LIMIT.ID,Y.LIMIT.AMT,Y.OFFICER,Y.PDT.TYPE,Y.LOAN.NAME,Y.POL.TYPE,Y.CUS.ID,Y.PORTF.NO)
    APAP.REDOAPAP.redoApapNofileCusCoDetGetDet(Y.LOAN.ID,Y.ALT.ID,Y.LOAN.STATUS,Y.AMOUNT,Y.LIMIT.ID,Y.LIMIT.AMT,Y.OFFICER,Y.PDT.TYPE,Y.LOAN.NAME,Y.POL.TYPE,Y.CUS.ID,Y.PORTF.NO)
RETURN
*-------------------------------------------------------------------------------------------------------------------------------------------
***************
OUTGOING.ARRAY:
