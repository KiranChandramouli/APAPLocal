* @ValidationCode : MjoyMTM2MTM4MTU0OkNwMTI1MjoxNzAwNDgwNTg1NTEwOklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
...
*28-08-2023    CONVERSION TOOL     R22 AUTO CONVERSION	   REM TO DISPLAY.MESSAGE(TEXT, ''),VM TO @VM,FM TO @FM,++ TO +=1,INSERT FILE MODIFIED
*28-08-2023    VICTORIA S          R22 MANUAL CONVERSION   DYN.TO.OFS to OFS.BUILD.RECORD,CALL ROUTINE ADDED
...
*R22 Manual CONVERSION
    APAP.TAM.redoCreArrAuthGetLimColl()
    IF END.ERROR THEN
...
*Y.OFS.MESSAGE = DYN.TO.OFS(R.LIMIT.INP, "LIMIT", ADDNL.INFO.INP)
        APP.NAME = 'LIMIT' ;*R22 MANUAL CONVERSION START-DYN.TO.OFS to OFS.BUILD.RECORD
        OFS.FUNCTION = 'I'
        OFS.PROCESS = 'PROCESS'
        OFS.VERSION = APP.NAME:',':Y.VER.LIMIT ;*R22 MANUAL CONVERSION- For TC-309/314
        Y.GTSMODE = ''
        NO.OF.AUTH = ''
        TRANSACTION.ID = Y.LIMIT.ID
        R.RECORD = R.LIMIT.INP
*       Y.OFS.STR='' ;*R22 MANUAL CONVERSION
        Y.OFS.MESSAGE = ''
        CALL OFS.BUILD.RECORD(APP.NAME,OFS.FUNCTION,OFS.PROCESS,OFS.VERSION,Y.GTSMODE,NO.OF.AUTH,TRANSACTION.ID,R.RECORD, Y.OFS.MESSAGE) ;*R22 MANUAL CONVERSION END
        IF ETEXT NE '' THEN
            E = ETEXT
            RETURN
        END
...
*        Y.OFS.STR=''
        Y.OFS.MESSAGE = '' ;*R22 MANUAL CONVERSION
        CALL OFS.BUILD.RECORD(APP.NAME,OFS.FUNCTION,OFS.PROCESS,OFS.VERSION,Y.GTSMODE,NO.OF.AUTH,TRANSACTION.ID,R.RECORD, Y.OFS.MESSAGE) ;*R22 MANUAL CONVERSION END
        IF ETEXT NE '' THEN
            E = ETEXT
            RETURN
...
*Y.OFS.MESSAGE = DYN.TO.OFS(R.COLL.RIGHT, "COLLATERAL.RIGHT", ADDNL.INFO.INP)
    ...
    Y.GTSMODE=''
    NO.OF.AUTH=''
    TRANSACTION.ID=Y.COL.RIGHT.ID
    R.RECORD=R.COLL.RIGHT
*   Y.OFS.STR='' ;*R22 MANUAL CONVERSION
    Y.OFS.MESSAGE = ''
    CALL OFS.BUILD.RECORD(APP.NAME,OFS.FUNCTION,OFS.PROCESS,OFS.VERSION,Y.GTSMODE,NO.OF.AUTH,TRANSACTION.ID,R.RECORD, Y.OFS.MESSAGE) ;*R22 MANUAL CONVERSION END
    IF ETEXT NE '' THEN
...
*------------------------------------------------------------------------------------------------------------------
    IF Y.BM.CAN.BE.USED THEN
        options = Y.OFS.SOURCE.ID : @FM : "OFS" ;*R22 AUTO CONVERSION
...
*CALL OFS.GLOBUS.MANAGER(Y.OFS.SOURCE.ID, Y.OFS.MESSAGE)
        CALL OFS.CALL.BULK.MANAGER(Y.OFS.SOURCE.ID, Y.OFS.MESSAGE, OFS.RESP, TXN.COMMIT) ;* R22 Manual conversion - End
...
*CALL OFS.GLOBUS.MANAGER(Y.OFS.SOURCE.ID, Y.OFS.MESSAGE)
            ...
            REPEAT
            GOSUB COMMON.RESTORE
            E = "ST-REDO.CRE.ARR.CREATING.LIMIT.COLL" : @VM : "LIMIT OR COLLATERAL COULD NOT BE CREATED, CHECK LOG" ;*R22 AUTO CONVERSION
        END
    END
RETURN
