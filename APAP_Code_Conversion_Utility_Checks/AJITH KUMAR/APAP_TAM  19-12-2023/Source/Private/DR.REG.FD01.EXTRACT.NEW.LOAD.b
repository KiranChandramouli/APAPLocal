* @ValidationCode : MjotMTU3NjY2NTkyOkNwMTI1MjoxNzAwODQyNzM1MTM1OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
...
*-----------------------------------------------------------------------------
$PACKAGE APAP.TAM
SUBROUTINE DR.REG.FD01.EXTRACT.NEW.LOAD
*-----------------------------------------------------------------------------
* Company Name   : APAP
...
*24/11/2023         Suresh             R22 Manual Conversion             BP Removed and changed INCLUDE TO INSERT
*----------------------------------------------------------------------------------------
...
*PACS00309078
    ...
    TXNTT.POS = ''; TXNFT.POS = '' ; GET.ID = 'SYSTEM'
    CALL CACHE.READ(FN.DR.REG.FD01.PARAM,GET.ID,R.DR.REG.FD01.PARAM,DR.REG.FD01.PARAM.ERR)
    IF R.DR.REG.FD01.PARAM THEN
        ...
        CHANGE @VM TO ' ' IN TT.CODES  ;*R22 Manual Conversion
        FX.CODES = R.DR.REG.FD01.PARAM<DR.FD01.PARAM.FX.DEAL.TYPE>
        CHANGE @VM TO ' ' IN FX.CODES ;*R22 Manual Conversion
    ...
    END
    Y.FXTXN.VAL.ARR = CHANGE(Y.FXTXN.VAL.ARR,@SM,@VM) ;*R22 Manual Conversion
    Y.FXTXN.DIS.ARR = CHANGE(Y.FXTXN.DIS.ARR,@SM,@VM) ;*R22 Manual Conversion
    ...
    END
    Y.CMPTXN.VAL.ARR = CHANGE(Y.CMPTXN.VAL.ARR,@SM,@VM) ;*R22 Manual Conversion
    Y.CMPTXN.DIS.ARR = CHANGE(Y.CMPTXN.DIS.ARR,@SM,@VM) ;*R22 Manual Conversion
