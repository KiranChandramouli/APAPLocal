* @ValidationCode : Mjo0MzAxNjIwOTA6Q3AxMjUyOjE3MDQ0NDUxODYzNjQ6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 05 Jan 2024 14:29:46
...
*14-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     FM TO @FM,SM TO @SM,VM TO @VM
*14-07-2023    VICTORIA S          R22 MANUAL CONVERSION   LINE COMMENTED
*-----------------------------------------------------------------------------------------------------
...
*---------------------------------------------------------------------------------------
    ...
    WHILE SEL.LIST.ID:POS
        IF Y.GROUP EQ SEL.LIST.ID THEN
            ...
            CHANGE @VM TO '_' IN Y.CODE ;*R22 AUTO CONVERSION START
            CHANGE @VM TO '_' IN Y.DES
*T(TEL.PRO.BRANCH.DES) = "":@FM:Y.DES ;*R22 AUTO CONVERSION END,R22 MANUAL CONVERSION-COMMENTED
            Y.FLAG = '1'
        END
    REPEAT

    IF NOT(Y.FLAG) THEN
*        T(TEL.PRO.BRANCH.DES)<3> = 'NOINPUT' *R22 MANUAL CONVERSION-COMMENTED
*        R.NEW(TEL.PRO.BRANCH.DES) = '' *R22 MANUAL CONVERSION-COMMENTED
    END
