* @ValidationCode : Mjo2MzcxNTM1MTc6Q3AxMjUyOjE3MDI5OTEwOTY0ODg6SVRTUzE6LTE6LTE6MDoxOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 19 Dec 2023 18:34:56
...
SUBROUTINE REDO.V.VAL.PDIS.AMT(Y.ID)
***********************************************************
*----------------------------------------------------------
* COMPANY NAME : APAP
...
*17-04-2023       Conversion Tool        R22 Auto Code conversion       FM TO @FM,VM TO @VM,SM TO @SM
...
* Y.CNT = DCOUNT(R.ACCOUNT.DETAILS<AA.AD.BILL.PAY.DATE>,@VM)
    Y.CNT = DCOUNT(R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillPayDate>,@VM)
    FLG.VAL = ''
    LOOP
    WHILE Y.CNT GT 0 DO
        FLG.VAL += 1
*Y.TYPE = R.ACCOUNT.DETAILS<AA.AD.BILL.TYPE,FLG.VAL>
        Y.TYPE = R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL>
...
* IF R.ACCOUNT.DETAILS<AA.AD.BILL.TYPE,FLG.VAL,1> EQ 'PAYMENT' THEN
    IF R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,1> EQ 'PAYMENT' THEN
*  Y.BILL.ID = R.ACCOUNT.DETAILS<AA.AD.BILL.ID,FLG.VAL,1>
        Y.BILL.ID = R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,1> ;*r22 mnaual code conversion
...
*IF R.ACCOUNT.DETAILS<AA.AD.BILL.TYPE,FLG.VAL,1> EQ 'ACT.CHARGE' THEN
    IF R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillType,FLG.VAL,1> EQ 'ACT.CHARGE' THEN
        FLG.PY += 1
    END
RETURN
*---------------------
...
* IF R.ACCOUNT.DETAILS<AA.AD.BILL.TYPE,FLG.VAL,FG.LG> EQ 'PAYMENT' THEN
        IF R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdAgeAllBillType,FLG.VAL,FG.LG> EQ 'PAYMENT' THEN;*R22 MANAUAL CODE CNVERSION
*Y.BILL.ID = R.ACCOUNT.DETAILS<AA.AD.BILL.ID,FLG.VAL,FG.LG>
            Y.BILL.ID = R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillId,FLG.VAL,FG.LG>
*CALL F.READ(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
* AA.PaymentSchedule.BillDetails.Read(FN.AA.BILL.DETAILS,Y.BILL.ID,R.AA.BILL.DETAILS,F.AA.BILL.DETAILS,BILL.ERR)
            R.AA.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BILL.ERR)
* Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.BD.SETTLE.STATUS,1>
            Y.SETTLE.STATUS = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdSettleStatus,1>
...
*  IF R.ACCOUNT.DETAILS<AA.AD.BILL.TYPE,FLG.VAL,FG.LG> EQ 'ACT.CHARGE' THEN
        IF R.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdAgeAllBillType,FLG.VAL,FG.LG> EQ 'ACT.CHARGE' THEN
...
*  Y.FIN.AMT += SUM(R.AA.BILL.DETAILS<AA.BD.OS.PROP.AMOUNT>)
    ...
    END ELSE
* Y.PAY.DATE = R.AA.BILL.DETAILS<AA.BD.PAYMENT.DATE>
        Y.PAY.DATE = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdPaymentType> ;*r22 manual code conversion
        IF Y.PAY.DATE NE Y.DUP.DATE THEN
            Y.DUP.DATE = Y.PAY.DATE
            FLG.PY += 1
        END
...
*  CALL AA.SCHEDULE.PROJECTOR(Y.ARRANGEMENT.ID, SIM.REF, "",CYCLE.DATE, TOT.PAYMENT, DUE.DATES, DUE.TYPES, DUE.DEFER.DATES, DUE.METHODS, DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(Y.ARRANGEMENT.ID, SIM.REF, "",CYCLE.DATE, TOT.PAYMENT, DUE.DATES, DUE.TYPES, DUE.DEFER.DATES, DUE.METHODS, DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
