* @ValidationCode : MjoyMzgyNTAyMTQ6Q3AxMjUyOjE2ODU2ODkxMzY5ODc6SVRTUzotMTotMTo5MTg6MTpmYWxzZTpOL0E6REVWXzIwMjEwOC4wOi0xOi0x
* @ValidationInfo : Timestamp         : 02 Jun 2023 12:28:56
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ITSS
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : 918
...
*DATE                       WHO                         REFERENCE                                   DESCRIPTION
*13-04-2023            Conversion Tool             R22 Auto Code conversion                FM TO @FM,VM TO @VM,F.READ TO CACHE.READ,IF CONDITION ADDED
...
*CALL REDO.S.VP.SEL.CHANNEL(APPLICATION,PGM.VERSION,TRANS.CODE,Y.CHANNEL,Y.MON.CHANNEL)
        APAP.TAM.redoSVpSelChannel(APPLICATION,PGM.VERSION,TRANS.CODE,Y.CHANNEL,Y.MON.CHANNEL)  ;*R22 MANAUAL CODE CONVERSION
        WS.DATA<3> = Y.CHANNEL
...
* 31.Pn_SobregiroRD, 32.Pn_SobregiroUS, 33.Pd_fecha_ult_estcta, 34.ID_Comportamiento, 35.Pi_CodigoMensaje, 36.Pv_DescripcionMensaje,
        ...
        Y.CARD.CLIENT = FIELD(WS.DATA<11>,'/',1)
        Y.CARD.CLIENT = Y.CARD.CLIENT * 1
*       R.NEW(ARC.BEN.BEN.ACCT.NO) = TRIM(Y.ACCOUNT,'0','L')
        R.NEW(ARC.BEN.BEN.ACCT.NO) = Y.ACCOUNT * 1

        OTH.CUS = FIELD(WS.DATA<11>,'/',1)
        OTH.CUS = OTH.CUS * 1

        IF WS.DATA<35> NE 0 THEN
            ETEXT ='EB-INVALID.ACCT'
            CALL STORE.END.ERROR
            RETURN
        END
...
*------------------------
    ...
    IF NOT(CUS.BEN.LIST.ER) THEN
        ...
        WHILE BEN.EACH.ID:BEN.LIST.POS
            ACCT.BEN.ID = FIELD(BEN.EACH.ID,"*",1)
            BEN.ID = FIELD(BEN.EACH.ID,"*",2)
            CALL CACHE.READ(FN.BENEFICIARY, BEN.ID, R.BENEFICIARY, BENEFICIARY.ERR)     ;*R22 AUTO CODE CONVERSION
