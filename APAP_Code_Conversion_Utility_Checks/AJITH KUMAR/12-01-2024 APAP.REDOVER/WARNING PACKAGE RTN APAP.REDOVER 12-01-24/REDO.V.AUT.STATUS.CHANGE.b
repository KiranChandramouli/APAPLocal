* @ValidationCode : MjotNjk1NzIwODQ5OkNwMTI1MjoxNzAzNzQ4MTIyMzIxOmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 28 Dec 2023 12:52:02
...
* CALL F.READ(FN.REDO.T.MSG.DET,Y.CUSTOMER.ID,R.MSG.DET,F.REDO.T.MSG.DET,ERR)
        CALL F.READU(FN.REDO.T.MSG.DET,Y.CUSTOMER.ID,R.MSG.DET,F.REDO.T.MSG.DET,ERR,'');*R22 MANUALCODE CONVERSION
...
*PACS00100804-S
    IF R.NEW(CUS.DOC.STATUS) EQ 2 AND R.OLD(CUS.DOC.STATUS) NE 2 THEN
        ...
        IF R.MSG.DET EQ '' THEN
            ID.VAR='SYSTEM' ;*R22 MAUAL CODE CONVERSION
            CALL CACHE.READ(FN.MSG.PARAMETER,ID.VAR,R.MSG.PARM,ERR)
            R.SECURE.MSG=''
