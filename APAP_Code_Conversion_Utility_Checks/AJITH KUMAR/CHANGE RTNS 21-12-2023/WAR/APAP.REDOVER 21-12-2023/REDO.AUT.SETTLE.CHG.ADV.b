* @ValidationCode : MjotNzkzMjYwNjAwOkNwMTI1MjoxNzAzMTM4ODQ2MDEwOmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 21 Dec 2023 11:37:26
...
SUBROUTINE REDO.AUT.SETTLE.CHG.ADV
*-----------------------------------------------------------------------------
*DESCRIPTION:
*------------
...
*04-04-2023         Conversion Tool       R22 Auto Code conversion      FM TO @FM,VM TO @VM , SM TO @SM
...
PROCESS:
    ...
    END
    ID.VAR='SYSTEM'
    CALL CACHE.READ(FN.REDO.H.AA.DIS.CHG,ID.VAR,R.REDO.H.AA.DIS.CHG,DIS.CHG.ERR)
...
* CALL AA.GET.PERIOD.BALANCES(ACCOUNT.ID, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "")
    AA.Framework.GetPeriodBalances(ACCOUNT.ID, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "") ;*R22 MANUAL CODE CONVERSION
    ...
    END ELSE
        IF PRIN.BALANCE LE Y.TOT.DIS.AMT THEN
            IF Y.DEB.AMT GE (PRIN.BALANCE - Y.TOT.AMT) THEN
                R.NEW(FT.CREDIT.AMOUNT) = PRIN.BALANCE - Y.TOT.AMT
                CURR.NO = DCOUNT(R.NEW(FT.OVERRIDE),@VM) + 1
                TEXT = 'REDO.CHG.ADV.AMT':@FM:Y.TOT.AMT
                CALL STORE.OVERRIDE(CURR.NO)

                Y.REM.AMT = PRIN.BALANCE - Y.TOT.AMT
                CURR.NO = DCOUNT(R.NEW(FT.OVERRIDE),@VM) + 1
                TEXT = 'REDO.REP.ADV.AMT':@FM:Y.REM.AMT
                CALL STORE.OVERRIDE(CURR.NO)
