* @ValidationCode : MjoxODM0MTc2MTkzOkNwMTI1MjoxNzAyOTYyMDYyODUxOmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 19 Dec 2023 10:31:02
...
SUBROUTINE REDO.V.TEMP.ADV.NO.INS
*-----------------------------------------------------------------------------
*DESCRIPTION:
*------------
...
* 05-06-2017        Edwin Charles D  R15 Upgrade         Initial Creation
...
PROCESS:
    ...
    Y.ADV.CNT = R.NEW(FT.TN.L.ADV.INS.CNT)
    Y.PART.CON = R.NEW(FT.TN.AA.PART.ALLOW)
...
* Y.BILLS = R.AA.ACCOUNT.DETAILS<AA.AD.BILL.ID>
    Y.BILLS = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillId>
...
* Y.ST = R.AA.BILL.DETAILS<AA.BD.SETTLE.STATUS,1>
        Y.ST = R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdSettleStatus,1>
    ...
    Y.ADV.BL = R.NEW(FT.TN.L.ADV.INS.CNT)
...
* Y.LAST.PART.AMT = SUM(R.AA.BILL.DETAILS<AA.BD.OS.PROP.AMOUNT>)* (Y.PARTIAL/100)
                Y.LAST.PART.AMT = SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>)* (Y.PARTIAL/100)
* Y.FIN.AMT = Y.FIN.AMT - SUM(R.AA.BILL.DETAILS<AA.BD.OS.PROP.AMOUNT>)+ Y.LAST.PART.AMT
                Y.FIN.AMT = Y.FIN.AMT - SUM(R.AA.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount>)+ Y.LAST.PART.AMT
            END ELSE
...
* Y.CNT = DCOUNT(R.AA.ACCOUNT.DETAILS<AA.AD.BILL.PAY.DATE>,@VM)
    Y.CNT = DCOUNT(R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetails.AdBillPayDate>,@VM)
    FLG = '' ; FIN.CNT = ''
    LOOP
    WHILE Y.CNT GT 0 DO
        FLG.VAL += 1
