* @ValidationCode : MjoxNjAxMzI5MTgyOkNwMTI1MjoxNzA0NzE1NDU0ODQ1OmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 08 Jan 2024 17:34:14
...
* <Rating>-139</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE REDO.V.VAL.DEF.LIM.AMT
***********************************************************
*----------------------------------------------------------
* COMPANY NAME    : APAP
...
*TUS change END
    ...
    CALL MULTI.GET.LOC.REF(APPLICATION,'L.FT.MSG.DESC',LREF.POS)
    FN.OVERRIDE='F.OVERRIDE'
    F.OVERRIDE=''
    CALL OPF(FN.OVERRIDE,F.OVERRIDE)
*    CALL F.READ(FN.OVERRIDE,'PART.PAY.WARN',R.OVERRIDE,F.OVERRIDE,ERR)
    IDVAR.1 = 'PART.PAY.WARN' ;* R22 UTILITY AUTO CONVERSION
    CALL F.READ(FN.OVERRIDE,IDVAR.1,R.OVERRIDE,F.OVERRIDE,ERR);* R22 UTILITY AUTO CONVERSION
    Y.PRT.WARN.MSG=R.OVERRIDE<EB.OR.MESSAGE>
    R.NEW(FT.LOCAL.REF)<1,LREF.POS>=''
    GOSUB LOC.REF
    GOSUB FUNDSTRANSFER
RETURN
*-------------------------------------------------------------
...
*-------------------------------------------------------------
    R.ECB = ''
    CALL CACHE.READ(FN.ECB,Y.ACCOUNT.ID,R.ECB,F.ECB.E.ECB) ;*R22 Manual Code Conversion-Call Method Format Modified
...
*    CALL AA.GET.PERIOD.BALANCES(OUT.ID, Y.BAL.TYPE, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "")
    AA.Framework.GetPeriodBalances(OUT.ID, Y.BAL.TYPE, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "");* R22 UTILITY AUTO CONVERSION
...
* if repayment is made through FT
    ...
    END
    ELSE
        IF R.NEW(FT.CREDIT.AMOUNT) LT Y.TOTAL.DUE THEN
            R.NEW(FT.LOCAL.REF)<1,LREF.POS>=Y.PRT.WARN.MSG
        END
    END
...
* To calculate the MIN & MAX amount and period for an arrangement
    ...
    R.ARRANGEMENT=AA.Framework.Arrangement.Read(Y.ARRANGEMENT.ID, ARR.ERR)
* Y.ARR.VALUE.DATE=R.ARRANGEMENT<AA.ARR.START.DATE>
    Y.ARR.VALUE.DATE=R.ARRANGEMENT<AA.Framework.Arrangement.ArrStartDate>
