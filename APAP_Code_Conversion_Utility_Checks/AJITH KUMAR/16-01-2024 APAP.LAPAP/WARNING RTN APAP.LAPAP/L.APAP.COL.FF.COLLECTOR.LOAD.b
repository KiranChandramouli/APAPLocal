* @ValidationCode : MjotMTQ2MDcxMDgyNzpDcDEyNTI6MTY5MTc1MTMyMzM1MTpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
* <Rating>-78</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE L.APAP.COL.FF.COLLECTOR.LOAD
**-----------------------------------------------------------------------------
* Basada en la logica de la rutina REDO.COL.FF.EXTRACT.LOAD routine to load the variables
*
*-----------------------------------------------------------------------------
*--------------------------------------------------------------------------------------------
...
* ----                  ----                                ----                 ----
* 09-08-2023           Ajith Kumar         R22 Manual Code Conversion     TAM.BP,LAPAP.BP,T24.BP IS REMOVED,VM FM to @FM,@VM
...
*$INSERT I_L.APAP.COL.CUSTOMER.COMMON
    ...
    $INSERT I_F.AA.PRODUCT ;*R22 MANUAL CODE CONVERSION
*-----------------------------------------------------------------------------
...
FN.REDO.INTERFACE.PARAM = 'F.REDO.INTERFACE.PARAM';* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.REDO.INTERFACE.PARAM, Y.RID.ID, R.REDO.INTERFACE.PARAM, Y.ERR);* R22 UTILITY AUTO CONVERSION
    IF Y.ERR THEN
...
*>> PACS00169639
    ...
    FV.AA.PRODUCT = ''
    CALL OPF (FN.AA.PRODUCT,FV.AA.PRODUCT)

**------------------------------------------------------------------todo
**--------------------nuevas tablas para escribir los registro temporales


    FN.LAPAP.CONCAT.TMP.CLIENTES = 'F.LAPAP.CONCAT.TMP.CLIENTES'
    FV.LAPAP.CONCAT.TMP.CLIENTES = '';
    CALL OPF (FN.LAPAP.CONCAT.TMP.CLIENTES,FV.LAPAP.CONCAT.TMP.CLIENTES)

    FN.LAPAP.CONCAT.TMP.TIPOGARANTIA = 'F.LAPAP.CONCAT.TMP.TIPOGARANTIA';
    FV.LAPAP.CONCAT.TMP.TIPOGARANTIA = ''
    CALL OPF (FN.LAPAP.CONCAT.TMP.TIPOGARANTIA,FV.LAPAP.CONCAT.TMP.TIPOGARANTIA)

    FN.LAPAP.CONCAT.TMP.GESGARANTIAS = 'F.LAPAP.CONCAT.TMP.GESGARANTIAS';
    FV.LAPAP.CONCAT.TMP.GESGARANTIAS = ''
    CALL OPF (FN.LAPAP.CONCAT.TMP.GESGARANTIAS,FV.LAPAP.CONCAT.TMP.GESGARANTIAS)
...
RAISE.ERROR:
*----------------------------------------------------------------------------
    SOURCE.INFO = "L.APAP.COL.FF.COLLECTOR.LOAD"
