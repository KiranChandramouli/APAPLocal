* @ValidationCode : Mjo0OTQ2NDY3MTA6Q3AxMjUyOjE3MDUzNzk2NDM2OTM6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 16 Jan 2024 10:04:03
...
*                        Ghayathri                       18/05/2023      R22 Manual Conversion - Call routine modified, insert $USING APAP.AAAPAP,$USING REDOSRTN,$USING APAP.TAM
*                                                        07/06/2023      R22 Manual Conversion - POS.L.AA.COL,POS.L.AA.COL.VAL and Y.CARGO.CANCELACION variable Added in I_REDO.B.MG.LNS.BY.DEBTOR.COMMON file
*                        Conversion Tool                 24/05/2023      R22 Auto Conversion - Changed T24.BP in insert, SM to @SM, VM to @VM, J to J.VAR,-- to -= 1 and ++ to +=1
*
*-----------------------------------------------------------------------------------------------------------------
* Include files
*-----------------------------------------------------------------------------------------------------------------
...
AA.Framework.GetArrangement(Y.AA.ARR.ID,R.ARRANGEMENT,ARR.ERR);* R22 UTILITY AUTO CONVERSION
...
*    YCO.CODE = R.ARRANGEMENT<AA.ARR.CO.CODE>
     ...
     Y.MAIN.PROD.GROUP = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProductGroup>
    Y.MAIN.ARR.STATUS = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrArrStatus>
    Y.MAIN.ARR.PRCT = R.ARRANGEMENT< AA.Framework.Arrangement.ArrProduct >
    YORG.CONT.DTE = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrOrigContractDate>
    YSTART.DTE = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrStartDate>
    YCO.CODE = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode>
...
**------------
    ...
    END
    ARR.ID = Y.AA.ARR.ID
    Y.CUSTOMER.ID = R.ARRANGEMENT< AA.Framework.ArrangementSim.ArrCustomer>
    Y.CURRENCY    = R.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency>
    AA.ID = Y.AA.ARR.ID
    APAP.AA.redoBConLnsByDebtorAaRecs(AA.ID,OUT.RECORD);*R22 Manual Conversion
...
*----------------------------------------------------------------------------------------------------------------------------------
    ...
    APAP.REDOSRTN.redoSRepCustomerExtract(Y.CUSTOMER.ID,Y.PRODUCT.GROUP,Y.REL.CODE,OUT.ARR);*R22 Manual Conversion
...
*-----------------------------
    Y.PRODUCT.LINE  = R.ARRANGEMENT< AA.Framework.ArrangementSim.ArrProductLine>
  ...
  *  Y.PROD          = R.ARRANGEMENT<AA.ARR.PRODUCT>
    ...
    Y.PROD          = R.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct>
*
    Y.LINKED.APPL    = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedAppl>
    Y.LINKED.APPL.ID = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>
...
**-----------------------------------------------------------
    ...
    APAP.LAPAP.redoSConCalcDisbDateAmt(IN.REC.VAL,OUT.REC.VAL);*R22 Manual Conversion
*
...
*-----------
    ...
    Y.GUAR.NO  = R.AA.TERM.AMOUNT<AA.AMT.LOCAL.REF,POS.L.AA.COL>;* R22 Manual conversion POS.L.AA.COL variable Added in I_REDO.B.MG.LNS.BY.DEBTOR.COMMON file
    Y.GUAR.VAL = R.AA.TERM.AMOUNT<AA.AMT.LOCAL.REF,POS.L.AA.COL.VAL>;* R22 Manual conversion POS.L.AA.COL.VAL variable Added in I_REDO.B.MG.LNS.BY.DEBTOR.COMMON file
...
*-------------------
    Y.CHG.RATE = ''
    Y.CHARGE.PROP = Y.CARGO.CANCELACION;*;* R22 Manual conversion Y.CARGO.CANCELACION variable Added in I_REDO.B.MG.LNS.BY.DEBTOR.COMMON file
*Y.CHARGE.PROP = 'PRCANCANTIC'
    ...
    R.CONDITION = ''
    ERR.MSG = ''
    APAP.AA.redoCrrGetConditions(Y.AA.ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION,ERR.MSG);*R22 Manual Conversion
...
*-----------------
    ...
    DUE.TYPE.AMTS = ''; DUE.PROPS = ''; DUE.PROP.AMTS = ''; TOT.PAY = ''; FLG.VAL1 = 0; FLG.VAL2 = 0; Y.VAR = 0
    ...
    J.VAR = 1
    LOOP
    WHILE J.VAR LE TOT.DUE.DATE
        SCHEDULE.DATE = DUE.DATES<J.VAR>
    ...
    IF YBILL.AMT EQ 0 OR NOT(YBILL.AMT) THEN
        ...
        FINDSTR 'ACCOUNT' IN DUE.PROPS<Y.VAR> SETTING FMPOS1,VMPOS1,SMPOS1 THEN
            YBILL.AMT = TOT.PAY<Y.VAR,FMPOS1>
        END
        FMPOS1 = ''; VMPOS1 = ''; SMPOS1 = ''
        FINDSTR 'PRINCIPALINT' IN DUE.PROPS<Y.VAR> SETTING FMPOS1,VMPOS1,SMPOS1 THEN
            YBILL.AMT = TOT.PAY<Y.VAR,FMPOS1>
        END
    END
RETURN
...
************
    FINDSTR 'ACCOUNT' IN DUE.PROPS<J.VAR> SETTING FMPOS,VMPOS,SMPOS THEN
        YACCT.RANGE.AMT = DUE.PROP.AMTS<J.VAR,VMPOS,SMPOS>
    END
    FINDSTR 'SOLO.CAPITAL' IN DUE.TYPES<J.VAR> SETTING FMS.POS,VMS.POS,SMS.POS THEN
        YSC.RANGE.AMT = DUE.PROP.AMTS<J.VAR,VMS.POS,SMS.POS>
    END
...
SCHED.VAL.2:
************
    FINDSTR 'ACCOUNT' IN DUE.PROPS<J.VAR> SETTING FMPOS1,VMPOS1,SMPOS1 THEN
        YBILL.AMT = TOT.PAY<J.VAR,FMPOS1>
    END
    IF YBILL.AMT EQ 0 OR NOT(YBILL.AMT) THEN
        FMPOS1 = ''; VMPOS1 = ''; SMPOS1 = ''
        FINDSTR 'PRINCIPALINT' IN DUE.PROPS<J.VAR> SETTING FMPOS1,VMPOS1,SMPOS1 THEN
            YBILL.AMT = TOT.PAY<J.VAR,FMPOS1>
        END
    END
RETURN
...
********************
   ...
   * CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.AA.ARR.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,ERR.AA.ACTIVITY.HISTORY)
    R.AA.ACTIVITY.HISTORY=AA.Framework.ActivityHistory.Read(Y.AA.ARR.ID, ERR.AA.ACTIVITY.HISTORY)
...
AA.Framework.GetPeriodBalances(Y.LOAN.CODE, BAL.TYPE1,REQUEST.TYPE, START.DATE, END.DATE, '',BAL.DETAILS, ERROR.MESSAGE);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.ACCT.ID, BAL.TYPE1,REQUEST.TYPE, START.DATE, END.DATE, '',BAL.DETAILS, ERROR.MESSAGE);* R22 UTILITY AUTO CONVERSION
