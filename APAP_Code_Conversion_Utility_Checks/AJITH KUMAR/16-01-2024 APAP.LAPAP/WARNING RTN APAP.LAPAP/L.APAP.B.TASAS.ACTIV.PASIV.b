* @ValidationCode : MjotMzM0Nzc1OTQyOkNwMTI1MjoxNzAyOTg4MzQ2MzI0OklUU1MxOi0xOi0xOjA6MTpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
...
*13-07-2023    AJITHKUMAR S        R22 MANUAL CONVERSION   CALL RTN FORMAT CAN BE CHANGED , VARAIABLE CHANGE
...
*   $INSERT I_F.AA.ACTIVITY.HISTORY
    ...
    $USING APAP.TAM
    $USING AA.Framework ;*R22 Manual Code Conversion_Utility Check
...
*           IF NOT(CUS.ID) THEN RETURN
            IF NOT(CUS.ID) THEN ;*R22 Manual Code Conversion_Utility Check
                RETURN
            END
...
*           CALL F.READ(FN.AA.ARRANGEMENT,REC.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,AA.ARRANGEMENT.ERR)
            R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(REC.ID, AA.ARRANGEMENT.ERR) ;*R22 Manual Code Conversion_Utility Check
            GOSUB PROCESS.AA
...
*    CALL DR.REP.AC.INT.CALC(REC.ID,R.ACCOUNT,INT.RATE)
    APAP.LAPAP.drRepAcIntCalc(REC.ID, R.ACCOUNT, INT.RATE) ;*R22 MANUAL CONVERSION
...
*    YCO.CODE = R.AA.ARRANGEMENT<AA.ARR.CO.CODE>
    ...
    CUS.ID = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency>
    STRT.DATE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrStartDate>
    YACCT.ID = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>
    Y.PRODUCT = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct>
    YCO.CODE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode>
...
AA.PaymentSchedule.ScheduleProjector(REC.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS,DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS);* R22 UTILITY AUTO CONVERSION
...
*CALL REDO.GET.DISBURSEMENT.DETAILS(Y.IDS.DETAILS,R.DISB.DETAILS,Y.COMMITED.AMT,Y.PEND.DISB)
    APAP.TAM.redoGetDisbursementDetails(Y.IDS.DETAILS,R.DISB.DETAILS,Y.COMMITED.AMT,Y.PEND.DISB)
...
*    CALL F.READ(FN.REDO.APAP.INSTIT.FINANC.PARAM,YCUS.RNC,R.REDO.APAP.INSTIT.FINANC.PARAM,F.REDO.APAP.INSTIT.FINANC.PARAM,ERR.REDO.APAP.INSTIT.FINANC.PARAM)
    CALL CACHE.READ(FN.REDO.APAP.INSTIT.FINANC.PARAM,YCUS.RNC,R.REDO.APAP.INSTIT.FINANC.PARAM,ERR.REDO.APAP.INSTIT.FINANC.PARAM);* R22 UTILITY AUTO CONVERSION
...
*   IF LEN(YDATE) <> 8 OR LEN(YDATE1) <> 8 THEN
    IF LEN(YDATE) NE 8 OR LEN(YDATE1) NE 8 THEN ;*R22 Manual Code Conversion_Utility Check
        Y.DAYS = 0
        RETURN
    END
*   IF YDATE[1,4] > YDATE1[1,4] THEN
    IF YDATE[1,4] GT YDATE1[1,4] THEN ;*R22 Manual Code Conversion_Utility Check
        TMP.YDATE1 = YDATE1
        TMP.YDATE = YDATE
        YDATE1 = TMP.YDATE
        YDATE = TMP.YDATE1
...
*    CALL F.READ(FN.AZ.PRODUCT.PARAMETER,CAT.VAL,R.AZ.PRODUCT.PARAMETER,F.AZ.PRODUCT.PARAMETER,AZ.PRODUCT.PARAMETER.ERR)
    CALL CACHE.READ(FN.AZ.PRODUCT.PARAMETER,CAT.VAL,R.AZ.PRODUCT.PARAMETER,AZ.PRODUCT.PARAMETER.ERR);* R22 UTILITY AUTO CONVERSION
