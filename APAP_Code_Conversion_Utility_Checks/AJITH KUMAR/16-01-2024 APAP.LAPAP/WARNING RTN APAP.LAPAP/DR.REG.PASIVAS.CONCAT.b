* @ValidationCode : MjozMzY5Nzg5NDA6Q3AxMjUyOjE2ODY2NzQyNDUyNDI6SVRTUzotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
...
* 06-MAY-2023     Conversion tool    R22 Auto conversion       FM TO @FM, VM to @VM, SM to @SM, ++ to +=, = to EQ, -- to -=
* 06-MAY-2023      Harishvikram C   Manual R22 conversion     R.DR.REG.PASIVAS.PARAM added in insert file 'I_DR.REG.PASIVAS.CONCAT.COMMON', CALL routine format modified
*----------------------------------------------------------------------------
...
********
    ...
    ERR.ACCT.LWORK = ''; R.ACCT.ENT.LWORK.DAY = ''; R.AZ.ACCOUNT = ''; AZ.ACCOUNT.ERR = ''; Y.VAR = 0
    BEGIN CASE
        ...
        CASE CONTROL.LIST<1,1> EQ "GRP.16"
            ...
            IF CR.DATE LT LAST.WORK.DAY THEN
                RETURN
            END
            Y.VAR = CNT.STMT.ID
            LOOP
            WHILE CTR.STMT.ID LE CNT.STMT.ID
                STMT.ID = R.ACCT.ENT.LWORK.DAY<Y.VAR>
...
***************
    ...
    IF BANK.EMP.FLAG EQ 'NO' THEN
        APAP.LAPAP.drRepAcIntCalc(REC.ID,R.ACCOUNT,INT.RATE) ;*Manual R22 conversion
...
*************
    ...
    IF LEN(YDATE) NE 8 OR LEN(YDATE1) NE 8 THEN
        ...
        RETURN
    END
    IF YDATE[1,4] GT YDATE1[1,4] THEN
        TMP.YDATE1 = YDATE1
        TMP.YDATE = YDATE
        YDATE1 = TMP.YDATE
        YDATE = TMP.YDATE1
