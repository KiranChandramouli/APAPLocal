...
*    IF INDEX(Y.ENQ.SEL.BRANCH,"DO",1) EQ 0 THEN ;* AUTO R22 CODE CONVERSION = TO EQ
    IF INDEX(Y.ENQ.SEL.BRANCH,"DO",1) EQ 0 THEN ;* AUTO R22 CODE CONVERSION EQ TO EQ;* R22 UTILITY AUTO CONVERSION
...
*            IF Y.MAX LT CURR.FROM.ID THEN ;* AUTO R22 CODE CONVERSION < TO LT
            IF Y.MAX LT CURR.FROM.ID THEN ;* AUTO R22 CODE CONVERSION LT TO LT;* R22 UTILITY AUTO CONVERSION
                Y.MAX=Y.MAX
            END ELSE
                Y.MAX=CURR.FROM.ID
            END
