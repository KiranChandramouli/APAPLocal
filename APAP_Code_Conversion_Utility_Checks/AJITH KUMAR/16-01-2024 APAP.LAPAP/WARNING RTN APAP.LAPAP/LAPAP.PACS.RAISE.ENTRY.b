* @ValidationCode : Mjo1MDk0Njg3Njc6Q3AxMjUyOjE3MDUwMzQyMzA3MDk6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 12 Jan 2024 10:07:10
...
*-----------------------------------------------------------------------------
*-----------------------------------------------------------------------------
SUBROUTINE LAPAP.PACS.RAISE.ENTRY(Y.LIST.NAME)
**==================================================================================================================================
*
...
*
* ARR*TRANSACTION.CODE#CURACCOUNT*100#DELACCOUNT*-50#PL51001*-30#GBP1400010001*-20
*
**==================================================================================================================================
************************************* CHANGE HISTORY ***************************************
* 08/03/2016 - Tamil.S - Performance fix on COPY.ACCOUNT
*
* 18/03/2016 - Sharmie - Routine modified to post entries correctly if customer account is involved in the SAVEDLISTS
*
* 30/03/2016 - Ramkumar
* Transaction code has to be passed in input file so that bank can show narrative in statement entry as per their requirement,
* JOURNAL.UPDATE added after deletion of ACCOUNT which has been moved from history to Live to generate correction entry.
* If, transaction code not passed then 851 will be defaulted
* ARR*TRANSACTION.CODE#CURACCOUNT*100#DELACCOUNT*-50#PL51001*-30#GBP1400010001*-20
*
* 31/03/2016 - Ramkumar
* Routine amended in such a way to raise accounting entry only if we pass account id. account id should be in Live.
* ARR_Account_number*TRANSACTION.CODE#CURACCOUNT*100#DELACCOUNT*-50#PL51001*-30#GBP1400010001*-20
*
* 31/03/2016 - Pandiarajan - TAFJ compatiablity  (We changed the name back to PACS.RAISE.ENTRY from PACS.RAISE.ENTRY.TAFC)
*
* 06/04/2016 - Pandiarajan - output SL name changed to include date and time ("CORR.ENTRIES_":COMO.DATE:"_":COMO.TIME)
*
* Modified By - Rajkumar T
* Date        - 18 May 2016
* Details     - If the history ACCOUNT record copied to live ACCOUNT record. Some cases F.READ is not working , so
*               READ command used to read the account record.
...
**==================================================================================================================================
    COMO.DATE=TODAY
    COMO.TIME=TIME()
    COMO.NAME = "PACS.RAISE.ENTRY_":COMO.DATE:"_":COMO.TIME
...
*LIST.NAME = "AA.ADJ.BAL"
    LIST.NAME = Y.LIST.NAME
RETURN
**==================================================================================================================================
*=======
PROCESS:
...
ST.CompanyCreation.LoadCompany(R.ARRANGEMENT<AA.ARR.CO.CODE>);* R22 UTILITY AUTO CONVERSION
            END
...
ST.CompanyCreation.LoadCompany(R.ACCOUNT<AC.CO.CODE>);* R22 UTILITY AUTO CONVERSION
            END
...
FN.AC.BALANCE.TYPE = "F.AC.BALANCE.TYPE";* R22 UTILITY AUTO CONVERSION
                CALL CACHE.READ(FN.AC.BALANCE.TYPE, BAL.TYPE, BALANCE.RECORD, BAL.ERR);* R22 UTILITY AUTO CONVERSION
...
AC.API.EbAccounting('AAA',"SAO",ENTRIES,"");* R22 UTILITY AUTO CONVERSION
...
*==========
    ...
    DTEXT = "   !!! Correction Completed !!!   Press Y/y To Exit....."
    LOOP
        CRT @(1,12):DTEXT
** TAFJ not working / S
*INPUT @(60,12):YIY
*IF YIY EQ "Y" OR YIY EQ "y" THEN EXIT
** TAFJ not working / E
        EXIT
    REPEAT
...
*CRT 'Invalid transaction code used for arrangement id :':ARR.NO:' (Correction entry wont get generated!!)'
        TEXT = 'Invalid transaction code used for arrangement id :':ARR.NO:' (Correction entry wont get generated!!)'
        E = TEXT
        EXIT.FLAG = 1
