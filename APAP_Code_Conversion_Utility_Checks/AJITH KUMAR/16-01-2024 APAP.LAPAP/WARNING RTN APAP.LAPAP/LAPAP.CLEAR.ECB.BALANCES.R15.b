* @ValidationCode : MjoyNDk1MDEwMzU6Q3AxMjUyOjE3MDQ4MDM4NjY4NDI6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 09 Jan 2024 18:07:46
...
$PACKAGE APAP.LAPAP
SUBROUTINE LAPAP.CLEAR.ECB.BALANCES.R15(Y.AA.ID,Y.INTERNA.ACCOUNT)
*=====================================================================
...
*21-04-2023      conversion tool     R22 Auto code conversion      = TO EQ,VM TO @VM
...
*=====================================================================
    PRINTTIME = TIMEDATE()
    EXECUTE 'COMO ON PACS.CLEAR.ECB.BALANCES-':PRINTTIME
    GOSUB INITIALISE
    GOSUB GET.ARRANGEMENT
    GOSUB CHECK.PROCESS.REQD
    IF DONT.PROCESS NE 1 THEN
        GOSUB UPDATE.ASA
        GOSUB CLEAR.ECB
        GOSUB UPDATE.BILLS
        GOSUB RAISE.ENTRY
...
*CALL JOURNAL.UPDATE(AA.ID)
        EXECUTE 'COMO OFF PACS.CLEAR.ECB.BALANCES-':PRINTTIME
    END
RETURN
*=====================================================================
INITIALISE:
...
*READ RREC FROM F$SAVEDLISTS, 'SL.CLEAR.AA' ELSE NULL
...
*CRT "ENTER THE INTERNAL ACCOUNT NUMBER U LIKE TO USE FOR THIS CORRECTION:"
*INPUT Y.INTERNAL.AC

    Y.INTERNAL.AC = Y.INTERNA.ACCOUNT
    AA.ID = Y.AA.ID
RETURN
*=====================================================================
GET.ARRANGEMENT:
   ...
   * CO.CODE = R.ARRANGEMENT<AA.ARR.CO.CODE,1>
    ARR.STATUS = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrArrStatus>

    AC.ID =  R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId,1>
    PRODUCT.LINE = R.ARRANGEMENT< AA.Framework.ArrangementSim.ArrProductLine>
    CO.CODE = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode ,1>
RETURN
*=====================================================================
CHECK.PROCESS.REQD:
    DONT.PROCESS = ""
...
*    CALL F.READ(FN.ASA, AA.ID, R.ASA, F.ASA, ASA.ERR)
    CALL F.READU(FN.ASA, AA.ID, R.ASA, F.ASA, ASA.ERR,'');* R22 UTILITY AUTO CONVERSION
    IF NOT(ASA.ERR) THEN
       * CNT.ACT = DCOUNT(R.ASA<AA.SCH.ACTIVITY.NAME>, @VM)
         CNT.ACT = DCOUNT(R.ASA<AA.Framework.ScheduledActivity.SchActivityName>, @VM)
           ...
           * IF R.ASA<AA.SCH.NEXT.DATE ,IDX> THEN
            *    R.ASA<AA.SCH.LAST.DATE ,IDX> = R.ASA<AA.SCH.NEXT.DATE ,IDX>
             *   R.ASA<AA.SCH.NEXT.DATE ,IDX> = ""
                IF R.ASA< AA.Framework.ScheduledActivity.SchNextDate ,IDX> THEN
                R.ASA< AA.Framework.ScheduledActivity.SchLastDate ,IDX> = R.ASA< AA.Framework.ScheduledActivity.SchNextDate ,IDX>
                R.ASA< AA.Framework.ScheduledActivity.SchNextDate ,IDX> = ""
            END
        NEXT IDX

        NEXT.ACT.ID = AA.ID:'-':R.ASA< AA.Framework.ScheduledActivity.SchNextRunDate>
        NEXT.LEAST.DATE = ""
        R.ASA< AA.Framework.ScheduledActivity.SchNextRunDate> = NEXT.LEAST.DATE

*        CALL F.READ(FN.ANA, NEXT.ACT.ID, R.ANA, F.ANA, ANA.ERR)
        CALL F.READU(FN.ANA, NEXT.ACT.ID, R.ANA, F.ANA, ANA.ERR,'');* R22 UTILITY AUTO CONVERSION
...
*    CALL F.READ(FN.AAD, AA.ID, R.AAD, F.AAD, AAD.ERR)
    CALL F.READU(FN.AAD, AA.ID, R.AAD, F.AAD, AAD.ERR,'');* R22 UTILITY AUTO CONVERSION
...
*    CALL F.READ(FN.ABD, BILL.ID, R.BILL.RECORD, F.ABD, ABD.ERR)
    CALL F.READU(FN.ABD, BILL.ID, R.BILL.RECORD, F.ABD, ABD.ERR,'');* R22 UTILITY AUTO CONVERSION
    ...
    IF NOT(ECB.ERR) THEN
        ...
        FOR BALANCE.TYPE.POS = 1 TO TOTAL.BALANCES
            ...
            IF BALANCE.TYPE.POS EQ TOTAL.BALANCES AND OPP.BAL.AMOUNT NE 0 THEN   ;* If its last balance type then need to add the opposite entry to internal account
                DIFF.ARR = DIFF.ARR:'#':Y.INTERNAL.AC:'*':OPP.BAL.AMOUNT*(-1)
            END
...
FN.AC.BALANCE.TYPE = "F.AC.BALANCE.TYPE";* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.AC.BALANCE.TYPE, BALANCE.TYPE, BALANCE.RECORD, TABLE.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(AC.ID, REQD.BALANCE.TYPE, REQUEST.TYPE, "TODAY", "", "", BALANCE.DETAILS, RET.ERR);* R22 UTILITY AUTO CONVERSION
