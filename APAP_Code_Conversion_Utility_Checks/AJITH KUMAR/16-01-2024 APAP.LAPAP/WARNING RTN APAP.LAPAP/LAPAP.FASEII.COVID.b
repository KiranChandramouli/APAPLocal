* @ValidationCode : MjotNTcwODk0Nzk2OkNwMTI1MjoxNzA1MDMzNDUwMDk3OmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 12 Jan 2024 09:54:10
...
*Lo anterior es válido la propiedad completa, es decir para los campos F.INICIO y F.FINAL.
*----------------------------------------------
*------------------------------------------------------------------------
...
*Verificar si el prestamo pertenece a la FASEI. Sino, excluir
        ...
        X.VAR = 0;
        FOR X.VAR = 1 TO Y.CNT1
            IF Y.PAYMENT.TYPE<X.VAR> EQ 'SOLO.INTERES' AND Y.START.DATE.PAY<X.VAR> EQ Y.END.DATE.PAY<X.VAR> THEN
                Y.POSEE.FASEI           = 'SI';
                Y.FECHA.EFECTIVA.GRUP4  = Y.END.DATE.PAY<X.VAR>;
            END
...
*Si la fecha de cualquiera de los payments del plan de pago es igual al TODAY, enviarle como fecha efectiva el LAST.WORK.DATE
            IF Y.START.DATE.PAY<X.VAR> EQ Y.TODAY THEN
...
*Si no vienen polizas o cargos en el archivo solo buscar el paymet CONSTANTE y  SOLO.INTERES
    ...
    END ELSE
        ...
        FOR J.VAR = 1 TO Y.CNT.POLIZA
            ...
            Y.DATA  = CHANGE(Y.TIPO.POLIZA.MONTO<J.VAR>,'*',@FM);
...
AA.Framework.GetPeriodBalances(Y.NUMERO.PRESTAMO, BAL.TYPES, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, ERROR.MESSAGE);* R22 UTILITY AUTO CONVERSION
