* @ValidationCode : MjotNDkzMzQxMjM6Q3AxMjUyOjE3MDUwNTI0Mjk5NzI6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 12 Jan 2024 15:10:29
...
*13-07-2023            Conversion Tool             R22 Auto Code conversion                FM TO @FM,VM TO @VM,SM TO @SM,++ TO +=1 ,INSERT FILE MODIFIED
...
*------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  *  $INSERT I_F.AA.ARRANGEMENT
   ...
   * $INSERT I_F.AA.ACTIVITY.HISTORY  ;*R22 AUTO CODE CONVERSION.END
...
*--------------
    ...
    IF Y.AA.OWNER THEN
        Y.DCNT.OWNER = DCOUNT(Y.AA.OWNER,@VM)  ;*R22 AUTO CODE CONVERSION
        Y.STA.COUNT = '1'
...
*-------------
    ...
    WHILE Y.STA.COUNT LE Y.DCNT.OWNER
        ...
        LOCATE AA.ARR.ID IN AA.ARR.TEMP<1> SETTING AA.POSN THEN
            Y.STA.COUNT += 1 ;*R22 AUTO CODE CONVERSION
            CONTINUE
        END
...
AA.Framework.GetArrangement(AA.ARR.ID,R.ARR.APPL,ARR.ERR);* R22 UTILITY AUTO CONVERSION
  ...
  *  Y.MAIN.STRT.DTE = R.ARR.APPL<AA.ARR.START.DATE>
    ...
    Y.MAIN.ARR.STATUS = R.ARR.APPL<AA.Framework.ArrangementSim.ArrProductLine>
    Y.MAIN.ARR.PRCT = R.ARR.APPL<AA.Framework.ArrangementSim.ArrProduct>
 ...
 *   Y.LINKED.APPL.ID = R.ARR.APPL<AA.ARR.LINKED.APPL.ID>
    ...
    Y.CURRENCY    = R.ARR.APPL<AA.Framework.ArrangementSim.ArrCurrency>
    Y.LINKED.APPL = R.ARR.APPL<AA.Framework.ArrangementSim.ArrLinkedAppl>
...
*------------
    ...
    IF R.EB.CON.BAL THEN
        ...
        CTR.BAL.TYPE = 1
        CNT.BAL.TYPE = DCOUNT(Y.ASSET.TYPE,@VM) ;*R22 AUTO CODE CONVERSION
        LOOP
        WHILE CTR.BAL.TYPE LE CNT.BAL.TYPE
            ACC.POS = ''
            BAL.TYPE1 = Y.ASSET.TYPE<1,CTR.BAL.TYPE>
            IF BAL.TYPE1[1,3] EQ 'UNC' THEN
                CTR.BAL.TYPE += 1 ;*R22 AUTO CODE CONVERSION
                CONTINUE
            END
...
***************
    ...
    IF YCOM.FLG EQ 1 AND DAT.BALANCES THEN
        Y.FIN.ECB.AMT.COM += DAT.BALANCES ;*R22 AUTO CODE CONVERSION
        IF REQ.LEN EQ 'ACCOUNT' THEN
            YFIN.ECB.COM += DAT.BALANCES ;*R22 AUTO CODE CONVERSION
        END
    END
    IF YCON.FLG EQ 1 AND DAT.BALANCES THEN
        Y.FIN.ECB.AMT.CONS += DAT.BALANCES ;*R22 AUTO CODE CONVERSION
    END
    IF YHIP.FLG EQ 1 AND DAT.BALANCES THEN
        Y.FIN.ECB.AMT.HIP += DAT.BALANCES ;*R22 AUTO CODE CONVERSION
    END
    DAT.BALANCES = ''
RETURN
