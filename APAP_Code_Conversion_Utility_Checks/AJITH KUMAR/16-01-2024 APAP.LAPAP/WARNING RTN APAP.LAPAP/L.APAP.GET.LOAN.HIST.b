* @ValidationCode : Mjo3NTMxNTAzMDE6Q3AxMjUyOjE3MDQ4MDAyMDE3MzI6YWppdGg6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 09 Jan 2024 17:06:41
...
*****************
   ...
   R.AA.ARRANGEMENT=  AA.Framework.Arrangement.Read(Y.ARR.ID, ERR.AA.ARRANGEMENT) 
    Y.LINKED.APPL = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedAppl>
    Y.LINKED.APPL.ID = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedAppl>
    Y.START = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrStartDate ,1>
    LOCATE "ACCOUNT" IN Y.LINKED.APPL<1,1> SETTING Y.LINK.POS THEN
        YACCT.ID =Y.LINKED.APPL.ID<Y.LINK.POS>
    END

    Y.ARR.ESTATUS = R.AA.ARRANGEMENT< AA.Framework.ArrangementSim.ArrArrStatus>
 ...
 *   CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.ARR.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,ERR.AA.ACTIVITY.HISTORY)
   R.AA.ACTIVITY.HISTORY= AA.Framework.ActivityHistory.Read(Y.ARR.ID, ERR.AA.ACTIVITY.HISTORY)
    YACT.ID.HIST = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistory.AhArcId>
...
******************CONSULTA AL LIVE HISTORICO******************
  *  CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.ARR.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,ERR.AA.ACTIVITY.HISTORY)
    R.AA.ACTIVITY.HISTORY=AA.Framework.ActivityHistory.Read(Y.ARR.ID, ERR.AA.ACTIVITY.HISTORY) 
    YACT.RECS = R.AA.ACTIVITY.HISTORY< AA.Framework.ActivityHistoryHist.AhActivity>
    YACT.IDR = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistory.AhActivityRef>
...
***************************************************************
    ...
    YACT.REF.BAL = R.AA.ACT.BAL<AA.Framework.ActivityBalances.ActBalActivityRef>
    ...
    WHILE YACT.ID:AA.POSN
        ...
        YORG.AMT  = R.AAA<AA.Framework.RrControl.ArrActOrigTxnAmt>
        YCO.CODE = R.AAA<AA.Framework.SimulationCapture.ArrActCoCode>[8,2]
        YEFF.DATE = R.AAA< AA.Framework.SimulationCapture.ArrActEffectiveDate>
        YACTIVITY = R.AAA<AA.Framework.RrControl.ArrActActivity>
        YFIELD.VALUE = R.AAA<AA.Framework.RrControl.ArrActActivity>
        YSTMT.NOS = R.AAA<AA.Framework.ArrangementActivity.ArrActStmtNos>
***---------------------------campos para nueva logica.
*** Si la actividad no tiene un FT, asociado se excluye
        Y.TXN.CONTRACT.ID = R.AAA< AA.Framework.ArrangementActivity.ArrActTxnContractId>
        Y.TXN.SYSTEM.ID = R.AAA<AA.Framework.SimulationCapture.ArrActTxnSystemId>
...
***------------------------------------------------------
        PAGO.DTE = YEFF.DATE
        Y.TRANSFER.DATETIME = PAGO.DTE : R.AAA< AA.Framework.ArrangementActivity.ArrActDateTime>[7,10]
        Y.ID.TRANSACCION =  R.AAA< AA.Framework.ArrangementActivity.ArrActDateTime>
...
********************
    ...
    WHILE Y.ACHIS DO
        ...
        YACT.RECS = R.AA.ACTIVITY.HISTORY.HIS< AA.Framework.ActivityHistoryHist.AhActivity>
        YACT.IDR = R.AA.ACTIVITY.HISTORY.HIS<AA.Framework.ActivityHistory.AhActivityRef>
        CHANGE @SM TO @FM IN YACT.IDR
        CHANGE @VM TO @FM IN YACT.IDR
        YCNT = DCOUNT(YACT.IDR,@FM)
        BAL.ERR = ''; R.AA.ACT.BAL = '' ; YACT.REF.BAL = ''
        CALL F.READ(FN.AA.ACT.BAL,Y.ARR.ID,R.AA.ACT.BAL,F.AA.ACT.BAL,BAL.ERR)
        YACT.REF.BAL = R.AA.ACT.BAL<AA.Framework.ActivityHistory.AhActivityRef>
        YTMP.CNT = 0
        ...
        WHILE YACT.ID:AA.POSN
            ...
            YORG.AMT  = R.AAA< AA.Framework.RrControl.ArrActOrigTxnAmt>
            YCO.CODE = R.AAA< AA.Framework.RrControl.ArrActOrigTxnAmt>
            YEFF.DATE = R.AAA< AA.Framework.RrControl.ArrActOrigTxnAmt>
            YACTIVITY = R.AAA<AA.Framework.RrControl.ArrActActivity>
            YFIELD.VALUE = R.AAA<AA.Framework.SimulationCapture.ArrActFieldValue>
            YSTMT.NOS = R.AAA<AA.Framework.ArrangementActivity.ArrActStmtNos>
*PAGO.DTE = OCONV(ICONV(YEFF.DATE,'D'),'D')
            PAGO.DTE = YEFF.DATE
            Y.TRANSFER.DATETIME = PAGO.DTE : R.AAA<AA.Framework.ArrangementActivity.ArrActDateTime>[7,10]
            Y.ID.TRANSACCION =  R.AAA<AA.Framework.ArrangementActivity.ArrActTxnContractId>
...
AA.Framework.GetPeriodBalances(YACCT.ID, BALANCE.NAME, VALUE.TRADE, Y.DISBURSE.DATE, Y.DISBURSE.DATE, "", BAL.DETS, ERR.PROCESS);* R22 UTILITY AUTO CONVERSION
...
********
    ...
    LOCATE YACT.ID IN YACT.REF.BAL<1,1> SETTING POS.AAA THEN
        ...
        Y.PROPS = R.AA.ACT.BAL<AA.Framework.ActivityBalances.ActBalProperty ,POS.AAA>

*MONTO ORIGINAL
        Y.PROPS.AMTS = R.AA.ACT.BAL<AA.Framework.ActivityBalances.ActBalPropertyAmt,POS.AAA>
