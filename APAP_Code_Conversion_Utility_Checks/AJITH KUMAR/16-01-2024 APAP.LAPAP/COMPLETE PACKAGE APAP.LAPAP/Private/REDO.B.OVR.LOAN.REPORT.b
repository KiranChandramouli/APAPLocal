* @ValidationCode : MjoxMDMzNDU3MDQ1OkNwMTI1MjoxNzA1MzgwODY1ODE0OmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 16 Jan 2024 10:24:25
...
*13/07/2023      Conversion tool            R22 Auto Conversion           FM TO @FM, VM TO @VM, SM TO @SM,++ TO +=, INCLUDE TO INSERT, BP removed in INSERT file, F.READ TO CACHE.READ
 ...
 *   Y.START = R.AA<AA.ARR.START.DATE,1>
    ...
    Y.PRD.GRP = R.AA<AA.Framework.ArrangementSim.ArrProductGroup,1>
    Y.PRD = R.AA<AA.Framework.ArrangementSim.ArrProduct,1>
    Y.PRODUCTO = R.AA<AA.Framework.ArrangementSim.ArrProduct,1>
    Y.CURR = R.AA<AA.Framework.Arrangement.ArrCurrency,1>
    Y.COMP = R.AA<AA.Framework.ArrangementSim.ArrCoCode,1>
    Y.AC.ID = R.AA<AA.Framework.ArrangementSim.ArrLinkedApplId,1>
    Y.CUS = R.AA<AA.Framework.ArrangementSim.ArrCustomer,1>
...
*---CODIGO SUCURSAL ORIGINACION
    ...
    Y.AA.CO.CODE = R.AA<AA.Framework.ArrangementSim.ArrCoCode, 1>
...
EB.LocalReferences.GetLocRef("CUSTOMER","L.CU.SCO.COB",Y.CU.CALIF.RIESGO.POS);* R22 UTILITY AUTO CONVERSION
...
*---OFICIAL DE CUENTA
    ...
    CALL CACHE.READ(FN.DEPT.ACCT.OFFICER, Y.CUS.ACCT.OFFICER, R.DEPT.ACCT, ERROR.DEPT.ACCT) ;*R22 Auto Conversion
...
*---SUCURSAL DE CARTERIZACION
    ...
    CALL CACHE.READ(FN.DEPT.ACCT.OFFICER, Y.CUS.OTHER.OFFICER, R.DEPT.ACCT.SUC, ERROR.DEPT.ACCT.SUC) ;*R22 Auto Conversion
...
EB.LocalReferences.GetLocRef("AA.ARR.OVERDUE", "L.STATUS.CHG.DT",L.STATUS.CHG.DT.POS);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetArrangementConditions(Y.AA.ID,'INTEREST','PRINCIPALINTWOF','','',RET.PR.OV,OV.ERR);* R22 UTILITY AUTO CONVERSION
            RET.PR.OV = RAISE(RET.PR.OV)
            Y.RATE = RET.PR.OV<AA.INT.EFFECTIVE.RATE>
        END
...
*******
    ...
    IF R.AA.HIS THEN
        ARC.CNT = DCOUNT(R.AA.HIS<AA.Framework.ActivityHistoryHist.AhArcDateTill>,@VM)
        IF ARC.CNT GE 1 THEN
            Y.START = R.AA.HIS<AA.Framework.ActivityHistoryHist.AhArcDateTill,ARC.CNT>
        END
    END

    REQD.MODE = ''; EFF.DATE = Y.START; R.AA.HIS = ''
*    CALL AA.READ.ACTIVITY.HISTORY(Y.AA.ID, REQD.MODE, EFF.DATE, R.AA.HIS)
AA.Framework.ReadActivityHistory(Y.AA.ID, REQD.MODE, EFF.DATE, R.AA.HIS);* R22 UTILITY AUTO CONVERSION
    Y.ACTY = R.AA.HIS<AA.Framework.ActivityHistoryHist.AhActivity> ; Y.ACTY = CHANGE(Y.ACTY,@SM,@VM) ; Y.ACTY.DUP = Y.ACTY
    Y.ACTY.REF = R.AA.HIS<AA.Framework.ActivityHistoryHist.AhActivityRef> ; Y.ACTY.REF = CHANGE(Y.ACTY.REF,@SM,@VM); Y.ACTY.REF.DUP = Y.ACTY.REF
    Y.ACT.STS = R.AA.HIS<AA.Framework.ActivityHistory.AhActStatus> ; Y.ACT.STS = CHANGE(Y.ACT.STS,@SM,@VM) ; CK = '' ; Y.ACT.STS.DUP = Y.ACT.STS
    Y.INT.TYPES = R.AA.HIS< AA.Framework.ActivityHistory.AhInitiation > ; Y.INT.TYPES = CHANGE(Y.INT.TYPES,@SM,@VM) ; Y.LAST.REP.DATE = '' ; Y.LAST.REP.AMT = ''
    ...
    WHILE CK EQ '' DO
        LOCATE 'TRANSACTION' IN Y.INT.TYPES<1,1> SETTING POS.LP THEN
            IF Y.ACT.STS<1,POS.LP> EQ 'AUTH' AND Y.ACTY<1,POS.LP> MATCHES 'LENDING-APPLYPAYMENT...' THEN
                ...
                Y.LAST.REP.DATE.TP = R.AAA<AA.Framework.SimulationCapture.ArrActEffectiveDate>
                Y.LAST.REP.AMT.TP = R.AAA<AA.Framework.RrControl.ArrActOrigTxnAmt>
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PR.PN, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PRO, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PR.I, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PR.MORA, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PR.SEG, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.AC.ID, Y.PR.I.SP, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, RET.ERROR);* R22 UTILITY AUTO CONVERSION
    Y.INT.BAL += ABS(BAL.DETAILS<4>)
...
*************
    ...
    Y.IDS.DETAILS<2> = "YES"
    APAP.LAPAP.redoLGetDisbursementDetails(Y.IDS.DETAILS,R.DISB.DETAILS,Y.COMMITED.AMT,Y.PEND.DISB) ;*R22 Manual Conversion
