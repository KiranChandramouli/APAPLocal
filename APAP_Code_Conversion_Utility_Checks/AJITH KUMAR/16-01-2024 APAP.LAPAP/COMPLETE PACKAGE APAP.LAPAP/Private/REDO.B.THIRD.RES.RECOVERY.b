* @ValidationCode : MjoxODgzNzMwMjE1OkNwMTI1MjoxNzA1MzgxNTQxNzExOmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 16 Jan 2024 10:35:41
...
*24/05/2023   Ghayathri T           MANUAL R22 CODE CONVERSION     Call routine modified,Changed AA.AH.ACTIVITY.ID to AA.AH.ACTIVITY.REF
*24/05/2023   Ghayathri T           AUTO R22 CODE CONVERSI         NEWLINES ADD,SM,VM,&FM CHANGE TO @SM,@VM,@FM
*---------------------------------------------------------------------------------------------
 ...
 *   $INSERT I_F.AA.ARRANGEMENT
  ...
  *  $INSERT I_F.AA.ACTIVITY.HISTORY
   ...
   $USING AA.Framework
*   $INSERT I_F.AA.INTEREST.ACCRUALS
    $INSERT I_REDO.B.THIRD.RES.RECOVERY.COMMON
 ...
 *   Y.START = R.ARRANGEMENT<AA.ARR.START.DATE,1>
    ...
    Y.ARR.CURRENCY   = R.ARRANGEMENT< AA.Framework.Arrangement.ArrCurrency>
    Y.LINKED.APPL    = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedAppl>
    Y.LINKED.ID      = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>
    Y.ARR.STATUS     = R.ARRANGEMENT< AA.Framework.ArrangementSim.ArrArrStatus>
    Y.CUSTOMER.ID    = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer>
    Y.START = R.ARRANGEMENT<AA.Framework.ArrangementSim.ArrStartDate,1>
...
AA.Framework.GetArrangementConditions(ARRANGEMENT.ID,PROP.CLASS,PROP.NAME,'',returnID,returnConditions,RET.ERR);* R22 UTILITY AUTO CONVERSION
*
RETURN
*---------------------------------------------------------------------------------------------
GET.EFFECTIVE.DATE:
...
AA.Framework.ReadActivityHistory(Y.AA.ARR.ID, REQD.MODE, EFF.DATE, R.AA.ACTIVITY.HISTORY);* R22 UTILITY AUTO CONVERSION
    Y.MIG.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity>
    YACT.ID.ARR = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivityRef>;* R22 Manual Conversion Changed AA.AH.ACTIVITY.ID to AA.AH.ACTIVITY.REF

    LOCATE "LENDING-TAKEOVER-ARRANGEMENT" IN YACT.ID.ARR<1,1> SETTING Y.POS.MIG THEN
        Y.DATE =R.ARRANGEMENT <AA.Framework.ArrangementSim.ArrOrigContractDate>
    END

    IF NOT(Y.DATE) THEN
        LOCATE "LENDING-DISBURSE-COMMITMENT" IN YACT.ID.ARR<1,1> SETTING YPOS.MIG THEN
            Y.DATE =R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate,YPOS.MIG,1>
        END
    END
RETURN
*---------------------------------------------------------------------------------------------
...
*-------------
    ...
    APAP.LAPAP.redoSRegCustomerExtract(Y.CUSTOMER.ID,Y.PRODUCT.GROUP,Y.REL.CODE,OUT.ARR);*MANUAL R22 CODE CONVERSION
*
