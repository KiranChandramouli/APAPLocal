* @ValidationCode : Mjo1OTkzNTkzMDpDcDEyNTI6MTY5MTc1MTMyMzUzOTpJVFNTMTotMTotMTowOjE6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 11 Aug 2023 16:25:23
...
* <Rating>100</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE L.APAP.COL.FF.COLLECTOR.TXN(Y.PROCESS.DATE, Y.AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.ID, Y.AGENCY.CODE, Y.CREDIT.TXN)
*******************************************************************************
...
* ----                  ----                                ----                 ----
*09-08-2023         Ajith Kumar         R22 Manual Code Conversion     TAM.BP,LAPAP.BP,T24.BP IS REMOVED,VM,SM,FMto @VM,@SM,@FM
...
*
    ...
    MSG = 'L.APAP.COL.FF.COLLECTOR.TXN( ' : Y.AA.ID : ') time=':Y.ELAPSED.TIME:'secs'
    CALL OCOMO(MSG)
...
FN.AA.ACTIVITY.HISTORY = 'F.AA.ACTIVITY.HISTORY';* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.AA.ACTIVITY.HISTORY,Y.AA.ID, R.AA.ACT.HIST, YERR);* R22 UTILITY AUTO CONVERSION
...
FN.AA.ARRANGEMENT.ACTIVITY = 'F.AA.ARRANGEMENT.ACTIVITY';* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.AA.ARRANGEMENT.ACTIVITY,Y.AA.ACT.ID,R.AA.ACT,YERR)      ;* PACS00169639;* R22 UTILITY AUTO CONVERSION
...
* ---------
**-------------------------------------------------------TODO
    Y.TMPMOVSECUENCIAL = CHANGE(Y.TMPMOVSECUENCIAL,".",@FM)
    Y.TMPMOVSECUENCIAL = Y.TMPMOVSECUENCIAL<1>
**------------------------------------------------------------
...
*CALL REDO.R.COL.GET.MAPPING(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, Y.MAP.TYPE, Y.MAP.VALUE)
    APAP.TAM.redoRColGetMapping(C.ID.STATIC.MAPPING, R.STATIC.MAPPING, 1, R.STATIC.MAPPING, Y.MAP.TYPE, Y.MAP.VALUE) ;* R22 Manual conversion
RETURN
...
FN.AA.ARRANGEMENT.ACTIVITY = 'F.AA.ARRANGEMENT.ACTIVITY';* R22 UTILITY AUTO CONVERSION
            CALL CACHE.READ(FN.AA.ARRANGEMENT.ACTIVITY,Y.TXN.CONTRACT,R.AA.ACT.NEW,YERR) ;;* R22 UTILITY AUTO CONVERSION
...
FN.TELLER = 'F.TELLER';* R22 UTILITY AUTO CONVERSION
            CALL CACHE.READ(FN.TELLER,Y.TXN.CONTRACT, R.TT, YERR);* R22 UTILITY AUTO CONVERSION
            IF YERR THEN
                YERR = ""
...
FN.FUNDS.TRANSFER = 'F.FUNDS.TRANSFER';* R22 UTILITY AUTO CONVERSION
            CALL CACHE.READ(FN.FUNDS.TRANSFER,Y.TXN.CONTRACT, R.FT, YERR);* R22 UTILITY AUTO CONVERSION
            IF YERR THEN
...
FN.STMT.ENTRY = 'F.STMT.ENTRY';* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.STMT.ENTRY,Y.STMT.ID,R.STMT.ENTRY,YERR);* R22 UTILITY AUTO CONVERSION
...
FN.TRANSACTION = 'F.TRANSACTION';* R22 UTILITY AUTO CONVERSION
        CALL CACHE.READ(FN.TRANSACTION,Y.TXN.CODE,R.TXN,YERR);* R22 UTILITY AUTO CONVERSION
