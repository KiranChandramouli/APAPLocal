* @ValidationCode : MjotNzc2NzQ1NzI5OkNwMTI1MjoxNzA0NzkyMDMxMDMwOmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 09 Jan 2024 14:50:31
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : ajith
* @ValidationInfo : Nb tests success  : N/A
* @ValidationInfo : Nb tests failure  : N/A
* @ValidationInfo : Rating            : N/A
* @ValidationInfo : Coverage          : N/A
* @ValidationInfo : Strict flag       : N/A
* @ValidationInfo : Bypass GateKeeper : false
* @ValidationInfo : Compiler Version  : R21_AMR.0
* @ValidationInfo : Copyright Temenos Headquarters SA 1993-2021. All rights reserved.
$PACKAGE APAP.LAPAP
*-----------------------------------------------------------------------------
* <Rating>1543</Rating>
*-----------------------------------------------------------------------------
* This subroutine allows to extract fields from Customer in T24
* duplicado del servicio de COLLECTOR
* MDP-3379:   *Para el archivo de cliente cada registro debe finalizar en cero en vez de la vergulilla.
*Para los demás archivos se debe remplazar por un valor vacio La virulilla
* Arguments: CUSTOMER.ID
* Basada en la logica de la rutina : L.APAP.COL.FF.COLLECTOR
* Service : BNK/L.APAP.COL.FF.COLLECTOR
* Date                  Who                               Reference           Description
* ----                  ----                                ----                 ----
* 09-08-2023         Ajith Kumar         R22 Manual Code Conversion     TAM.BP,LAPAP.BP,T24.BP IS REMOVED,CALL RTN FORMAT CAN BE CHANGED


* ----------------------------------------------------------------------------

SUBROUTINE L.APAP.COL.FF.COLLECTOR(CUSTOMER.ID)
    $INSERT I_EQUATE
    $INSERT I_COMMON
    $INSERT I_F.CUSTOMER ;* R22 Manual Code Conversion -start
    $INSERT I_F.DE.ADDRESS
    $INSERT I_F.REDO.INTERFACE.PARAM
    $INSERT I_L.APAP.COL.COLLECTOR.COMMON
    $INSERT I_F.REDO.COL.TRACE.PHONE
    $INSERT I_BATCH.FILES
   * $INSERT I_F.AA.ARRANGEMENT ;* R22 Manual Code Conversion -end
   $USING AA.Framework

    BEGIN CASE
*        CASE CONTROL.LIST<1,1> = 'Y.AP.LOANS'
        CASE CONTROL.LIST<1,1> EQ 'Y.AP.LOANS';* R22 UTILITY AUTO CONVERSION
            GOSUB PROCESS.AP.LOANS
*        CASE CONTROL.LIST<1,1> = 'Y.AP.CARDS'
        CASE CONTROL.LIST<1,1> EQ 'Y.AP.CARDS';* R22 UTILITY AUTO CONVERSION
            GOSUB PROCESS.AP.CARDS
    END CASE
RETURN

*****************
PROCESS.AP.LOANS:
*****************
    GOSUB INITIALISE
    IF CUSTOMER.ID[1,2] EQ 'AA' THEN
        Y.AA.ARR.ID = CUSTOMER.ID; R.ARRANGEMENT = ''; ARR.ERR = ''
*        CALL AA.GET.ARRANGEMENT(Y.AA.ARR.ID,R.ARRANGEMENT,ARR.ERR)
AA.Framework.GetArrangement(Y.AA.ARR.ID,R.ARRANGEMENT,ARR.ERR);* R22 UTILITY AUTO CONVERSION
        *CUSTOMER.ID = R.ARRANGEMENT<AA.ARR.CUSTOMER>
         CUSTOMER.ID = R.ARRANGEMENT<AA.Framework.Arrangement.ArrCustomer>
    END
    GOSUB PROCESS
RETURN

*****************
PROCESS.AP.CARDS:
*****************
    GOSUB INITIALISE
    GOSUB REP.AP.CARDS
RETURN

*----------------------------
INITIALISE:         * Initialise Variables, Files, etc
*----------------------------
    Y.MSG = ''
    Y.TOTAL.INSERT = 0
    Y.SIZE =  500
    DIM LIST.INSERTS.STMT(Y.SIZE)

    Y.NO.ID = "" ;     Y.CLI.TIPO.ID = ""    ;    Y.CLI.NUM.ID = ""
    Y.CLI.PRIMER.NOMBRE     = "" ;     Y.CLI.PRIMER.APELLIDO = ""
    Y.CLI.NOM.CONYUGE = ""       ;     Y.CLI.APE.CONYUGE = ""       ;    Y.CLI.TIPO.PERSONA = ""
    Y.CLI.RAZON.SOCIAL.NOM1 = "" ;     Y.CLI.RAZON.SOCIAL.NOM2 = "" ;    Y.CLI.FECHA.NACIMIENTO = ""
    Y.CLI.LUGAR.TRABAJO = ""     ;     Y.CLI.EMAIL = ""             ;    Y.CLI.FECHA.INGRESO = ""
    Y.CLI.SEXO = ""              ;     Y.CLI.ESTADO.CIVIL = ""      ;    Y.CLI.CONTACTO = ""
    Y.CLI.TELF.CONYUGE.AREA = "" ;     Y.CLI.TELF.CONYUGE.NO = ""   ;    Y.CLI.TELF.CONYUGE.EXT = ""
    Y.CLI.NOM.REPRES = ""        ;     Y.CLI.APE.REPRES = ""        ;    Y.CLI.ID.REPRES = ""
    Y.CLI.NUM.REPRES = ""
    Y.CLI.ADD.POR = ""
    Y.CLI.FECHA.ADICION = ""
    Y.CLI.MODIF.POR = ""
    Y.CLI.FECHA.MODIF = ""
    Y.CLI.CODIGO.ORIGEN = ""
    Y.CLI.TELF.TYPE = ""
    Y.CLI.CAMPO1.09 =  ""
    Y.CLI.CAMPO2.09 = ""
    Y.CLI.CAMPO3.09 = ""
    Y.CLI.CAMPO4.09 = ""
    Y.CLI.CAMPO5.09 = ""
    Y.CLI.CAMPO6.09 = ""
    Y.CLI.TELF.AREA.10 = ""
    Y.CLI.TELF.NO.10 = ""
    Y.CLI.TELF.EXT.10 = ""
    Y.CLI.TELF.AREA.11 = ""
    Y.CLI.TELF.NO.11 = ""
    Y.CLI.TELF.AREA.13 = ""
    Y.CLI.TELF.NO.13 = ""


    Y.TMPCLIENTES = 'tmpclientes'
    Y.TMPTELEFONOSCLIENTE = 'tmptelefonoscliente'
    Y.TMPDIRECCIONESCLIENTE = 'tmpdireccionesclientebase'

RETURN
*------------------------------
PROCESS:  * Process Each entry for the current customer
*------------------------------

    GOSUB GET.CUSTOMER.REC
    Y.CU.SCO.COB = R.CUSTOMER<EB.CUS.LOCAL.REF,C.L.CUS.SCO.COB.POS>   ;* Get SCO.COB from Customer Table

    IF Y.PROCESS.FLAG.TABLE<1,1> EQ "tmpclientes" THEN
        GOSUB CUSTOMER.STMT   ;*Ir al parrafo para extraer la informacion para la tabla de customer
    END
    IF Y.PROCESS.FLAG.TABLE<1,2> EQ "tmptelefonoscliente" THEN
        GOSUB PHONES.STMT     ;*Ir al Parrafo de para extraer y crear los Inserts de Phones
    END

    IF Y.PROCESS.FLAG.TABLE<1,3> EQ "tmpdireccionescliente" THEN
        GOSUB ADDRESS.STMT    ;*Ir al Parrafo de para extraer y crear los Inserts de Address
    END

    IF Y.PROCESS.FLAG.TABLE<1,4> EQ "tmpmovimientos" OR Y.PROCESS.FLAG.TABLE<1,5> EQ "tmpcredito" OR Y.PROCESS.FLAG.TABLE<1,6> EQ "gescreditointegracion" OR Y.PROCESS.FLAG.TABLE<1,7> EQ "gestipogarantias" OR Y.PROCESS.FLAG.TABLE<1,8> EQ "gesgarantias" OR Y.PROCESS.FLAG.TABLE<1,9> EQ "gescreditogarantias" THEN
        GOSUB PROCESS.AA      ;*Ir al Parrafo de para extraer y crear los Inserts de AA
    END

    IF Y.CREDIT THEN
        IF R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TARJ.CR.POS> EQ "YES" THEN
            R.REDO.CL.LOAN.CUST=CUSTOMER.ID
            CALL F.WRITE(FN.REDO.CL.LOAN.CUST,CUSTOMER.ID,R.REDO.CL.LOAN.CUST)
        END
*------------------------------------------------------
*se guarda aqui los cliente ya procesados
*tmpcliente,tmptelefonoscliente,tmpdireccionesclientebase
        GOSUB WRITE.TEMP.CUSTOMER
*---------------------------------------------------------
        GOSUB  FINAL.WRITE.RECORD
    END

RETURN

*************
REP.AP.CARDS:
*************
    GOSUB GET.CUSTOMER.REC
*    CALL F.READ(FN.REDO.CL.LOAN.CUST,CUSTOMER.ID,R.REDO.CL.LOAN.CUST,F.REDO.CL.LOAN.CUST,Y.RCLC.ERR)
    CALL F.READU(FN.REDO.CL.LOAN.CUST,CUSTOMER.ID,R.REDO.CL.LOAN.CUST,F.REDO.CL.LOAN.CUST,Y.RCLC.ERR,'');* R22 UTILITY AUTO CONVERSION

    IF R.REDO.CL.LOAN.CUST THEN
        CALL F.DELETE(FN.REDO.CL.LOAN.CUST,CUSTOMER.ID)
    END ELSE
***todo----------------------------------------------------------------------------------------------------------------------------
        Y.ID.COLLECT = CUSTOMER.ID
*        CALL F.READ (FN.LAPAP.CONCAT.TMP.CLIENTES,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.CLIENTES,FV.LAPAP.CONCAT.TMP.CLIENTES,ERROR.CLIENTES)
        CALL F.READU (FN.LAPAP.CONCAT.TMP.CLIENTES,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.CLIENTES,FV.LAPAP.CONCAT.TMP.CLIENTES,ERROR.CLIENTES,'');* R22 UTILITY AUTO CONVERSION
        IF R.LAPAP.CONCAT.TMP.CLIENTES THEN
            RETURN
        END
***-----------------------------------------------------------------------------------------------------------------------------
        Y.CU.SCO.COB = R.CUSTOMER<EB.CUS.LOCAL.REF,C.L.CUS.SCO.COB.POS>         ;* Get SCO.COB from Customer Table

        IF Y.PROCESS.FLAG.TABLE<1,1> EQ "tmpclientes" THEN
            GOSUB CUSTOMER.STMT         ;*Ir al parrafo para extraer la informacion para la tabla de customer
        END
        IF Y.PROCESS.FLAG.TABLE<1,2> EQ "tmptelefonoscliente" THEN
            GOSUB PHONES.STMT ;*Ir al Parrafo de para extraer y crear los Inserts de Phones
        END

        IF Y.PROCESS.FLAG.TABLE<1,3> EQ "tmpdireccionescliente" THEN
            GOSUB ADDRESS.STMT          ;*Ir al Parrafo de para extraer y crear los Inserts de Address
        END
        GOSUB  FINAL.WRITE.RECORD
    END
RETURN

GET.CUSTOMER.REC:
*****************
    R.CUSTOMER = ''; YERR = ''
    CALL F.READ(FN.CUSTOMER, CUSTOMER.ID, R.CUSTOMER, F.CUSTOMER, YERR)

    CUST.ID.HST = CUSTOMER.ID:';1'
    ERR.CUSTOMER.HST = ''; R.CUSTOMER.HST = ''
    CALL F.READ(FN.CUSTOMER.HST,CUST.ID.HST,R.CUSTOMER.HST,F.CUSTOMER.HST,ERR.CUSTOMER.HST)
    IF NOT(R.CUSTOMER.HST) THEN
        R.CUSTOMER.HST = R.CUSTOMER
    END
    IF R.CUSTOMER THEN
        Y.L.AUDIT.POS = DCOUNT(R.CUSTOMER<EB.CUS.INPUTTER>, @VM)
        Y.CLI.FECHA.MODIF.TP = R.CUSTOMER<EB.CUS.DATE.TIME,Y.L.AUDIT.POS>
        Y.CLI.FECHA.MODIF = ICONV(Y.CLI.FECHA.MODIF.TP,'DI')
        Y.CLI.FECHA.MODIF = OCONV(Y.CLI.FECHA.MODIF,'D4')
        Y.CLI.FECHA.MODIF = Y.CLI.FECHA.MODIF[7,4]:Y.CLI.FECHA.MODIF[4,2]:Y.CLI.FECHA.MODIF[1,2]
    END
RETURN

*-----------------------------------------------------------------------------------
GET.CUS.ID.TYPE:    *Extrae el tipo de Documento del Customer
*-----------------------------------------------------------------------------------
    BEGIN CASE
*        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS> NE ""      ;*CEDULA = 1
        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS> NE ""      ;*CEDULA EQ 1;* R22 UTILITY AUTO CONVERSION
            Y.CLI.TIPO.ID = 01
            Y.CLI.NUM.ID = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS>

*<< PP, LEGAL.ID is mv, there was no definition, then take first position
*        CASE R.CUSTOMER<EB.CUS.LEGAL.ID,1> NE ""      ;*PASAPORTE = 4
        CASE R.CUSTOMER<EB.CUS.LEGAL.ID,1> NE ""      ;*PASAPORTE EQ 4;* R22 UTILITY AUTO CONVERSION
            Y.CLI.TIPO.ID = 04
            Y.CLI.NUM.ID = R.CUSTOMER<EB.CUS.LEGAL.ID,1>
*>>

*        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS> NE ""     ;*RCN = 2
        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS> NE ""     ;*RCN EQ 2;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.ID = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS>
            Y.CLI.TIPO.ID = 02

*        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS> NE "" ;*ACTA NACIMIENTO = 8
        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS> NE "" ;*ACTA NACIMIENTO EQ 8;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.ID = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS>
            Y.CLI.TIPO.ID = 08

*        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS> NE "" ;*NUMERO UNICO = 15
        CASE R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS> NE "" ;*NUMERO UNICO EQ 15;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.ID = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS>
            Y.CLI.TIPO.ID = 15

    END CASE

RETURN
*-----------------------
CUSTOMER.STMT:      *EXTRACT INFORMATION ABOUT CUSTOMER
*-----------------------

    GOSUB GET.CUS.ID.TYPE

*BUCLE PARA LA LECTURA DE RELATION.CODE PARA SABER SU CONYUGE
    Y.RELATION.CODE = R.CUSTOMER<EB.CUS.RELATION.CODE>
    NRO.REL.CODE = DCOUNT(Y.RELATION.CODE,@VM)

    I = NRO.REL.CODE
    LOOP WHILE I GE 1
        IF Y.RELATION.CODE<1,I> MATCHES "7" : @VM : "8" THEN
            Y.CUS.CODE.CONYUGE = R.CUSTOMER<EB.CUS.REL.CUSTOMER,I>
* LECTURA DE LOS CAMPOS DE CORE DEL CONYUGE
            CALL F.READ(FN.CUSTOMER, Y.CUS.CODE.CONYUGE, R.CUSTOMER.CONYUGE, F.CUSTOMER, YERR)
            IF YERR NE '' THEN
                Y.MSG = yRecordNotFound : @FM : Y.CUS.CODE.CONYUGE : @VM : "F.CUSTOMER"
                Y.TABLE.ID = Y.TMPCLIENTES
                Y.PROCESS.FLAG.TABLE<1,1> = ""
                GOSUB TRACE.ERROR
                RETURN
            END
            Y.L.AUDIT.POSCC = DCOUNT(R.CUSTOMER.CONYUGE<EB.CUS.INPUTTER>, @VM)
            Y.CLI.FECHA.MODIF.TPCC = R.CUSTOMER.CONYUGE<EB.CUS.DATE.TIME,Y.L.AUDIT.POSCC>
            Y.CLI.FECHA.MODIF.CC = ICONV(Y.CLI.FECHA.MODIF.TPCC,'DI')
            Y.CLI.FECHA.MODIF.CC = OCONV(Y.CLI.FECHA.MODIF,'D4')
            Y.CLI.FECHA.MODIF.CC = Y.CLI.FECHA.MODIF.CC[7,4]:Y.CLI.FECHA.MODIF.CC[4,2]:Y.CLI.FECHA.MODIF.CC[1,2]
*ASIGNACION DE LA INFORMACION RESPECTO A EL CONYUGE
            Y.CLI.NOM.CONYUGE = R.CUSTOMER.CONYUGE<EB.CUS.GIVEN.NAMES>
            Y.CLI.APE.CONYUGE = R.CUSTOMER.CONYUGE<EB.CUS.FAMILY.NAME>
            Y.CLI.TELF.CONYUGE.AREA = R.CUSTOMER.CONYUGE<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,1>
            Y.CLI.TELF.CONYUGE.NO = R.CUSTOMER.CONYUGE<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,1>
            Y.CLI.TELF.CONYUGE.EXT = R.CUSTOMER.CONYUGE<EB.CUS.LOCAL.REF,Y.CLI.TELF.EXT.POS,1>
        END

        IF Y.RELATION.CODE<1,I> EQ 104 THEN       ;*ASIGNACION DEL TIPO DE DOCUMENTO DEL REPRESENTANTE
            Y.CUS.CODE.REPRES = R.CUSTOMER<EB.CUS.REL.CUSTOMER,I>
            CALL F.READ(FN.CUSTOMER, Y.CUS.CODE.REPRES, R.CUSTOMER.REPRES, F.CUSTOMER, YERR)
            IF YERR NE '' THEN
                Y.MSG = yRecordNotFound : @FM : Y.CUS.CODE.REPRES : @VM : "F.CUSTOMER"
                Y.TABLE.ID = Y.TMPCLIENTES
                Y.PROCESS.FLAG.TABLE<1,1> = ""
                GOSUB TRACE.ERROR
                RETURN
            END
            Y.L.AUDIT.POSCR = DCOUNT(R.CUSTOMER.REPRES<EB.CUS.INPUTTER>, @VM)
            Y.CLI.FECHA.MODIF.TPCR = R.CUSTOMER.REPRES<EB.CUS.DATE.TIME,Y.L.AUDIT.POSCR>
            Y.CLI.FECHA.MODIF.CR = ICONV(Y.CLI.FECHA.MODIF.TPCR,'DI')
            Y.CLI.FECHA.MODIF.CR = OCONV(Y.CLI.FECHA.MODIF.CR,'D4')
            Y.CLI.FECHA.MODIF.CR = Y.CLI.FECHA.MODIF.CR[7,4]:Y.CLI.FECHA.MODIF.CR[4,2]:Y.CLI.FECHA.MODIF.CR[1,2]
            GOSUB GET.REPRES.ID.TYPE
            Y.CLI.NOM.REPRES = R.CUSTOMER.REPRES<EB.CUS.GIVEN.NAMES>
            Y.CLI.APE.REPRES = R.CUSTOMER.REPRES<EB.CUS.FAMILY.NAME>
        END
        I--
    REPEAT

*Tipo de Customer Registrado(Persona Fisica, Persona Jurida, etc...)
    Y.CLI.TIPO.PERSONA = R.CUSTOMER<EB.CUS.LOCAL.REF, Y.CLI.TIPO.CL.POS>

    NRO.REL.CODE.PER = DCOUNT(Y.CLI.TIPO.PERSONA,@SM)

* PP, take first position   for NAME.1 & NAME.2
    I = NRO.REL.CODE.PER
    LOOP WHILE I GE 1
        Y.TIPO.CLIENTE = Y.CLI.TIPO.PERSONA<1,1,I>
        GOSUB GET.TIPO.PERSONA
        Y.CLI.TIPO.PERSONA<1,1,I> = Y.TIPO.CLIENTE
        IF Y.CLI.TIPO.PERSONA<1,1,I> EQ 0 THEN    ;*ASIGNACION NOMBRE CLIENTE JURIDICA
            Y.CLI.RAZON.SOCIAL.NOM1 = R.CUSTOMER<EB.CUS.NAME.1, 1>
            Y.CLI.RAZON.SOCIAL.NOM2 = R.CUSTOMER<EB.CUS.NAME.2, 1>
        END  ELSE
            IF Y.CLI.TIPO.PERSONA<1,1,I> EQ 1 THEN          ;*ASIGNACION NOMBRE Y APELLIDO CLIENTE NATURAL
                Y.CLI.PRIMER.NOMBRE = R.CUSTOMER<EB.CUS.GIVEN.NAMES>
                Y.CLI.PRIMER.APELLIDO = R.CUSTOMER<EB.CUS.FAMILY.NAME>
            END
        END
        I--
    REPEAT

*Extracion de datos Generales de Customer
    Y.CLI.FECHA.NACIMIENTO = R.CUSTOMER<EB.CUS.DATE.OF.BIRTH>
* PP take first position for Employers.Name & Email, no definition available
    Y.CLI.LUGAR.TRABAJO = R.CUSTOMER<EB.CUS.EMPLOYERS.NAME, 1>
    Y.CLI.EMAIL = R.CUSTOMER<EB.CUS.EMAIL.1, 1>
    Y.CLI.FECHA.INGRESO = R.CUSTOMER<EB.CUS.CUSTOMER.SINCE>
    Y.CLI.SEXO = R.CUSTOMER<EB.CUS.GENDER>
    Y.CLI.ESTADO.CIVIL = R.CUSTOMER<EB.CUS.MARITAL.STATUS>[1,1]
    Y.CLI.APELLIDO = R.CUSTOMER<EB.CUS.GIVEN.NAMES>:' ':R.CUSTOMER<EB.CUS.FAMILY.NAME>
    Y.CALC.RISK.CLASS = R.CUSTOMER<EB.CUS.CALC.RISK.CLASS,1>

*    Y.CLI.CONTACTO =  R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TEL.P.CONT>

*CAMPOS DE AUDITORIA DEL REGISTRO DE CUSTOMER
    Y.CLI.ADD.POR = FIELD(R.CUSTOMER.HST<EB.CUS.INPUTTER, 1>,'_',2)   ;* Who created the row
    Y.CLI.FECHA.ADICION.TP = R.CUSTOMER.HST<EB.CUS.DATE.TIME, 1>      ;* When the row was created
    Y.CLI.FECHA.ADICION = ICONV(Y.CLI.FECHA.ADICION.TP,'DI')
    Y.CLI.FECHA.ADICION = OCONV(Y.CLI.FECHA.ADICION,'D4')
    Y.CLI.FECHA.ADICION = Y.CLI.FECHA.ADICION[7,4]:Y.CLI.FECHA.ADICION[4,2]:Y.CLI.FECHA.ADICION[1,2]
    Y.CLI.MODIF.POR = FIELD(R.CUSTOMER<EB.CUS.INPUTTER, Y.L.AUDIT.POS>,'_',2)   ;* Last inputter & date

*Y.CLI.CODIGO.ORIGEN = TRIM(CUSTOMER.ID,"","B")          ;* CN008947
    Y.CLI.CAMPO1.09 = CHANGE(R.CUSTOMER<EB.CUS.ADDRESS>,@VM,' ')
    Y.CLI.CAMPO1.09 = CHANGE (Y.CLI.CAMPO1.09, @SM, ' ')
    Y.CLI.CAMPO2.09 = CHANGE(R.CUSTOMER<EB.CUS.STREET>, @VM, ' ')
    Y.CLI.CAMPO3.09 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.CAMPO3.09.POS >
    Y.CLI.CAMPO4.09 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.CAMPO4.09.POS >
    Y.CLI.CAMPO5.09 = R.CUSTOMER<EB.CUS.COUNTRY, 1>         ;* No definition, take first position
    Y.CLI.CAMPO6.09 = R.CUSTOMER<EB.CUS.TOWN.COUNTRY, 1>    ;* No definition, take first position

* VERIFICAR CAMPO DE DIRECCION ;* cambia los espacios de la direccion almacenada en el R.CUSTOMER<EB.CUS.ADDRESS> por MV
    Y.ADDRESS.09 = CHANGE(Y.CLI.CAMPO1.09,@VM,' ')

    GOSUB GET.CUSTOMER.PHONES

*CONCATENAR INFORMACION DEL CLIENTE
    Y.RAZON.SOCIAL = Y.CLI.RAZON.SOCIAL.NOM1:' ':Y.CLI.RAZON.SOCIAL.NOM2
    Y.TEL.CONYUGE = Y.CLI.TELF.CONYUGE.AREA:' ':Y.CLI.TELF.CONYUGE.NO:' ':Y.CLI.TELF.CONYUGE.EXT
    Y.NOM.APE.REPRES = Y.CLI.NOM.REPRES:' ':Y.CLI.APE.REPRES

*DIRECCION
    Y.CAMPO.9 = Y.ADDRESS.09:' ':Y.CLI.CAMPO2.09:' ':Y.CLI.CAMPO3.09:' ':Y.CLI.CAMPO4.09:' ':Y.CLI.CAMPO5.09:' ':Y.CLI.CAMPO6.09

*TELEFONO
    Y.CAMPO.10 = Y.CLI.TELF.AREA.10:' ':Y.CLI.TELF.NO.10:' ':Y.CLI.TELF.EXT.10
    Y.CAMPO.11 = Y.CLI.TELF.AREA.11:' ':Y.CLI.TELF.NO.11
    Y.CAMPO.13 = Y.CLI.TELF.AREA.13:' ':Y.CLI.TELF.NO.13

*** :Y.DELIM:"BPR"

    Y.LINEA=''
    Y.DELIM='~'
    Y.VALOR.CERO = '0';
*    Y.LINEA := "000001":Y.DELIM:TRIM(Y.CLI.ADD.POR[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.PRIMER.APELLIDO[1,60],"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(Y.CLI.ID.REPRES,"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.APELLIDO[1,300],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.EMAIL[1,60],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.CLI.ESTADO.CIVIL[1,10],"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.ADICION,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.NACIMIENTO,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.INGRESO,"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.MODIF,"","B"):Y.DELIM:TRIM(Y.CLI.NUM.REPRES[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.LUGAR.TRABAJO[1,250],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.CLI.MODIF.POR[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.NOM.CONYUGE[1,40],"","B"):Y.DELIM:TRIM(Y.NOM.APE.REPRES[1,250],"","B"):Y.DELIM:TRIM(Y.CLI.NUM.ID[1,20],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.PRIMER.APELLIDO[1,60],"","B"):Y.DELIM:TRIM(Y.CLI.PRIMER.NOMBRE[1,60],"","B")
*    Y.LINEA := Y.DELIM:TRIM(Y.RAZON.SOCIAL[1,200],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.SEXO[1,1],"","B"):Y.DELIM:TRIM(Y.TEL.CONYUGE[1,10],"","B"):Y.DELIM:TRIM(Y.CALC.RISK.CLASS[1,40],"","B"):Y.DELIM:TRIM(Y.CLI.TIPO.ID,"","B"):Y.DELIM:TRIM(Y.TIPO.CLIENTE,"","B"):Y.DELIM:"BPR"
*    Y.LINEA := Y.DELIM:TRIM(Y.CAMPO.9[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.10[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.11[1,400],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CAMPO.13[1,400],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:""
*    Y.LINEA := Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:""
*    Y.LINEA := Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM

    IF Y.CLI.TIPO.ID THEN
        Y.LINEA := "1":Y.DELIM:"BPR":Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(Y.CLI.TIPO.ID,"","B"):Y.DELIM:TRIM(Y.CLI.NUM.ID[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.PRIMER.NOMBRE[1,60],"","B"):Y.DELIM:"":Y.DELIM
        Y.LINEA := TRIM(Y.CLI.PRIMER.APELLIDO[1,60],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.NOM.CONYUGE[1,40],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.TIPO.PERSONA[1,1],"","B"):Y.DELIM:TRIM(Y.RAZON.SOCIAL[1,200],"","B"):Y.DELIM
        Y.LINEA := TRIM(Y.CLI.FECHA.NACIMIENTO,"","B"):Y.DELIM:TRIM(Y.CLI.LUGAR.TRABAJO[1,250],"","B"):Y.DELIM:TRIM(Y.CLI.EMAIL[1,30],"","B"):Y.DELIM
        Y.LINEA := TRIM(Y.CLI.FECHA.INGRESO,"","B"):Y.DELIM:TRIM(Y.CLI.SEXO[1,1],"","B"):Y.DELIM:TRIM(Y.CLI.ESTADO.CIVIL[1,30],"","B"):Y.DELIM
        Y.LINEA := TRIM(Y.CLI.CONTACTO[1,300],"","B"):Y.DELIM:TRIM(Y.TEL.CONYUGE[1,10],"","B"):Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.ID.REPRES,"","B"):Y.DELIM:TRIM(Y.CLI.NUM.REPRES[1,20],"","B"):Y.DELIM:TRIM(Y.NOM.APE.REPRES[1,250],"","B")
        Y.LINEA := Y.DELIM:"":Y.DELIM:"":TRIM(Y.CLI.ADD.POR[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.FECHA.ADICION,"","B"):Y.DELIM:TRIM(Y.CLI.MODIF.POR[1,20],"","B"):Y.DELIM
        Y.LINEA := TRIM(Y.CLI.FECHA.MODIF,"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM
        Y.LINEA := TRIM(Y.CAMPO.9[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.10[1,400],"","B"):Y.DELIM:TRIM(Y.CAMPO.11[1,400],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CAMPO.13[1,400],"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM
        Y.LINEA := "":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:TRIM(Y.CU.SCO.COB,"","B"):Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM
        Y.LINEA :="":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.DELIM:"":Y.VALOR.CERO
*:"":Y.DELIM

        R.COL.QUEUE = ''
        R.COL.QUEUE<1> = Y.TMPCLIENTES
        R.COL.QUEUE<2> = Y.LINEA

        GOSUB WRITE.RECORD
    END
RETURN
*-------------------
PHONES.STMT:
*-------------------
    GOSUB GET.CUSTOMER.PHONES
    IF Y.CLI.TELF.TYPE EQ "" THEN       ;*PHONE TYPE VALIDATION
        Y.MSG = "EL CLIENTE " : CUSTOMER.ID : " NO TIENE TELEFONOS REGISTRADOS "          ;* Warning Message
        CALL OCOMO(Y.MSG)
        RETURN
    END

*    CALL CACHE.READ('F.REDO.COL.TRACE.PHONE',CUSTOMER.ID,R.COL.TRACE.PHONE,Y.ERR)
    CALL F.READ(FN.REDO.COL.TRACE.PHONE, CUSTOMER.ID, R.COL.TRACE.PHONE, F.REDO.COL.TRACE.PHONE, YERR)


    Y.CLI.TELF.TYPE = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.TYPE.POS>
    NRO.REL.CODE.TEL = DCOUNT(Y.CLI.TELF.TYPE,@SM)

    Y.TEL.TYPE.LIST = ''
    Y.TEL.TYPE.SEQ  = ''
    I = 1
    LOOP WHILE I LE NRO.REL.CODE.TEL
*        Y.INSERT.TEL = C_DML_STMT_TELEFONOS
        Y.CLI.TELF.TYPE = R.CUSTOMER<EB.CUS.LOCAL.REF, Y.CLI.TELF.TYPE.POS, I>
        IF Y.CLI.TELF.TYPE NE '' THEN
            GOSUB PROC.PHONES
        END
        I++
    REPEAT

RETURN

*-------------------
PROC.PHONES:
*-------------------
    IF NUM(Y.CLI.TELF.TYPE) EQ @FALSE THEN        ;* Warning Message - Phone Number is not Numeric
        Y.MSG = "EL TIPO DE TELEFONO -&- DEBE SER NUMERICO EN CLIENTE & " : @FM : Y.CLI.TELF.TYPE : @VM : CUSTOMER.ID
        CALL OCOMO(Y.MSG)
        RETURN
    END

    Y.TEL.DATE.TIME.ADD = "" ;     Y.TEL.USER          = ""    ;     Y.TEL.DATE.TIME.UPD = ""
    GOSUB GET.TEL.AUDIT.INFO

    Y.CLI.TELF.NO = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
    Y.CLI.TELF.AREA.DC= R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
    Y.CLI.TELF.NO=Y.CLI.TELF.AREA.DC:Y.CLI.TELF.NO
    Y.CLI.TELF.EXT = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.EXT.POS,I>
    YSTATUS = ''; Y.STATUS = ''; Y.STATUS = R.CUSTOMER<EB.CUS.CUSTOMER.STATUS>
    IF Y.STATUS EQ '1' THEN
        YSTATUS = 'S'
    END ELSE
        YSTATUS = 'N'
    END
    GOSUB GET.TEL.SEQUENCE

    Y.INSERT.TEL=''
    Y.DELIM='~'
    Y.INSERT.TEL=''
    Y.INSERT.TEL := "000001":Y.DELIM:TRIM(CUSTOMER.ID,"","B"):Y.DELIM:TRIM(Y.CLI.TELF.TYPE,"","B"):Y.DELIM:TRIM(Y.CURR.TEL.SEQ,"","B") :Y.DELIM:"1":Y.DELIM:TRIM(Y.CLI.TELF.NO[1,56],"","B"):Y.DELIM
    Y.INSERT.TEL := TRIM(Y.CLI.TELF.EXT[1,5],"","B"):Y.DELIM:TRIM(Y.CLI.ADD.POR[1,20],"","B"):Y.DELIM
    Y.INSERT.TEL := TRIM(Y.TEL.DATE.TIME.ADD,"","B"):Y.DELIM:TRIM(Y.TEL.USER[1,20],"","B"):Y.DELIM
* 20171020 /S
    Y.INSERT.TEL := TRIM(Y.TEL.DATE.TIME.UPD,"","B"):Y.DELIM:'T24':Y.DELIM:YSTATUS:Y.DELIM:'':Y.DELIM
*:'':Y.DELIM
* 20171020 /E
    R.COL.QUEUE = ''
    R.COL.QUEUE<1> = Y.TMPTELEFONOSCLIENTE
    R.COL.QUEUE<2> = Y.INSERT.TEL
    GOSUB WRITE.RECORD
RETURN

*-------------------
GET.TIPO.PERSONA:
*-------------------
    IF Y.TIPO.CLIENTE MATCHES "...FISICA" OR   Y.TIPO.CLIENTE  MATCHES "...MENOR" THEN
        Y.TIPO.CLIENTE = 1
    END
    IF Y.CLI.TIPO.PERSONA<1,1,I> MATCHES "...JURIDICA" THEN
        Y.TIPO.CLIENTE = 0
    END

RETURN

*-------------------
GET.CUSTOMER.PHONES:*BUCLE PARA LA LECTURA DEL CAMPO LOCAL DEL CLIENTE CORRESPONDIENTE A TELEFONOS
*-------------------
IF Y.CLI.TELF.TYPE NE "" THEN
RETURN
END

Y.CLI.TELF.TYPE = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.TYPE.POS>
NRO.REL.CODE.TEL = DCOUNT(Y.CLI.TELF.TYPE,@SM)
Y.CLI.CONTACTO=''
I = NRO.REL.CODE.TEL
LOOP WHILE I GE 1
BEGIN CASE
    CASE Y.CLI.TELF.TYPE<1,1,I> EQ "05"
        Y.CLI.TELF.AREA.10 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
        Y.CLI.TELF.NO.10   = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
        Y.CLI.TELF.EXT.10  = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.EXT.POS,I>
        Y.CLI.CONTACTO     =  R.CUSTOMER<EB.CUS.LOCAL.REF,Y.L.CU.TEL.P.CONT,I>
    CASE Y.CLI.TELF.TYPE<1,1,I> EQ "01"
        Y.CLI.TELF.AREA.11 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
        Y.CLI.TELF.NO.11 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
    CASE Y.CLI.TELF.TYPE<1,1,I> EQ "06"
        Y.CLI.TELF.AREA.13 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.AREA.POS,I>
        Y.CLI.TELF.NO.13 = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TELF.NO.POS,I>
END CASE
I--
REPEAT
RETURN
*-------------
ADDRESS.STMT:       *OBTENER TMPCLIENTEDIRECCIONES   18-inicio
*-------------
    Y.CLI.COD.EMPRESA = 000001
    Y.CLI.COD.CLIENTE = CUSTOMER.ID
    Y.CLI.REFER.GEOGRAFICA = 1
*GET TMPDirClienteModificaco
    Y.CLI.DIR.MODIFICADO = 0

* BUILD THE QUERY TO GET THE ADDRESS ID'S  19- inicio'
    Y.ADDRESS = ID.COMPANY : ".C-" : CUSTOMER.ID : ".PRINT."          ;*GB0010001.C-100589.PRINT..
    Y.SEQ = 1

    LOOP WHILE Y.SEQ > 0 DO
        Y.DE.ADDRESS.LIST.ID = Y.ADDRESS:Y.SEQ
        CALL F.READ(FN.DE.ADDRESS, Y.DE.ADDRESS.LIST.ID,R.DE.ADDRESS, F.DE.ADDRESS, YERR)

        IF YERR NE '' AND Y.SEQ EQ 1 THEN
            Y.MSG = yRecordNotFound : @FM : Y.DE.ADDRESS.LIST.ID : @VM : ' DE.ADDRESS APPLICATION'
            Y.TABLE.ID = Y.TMPDIRECCIONESCLIENTE
            Y.PROCESS.FLAG.TABLE<1,3> = ""
            GOSUB TRACE.ERROR
            RETURN
        END
        IF YERR NE '' THEN
            Y.SEQ = 0
            RETURN  ;*CONTINUE
        END
        R.DE.ADDRESS.OLD = ''
        CALL F.READ(FN.DE.ADDRESS$HIS, Y.DE.ADDRESS.LIST.ID:";1", R.DE.ADDRESS.OLD, F.DE.ADDRESS$HIS, YERR)
        IF R.DE.ADDRESS.OLD EQ '' THEN
            R.DE.ADDRESS.OLD = R.DE.ADDRESS
        END

        Y.CLI.COUNTRY = R.DE.ADDRESS<DE.ADD.COUNTRY,1>      ;* PACS00169639
        Y.CLI.COUNTRY.TOWN =  R.DE.ADDRESS<DE.ADD.TOWN.COUNTY,1>      ;* PACS00169639
        Y.L.AUD.POSITION = DCOUNT(R.DE.ADDRESS<DE.ADD.INPUTTER>, @VM)
        Y.CLI.INPUTTER.LAST = FIELD(R.DE.ADDRESS<DE.ADD.INPUTTER, Y.L.AUD.POSITION>,'_',2)
        Y.CLI.DATE.TIME.LAST = R.DE.ADDRESS<DE.ADD.DATE.TIME, Y.L.AUD.POSITION>
        Y.CLI.DATE.TIME.LAST = ICONV(Y.CLI.DATE.TIME.LAST,'DI')
        Y.CLI.DATE.TIME.LAST = OCONV(Y.CLI.DATE.TIME.LAST,'D4')
        Y.CLI.DATE.TIME.LAST = Y.CLI.DATE.TIME.LAST[7,4]:Y.CLI.DATE.TIME.LAST[4,2]:Y.CLI.DATE.TIME.LAST[1,2]

        Y.CLI.INPUTTER.ADD = FIELD(R.DE.ADDRESS.OLD<DE.ADD.INPUTTER,1>,'_',2)
        Y.CLI.DATE.TIME.ADD = R.DE.ADDRESS.OLD<DE.ADD.DATE.TIME, 1>
        Y.CLI.DATE.TIME.ADD = ICONV(Y.CLI.DATE.TIME.ADD,'DI')
        Y.CLI.DATE.TIME.ADD = OCONV(Y.CLI.DATE.TIME.ADD,'D4')
        Y.CLI.DATE.TIME.ADD = Y.CLI.DATE.TIME.ADD[7,4]:Y.CLI.DATE.TIME.ADD[4,2]:Y.CLI.DATE.TIME.ADD[1,2]

        Y.CLI.TIPO.DIRECCION = R.DE.ADDRESS<DE.ADD.LOCAL.REF,Y.CLI.TIPO.DIRECCION.POS>
        Y.CLI.APR.POSTAL = R.DE.ADDRESS<DE.ADD.LOCAL.REF,Y.CLI.APR.POSTAL.POS>
        Y.CLI.NO.DIRECCION = R.DE.ADDRESS<DE.ADD.LOCAL.REF,Y.CLI.NO.DIRECCION.POS>
        Y.TMPDIRCLIENTEBASEPRINCIPAL='N'
        IF Y.CLI.NO.DIRECCION EQ 'DOMICILIO.PRINCIPAL' THEN
            Y.TMPDIRCLIENTEBASEPRINCIPAL='S'
        END
        Y.MSG = ""
*20.4
        IF Y.CLI.TIPO.DIRECCION NE '' THEN
            GOSUB PROCESS.CUS.ADDRESS
            IF Y.MSG THEN
                RETURN
            END
        END ELSE
            Y.CLI.TIPO.DIRECCION = 'DOMICILIO.PRINCIPAL'
            GOSUB PROCESS.CUS.ADDRESS
            IF Y.MSG THEN
                RETURN
            END
        END

        Y.SEQ = Y.SEQ + 1

    REPEAT
RETURN
*-----------------------------------------------------------------------------------
PROCESS.CUS.ADDRESS:* Call to process each AA
*-----------------------------------------------------------------------------------
GOSUB GET.ADDRESS.TYPE.COL
IF Y.MSG THEN
RETURN
END

IF Y.CLI.NO.DIRECCION EQ "" THEN    ;*ADDRESS CODE VALIDATION
Y.CLI.NO.DIRECCION = 1          ;* BY Default set 1, PACS00169639
END

Y.CLI.STREET.ADDR.DESC = Y.CLI.NO.DIRECCION:" ":R.DE.ADDRESS<DE.ADD.STREET.ADDRESS,1>:" ":Y.CLI.CAMPO3.09.POS:" ":Y.CLI.CAMPO4.09.POS:" ":Y.CLI.COUNTRY:" ":Y.CLI.COUNTRY.TOWN

Y.LINEA.DIR.CLI = ""
Y.CLI.NO.DIRECCION = Y.SEQ

Y.LINEA.DIR.CLI = ""
Y.DELIM='~'
Y.CLI.NO.DIRECCION = Y.SEQ
Y.TMPDIRCLIENTEBARRIO ='999'

Y.TMPDIRCLIENTEESTADO='999'
Y.TMPDIRCLIENTEMUNICIPIO='999'
Y.TMPDIRCLIENTEPAIS='1'
Y.TMPDIRCLIENTESECTOR='999'

Y.LINEA.DIR.CLI   = TRIM(Y.CLI.COD.EMPRESA,"","B"):Y.DELIM:TRIM(Y.CLI.COD.CLIENTE,"","B"):Y.DELIM:Y.TMPDIRCLIENTEBARRIO:Y.DELIM:"1":Y.DELIM:"T24"
Y.LINEA.DIR.CLI   := Y.DELIM:TRIM(Y.CLI.STREET.ADDR.DESC[1,200],"","B"):Y.DELIM:TRIM(Y.CLI.APR.POSTAL[1,100],"","B"):Y.DELIM:"":Y.DELIM:TRIM(Y.CLI.INPUTTER.ADD[1,20],"","B")
Y.LINEA.DIR.CLI   := Y.DELIM:TRIM(Y.CLI.DATE.TIME.ADD,"","B"):Y.DELIM:TRIM(Y.CLI.INPUTTER.LAST[1,20],"","B"):Y.DELIM:TRIM(Y.CLI.DATE.TIME.LAST,"","B")
Y.LINEA.DIR.CLI   := Y.DELIM:Y.TMPDIRCLIENTEPAIS:Y.DELIM:Y.TMPDIRCLIENTEESTADO:Y.DELIM:Y.TMPDIRCLIENTEMUNICIPIO:Y.DELIM:Y.TMPDIRCLIENTESECTOR
Y.LINEA.DIR.CLI   := Y.DELIM:Y.TMPDIRCLIENTEBARRIO:Y.DELIM:Y.TMPDIRCLIENTEBASEPRINCIPAL:Y.DELIM
*:"":Y.DELIM

R.COL.QUEUE = ''
R.COL.QUEUE<1> = Y.TMPDIRECCIONESCLIENTE
R.COL.QUEUE<2> = Y.LINEA.DIR.CLI
GOSUB WRITE.RECORD
RETURN
*-----------------------------------------------------------------------------------
PROCESS.AA:         * Call to process each AA
*-----------------------------------------------------------------------------------
    Y.CREDIT = ""; PR.GESTIPOGARANTIAS = ''; Y.CREDIT.TXN = ""; PR.GESGARANTIAS = ''; PR.GESCREDITOSGARANTIAS = '';P.GESCREDITOINTEGRACION = ''
*todo cambio de nombre en rutina, anterior :L.APAP.COL.FF.EXTRACT.CREDIT
*CALL L.APAP.COL.FF.COLLECTOR.CREDIT(CUSTOMER.ID, Y.CREDIT, Y.CREDIT.TXN,PR.GESGARANTIAS,PR.GESCREDITOSGARANTIAS,PR.GESTIPOGARANTIAS,P.GESCREDITOINTEGRACION)
    APAP.LAPAP.lApapColFfCollectorCredit(CUSTOMER.ID, Y.CREDIT, Y.CREDIT.TXN,PR.GESGARANTIAS,PR.GESCREDITOSGARANTIAS,PR.GESTIPOGARANTIAS,P.GESCREDITOINTEGRACION);*R22 MANUAL CODE COVERSION
*   IF E THEN
*       RETURN
*   END
    IF Y.PROCESS.FLAG.TABLE<1,5> EQ "tmpcredito" THEN
        Y.TOTAL.E = DCOUNT(Y.CREDIT,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "tmpcredito"
            R.COL.QUEUE<2> = Y.CREDIT<Y.I,1>
            R.COL.QUEUE<4> = Y.CREDIT<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,4> EQ "tmpmovimientos" THEN
        Y.TOTAL.E = DCOUNT(Y.CREDIT.TXN,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "tmpmovimientos"
            R.COL.QUEUE<2> = Y.CREDIT.TXN<Y.I,1>
            R.COL.QUEUE<4> = Y.CREDIT.TXN<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END

    IF Y.PROCESS.FLAG.TABLE<1,7> EQ "gestipogarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESTIPOGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "gestipogarantias"
            R.COL.QUEUE<2> = PR.GESTIPOGARANTIAS<Y.I,1>
            R.COL.QUEUE<4> = PR.GESTIPOGARANTIAS<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,6> EQ "gescreditointegracion" THEN
        Y.TOTAL.E = DCOUNT(P.GESCREDITOINTEGRACION,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "gescreditointegracion"
            R.COL.QUEUE<2> = P.GESCREDITOINTEGRACION<Y.I,1>
            R.COL.QUEUE<4> = P.GESCREDITOINTEGRACION<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,8> EQ "gesgarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "gesgarantias"
            R.COL.QUEUE<2> = PR.GESGARANTIAS<Y.I,1>
            R.COL.QUEUE<4> = PR.GESGARANTIAS<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END
    IF Y.PROCESS.FLAG.TABLE<1,9> EQ "gescreditogarantias" THEN
        Y.TOTAL.E = DCOUNT(PR.GESCREDITOSGARANTIAS,@FM)
        Y.I = 1
        LOOP WHILE Y.I LE Y.TOTAL.E
            R.COL.QUEUE = ""
            R.COL.QUEUE<1> = "gescreditogarantias"
            R.COL.QUEUE<2> = PR.GESCREDITOSGARANTIAS<Y.I,1>
            R.COL.QUEUE<4> = PR.GESCREDITOSGARANTIAS<Y.I,2>
            GOSUB WRITE.RECORD
            Y.I++
        REPEAT
    END
RETURN
*-----------------------------------------------------------------------------------
WRITE.RECORD:
*-----------------------------------------------------------------------------------
    Y.TOTAL.INSERT++
    LIST.INSERTS.STMT(Y.TOTAL.INSERT) = R.COL.QUEUE
RETURN
*-----------------------------------------------------------------------------------
FINAL.WRITE.RECORD:
*-----------------------------------------------------------------------------------
    Y.CONTADOR = '30000';
    I = 1
    LOOP WHILE I LE Y.TOTAL.INSERT
        Y.QUEUE.ID = ''; Y.ID = ''; Y.ID = RND(Y.CONTADOR)
        CALL ALLOCATE.UNIQUE.TIME(Y.QUEUE.ID)
        Y.QUEUE.ID = DATE():Y.QUEUE.ID

        R.REDO.COL.QUEUE = LIST.INSERTS.STMT(I)

*---------------------------------------------------------------------------------------------------
        BEGIN CASE
            CASE R.REDO.COL.QUEUE<1> EQ 'gesgarantias'
                Y.ID.COLLECT = ''; Y.ID.COLLECT = R.REDO.COL.QUEUE<2>
                Y.ID.COLLECT = FIELD(Y.ID.COLLECT,'~',1)
                Y.ID.COLLECT = Y.ID.COLLECT:"|":Y.ID:Y.QUEUE.ID
                R.LAPAP.CONCAT.TMP.GESGARANTIAS = ''; ERROR.TMP.GESGARANTIA = '';
*                CALL F.READ(FN.LAPAP.CONCAT.TMP.GESGARANTIAS,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.GESGARANTIAS,FV.LAPAP.CONCAT.TMP.GESGARANTIAS,ERROR.TMP.GESGARANTIA)
                CALL F.READU(FN.LAPAP.CONCAT.TMP.GESGARANTIAS,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.GESGARANTIAS,FV.LAPAP.CONCAT.TMP.GESGARANTIAS,ERROR.TMP.GESGARANTIA,'');* R22 UTILITY AUTO CONVERSION
                IF NOT (R.LAPAP.CONCAT.TMP.GESGARANTIAS) THEN
                    R.LAPAP.CONCAT.TMP.GESGARANTIAS = R.REDO.COL.QUEUE<2>;
                    CALL F.WRITE(FN.LAPAP.CONCAT.TMP.GESGARANTIAS,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.GESGARANTIAS )
                    CALL OCOMO("Escribiendo el registro: ":Y.ID.COLLECT:" en la tabla ":FN.LAPAP.CONCAT.TMP.GESGARANTIAS)
                END
                I++
                CONTINUE;

            CASE R.REDO.COL.QUEUE<1> EQ 'gestipogarantias'
                Y.ID.COLLECT = ''; Y.ID.COLLECT = R.REDO.COL.QUEUE<2>;
                Y.ID.COLLECT = CHANGE(Y.ID.COLLECT,' ','')
                Y.ID.COLLECT = CHANGE(Y.ID.COLLECT,'~','*')
                Y.ID.COLLECT = Y.ID.COLLECT:"|":Y.ID:Y.QUEUE.ID
                R.LAPAP.CONCAT.TMP.TIPOGARANTIA = '';
                ERROR.TIPOGARANTIA = '';
*                CALL F.READ (FN.LAPAP.CONCAT.TMP.TIPOGARANTIA,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.TIPOGARANTIA,FV.LAPAP.CONCAT.TMP.TIPOGARANTIA,ERROR.TIPOGARANTIA)
                CALL F.READU (FN.LAPAP.CONCAT.TMP.TIPOGARANTIA,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.TIPOGARANTIA,FV.LAPAP.CONCAT.TMP.TIPOGARANTIA,ERROR.TIPOGARANTIA,'');* R22 UTILITY AUTO CONVERSION
                IF NOT (R.LAPAP.CONCAT.TMP.TIPOGARANTIA) THEN
                    R.LAPAP.CONCAT.TMP.TIPOGARANTIA = R.REDO.COL.QUEUE<2>;
                    CALL F.WRITE(FN.LAPAP.CONCAT.TMP.TIPOGARANTIA,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.TIPOGARANTIA)
                    CALL OCOMO("Escribiendo el registro: ":Y.ID.COLLECT:" en la tabla ":FN.LAPAP.CONCAT.TMP.TIPOGARANTIA)
                END
                I++
                CONTINUE;
        END CASE

        Y.EXTRACT.OUT.PATH=R.REDO.INTERFACE.PARAM<REDO.INT.PARAM.FI.AUTO.PATH>

        Y.FILE.ID='SESSION':R.REDO.COL.QUEUE<1>:SESSION.NO

        OPENSEQ Y.EXTRACT.OUT.PATH, Y.FILE.ID TO Y.FILE.PATH ELSE
            CREATE Y.FILE.PATH ELSE
                CALL OCOMO("CANNOT OPEN SESSION FILE PATH")
            END
        END
        WRITESEQ R.REDO.COL.QUEUE<2> APPEND TO Y.FILE.PATH ELSE
            CALL OCOMO("CANNOT WRITE TO SESSION FILE OF ID ":Y.FILE.ID)
        END
        I++
    REPEAT
RETURN
*-----------------------------------------------------------------------------------
GET.TEL.SEQUENCE:
*-----------------------------------------------------------------------------------
    LOCATE Y.CLI.TELF.TYPE IN Y.TEL.TYPE.LIST<1> SETTING Y.POS THEN
        Y.CURR.TEL.SEQ = Y.TEL.TYPE.SEQ<Y.POS>
        Y.TEL.TYPE.SEQ<Y.POS> = (Y.CURR.TEL.SEQ + 1)
    END ELSE
        Y.CURR.TEL.SEQ = 1
        Y.TEL.TYPE.SEQ<Y.POS> = 2
        Y.TEL.TYPE.LIST<Y.POS> = Y.CLI.TELF.TYPE
    END
RETURN
*-----------------------------------------------------------------------------------
GET.REPRES.ID.TYPE:
*-----------------------------------------------------------------------------------
    BEGIN CASE
*        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS> NE ""         ;*CEDULA = 1
        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS> NE ""         ;*CEDULA EQ 1;* R22 UTILITY AUTO CONVERSION
            Y.CLI.ID.REPRES = 01
            Y.CLI.NUM.REPRES = R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.CI.POS>
*        CASE R.CUSTOMER.REPRES<EB.CUS.LEGAL.ID> NE "" ;*PASAPORTE = 4
        CASE R.CUSTOMER.REPRES<EB.CUS.LEGAL.ID> NE "" ;*PASAPORTE EQ 4;* R22 UTILITY AUTO CONVERSION
            Y.CLI.ID.REPRES = 04
            Y.CLI.NUM.REPRES = R.CUSTOMER<EB.CUS.LEGAL.ID>
*        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS> NE ""        ;*RCN = 2
        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS> NE ""        ;*RCN EQ 2;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.REPRES = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.RCN.POS>
            Y.CLI.ID.REPRES = 02
*        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS> NE ""    ;*ACTA NACIMIENTO = 8
        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS> NE ""    ;*ACTA NACIMIENTO EQ 8;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.REPRES = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.ACTANAC.POS>
            Y.CLI.ID.REPRES = 08
*        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS> NE ""    ;*NUMERO UNICO = 15
        CASE R.CUSTOMER.REPRES<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS> NE ""    ;*NUMERO UNICO EQ 15;* R22 UTILITY AUTO CONVERSION
            Y.CLI.NUM.REPRES = R.CUSTOMER<EB.CUS.LOCAL.REF,Y.CLI.TIPO.ID.NOUNICO.POS>
            Y.CLI.ID.REPRES = 15
    END CASE
RETURN
*-----------------------------------------------------------------------------------
GET.TEL.AUDIT.INFO:
*-----------------------------------------------------------------------------------
    FIND Y.CLI.TELF.TYPE IN R.COL.TRACE.PHONE<REDO.COL.TP.PHONE.TYPE,1> SETTING Y.FM.POS,Y.VM.POS THEN
        Y.TEL.DATE.TIME.ADD = R.COL.TRACE.PHONE<REDO.COL.TP.CREATION.DATE,Y.VM.POS>
        Y.TEL.USER          = R.COL.TRACE.PHONE<REDO.COL.TP.LAST.IMPUTTER,Y.VM.POS>
        Y.TEL.DATE.TIME.UPD = R.COL.TRACE.PHONE<REDO.COL.TP.LAST.UPD.DATE,Y.VM.POS>
    END ELSE
        Y.TEL.DATE.TIME.ADD = Y.CLI.FECHA.ADICION
        Y.TEL.USER          = Y.CLI.MODIF.POR
        Y.TEL.DATE.TIME.UPD = Y.CLI.FECHA.MODIF
    END
    Y.TEL.USER = Y.TEL.USER[1,20]

RETURN
*-----------------------------------------------------------------------------------
GET.ADDRESS.TYPE.COL:
*-----------------------------------------------------------------------------------
    Y.ADD.TYPE.LIST = ""      ;*valor statico de  Y.ADD.TYPE.LIST
    Y.ADD.TYPE.LIST = C.ADD.TYPE.LIST

    LOCATE Y.CLI.TIPO.DIRECCION IN Y.ADD.TYPE.LIST<1,1> SETTING Y.POS THEN
        Y.CLI.TIPO.DIRECCION = Y.ADD.TYPE.LIST<2,Y.POS>
    END ELSE
        Y.MSG = yNonMappingValue : @FM : Y.CLI.TIPO.DIRECCION : @VM : "TIPO.DIRECCION" : @VM : CUSTOMER.ID
        Y.TABLE.ID = Y.TMPDIRECCIONESCLIENTE
        Y.PROCESS.FLAG.TABLE<1,3> = ""
        GOSUB TRACE.ERROR
        RETURN
    END
RETURN
*-----------------------------------------------------------------------------------
TRACE.ERROR:
*-----------------------------------------------------------------------------------

RETURN
*-----------------------------------------------------------------------------------
*--------------------
WRITE.TEMP.CUSTOMER:
*---------------------
    Y.ID.COLLECT = CUSTOMER.ID;
*    CALL F.READ (FN.LAPAP.CONCAT.TMP.CLIENTES,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.CLIENTES,FV.LAPAP.CONCAT.TMP.CLIENTES,ERROR.CLIENTES)
    CALL F.READU (FN.LAPAP.CONCAT.TMP.CLIENTES,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.CLIENTES,FV.LAPAP.CONCAT.TMP.CLIENTES,ERROR.CLIENTES,'');* R22 UTILITY AUTO CONVERSION
    IF NOT(R.LAPAP.CONCAT.TMP.CLIENTES) THEN
        R.LAPAP.CONCAT.TMP.CLIENTES = Y.ID.COLLECT;
        CALL F.WRITE(FN.LAPAP.CONCAT.TMP.CLIENTES,Y.ID.COLLECT,R.LAPAP.CONCAT.TMP.CLIENTES)

    END
RETURN

END
