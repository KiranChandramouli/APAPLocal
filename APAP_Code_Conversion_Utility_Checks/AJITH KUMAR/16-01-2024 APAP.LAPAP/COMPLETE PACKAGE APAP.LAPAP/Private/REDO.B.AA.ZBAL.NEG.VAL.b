* @ValidationCode : MjotMjg5NjE5Njc2OkNwMTI1MjoxNzA1MDQ2MDMzNTQ2OmFqaXRoOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 12 Jan 2024 13:23:53
...
*13-07-2023            Conversion Tool             R22 Auto Code conversion                FM TO @FM,VM TO @VM,SM TO @SM,<= TO LE ,X TO X.VAR
...
*************
  ...
  *  Y.MAIN.STRT.DTE = R.AA.ARR<AA.ARR.START.DATE>
    ...
    Y.MAIN.PROD.GROUP = R.AA.ARR<AA.Framework.ArrangementSim.ArrProductGroup>
    Y.MAIN.ARR.STATUS = R.AA.ARR<AA.Framework.ArrangementSim.ArrArrStatus>
    Y.MAIN.ARR.PRCT = R.AA.ARR<AA.Framework.Arrangement.ArrProduct>
    Y.MAIN.STRT.DTE = R.AA.ARR<AA.Framework.ArrangementSim.ArrStartDate>
    IF Y.MAIN.STRT.DTE GT YLST.TODAY THEN
        RETURN
    END
    Y.CUSTOMER.ID = R.AA.ARR< AA.Framework.ArrangementSim.ArrCustomer>
    Y.CURRENCY    = R.AA.ARR<AA.Framework.Arrangement.ArrCurrency>
    Y.LINKED.APPL = R.AA.ARR<AA.Framework.ArrangementSim.ArrLinkedAppl >
    Y.LINKED.APPL.ID = R.AA.ARR<AA.Framework.ArrangementSim.ArrLinkedAppl >
...
************
    ...
    Y.DCNT.CUR.ASS = DCOUNT(Y.CUR.ASSET.TYPE,@VM) ;*R22 AUTO CODE CONVERSION
    Y.CNT.CUR.TYPE = '1'
    LOOP
    WHILE Y.CNT.CUR.TYPE LE Y.DCNT.CUR.ASS
        ...
        IF SYS.DATE LE YLST.TODAY THEN  ;*R22 AUTO CODE CONVERSION
...
**************
    ...
    CHANGE @SM TO @FM IN  Y.BILL.LIST  ;*R22 AUTO CODE CONVERSION
    CHANGE @SM TO @FM IN  Y.BILL.TYPE  ;*R22 AUTO CODE CONVERSION
    CHANGE @SM TO @FM IN  Y.BILL.STATUS  ;*R22 AUTO CODE CONVERSION

    CHANGE @VM TO @FM IN  Y.BILL.LIST  ;*R22 AUTO CODE CONVERSION
    CHANGE @VM TO @FM IN  Y.BILL.TYPE  ;*R22 AUTO CODE CONVERSION
    ...
    YBILL.CNT = DCOUNT(Y.BILL.LIST,@FM)  ;*R22 AUTO CODE CONVERSION
...
PRICNI.MVMT.APPL:
*---------------
    YARR.CNT = 0; X.VAR = 1; TOTCOMMITMENT.VAL = 0   ;*R22 AUTO CODE CONVERSION
    YARR.CNT = DCOUNT(YARR.ASST,@FM)   ;*R22 AUTO CODE CONVERSION
    LOOP
    UNTIL X.VAR GT YARR.CNT   ;*R22 AUTO CODE CONVERSION
        ECB.AMT = 0; ORG.ECB.AMT = 0; ASSET.VAL = ''; TEMP.FLG = 0
        ASSET.VAL = YARR.ASST<X.VAR>  ;*R22 AUTO CODE CONVERSION
        ECB.AMT = Y.BAL.PRIC.GP<X.VAR> ;*R22 AUTO CODE CONVERSION
        ORG.ECB.AMT = Y.BAL.PRIC.ORG<X.VAR>  ;*R22 AUTO CODE CONVERSION
...
AA.Framework.GetPeriodBalances(Y.ACCOUNT, BALANCE.TO.CHECK,REQUEST.TYPE, START.DATE, END.DATE, '',BAL.DETAILS, ERROR.MESSAGE);* R22 UTILITY AUTO CONVERSION
...
AA.Framework.ReadActivityHistory(AA.ID, REQD.MODE, EFF.DATE, R.AA.ACTIVITY.HISTORY);* R22 UTILITY AUTO CONVERSION
    ...
    END
    Y.PROD.GUP = R.AA.ARR<AA.Framework.ArrangementSim.ArrProductGroup>
...
*    CALL F.READ(FN.REDO.APAP.PROPERTY.PARAM,Y.PROD.GUP,R.REDO.APAP.PROPERTY.PARAM,F.REDO.APAP.PROPERTY.PARAM,ERR.REDO.APAP.PROPERTY.PARAM)
    CALL CACHE.READ(FN.REDO.APAP.PROPERTY.PARAM,Y.PROD.GUP,R.REDO.APAP.PROPERTY.PARAM,ERR.REDO.APAP.PROPERTY.PARAM);* R22 UTILITY AUTO CONVERSION
