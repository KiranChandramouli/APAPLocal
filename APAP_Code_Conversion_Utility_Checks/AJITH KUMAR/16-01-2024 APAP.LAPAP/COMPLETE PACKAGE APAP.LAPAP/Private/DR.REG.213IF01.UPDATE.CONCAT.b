...
*13-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     VM to @VM, FM to @FM ,T24.BP is Removed ,= to EQ
...
*----------------------------------------------------------------------------------------
    ...
    $INSERT I_F.TELLER
    $INSERT I_F.TRANSACTION ;*R22 Auto code conversion
    $INSERT I_F.CURRENCY
    $INSERT I_F.ACCOUNT
    ...
    $INSERT I_F.T24.FUND.SERVICES ;*R22 Auto code conversion
    ...
    BEGIN CASE
        CASE CONTROL.LIST<1,1> EQ 'ACCT.ENT.LWORK.DAY' ;*R22 Auto code conversion
            GOSUB PROCESS.ACCT
        CASE CONTROL.LIST<1,1> EQ 'REDO.TRANSACTION'
            GOSUB PROCESS.AA
        CASE CONTROL.LIST<1,1> EQ 'REDO.AA.OVERPAY' ;*R22 Auto code conversion
...
*************
    ...
    IF R.ACCT.ENT.LWORK.DAY THEN
        CTR.STMT.ID = 1
        CNT.STMT.ID = DCOUNT(R.ACCT.ENT.LWORK.DAY,@FM) ;*R22 Auto code conversion
        LOOP
...
*    CALL F.READ(FN.DR.REG.213IF01.CONCAT,CONCAT.ID,R.DR.REG.213IF01.CONCAT,F.DR.REG.213IF01.CONCAT,DR.REG.213IF01.CONCAT.ERR)
    CALL F.READU(FN.DR.REG.213IF01.CONCAT,CONCAT.ID,R.DR.REG.213IF01.CONCAT,F.DR.REG.213IF01.CONCAT,DR.REG.213IF01.CONCAT.ERR,'');* R22 UTILITY AUTO CONVERSION
    IF R.DR.REG.213IF01.CONCAT THEN
        ...
        FINDSTR Y.T24.FS.REF IN YTEMP.YSTMT.VAL<1,1> SETTING LPOSN,LSM,LVM THEN
            RETURN
        END
        CNT.STMT = DCOUNT(YTEMP.YSTMT.VAL,@VM) ;*R22 Auto code conversion
        CTR.STMT = CNT.STMT + 1
