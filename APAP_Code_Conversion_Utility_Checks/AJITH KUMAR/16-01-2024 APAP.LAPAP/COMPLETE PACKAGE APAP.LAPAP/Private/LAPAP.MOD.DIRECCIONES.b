...
* 13-07-2023    Conversion Tool        R22 Auto Conversion     BP is removed in insert file,M to M.VAR,B to B.VAR
...
EB.LocalReferences.GetLocRef("CUSTOMER","L.CU.CIDENT",POSCIDENT);* R22 UTILITY AUTO CONVERSION
...
EB.LocalReferences.GetLocRef("CUSTOMER","L.CU.TEL.NO",TEL.POS);* R22 UTILITY AUTO CONVERSION
    CUSTOMER.PHONE = R.CUSTOMER<EB.CUS.LOCAL.REF,TEL.POS>
*    CALL GET.LOC.REF("CUSTOMER","L.CU.RES.SECTOR",SECTOR.POS)
EB.LocalReferences.GetLocRef("CUSTOMER","L.CU.RES.SECTOR",SECTOR.POS);* R22 UTILITY AUTO CONVERSION
...
*   Email Validation
    ...
    M.VAR  = DCOUNT(ACTUAL.MAIL,@VM)
    AA = M.VAR - 1
    PREVIOUS.MAIL = ACTUAL.MAIL<1,M.VAR>
...
*   Phone Validation
    ...
    FOR B.VAR = 1 TO MM STEP 1
        FIND ACTUAL.PHONE<1,B.VAR> IN ARR.PHONE SETTING V.FLD, V.VAL ELSE
            MDIR.PHONE<-1> = ACTUAL.AREA<1,B.VAR>:ACTUAL.PHONE<1,B.VAR>
...
*   Address validation
    ...
    IF VALUES.TO.INSERT NE '' THEN
        ...
        APAP.LAPAP.lapapBuildMonitorFromVersion(ARR) ;*R22 Manual Conversion
