* @ValidationCode : MjotMTk5OTgyODU4NjpDcDEyNTI6MTcwNTM4MzA5NTgwODphaml0aDotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Jan 2024 11:01:35
    ...
    WHILE Y.VAR1 LE Y.OWNER.CNT
        ...
        IF Y.ARR.ID THEN
           * CALL F.READ(FN.AA.ARRANGEMENT,Y.ARR.ID,R.ARR.REC,F.AA.ARRANGEMENT,ARR.ERR)
            R.ARR.REC=AA.Framework.Arrangement.Read(Y.ARR.ID, ARR.ERR)
            Y.PRODUCT=R.ARR.REC<AA.Framework.Arrangement.ArrProduct>
            Y.STATUS = R.ARR.REC<AA.Framework.ArrangementSim.ArrArrStatus>
...
*--TODO
            ...
            Y.ARRAY:='*':R.ARR.REC<AA.Framework.Arrangement.ArrCurrency >
            Y.ARRAY:='*':R.ARR.REC<AA.Framework.ArrangementSim.ArrStartDate>:@FM
        END
        Y.VAR1 += 1 ;*AUTO R22 CODE CONVERSION
    REPEAT
    ...
    WHILE Y.VAR2 LE Y.OTHER.CNT
        ...
        IF Y.ARR.ID THEN
           * CALL F.READ(FN.AA.ARRANGEMENT,Y.ARR.ID,R.ARR.REC,F.AA.ARRANGEMENT,ARR.ERR)
            R.REC.REC=AA.Framework.Arrangement.Read(Y.ARR.ID, ARR.ERR)
            Y.PRODUCT=R.ARR.REC< AA.Framework.Arrangement.ArrProduct>
            Y.STATUS = R.ARR.REC<AA.Framework.ArrangementSim.ArrArrStatus>
            IF Y.STATUS EQ 'AUTH' THEN
...
*--TODO
            ...
            GOSUB GET.NEXT.PAYMENT
            Y.ARRAY:='*':Y.NEXTPAY.AMT
            Y.ARRAY:='*':R.ARR.REC<AA.Framework.ArrangementSim.ArrCurrency>
            Y.ARRAY:='*':R.ARR.REC<AA.Framework.ArrangementSim.ArrStartDate>:@FM
        END
        Y.VAR2 += 1 ;*AUTO R22 CODE CONVERSION
    REPEAT
...
AA.Framework.GetArrangementConditions(ARRANGEMENT.ID,PROP.CLASS,PROP.NAME,'','',R.AA,RET.ERR);* R22 UTILITY AUTO CONVERSION
