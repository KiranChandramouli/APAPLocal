* @ValidationCode : MjoxNjI4NTY3Njg0OlVURi04OjE3MDI5MDE3NDU5Nzg6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 18 Dec 2023 17:45:45
...
*-----------------------------------------------------------------------------
* Read Main Files
*APAP.REDORETAIL.REDO.COL.EXTRACT.CREDIT.READ(AA.ID,R.AA,R.ACT.HIST,R.PRINCIPALINT,R.PENALTYINT,R.AA.ACCOUNT.DETAILS)
    APAP.REDORETAIL.redoColExtractCreditRead(AA.ID,R.AA,R.ACT.HIST,R.PRINCIPALINT,R.PENALTYINT,R.AA.ACCOUNT.DETAILS);* MANUAL R22 CODE CONVERSION
    IF E NE "" THEN
        RETURN
    END
* << PACS00169639
...
*APAP.TAM.REDO.S.COL.GET.BILL.DETAILS(R.AA.ACCOUNT.DETAILS,  Y.PROCESS.DATE,  Y.MORA.CTA.VEN, Y.TMPCREDITODIASATRASO, Y.TMPCREDITOCUOTASVENCIDAS, Y.TMPCREDITOCUOTASPAGADAS, Y.TMPCREDITOMONTOMOROSO )
...
*APAP.REDOSRTN.REDO.S.COL.GET.PAY.FREQ(AA.ID,  R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOTIPOCUOTA)
...
*Y.TMPCREDITOFECHAVCTO = redoOracleDate(R.AA.ACCOUNT.DETAILS<AA.AD.MATURITY.DATE>,'yyyyMMdd')      ;* Maturity Date
    Y.TMPCREDITOFECHAVCTO = redoOracleDate(R.AA.ACCOUNT.DETAILSAA.PaymentSchedule.AccountDetails.AdMaturityDate,'yyyyMMdd') ;*Manual R22 Conversion
...
*APAP.REDOSRTN.REDO.S.COL.GET.PAY.TYPE(AA.ID, R.STATIC.MAPPING, Y.PROCESS.DATE, Y.TMPCREDITOFORMAPAGO)
...
*
*APAP.REDORETAIL.REDO.COL.EXTRACT.CREDIT.2("GET.AA.CUSTOMER", Y.ACCOUNT.ID, AA.ID, "", P.GET.AA.CUSTOMER, "")
    APAP.REDORETAIL.redoColExtractCredit2("GET.AA.CUSTOMER", Y.ACCOUNT.ID, AA.ID, "", P.GET.AA.CUSTOMER, "");* MANUAL R22 CODE CONVERSION
    IF E THEN
        RETURN
    END
...
*
*APAP.REDORETAIL.REDO.COL.EXTRACT.CREDIT.2("GET.CREDIT.GRANTED", Y.ACCOUNT.ID, AA.ID, P.GET.CREDIT.GRANTED, "", "")
    APAP.REDORETAIL.redoColExtractCredit2("GET.CREDIT.GRANTED", Y.ACCOUNT.ID, AA.ID, P.GET.CREDIT.GRANTED, "", "");* MANUAL R22 CODE CONVERSION
    IF E THEN
        RETURN
    END
    ...
    GOSUB INSERT.STMT
*APAP.REDORETAIL.REDO.COL.EXTRACT.TXN(Y.PROCESS.DATE, AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.GROUP, Y.AGENCY.CODE, Y.CREDIT.TXN)
    APAP.REDORETAIL.redoColExtractTxn(Y.PROCESS.DATE, AA.ID, R.STATIC.MAPPING, Y.ACCOUNT.ID, Y.PRODUCT.GROUP, Y.AGENCY.CODE, Y.CREDIT.TXN);* MANUAL R22 CODE CONVERSION
...
* CALL AA.SCHEDULE.PROJECTOR(AA.ID, SIM.REF, "",CYCLE.DATE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS, DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)  ;* Routine to Project complete schedules
    CALL AA.PaymentSchedule.ScheduleProjector(AA.ID, SIM.REF, "",CYCLE.DATE, TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS, DUE.TYPE.AMTS, DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS) ;*Manual R22 Conversion
...
*APAP.REDORETAIL.REDO.COL.EXTRACT.CREDIT.2("PROCESS.ITEMS.DETAIL", Y.ACCOUNT.ID, AA.ID, "", "", P.PROCESS.ITEMS.DETAILS)
    APAP.REDORETAIL.redoColExtractCredit2("PROCESS.ITEMS.DETAIL", Y.ACCOUNT.ID, AA.ID, "", "", P.PROCESS.ITEMS.DETAILS);* MANUAL R22 CODE CONVERSION
...
*APAP.TAM.REDO.R.COL.PROCESS.TRACE("EXTRACT", "20", Y.CONTADOR, "TMPCREDITO", Y.PRODUCT.GROUP, C.DESC)
    APAP.TAM.redoRColProcessTrace("EXTRACT", "20", Y.CONTADOR, "TMPCREDITO", Y.PRODUCT.GROUP, C.DESC);* R22 Manual conversion - CALL method format changed
*APAP.TAM.REDO.R.COL.EXTRACT.ERROR(C.DESC, "REDO.COL.EXTRACT.CREDIT",P.TABLE)
    APAP.TAM.redoRColExtractError(C.DESC, "REDO.COL.EXTRACT.CREDIT",P.TABLE);* R22 Manual conversion - CALL method format changed
