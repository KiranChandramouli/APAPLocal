* @ValidationCode : MjotMjA1MjA1OTcyNzpVVEYtODoxNzA2MTAwMzE2MTc1OkFkbWluOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 24 Jan 2024 18:15:16
...
* 24-01-2024     Narmadha V        R22 Utility Changes        Call routine format modified
*----------------------------------------------------------------------------------------------------------------------
*
    $INSERT I_COMMON
    $INSERT I_EQUATE
    ...
    $USING APAP.AA
    $USING AA.PaymentSchedule ;*R22 Utility Changes
...
* Y.DEAL.ARRAY<21,Y.VAR2> = FMT(Y.NEXT.PAY.AMT,"L2,#15")  ;* Next Payment Amount
    ...
    END
    Y.GESTION.CHARGE = FIELD(TOTAL.AMT,'*',7)
    Y.DEAL.ARRAY<22,Y.VAR2> = Y.GESTION.CHARGE
...
* AA.PaymentScheule.ScheduleProjector(ARR.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, PAYMENT.DATES, DUE.DEFER.DATES, PAYMENT.TYPES, DUE.METHODS, DUE.TYPE.AMTS, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(ARR.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, PAYMENT.DATES, DUE.DEFER.DATES, PAYMENT.TYPES, DUE.METHODS, DUE.TYPE.AMTS, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, DUE.OUTS) ;*R22 Utility Changes
