* @ValidationCode : MjoxOTg3Mzg1OTY4OlVURi04OjE3MDI1NDYyMzM3Mjk6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 14 Dec 2023 15:00:33
...
* 14-04-2023          Narmadha V           Manual 22 Conversion     Changed hardcoded value to FN variable and ID variable,Add call OPF.
*-------------------------------------------------------------------------------------
*
*----------------------------------------------------------------------
*
...
*
    ...
    R.AC.STMT.PARAM = ""
    FN.AC.STMT.PARAMETER = "F.AC.STMT.PARAMETER" ;*Manual R22 Conversion - initalise FN variable
    Y.ID = 'SYSTEM' ;*Manual R22 Conversion - initalise FN variable
    IF R.ACCOUNT.PARAMETER<AC.PAR.VALUE.DATED.ACCTNG> EQ 'Y' THEN
*CALL CACHE.READ('F.AC.STMT.PARAMETER', 'SYSTEM', R.AC.STMT.PARAM, '')
        CALL CACHE.READ(FN.AC.STMT.PARAMETER, Y.ID, R.AC.STMT.PARAM, '') ;*Manual R22 conversion - Change Hardcoded value to FN variable
...
*CALL F.READ('F.ACCOUNT.STATEMENT', SUB.ACCT.NO, R.ACCT.STMT, F.ACCOUNT.STATEMENT, YERR)
    CALL F.READ(FN.ACCOUNT.STATEMENT, SUB.ACCT.NO, R.ACCT.STMT, F.ACCOUNT.STATEMENT, YERR) ;*Manual R22 Conversion - Change hardcoded value to FN variable.
    IF YERR THEN
        YERR = ''
...
*
    FN.ACCOUNT.STATEMENT = "F.ACCOUNT.STATEMENT" ;*Manual R22 Convrsion - initalise the variable
    YR.ACCOUNT.STATEMENT = '' ; YERR = '' ; YACCT.NO = ''
* CALL F.READ('F.ACCOUNT.STATEMENT',Y.AC.NO,YR.ACCOUNT.STATEMENT,F.ACCOUNT.STATEMENT,YERR)
    CALL F.READ(FN.ACCOUNT.STATEMENT,Y.AC.NO,YR.ACCOUNT.STATEMENT,F.ACCOUNT.STATEMENT,YERR) ;*Manual R22 Conversion- Change hardcoded value to FN variable.
    IF YERR THEN
        YERR = ''
        YACCT.NO = Y.AC.NO
...
*
    ...
    READ R.ACCT FROM F.ACCOUNT,Y.AC.NO THEN
        IF R.ACCT<AC.CO.CODE> NE ID.COMPANY THEN
            ...
            END
        END
        FN.AC.SUB.ACCOUNT = "F.AC.SUB.ACCOUNT" ;*Manual R22 Conversion - initalise FN Variable
        F.AC.SUB.ACCOUNT = ""
        CALL OPF(FN.AC.SUB.ACCOUNT,F.AC.SUB.ACCOUNT) ;*Manual R22 Conversion -  Add Call OPF.
...
* CALL F.READ("F.AC.SUB.ACCOUNT",Y.AC.NO,R.SUB.ACC.LIST,F.AC.SUB.ACCOUNT,ER)
            CALL F.READ(FN.AC.SUB.ACCOUNT,Y.AC.NO,R.SUB.ACC.LIST,F.AC.SUB.ACCOUNT,ER) ;*Manual R22 Conversion - Change hardcoded value to FN varaible.
