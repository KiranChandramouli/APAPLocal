* @ValidationCode : MjozMzcwMzAxMzU6VVRGLTg6MTcwMjY5ODY3OTQzMzpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Dec 2023 09:21:19
...
* 15-12-2023      Narmadha V        Manual R22 Conversion      Call Routine Format Modified, F.READ to CACHE.READ
*-----------------------------------------------------------------------------
...
*CALL RECORDID.INPUT
        EB.TransactionControl.RecordidInput() ;*Manual R22 Conversion - Call Routine Format Modified
...
*CALL RECORD.READ
            EB.TransactionControl.RecordRead() ;*Manual R22 Conversion - Call Routine Format Modified.
...
* CALL UNAUTH.RECORD.WRITE
            EB.TransactionControl.UnauthRecordWrite() ;*Manual R22 Conversion - Call Routine Format Modified
...
*CALL AUTH.RECORD.WRITE
            EB.TransactionControl.AuthRecordWrite() ;*Manual R22 Conversion - Call Routine Format modified
...
*CALL MB.SDB.TYPE.CHECK.FIELDS
    APAP.REDOEB.mbSdbTypeCheckFields() ;*Manual R22 conversion
    IF E THEN
        T.SEQU = "IFLD"
        CALL ERR
    END
...
* CALL TRANSACTION.ABORT
        EB.TransactionControl.TransactionAbort() ;*Manual R22 Conversion - Call Routine format Modified
        V$ERROR = 1
        MESSAGE = "ERROR"
        RETURN
    END
...
*CALL F.READ(FN.MB.SDB.PARAM,MB.SDB.PARAM.ID,R.MB.SDB.PARAM,F.MB.SDB.PARAM,YERR)
    CALL CACHE.READ(FN.MB.SDB.PARAM,MB.SDB.PARAM.ID,R.MB.SDB.PARAM,YERR) ;*Manual R22 Conversion- Call Routine Format Modified
...
* CALL MB.SDB.TYPE.FIELD.DEFINITIONS
    APAP.REDOEB.mbSdbTypeFieldDefinitions();*Manual R22 conversion
RETURN
