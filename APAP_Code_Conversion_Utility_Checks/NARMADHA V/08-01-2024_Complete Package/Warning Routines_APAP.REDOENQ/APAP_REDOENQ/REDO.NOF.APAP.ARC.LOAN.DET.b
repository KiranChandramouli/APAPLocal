* @ValidationCode : MjotNTAxMjI3OTg0OlVURi04OjE3MDQyODk3Nzg3MDk6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 Jan 2024 19:19:38
...
*Y.LOAN.TYPE          = R.AA.ARRANGEMENT<AA.ARR.PRODUCT,1>
        Y.LOAN.TYPE          = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct,1>
...
*Y.CUSTOMER           = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
        Y.CUSTOMER           = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer>
...
*Y.AA.STATUS          = R.AA.ARRANGEMENT<AA.ARR.ARR.STATUS>
        Y.AA.STATUS          = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrArrStatus>
...
*Y.AA.ARR.LINK.APPLN = R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL>
        Y.AA.ARR.LINK.APPLN = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedAppl>
    END
    IF Y.AA.ARR.LINK.APPLN EQ 'ACCOUNT' THEN
*Y.AA.ACCT.NO =  R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
        Y.AA.ACCT.NO =  R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>
    END
...
*    CALL F.READ(FN.AA.ARRANGEMENT,AA.ARRANGEMENT.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,AA.ARRANGEMENT.ER)
    R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(AA.ARRANGEMENT.ID, AA.ARRANGEMENT.ER)
RETURN
...
* CALL F.READ(FN.AA.ARRANGEMENT.ACTIVITY,EXACT.AAA.ID,R.ARR.ACT,F.AA.ARRANGEMENT.ACTIVITY,ARR.ACT.ERR)
    R.ARR.ACT = AA.Framework.ArrangementActivity.Read(EXACT.AAA.ID, ARR.ACT.ERR)
*Y.ACT.CLASS = R.ARR.ACT<AA.ARR.ACT.ACTIVITY.CLASS>
    Y.ACT.CLASS = R.ARR.ACT<AA.Framework.SimulationCapture.ArrActActivityClass>
    IF NOT(ARR.ACT.ERR) AND Y.ACT.CLASS NE 'LENDING-DISBURSE-TERM.AMOUNT' THEN
*LAST.PAID.AMT =R.ARR.ACT<AA.ARR.ACT.TXN.AMOUNT.LCY>
        LAST.PAID.AMT =R.ARR.ACT<AA.Framework.ArrangementActivity.ArrActTxnAmountLcy>
    END
...
*  CALL F.READ(FN.AA.ACTIVITY.HISTORY,AA.ACTIVITY.HISTORY.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,AA.ACTIVITY.HISTORY.ER)
    R.AA.ACTIVITY.HISTORYR.AA.ACTIVITY.HISTORY =  AA.Framework.ActivityHistory.Read(AA.ACTIVITY.HISTORY.ID, AA.ACTIVITY.HISTORY.ER)
    IF R.AA.ACTIVITY.HISTORY NE '' THEN
        Y.CTR.ED = 1
        LOOP
*           Y.EFFECTIVE.DATE.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE>,@VM)
            Y.EFFECTIVE.DATE.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate>,@VM)
        WHILE Y.CTR.ED LE Y.EFFECTIVE.DATE.HIS
* Y.ACTIVITY.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.CTR.ED>,@SM)
            Y.ACTIVITY.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity>,@SM)
            Y.CTR = 1
            LOOP
            WHILE Y.CTR LE Y.ACTIVITY.HIS
*               Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.CTR.ED,Y.CTR>
                Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity,Y.CTR.ED,Y.CTR>
...
*                  LAST.TXN.DATE =  R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE,Y.CTR.ED>
                    LAST.TXN.DATE =  R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate,Y.CTR.ED>
