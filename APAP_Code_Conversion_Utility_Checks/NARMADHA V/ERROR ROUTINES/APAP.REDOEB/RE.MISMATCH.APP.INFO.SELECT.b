* @ValidationCode : MjotMTg5NzQ2NDY0ODpVVEYtODoxNzAyNzEzNDcwMDg5OkFkbWluOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 16 Dec 2023 13:27:50
...
*16-12-2023    Narmadha V          Manual R22 Conversion   Call Routine format Modified, = to EQ
*----------------------------------------------------------------------------------------
...
*CALL EB.CLEAR.FILE(FN.MISMATCH.CONT.FILE:@FM:SEL.CMD, F.MISMATCH.CONT.FILE) ;*R22 MANUAL CONVERSION
                EB.Service.ClearFile(FN.MISMATCH.CONT.FILE:@FM:SEL.CMD, F.MISMATCH.CONT.FILE) ;*Manual R22 Conversion
            END ELSE
                CRT '***** CLEARING MISMATCH.CONT.FILE *****'
*CALL EB.CLEAR.FILE(FN.MISMATCH.CONT.FILE, F.MISMATCH.CONT.FILE)
                EB.Service.ClearFile(FN.MISMATCH.CONT.FILE, F.MISMATCH.CONT.FILE) ;*Manul R22 Conversion - Call Routine Format Modified
            END
...
*IF ID.LIST = '' THEN
    IF ID.LIST EQ '' THEN ;*Manual R22 cConversion = to EQ
        NULL
    END ELSE
...
*IF Y.SEL.REP.NAME = Y.REPORT THEN
            IF Y.SEL.REP.NAME EQ Y.REPORT THEN ;*Manual R22 cConversion = to EQ
...
*IF R.NEW(RE.SMM.LINE.NO.END)<1,YY,YYY> = "" THEN
                    IF R.NEW(RE.SMM.LINE.NO.END)<1,YY,YYY> EQ "" THEN ;*Manual R22 cConversion = to EQ
*IF Y.COMPARE.REP.LINE.NO = R.NEW(RE.SMM.LINE.NO.ST)<1,YY,YYY> THEN
                        IF Y.COMPARE.REP.LINE.NO EQ R.NEW(RE.SMM.LINE.NO.ST)<1,YY,YYY> THEN ;*Manual R22 cConversion = to EQ
                            Y.LINE.MATCH = 1
                            NO.OF.REC +=1
                            GOSUB EXT.APP.KEYS
                        END
