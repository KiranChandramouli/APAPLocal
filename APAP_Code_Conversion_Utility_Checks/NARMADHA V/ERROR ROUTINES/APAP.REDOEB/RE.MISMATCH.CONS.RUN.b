* @ValidationCode : Mjo1MDk3NjM1Njc6VVRGLTg6MTcwMjc0NzE5NTcwMDpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 16 Dec 2023 22:49:55
...
*16-12-2023    Narmadha V          Manual R22 Conversion   = to EQ,> to GE, < to LE
*----------------------------------------------------------------------------------------
...
* IF Y7.REC<RE.SLC.TYPE> = "HEADING" THEN
        IF Y7.REC<RE.SLC.TYPE> EQ "HEADING" THEN ;*Manual R22 Conversion = to EQ
...
* IF Y1.REC<RE.SRH.SPLIT> = "BY.FOREIGN" THEN
                IF Y1.REC<RE.SRH.SPLIT> EQ "BY.FOREIGN" THEN ;*Manual R22 Conversion = to EQ
                    YPRINT.LINES = 0
                END
            END
...
*---- Processing depends on SPLIT and CURRENCY
        ...
        BEGIN CASE
*CASE Y.CHECK = "ALL"
            CASE Y.CHECK EQ "ALL" ;*Manual R22 Conversion = to EQ
                GOSUB PROCESS.ASSET.RECORDS

*CASE Y.CHECK = "LOCAL"
            CASE Y.CHECK EQ "LOCAL" ;*Manual R22 Conversion = to EQ
                IF Y.CURR.CCY = LCCY THEN
                    GOSUB PROCESS.ASSET.RECORDS
                END

*CASE Y.CHECK = "CURRENCY"
            CASE Y.CHECK EQ "CURRENCY" ;*Manual R22 Conversion = to EQ
                IF Y.FIRST.RUN THEN
*IF Y.CURR.CCY = LCCY THEN
                    IF Y.CURR.CCY EQ LCCY THEN ;*Manual R22 Conversion = to EQ
                        GOSUB PROCESS.ASSET.RECORDS
                    END ELSE
...
*IF Y.CURR.CCY = Y.RPT.CCY THEN
                    IF Y.CURR.CCY EQ Y.RPT.CCY THEN ;*Manual R22 Conversion = to EQ
                        GOSUB PROCESS.ASSET.RECORDS
                    END
                END

*CASE Y.CHECK = "TOTAL.FOREIGN"
            CASE Y.CHECK EQ "TOTAL.FOREIGN" ;*Manual R22 Conversion = to EQ
                IF Y.CURR.CCY <> LCCY THEN
                    GOSUB PROCESS.ASSET.RECORDS
                END

*CASE Y.CHECK = "BY.FOREIGN"
            CASE Y.CHECK EQ "BY.FOREIGN" ;*Manual R22 Conversion = to EQ
                IF Y.FIRST.RUN THEN
                    IF Y.CURR.CCY <> LCCY THEN
                        LOCATE Y.CURR.CCY IN Y.FOREIGN.CURRENCIES<1> BY "AL" SETTING X ELSE
                            INS Y.CURR.CCY BEFORE Y.FOREIGN.CURRENCIES<X>
...
*---- Consolidation key & Balances printed here
    ...
    Y.EXT.INDIC = ""
* IF YLEN > YWIDTH THEN
    IF YLEN GE YWIDTH THEN ;*Manual R22 Conversion > to GE
...
*IF Y.BALANCE.FILE = "" THEN
    IF Y.BALANCE.FILE EQ "" THEN ;*Manual R22 Conversion = to EQ
        Y.BALANCE.FILE = 0
    END
*
...
* IF Y.CAL.REC<RE.ASL.SCHD.AMOUNT,Y.TYPE.POS> = "" THEN
    IF Y.CAL.REC<RE.ASL.SCHD.AMOUNT,Y.TYPE.POS> EQ "" THEN ;*Manual R22 Conversion = to EQ
*IF YMATURITY.DATE.RANGE[1,1] = '/' THEN
        IF YMATURITY.DATE.RANGE[1,1] EQ '/' THEN ;*Manual R22 Conversion = to EQ
            Y.CCY.BAL = Y.CAL.REC<RE.ASL.BALANCE,Y.TYPE.POS> + Y.CAL.REC<RE.ASL.CREDIT.MOVEMENT,Y.TYPE.POS> + Y.CAL.REC<RE.ASL.DEBIT.MOVEMENT,Y.TYPE.POS>
            Y.LOCAL.BAL = Y.CAL.REC<RE.ASL.LOCAL.BALANCE,Y.TYPE.POS> + Y.CAL.REC<RE.ASL.LOCAL.CREDT.MVE,Y.TYPE.POS> + Y.CAL.REC<RE.ASL.LOCAL.DEBIT.MVE,Y.TYPE.POS>
        END
        RETURN
...
*IF YSCHD.MAT.DATE = 0 THEN
            IF YSCHD.MAT.DATE EQ 0 THEN ;*Manual R22 Conversion = to EQ
* call type contracts
                YSCHD.MAT.DATE = YPROC.DATE
            END ELSE
*IF LEN(YSCHD.MAT.DATE) < 4 THEN
                IF LEN(YSCHD.MAT.DATE) LE 4 THEN ;*Manual R22 Conversion < to LE
...
* IF YSCHD.MAT.DATE > YSTART.MAT.DATE.RANGE THEN
        IF YSCHD.MAT.DATE GE YSTART.MAT.DATE.RANGE THEN ;*Manual R22 Conversion > to GE
...
*IF YP.SPACE.BEFORE > 0 THEN
    IF YP.SPACE.BEFORE GE 0 THEN ;*Manual R22 Conversion > to GE
...
* IF YP.SPACE.AFTER > 0 THEN
    IF YP.SPACE.AFTER GE 0 THEN ;*Manual R22 Conversion > to GE
...
* IF Y.RPT.CCY = "" THEN
    IF Y.RPT.CCY EQ "" THEN ;*Manual R22 Conversion = to EQ
        YYCCY = LCCY
    END ELSE
        YYCCY = Y.RPT.CCY
    END
...
*
    ...
    YMM = YMM + YMONTHS
*IF YMM > 12 THEN
    IF YMM GE 12 THEN ;*Manual R22 Conversion > to GE
        YYY = INT(YMM/12)
        YMM = YMM - YYY * 12
* IF YMM = 0 THEN
        IF YMM EQ 0 THEN ;*Manual R22 Conversion = to EQ
            YMM = 12
            YYY = YYY - 1
        END
        YYR = YYR + YYY
...
* IF YDD > YLAST.DAY THEN
        IF YDD GE YLAST.DAY THEN ;*Manual R22 Conversion > to GE
            YDD = YLAST.DAY
        END
    END
*IF LEN(YMM) = 1 THEN
    IF LEN(YMM) EQ 1 THEN ;*Manual R22 Conversion = to EQ
        YMM = "0":YMM
    END
...
*IF MOD(YYR,4) = 0 THEN
        IF MOD(YYR,4) EQ 0 THEN ;*Manual R22 Conversion = to EQ
            YLAST.DAY = "29"
        END ELSE
            YLAST.DAY = "28"
        END
...
*IF INDEX("04 06 09 11",YMM,1) > 0 THEN
        IF INDEX("04 06 09 11",YMM,1) GE 0 THEN ;*Manual R22 Conversion > to GE
            YLAST.DAY = "30"
        END ELSE
            YLAST.DAY = "31"
        END
...
*
    ...
    FOR YD.NO = 1 TO 2
*IF YD.NO = 1 THEN
        IF YD.NO EQ 1 THEN ;*Manual R22 conversion = to EQ
...
*CASE YCHECK.RANGE = "1B"
            CASE YCHECK.RANGE EQ "1B" ;*Manual R22 Conversion = to EQ
                YEND.DATE = YNEXT.RUN.DATE

*CASE YCHECK.RANGE = ""
            CASE YCHECK.RANGE EQ "" ;*Manual R22 Conversion = to EQ
                YEND.DATE = 0

*CASE YCHECK.RANGE = "REST"
            CASE YCHECK.RANGE EQ "REST" ;*Manual R22 Conversion = to EQ
