* @ValidationCode : MjotMTQ5NTMwNzIwNzpVVEYtODoxNzA1OTIzMzkyMTM3OkFkbWluOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 22 Jan 2024 17:06:32
...
* 08-01-2024         Narmadha V      Manual R22 Conversion,R22 Utility Changes   Changed hardcoded value to variable, F.READ to CACHE.READ,ID variable changed instead of Hardcoding
*------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
*CALL CACHE.READ(FN.REDO.TRANS.CODE.PARAM,"SYSTEM",R.REDO.TRANS.CODE.PARAM,REDO.TRANS.CODE.PARAM.ERR)
    ...
    CALL CACHE.READ(FN.REDO.TRANS.CODE.PARAM,IDVAR.1,R.REDO.TRANS.CODE.PARAM,REDO.TRANS.CODE.PARAM.ERR);* R22 Utility Changes
...
*------------
    ...
    ELSE
        ...
        IF Y.RESPONSE.MSG EQ 'FAIL' OR Y.RESPONSE THEN
            ...
            Y.VAR =  "EB-TP.CON.FAIL.CODE" ;*Manual R22 Conversion
            IF Y.RESPONSE THEN
*CALL CACHE.READ(FN.EB.ERROR, "EB-TP.CON.FAIL.CODE", R.EB.ERROR, ERR)	;*R22 Auto Conversion
            ...
            RE.EX.USER = ''
            RE.EX.PC = ''
            APAP.REDOCHNLS.redoInterfaceRecAct(RE.INT.CODE,RE.INT.TYPE,RE.BAT.NO,RE.BAT.TOT,RE.INFO.OR,RE.INFO.DE,RE.ID.PROC,RE.MON.TP,RE.DESC,RE.REC.CON,RE.EX.USER,RE.EX.PC);*R22 Manual Conversion
