* @ValidationCode : Mjo3MzU4MjcwNTU6VVRGLTg6MTcwNTkyNTQzMDA3MzpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Jan 2024 17:40:30
...
* 19-01-2024     Narmadha V    R22 Utility Changes,Manual R22 Conversion    Call Routine Modified, ID Variable used instead of hardcoding
*-------------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
*CALL CACHE.READ(FN.REDO.PAYMENT.DISBURSE.DESC,'SYSTEM',R.REDO.PAYMENT.DISBURSE.DESC,DESC.ERR)
    ...
    CALL CACHE.READ(FN.REDO.PAYMENT.DISBURSE.DESC,IDVAR.1,R.REDO.PAYMENT.DISBURSE.DESC,DESC.ERR);* R22 Utility Changes
...
*    Y.AC.ID = R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
    Y.AC.ID = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId> ;*Manual R22 Conversion
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.AA.ID,'INTEREST','','',RET.PROP,RET.COND,RET.ERR)
    AA.Framework.GetArrangementConditions(Y.AA.ID,'INTEREST','','',RET.PROP,RET.COND,RET.ERR);* R22 Utility Changes
...
*    Y.AAA.IDS = R.AA.ACT.HIS<AA.AH.ACTIVITY.REF> ; Y.AAA.IDS = CHANGE(Y.AAA.IDS,@VM,@SM)    ;*R22 Manual Conversion - changed to AA.AH.ACTIVITY.REF
    Y.AAA.IDS = R.AA.ACT.HIS<AA.Framework.ActivityHistory.AhActivityRef> ;*Manual R22 Conversion
    Y.AAA.IDS = CHANGE(Y.AAA.IDS,@VM,@SM)    ;*R22 Manual Conversion - changed to AA.AH.ACTIVITY.REF
*    Y.AAA.STS = R.AA.ACT.HIS<AA.AH.ACT.STATUS> ; Y.AAA.STS = CHANGE(Y.AAA.STS,@VM,@SM)
    Y.AAA.STS = R.AA.ACT.HIS<AA.Framework.ActivityHistory.AhActStatus>;* Manual R22 Conversion
    Y.AAA.STS = CHANGE(Y.AAA.STS,@VM,@SM) ;*Manual R22 Conversion
...
*CALL AA.GET.PERIOD.BALANCES(Y.AC.ID, BALANCE.TO.CHECK, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, ERROR.MESSAGE)
        AA.Framework.GetPeriodBalances(Y.AC.ID, BALANCE.TO.CHECK, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, ERROR.MESSAGE);* R22 Utility Changes
