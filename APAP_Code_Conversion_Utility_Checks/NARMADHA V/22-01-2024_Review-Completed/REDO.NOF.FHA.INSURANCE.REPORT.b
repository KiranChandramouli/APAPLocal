* @ValidationCode : MjotNDczMTQxNDgzOlVURi04OjE3MDU5MTgzNTA2NjM6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Jan 2024 15:42:30
...
SUBROUTINE REDO.NOF.FHA.INSURANCE.REPORT(Y.FINAL.ARRAY)
*------------------------------------------------------------------------------------------------------------------
...
*CALL F.READ(FN.AA.ARRANGEMENT,Y.NOT.FHA.AA.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.AA.ARRANGE.ERR)
        R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(Y.NOT.FHA.AA.ID, Y.AA.ARRANGE.ERR) ;*Manual R22 Conversion
...
*            Y.SEL.LOAN.READ.PRD.VALUE = R.AA.ARRANGEMENT<AA.ARR.PRODUCT.GROUP>
            Y.SEL.LOAN.READ.PRD.VALUE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProductGroup> ;*Manual R22 Conversion
            Y.ARRANGEMENT.ID = Y.NOT.FHA.AA.ID
            GOSUB FIND.PROD.GROUP
*            IF R.AA.ARRANGEMENT<AA.ARR.PRODUCT.GROUP> EQ 'HIPOTECARIO' THEN
            IF R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProductGroup> EQ 'HIPOTECARIO' THEN ;*Manual R22 Conversion
...
FIND.PROD.GROUP:
*-----------------------------------------------------------------------------------------------------------------
    IF Y.SEL.LOAN.PRD.VALUE EQ 'HIPOTECARIO' OR Y.SEL.LOAN.PRD.VALUE EQ '' THEN
*        CALL F.READ(FN.AA.ARRANGEMENT,Y.ARRANGEMENT.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.AA.ARRANGE.ERR)
        R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(Y.ARRANGEMENT.ID, Y.AA.ARRANGE.ERR) ;*Manual R22 Conversion
*        Y.PROD.GROUP.VALUE = R.AA.ARRANGEMENT<AA.ARR.PRODUCT.GROUP>
        Y.PROD.GROUP.VALUE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProductGroup> ;*Manual R22 Conversion
    END ELSE
...
PROCESS.PARA:
*------------------------------------------------------------------------------------------------------------------
    IF Y.PROD.GROUP.VALUE  EQ 'HIPOTECARIO' THEN
*        Y.AA.REC.ORIGIN.AGENCY =  R.AA.ARRANGEMENT<AA.ARR.CO.CODE>
        Y.AA.REC.ORIGIN.AGENCY =  R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode> ;*Manual R22 Conversion
*        Y.AA.REC.START.DATE = R.AA.ARRANGEMENT<AA.ARR.START.DATE>
        Y.AA.REC.START.DATE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrStartDate> ;*Manual R22 Conversion
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(ARR.ID1,PROP.CLASS1,"","",RETURN.IDS1,RETURN.COND1,RETURN.ERR1)
    AA.Framework.GetArrangementConditions(ARR.ID1,PROP.CLASS1,"","",RETURN.IDS1,RETURN.COND1,RETURN.ERR1);* R22 Utility Changes
...
*    Y.ARRANGE.CUST = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
    Y.ARRANGE.CUST = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer> ;*Manual R22 Conversion
...
*    CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.LOAN.NO,R.AA.ACTVITY.HISTORY,F.AA.ACTIVITY.HISTORY,Y.AA.ACT.HIS.ERR)
    R.AA.ACTVITY.HISTORY = AA.Framework.ActivityHistory.Read(Y.LOAN.NO, Y.AA.ACT.HIS.ERR) ;*Manual R22 Conversion
...
*        Y.AA.ACTIVITY.HIS.ACT.ID = R.AA.ACTVITY.HISTORY<AA.AH.ACTIVITY.REF>     ;*R22 Manual Conversion - changed to AA.AH.ACTIVITY.REF
        Y.AA.ACTIVITY.HIS.ACT.ID = R.AA.ACTVITY.HISTORY<AA.Framework.ActivityHistory.AhActivityRef> ;*Manual R22 Conversion
    END
...
*        Y.DISBURSED.VALUE = R.AA.ACTVITY.HISTORY<AA.AH.ACTIVITY.AMT,Y.AA.ACTIVITY.HIS.ACT.POS,1>  ;*R22 Manual Conversion - changed from AA.AH.ACT.AMOUNT to AA.AH.ACTIVITY.AMT
        Y.DISBURSED.VALUE = R.AA.ACTVITY.HISTORY<AA.Framework.ActivityHistory.AhActivityAmt,Y.AA.ACTIVITY.HIS.ACT.POS,1>;*Manual R22 Conversion
    END
RETURN
*------------------------------------------------------------------------------------------------------------------
TERM.AMOUNT:
