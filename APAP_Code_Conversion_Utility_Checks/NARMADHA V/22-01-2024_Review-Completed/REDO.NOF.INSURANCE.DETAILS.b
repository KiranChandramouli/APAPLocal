* @ValidationCode : MjotMjM3MDcxNDgzOlVURi04OjE3MDU5MjMxNzU3Mzc6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Jan 2024 17:02:55
...
*18-12-2023         VIGNESHWARI            R22 Manual Conversion - CALL RTN MODIFIED,F.READ IS CHANGED
...
*   $INSERT I_F.AA.BILL.DETAILS ;*Manual R22 Conversion
    $INSERT I_ENQUIRY.COMMON
    $INSERT I_F.REDO.ISSUE.CLAIMS
    $INSERT I_F.CUSTOMER
    $INSERT I_F.COLLATERAL
*    $INSERT I_F.AA.ACCOUNT.DETAILS ;*Manual R22 Conversion
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.ARR.ID,'CUSTOMER','','',RET.PROP.CUS,RET.COND.CUS,CUS.AA.ERR)
    AA.Framework.GetArrangementConditions(Y.ARR.ID,'CUSTOMER','','',RET.PROP.CUS,RET.COND.CUS,CUS.AA.ERR);* R22 Utility Changes
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.ARR.ID,'ACCOUNT','','',RET.PROP.AC,RET.COND.AC,AC.AA.ERR)
    AA.Framework.GetArrangementConditions(Y.ARR.ID,'ACCOUNT','','',RET.PROP.AC,RET.COND.AC,AC.AA.ERR);* R22 Utility Changes
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.ARR.ID,'TERM.AMOUNT','','',RET.PROP.TM,RET.COND.TM,ERR.ATR)
    AA.Framework.GetArrangementConditions(Y.ARR.ID,'TERM.AMOUNT','','',RET.PROP.TM,RET.COND.TM,ERR.ATR);* R22 Utility Changes
...
*    CALL F.READ(FN.AA.ARRANGEMENT,Y.ARR.ID,R.AA.ARR,F.AA.ARRANGEMENT,AA.ARR.ERR)
    R.AA.ARR = AA.Framework.Arrangement.Read(Y.ARR.ID, AA.ARR.ERR) ;*Manual R22 Conversion
*    Y.PRODUCT = R.AA.ARR<AA.ARR.PRODUCT>
    Y.PRODUCT = R.AA.ARR<AA.Framework.Arrangement.ArrProduct> ;*Manual R22 Conversion
*    Y.CURRENCY = R.AA.ARR<AA.ARR.CURRENCY>
    Y.CURRENCY = R.AA.ARR<AA.Framework.Arrangement.ArrCurrency> ;*Manual R22 Conversion
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.ARR.ID,'CHARGE',Y.PROP.CR,'',RET.PROP,RET.COND.CR,CR.ERR)
        AA.Framework.GetArrangementConditions(Y.ARR.ID,'CHARGE',Y.PROP.CR,'',RET.PROP,RET.COND.CR,CR.ERR);* R22 Utility Changes
        IF RET.COND.CR THEN
    ...
    WHILE CNT.VM GT 0 DO
        ...
        IF Y.POL.TYPE EQ POL.TYPE THEN
            ...
            IF Y.DUP.ARR.ID NE Y.ARR.ID THEN
                R.AA.ACCOUNT.DETAILS = AA.PaymentSchedule.AccountDetails.Read(Y.ARR.ID, AA.ACC.ERR);*R22 Manual Conversion -F.READ IS CHANGED
...
*             SEL.LIST.BILL = R.AA.ACCOUNT.DETAILS<AA.AD.BILL.ID>
                SEL.LIST.BILL = R.AA.ACCOUNT.DETAILS<AA.PaymentSchedule.AccountDetailsHist.AdBillId> ;*Manual R22 Conversion
...
*               CALL AA.SCHEDULE.PROJECTOR(Y.ARR.ID,"", "","", TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS, DUE.TYPE.AMTS,DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS)
                AA.PaymentSchedule.ScheduleProjector(Y.ARR.ID,"", "","", TOT.PAYMENT, DUE.DATES, DUE.DEFER.DATES, DUE.TYPES, DUE.METHODS, DUE.TYPE.AMTS,DUE.PROPS, DUE.PROP.AMTS, DUE.OUTS);* R22 Manual Conversion - CALL RTN MODIFIED
...
*-----------------------------------------------------------------------------
    ...
    WHILE Y.BILL.ID:BILL.POS
        R.BILL.DETAILS = AA.PaymentSchedule.BillDetails.Read(Y.BILL.ID, BIL.A.ERR) ;*Manual R22 Conversion
...
*      Y.PROPERTIES = R.BILL.DETAILS<AA.BD.PROPERTY>
        Y.PROPERTIES = R.BILL.DETAILS<AA.PaymentSchedule.BillDetailsHist.BdProperty> ;*Manual R22 Conversion
...
* Y.OS.PROP.AMT = Y.OS.PROP.AMT + R.BILL.DETAILS<AA.BD.OS.PROP.AMOUNT,POS.PR>
            Y.OS.PROP.AMT = Y.OS.PROP.AMT + R.BILL.DETAILS<AA.PaymentSchedule.BillDetails.BdOsPropAmount,POS.PR> ;*Manual R22 Conversion
        END
...
*    CALL F.READ(FN.AA.ARRANGEMENT.DATED.XREF,Y.ARR.ID,R.AA.ARRANGEMENT.DA.XR,F.AA.ARRANGEMENT.DATED.XREF,ERR.DTA.XREF)
    R.AA.ARRANGEMENT.DA.XR = AA.Framework.ArrangementDatedXref.Read(Y.ARR.ID, ERR.DTA.XREF) ;*Manual R22 Conversion
