* @ValidationCode : MjotMjQxNDEwMzM4OlVURi04OjE3MDU5MTc4OTUwOTE6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Jan 2024 15:34:55
...
* 19-01-2024  Narmadha V      Manual R22 Conversion, R22 Utility changes - Call Routine Modified
*********************************************************************************************************
...
*Y.LOAN.TYPE          = R.AA.ARRANGEMENT<AA.ARR.PRODUCT,1>
        Y.LOAN.TYPE          = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct,1> ;*Manual R22 Conversion
...
*Y.CUSTOMER  = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
        Y.CUSTOMER  = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer> ;*Manual R22 Conversion
*Y.CURRENCY  = R.AA.ARRANGEMENT<AA.ARR.CURRENCY>
        Y.CURRENCY  = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency> ;*Manual R22 Conversion
*Y.AA.STATUS  = R.AA.ARRANGEMENT<AA.ARR.ARR.STATUS>
        Y.AA.STATUS  = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrArrStatus> ;*Manual R22 Conversion
*Y.AGENCY  = R.AA.ARRANGEMENT<AA.ARR.CO.CODE>
        Y.AGENCY = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode> ;*Manual R22 Conversion
...
*Y.AA.ACCT.NO =  R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
        Y.AA.ACCT.NO =  R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId> ;*Manual R22 Conversion
    END
    ...
    APAP.TAM.redoGetPropertyName(AA.ARRANGEMENT.ID,IN.PROPERTY.CLASS,R.OUT.AA.RECORD,OUT.PROPERTY,OUT.ERR)
...
****************************
    ...
    TOT.DUE = ABS(TOT.DUE)
    Y.DATE = TODAY
    APAP.TAM.redoGetNextPaymentAmount(AA.ARRANGEMENT.ID,Y.DATE,Y.FEE)
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(AA.ARRANGEMENT.ID, Y.PROPERTY,'','', RET.IDS, R.RECORD, RET.ERR)
    AA.Framework.GetArrangementConditions(AA.ARRANGEMENT.ID, Y.PROPERTY,'','', RET.IDS, R.RECORD, RET.ERR);* R22 Utility Changes
...
*    CALL F.READ(FN.AA.ARRANGEMENT,AA.ARRANGEMENT.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,AA.ARRANGEMENT.ER)
    R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(AA.ARRANGEMENT.ID, AA.ARRANGEMENT.ER) ;*Manual R22 Conversion
RETURN
...
*CALL AA.GET.PERIOD.BALANCES(Y.AA.ACCT.NO, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "")
    AA.Framework.GetPeriodBalances(Y.AA.ACCT.NO, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "");* R22 Utility Changes
...
* CALL F.READ(FN.AA.ARRANGEMENT.ACTIVITY,EXACT.AAA.ID,R.ARR.ACT,F.AA.ARRANGEMENT.ACTIVITY,ARR.ACT.ERR)
    R.ARR.ACT = AA.Framework.ArrangementActivity.Read(EXACT.AAA.ID, ARR.ACT.ERR) ;*Manual R22 Conversion
*Y.ACT.CLASS = R.ARR.ACT<AA.ARR.ACT.ACTIVITY.CLASS>
    Y.ACT.CLASS = R.ARR.ACT<AA.Framework.SimulationCapture.ArrActActivityClass> ;*Manual R22 Conversion
    IF NOT(ARR.ACT.ERR) AND Y.ACT.CLASS NE 'LENDING-DISBURSE-TERM.AMOUNT' THEN
*LAST.PAID.AMT =R.ARR.ACT<AA.ARR.ACT.TXN.AMOUNT.LCY>
        LAST.PAID.AMT =R.ARR.ACT<AA.Framework.ArrangementActivity.ArrActTxnAmountLcy> ;*Manual R22 Conversion
    END
...
*  CALL F.READ(FN.AA.ACTIVITY.HISTORY,AA.ACTIVITY.HISTORY.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,AA.ACTIVITY.HISTORY.ER)
    R.AA.ACTIVITY.HISTORYR.AA.ACTIVITY.HISTORY =  AA.Framework.ActivityHistory.Read(AA.ACTIVITY.HISTORY.ID, AA.ACTIVITY.HISTORY.ER) ;*Manual R22 Conversion
    IF R.AA.ACTIVITY.HISTORY NE '' THEN
        Y.CTR.ED = 1
        LOOP
*           Y.EFFECTIVE.DATE.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE>,@VM)
            Y.EFFECTIVE.DATE.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate>,@VM) ;*Manual R22 Conversion
        WHILE Y.CTR.ED LE Y.EFFECTIVE.DATE.HIS
* Y.ACTIVITY.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.CTR.ED>,@SM)
            Y.ACTIVITY.HIS = DCOUNT(R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity>,@SM) ;*Manual R22 Conversion
            Y.CTR = 1
            LOOP
            WHILE Y.CTR LE Y.ACTIVITY.HIS
*               Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.CTR.ED,Y.CTR>
                Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity,Y.CTR.ED,Y.CTR> ;*Manual R22 Conversion
...
*                  LAST.TXN.DATE =  R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE,Y.CTR.ED>
                    LAST.TXN.DATE =  R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate,Y.CTR.ED> ;*Manual R22 Conversion
