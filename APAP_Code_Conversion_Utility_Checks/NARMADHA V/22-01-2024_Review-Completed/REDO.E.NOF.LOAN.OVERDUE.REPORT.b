* @ValidationCode : MjotMTQ4NjY0MTM1OlVURi04OjE3MDU5MTcwOTA4NjM6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Jan 2024 15:21:30
...
*DATE              WHO                REFERENCE                        DESCRIPTION
*23-05-2023      HARSHA        AUTO R22 CODE CONVERSION           VM TO @VM,FM TO @FM,SM TO @SM,F.READ TO CACHE.READ
...
*    $INSERT I_F.AA.ACTIVITY.HISTORY ;*Manual R22 Conversion
*    $INSERT I_F.AA.ARRANGEMENT.ACTIVITY ;*Manual R22 Conversion
...
*----------------------------------------------------------------------------------------
    Y.PRI.OWNER = Y.AA.ARR.CUSTOMER<AA.CUS.CUSTOMER>    ;*R22 Manual Conversion - changed to AA.CUS.CUSTOMER
    CALL F.READ(FN.CUSTOMER,Y.PRI.OWNER,R.CUSTOMER,F.CUSTOMER,CUS.ERR)
    Y.CLIENT.NAME = R.CUSTOMER<EB.CUS.SHORT.NAME,1>
    Y.OWNER.LIST = Y.AA.ARR.CUSTOMER<AA.CUS.CUSTOMER>   ;*R22 Manual Conversion - changed to AA.CUS.CUSTOMER
...
*    CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,AA.ARR.ERR)
    R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(Y.AA.ID, AA.ARR.ERR) ;*Manual R22 Conversion
*    X.LOAN.BOOK.TYPE     = R.AA.ARRANGEMENT<AA.ARR.PRODUCT.GROUP>
    X.LOAN.BOOK.TYPE     = R.AA.ARRANGEMENT< AA.Framework.ArrangementSim.ArrProductGroup> ;*Manual R22 Conversion
* X.LOAN.PRODUCT.TYPE  = R.AA.ARRANGEMENT<AA.ARR.PRODUCT> ;  X.LOAN.NUMBER        = Y.AA.ID
    X.LOAN.PRODUCT.TYPE  = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct> ;  X.LOAN.NUMBER        = Y.AA.ID  ;*Manual R22 Conversion
*X.CURRENCY           = R.AA.ARRANGEMENT<AA.ARR.CURRENCY>;  X.LOAN.SOURCE.AGENCY = R.AA.ARRANGEMENT<AA.ARR.CO.CODE>
    X.CURRENCY           = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency>;  X.LOAN.SOURCE.AGENCY = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode> ;*Manual R22 Conversion
RETURN
*----------------------------------------------------------------------------------------
GET.ARR.CONDITION:
*----------------------------------------------------------------------------------------
...
*ARR.INFO = ''        ;       Y.EFF.DATE = R.AA.ARRANGEMENT<AA.ARR.PROD.EFF.DATE>
    ARR.INFO = ''        ;       Y.EFF.DATE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProdEffDate> ;*Manual R22 Conversion
    ARR.INFO<1> =Y.AA.ID ; R.ARRANGEMENT = ''
*CALL AA.GET.ARRANGEMENT.PROPERTIES(ARR.INFO, Y.EFF.DATE, R.ARRANGEMENT, PROP.LIST)
    AA.Framework.GetArrangementProperties(ARR.INFO, Y.EFF.DATE, R.ARRANGEMENT, PROP.LIST);* R22 Utility Changes
...
*CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.AA.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,ACT.HIS.ERR)
    R.AA.ACTIVITY.HISTORY = AA.Framework.ActivityHistory.Read(Y.AA.ID, ACT.HIS.ERR) ;*Manual R22 Conversion
    IF R.AA.ACTIVITY.HISTORY THEN
*Y.ACTIVITY.REF   = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY.REF>
        Y.ACTIVITY.REF   = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistory.AhActivityRef> ;*Manual R22 Conversion
*Y.ACTIVITY       = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY>
        Y.ACTIVITY       = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity> ;*Manual R22 Conversion
    END
    LOOP
...
*CALL F.READ(FN.AA.ARRANGEMENT.ACTIVITY,Y.ACTIVITY.REF.ID,R.AA.ARRANGEMENT.ACTIVITY,F.AA.ARRANGEMENT.ACTIVITY,ARR.ACTY.ERR)
    R.AA.ARRANGEMENT.ACTIVITY = AA.Framework.ArrangementActivity.Read(Y.ACTIVITY.REF.ID, ARR.ACTY.ERR) ;*Manual R22 Conversion
    IF Y.EFFECTIVE.DATE EQ '' THEN
*   Y.EFFECTIVE.DATE     = R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.EFFECTIVE.DATE>
        Y.EFFECTIVE.DATE     = R.AA.ARRANGEMENT.ACTIVITY<AA.Framework.SimulationCapture.ArrActEffectiveDate> ;*Manual R22 Conversion
*   Y.TXN.AMOUNT         = R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.TXN.AMOUNT>
        Y.TXN.AMOUNT         = R.AA.ARRANGEMENT.ACTIVITY<AA.Framework.SimulationCapture.ArrActTxnAmount> ;*Manual R22 Conversion
    END ELSE
*   Y.EFFECTIVE.DATE<-1> = R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.EFFECTIVE.DATE>
        Y.EFFECTIVE.DATE<-1> = R.AA.ARRANGEMENT.ACTIVITY<AA.Framework.SimulationCapture.ArrActEffectiveDate> ;*Manual R22 Conversion
*    Y.TXN.AMOUNT<-1>     = R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.TXN.AMOUNT>
        Y.TXN.AMOUNT<-1>     = R.AA.ARRANGEMENT.ACTIVITY<AA.Framework.SimulationCapture.ArrActTxnAmount> ;*Manual R22 Conversion
    END
RETURN
*----------------------------------------------------------------------------------------
...
*       Y.DISB.AMT += R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.TXN.AMOUNT>
        Y.DISB.AMT += R.AA.ARRANGEMENT.ACTIVITY< AA.Framework.SimulationCapture.ArrActTxnAmount> ;*Manual R22 Conversion
    END
RETURN
*----------------------------------------------------------------------------------------
...
*   Y.CUS = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
    Y.CUS = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer> ;*Manual R22 Conversion
...
CHANGE.SUB.PROCESS:
*----------------------------------------------------------------------------------------
    CALL CACHE.READ(FN.AA.PROPERTY, Y.AA.PROP.ID, R.AA.PROPERTY, AA.PROP.ERR) ;*AUTO R22 CODE CONVERSION
...
*    Y.AA.PROPERTY = R.AA.ARRANGEMENT<AA.ARR.PROPERTY>
    Y.AA.PROPERTY = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProperty> ;*Manual R22 Conversion
    LOOP
...
*    Y.EFF.DATE = R.AA.ARRANGEMENT<AA.ARR.PROD.EFF.DATE>
    Y.EFF.DATE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProdEffDate> ;*Manual R22 Conversion
    ARR.INFO<1> = Y.AA.ID     ;  R.ARRANGEMENT = ''
*CALL AA.GET.ARRANGEMENT.PROPERTIES(ARR.INFO,Y.EFF.DATE,R.ARRANGEMENT,PROP.LIST)
    AA.Framework.GetArrangementProperties(ARR.INFO,Y.EFF.DATE,R.ARRANGEMENT,PROP.LIST);* R22 Utility Changes
