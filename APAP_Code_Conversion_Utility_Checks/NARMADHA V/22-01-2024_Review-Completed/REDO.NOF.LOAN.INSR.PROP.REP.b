* @ValidationCode : MjotNjM0MDMwNDM5OlVURi04OjE3MDU5MjMyODU5NjM6QWRtaW46LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Jan 2024 17:04:45
...
* 19-01-2024         Narmadha V            R22 Utility Changes,Manual R22 Conversion     ID variable used instead of hardcoding
*-----------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
*    Y.AGENCY = R.AA<AA.ARR.CO.CODE>
    Y.AGENCY = R.AA<AA.Framework.ArrangementSim.ArrCoCode> ;*Manual R22 Conversion
*    Y.CUR = R.AA<AA.ARR.CURRENCY>
    Y.CUR = R.AA<AA.Framework.Arrangement.ArrCurrency> ;*Manual R22 Conversion
    GOSUB GET.LOAN.ST
...
*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.AA.ID,'OVERDUE','','',RET.PRO,RET.COND,RET.ERR)
    AA.Framework.GetArrangementConditions(Y.AA.ID,'OVERDUE','','',RET.PRO,RET.COND,RET.ERR);* R22 Utility Changes
...
*Y.LN.AGE.ST = R.AA.AC<AA.AD.ARR.AGE.STATUS>
    Y.LN.AGE.ST = R.AA.AC<AA.PaymentSchedule.AccountDetailsHist.AdArrAgeStatus> ;*Manual R22 Conversion
...
*            CALL AA.GET.ECB.BALANCE.AMOUNT(Y.AC.ID, Y.BAL.TYPE, REQUEST.DATE, BALANCE.AMOUNT,RET.ERROR)
            AA.Framework.GetEcbBalanceAmount(AccountId, BalanceType, RequestDate, BalanceAmount, RetError) ;*Manual R22 Conversion
...
*       CALL AA.GET.BILL(Y.AA.ID, "", "", DEFER.DATES , "", BILL.TYPE, PAYMENT.METHOD, BILL.STATUS, "", "", "", "", Y.BILL.REFERENCE, RET.ERROR)
        AA.PaymentSchedule.GetBill(Y.AA.ID, "", "", DEFER.DATES , "", BILL.TYPE, PAYMENT.METHOD, BILL.STATUS, "", "", "", "", Y.BILL.REFERENCE, RET.ERROR);*R22 Manual Conversion - CALL RTN MODIFIED
...
*       Y.PRS = R.BILL<AA.BD.PROPERTY>
        Y.PRS = R.BILL<AA.PaymentSchedule.BillDetailsHist.BdProperty> ;*Manual R22 Conversion
        LOCATE Y.INS.PROP IN Y.PRS<1,1> SETTING POS.KO THEN
*    Y.TOT.BAL = R.BILL<AA.BD.OR.PROP.AMOUNT,POS.KO>
            Y.TOT.BAL = R.BILL<AA.PaymentSchedule.BillDetailsHist.BdOrPropAmount,POS.KO> ;*Manual R22 Conversion
            Y.NCM = 0
        END
        Y.NCM -= 1
    REPEAT
