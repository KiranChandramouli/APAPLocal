* @ValidationCode : Mjo2NzkyMjMxMDY6VVRGLTg6MTcwNTkyNDExNDU1NzpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Jan 2024 17:18:34
...
* 05-01-2024       Narmadha V              Manual R22 Changes     Call Routine Modified
*------------------------------------------------------------------------
    ...
    $INSERT I_F.USER
*    $INSERT I_F.AA.ACCOUNT.DETAILS ;*Manual R22 Changes
    $INSERT I_F.REDO.CUSTOMER.ARRANGEMENT
    $USING APAP.TAM
    $USING APAP.AA
    $USING AA.PaymentSchedule ;*Manual R22 Changes
...
* CALL REDO.AA.GET.OUT.BALANCE(Y.ARR.ID,Y.BALANCE)
            ...
            APAP.TAM.redoGetTotalOutstandingSinUncUnd(Y.ARR.ID,Y.BAL,Y.TOT.BAL) ;*Manual R22 conversion
            Y.BALANCE = Y.BAL<1>
            Y.ARRAY:='*':Y.BALANCE
...
*  CALL F.READ(FN.AA.ACCOUNT.DETAILS,Y.ARR.ID,R.AA.ACC.DET,F.AA.ACCOUNT.DETAILS,ACT.ERR)
    R.AA.ACC.DET = AA.PaymentSchedule.AccountDetails.Read(Y.ARR.ID, ACT.ERR) ;*Manual R22 Changes
*Y.PAY.START.DATE = R.AA.ACC.DET<AA.AD.PAYMENT.START.DATE>
    Y.PAY.START.DATE = R.AA.ACC.DET<AA.PaymentSchedule.AccountDetailsHist.AdPaymentStartDate> ;*Manual R22 Changes
...
*   CALL AA.SCHEDULE.PROJECTOR(Y.ARR.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, PAYMENT.DATES, DUE.DEFER.DATES, PAYMENT.TYPES, DUE.METHODS,DUE.TYPE.AMTS, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, DUE.OUTS)
    AA.PaymentSchedule.ScheduleProjector(Y.ARR.ID, SIMULATION.REF, NO.RESET, DATE.RANGE, TOT.PAYMENT, PAYMENT.DATES, DUE.DEFER.DATES, PAYMENT.TYPES, DUE.METHODS,DUE.TYPE.AMTS, PAYMENT.PROPERTIES, PAYMENT.PROPERTIES.AMT, DUE.OUTS);*R22 Manual Conversion - CALL RTN MODIFIED
...
* Gets the role of the customer in that arrangement
    ...
    ERR.MSG = ''
    APAP.AA.redoCrrGetConditions(Y.ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.CUSTOMER,ERR.MSG) ;*Manual R22 conversion
    ...
    LOCATE Y.CUSTOMER.ID IN Y.OTHER.PARTY<1,1> SETTING POS1 THEN
        Y.VAR = "AA.PARTY.ROLE";*Manual R22 Changes
        Y.ROLE.CUS=Y.ROLE<1,POS1>
        Y.VAR1 = Y.VAR:'*':Y.ROLE.CUS ;*Manual R22 Changes
*CALL F.READ(FN.EB.LOOKUP,'AA.PARTY.ROLE*':Y.ROLE.CUS,R.EB.LOOKUP,F.EB.LOOKUP,LOOK.ERR)
		CALL F.READ(FN.EB.LOOKUP,Y.VAR1,R.EB.LOOKUP,F.EB.LOOKUP,LOOK.ERR) ;*Manual R22 Changes
        IF R.USER<EB.USE.LANGUAGE> EQ 1 THEN
