* @ValidationCode : MjotMTk0ODIyNDQ5OkNwMTI1MjoxNzAyOTA2NTAwMzgxOnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 18 Dec 2023 19:05:00
...
TOT.BILL.DET = R.RECORD<AA.PaymentSchedule.AccountDetailsHist.AdBillId>
...
TOT.BILL.STATUS = R.RECORD<AA.PaymentSchedule.AccountDetails.AdSetStatus>
...
**********************
    ...
    WHILE BILL.CNT LE TOT.BILL.CNT
        ...
        *EACH.BILL.ID=GET.FULL.DET<AA.AD.BILL.ID,BILL.CNT>
        EACH.BILL.ID=GET.FULL.DET<  AA.PaymentSchedule.AccountDetailsHist.AdBillId,BILL.CNT>
        *EACH.BILL.STATUS = GET.FULL.DET<AA.AD.SET.STATUS,BILL.CNT>
        EACH.BILL.STATUS = GET.FULL.DET<AA.PaymentSchedule.AccountDetails.AdSetStatus,BILL.CNT>
        IF EACH.BILL.STATUS EQ 'UNPAID' THEN
            ...
            IF R.BILL.DET THEN
               * Y.PROPERTY.LIST = R.BILL.DET<AA.BD.PROPERTY>
                Y.PROPERTY.LIST = R.BILL.DET<AA.PaymentSchedule.BillDetailsHist.BdProperty>
...
*    Y.EACH.BILL.AMT + = R.BILL.DET<AA.BD.OS.PROP.AMOUNT,Y.PROP.CNT>
                 Y.EACH.BILL.AMT + = R.BILL.DET<AA.PaymentSchedule.BillDetails.BdOsPropAmount,Y.PROP.CNT>
                    Y.PROP.CNT += 1
...
*        CHECK.BILL.AMT=R.BILL.DET<AA.BD.OR.TOTAL.AMOUNT>*(PART.PCNT/100)
    CHECK.BILL.AMT=R.BILL.DET<AA.PaymentSchedule.BillDetailsHist.BdOrTotalAmount>*(PART.PCNT/100)
        FIRST.BILL.PART.AMT=CHECK.BILL.AMT
 *       FIRST.AMT.TO.SUB   =SUM(R.BILL.DET<AA.BD.OS.PROP.AMOUNT>)
        FIRST.AMT.TO.SUB   =SUM(R.BILL.DET<AA.PaymentSchedule.BillDetails.BdOsPropAmount>) 
...
*******************************
    IF BILL.CNT GT '1' THEN
        *EXACT.PART.AMT +=SUM(R.BILL.DET<AA.BD.OS.PROP.AMOUNT>)
        EXACT.PART.AMT +=SUM(R.BILL.DET<AA.PaymentSchedule.BillDetails.BdOsPropAmount>)
    END
