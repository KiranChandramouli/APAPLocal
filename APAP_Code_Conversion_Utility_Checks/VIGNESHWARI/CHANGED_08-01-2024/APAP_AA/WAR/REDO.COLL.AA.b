* @ValidationCode : MjotMjU0MjcxODA4OkNwMTI1MjoxNzA0MjU5MDk2NjY2OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 03 Jan 2024 10:48:16
...
*=============================
   * CALL F.READ(FN.AA.ARRANGEMENT, VAR.CRED, R.AA.ARRANGEMENT, F.AA.ARRANGEMENT, Y.ERR)
    R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(VAR.CRED, Y.ERR);*r22 manual code convertion
...
*    AAA.REQUEST<AA.ARR.ACT.EFFECTIVE.DATE> = TODAY
 ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS MODIFIED-START   
     AAA.REQUEST<AA.Framework.ArrangementActivity.ArrActArrangement> = ARR.ID
    AAA.REQUEST<AA.Framework.RrControl.ArrActActivity> = 'LENDING-CHANGE.TERM-COMMITMENT'
    AAA.REQUEST<AA.Framework.SimulationCapture.ArrActEffectiveDate> = TODAY
...
* This forms the OFS message request for CHARGE property
   ...
   * AAA.REQUEST<AA.ARR.ACT.PROPERTY,-1> = AA.PROPERTY         ;*R.NEW(INS.DET.CHARGE)<1,1>  ;* Charge property to be updated as part of Insurance
   AAA.REQUEST<AA.Framework.RrControl.ArrActProperty,-1> = AA.PROPERTY         ;*R.NEW(INS.DET.CHARGE)<1,1>  ;* Charge property to be updated as part of Insurance ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK CHANGED
...
;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK CHANGED-START
         AAA.REQUEST<AA.Framework.RrControl.ArrActFieldName,1,-1> = 'L.AA.COL:':NUM.ITR:':1'
        AAA.REQUEST<AA.Framework.SimulationCapture.ArrActFieldValue,1,-1> = COLL.ID
        AAA.REQUEST<AA.Framework.RrControl.ArrActFieldName,1,-1> = 'L.AA.COL.VAL:':NUM.ITR:':1'      ;*Para actualizar el VNG en AA
        AAA.REQUEST<AA.Framework.SimulationCapture.ArrActFieldValue,1,-1> = Y.AA.COL.VAL<1,NUM.ITR>          ;*Para actualizar el VNG en AA
* PACS00310867 - S
        AAA.REQUEST<AA.Framework.RrControl.ArrActFieldName,1,-1> = 'L.AA.COL.DESC:':NUM.ITR:':1'     ;*Description AA updation
        AAA.REQUEST<AA.Framework.SimulationCapture.ArrActFieldValue,1,-1> = Y.AA.COL.DESC<1,NUM.ITR>         ;*Description AA updation
* PACS00310867 - E
* PACS00307565 - S
        AAA.REQUEST<AA.Framework.RrControl.ArrActFieldName,1,-1> = 'L.AA.AV.COL.BAL:':NUM.ITR:':1'   ;*AA Balance at attachement moment
        AAA.REQUEST<AA.Framework.SimulationCapture.ArrActFieldValue,1,-1> = Y.AA.COL.DESC<1,NUM.ITR>         ;*AA Balance at attachement moment
...
AA.Framework.GetArrangementConditions(idArrangementComp, idPropertyClass, idProperty, effectiveDate, returnIds, returnConditions, returnError);*R22 MANUAL CONVERSION-CALL RTN MODIFIED
 ...
 *   CALL AA.GET.ARRANGEMENT.PROPERTIES(ARR.INFO, ARR.DATE, R.ARRANGEMENT, PROP.LIST)
    AA.Framework.GetArrangementProperties(ARR.INFO, ARR.DATE, R.ARRANGEMENT, PROP.LIST);*R22 MANUAL CODE CONVERSION-CALL RTN MODIFIED
    CLASS.LIST = ''
