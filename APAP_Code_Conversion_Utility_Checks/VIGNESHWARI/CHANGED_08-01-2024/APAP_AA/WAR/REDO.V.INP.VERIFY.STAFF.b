* @ValidationCode : MjotODEwODMyNDcxOkNwMTI1MjoxNzA0NzAyMDc1MDg2OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 08 Jan 2024 13:51:15
...
*14-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION    VM,SM,FM to @VM,@FM,@SM,++ to +=
...
*01-01-2024 Vigneshwari         R22 MANUAL CODE CONVERSION          AA.FRAMEWORK IS CHANGED 
*------------------------------------------------------------------------
...
Y.CUS.DC=R.NEW(AA.Framework.SimulationCapture.ArrActCustomer)   ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
Y.ACC.DC=R.NEW(AA.Framework.ArrangementActivity.ArrActArrangement)  ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
Y.ARR.ID=Y.ACC.DC
*CALL F.READ(FN.AA.ARRANGEMENT,Y.ACC.DC,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.ARR.ERR)
R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(Y.ACC.DC, Y.ARR.ERR)   ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
*Y.ACC.DC=R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
Y.ACC.DC=R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>  ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
IF Y.CUS.DC THEN
LOCATE Y.CUS.DC IN Y.EMP.REL.CUS<1,1> SETTING EMP.POS THEN
OVERRIDE.FIELD.VALUE = R.NEW(FT.OVERRIDE)
CURR.NO = DCOUNT(OVERRIDE.FIELD.VALUE,@VM) + 1
...
AF = AA.Framework.ArrangementActivity.ArrActArrangement ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
GOSUB SEND.ERROR

*CALL AA.GET.ARRANGEMENT.CONDITIONS(Y.ARR.ID,'CUSTOMER', '','',RET.IDS, ACC.COND, RET.ERR)
AA.Framework.GetArrangementConditions(Y.ARR.ID,'CUSTOMER', '','',RET.IDS, ACC.COND, RET.ERR);* R22 AUTO CONVERSION
...
OVERRIDE.FIELD.VALUE = R.NEW(AA.Framework.RrControl.ArrActOverride)    ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED 
...
CALL F.READ(FN.CUSTOMER,Y.CUS.VAL.JOIN,R.REL.CUSTOMER,F.CUSTOMER,CUS.ERR)
*AF = AA.ARR.ACT.ARRANGEMENT
AF = AA.Framework.ArrangementActivity.ArrActArrangement
GOSUB SEND.ERROR

Y.CUS.VAL = R.REL.CUSTOMER<EB.CUS.REL.CUSTOMER>
*OVERRIDE.FIELD.VALUE = R.NEW(AA.ARR.ACT.OVERRIDE)
OVERRIDE.FIELD.VALUE = R.NEW(AA.Framework.RrControl.ArrActOverride)
GOSUB CUST.CHECK
RETURN
*-------------
AA.API.PROCESS:
