* @ValidationCode : MjotMjAyMDY4NzI5MzpDcDEyNTI6MTcwNTA0Mzc5NDk5MDp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 12 Jan 2024 12:46:34
...
*17-5-2023    CONVERSION TOOL     R22 AUTO CONVERSION     FM TO @FM,VM TO @VM,SM TO @SM,T TO C$T24.SESSION
*17-5-2023    VICTORIA S          R22 MANUAL CONVERSION   NO CHANGE
...
*    CALL F.READ(FN.REDO.AA.DISBURSE.METHOD,Y.AA.ID,R.REDO.AA.DISBURSE.METHOD,F.REDO.AA.DISBURSE.METHOD,AA.DIS.ERR)
    ...
    IF R.REDO.AA.DISBURSE.METHOD THEN
        ...
        LOCATE Y.TEMP.ID IN R.REDO.AA.DISBURSE.METHOD<DIS.MET.FC.DISBURSE.ID,1> SETTING PS.FC THEN
            ...
            Y.REF.ID.CNT = DCOUNT(R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.REF.ID,PS.FC>,@SM) ;*R22 AUTO CONVERSION
            ...
            END
        END ELSE
            Y.REF.FC.CNT = DCOUNT(R.REDO.AA.DISBURSE.METHOD<DIS.MET.FC.DISBURSE.ID>,@VM) ;*R22 AUTO CONVERSION
            R.REDO.AA.DISBURSE.METHOD<DIS.MET.FC.DISBURSE.ID,Y.REF.FC.CNT+1> = Y.TEMP.ID

            Y.REF.ID.CNT = DCOUNT(R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.REF.ID>,@SM) ;*R22 AUTO CONVERSION
...
*   Y.OWN = RET.COND<AA.CUS.OWNER>
    ...
    Y.PR.OWN = RET.COND<AA.CUS.CUSTOMER>
    Y.OWN = RET.COND<AA.CUS.CUSTOMER> ;*R22 MANUAL CONVERSION END
    Y.OT.PARTY = RET.COND<AA.CUS.OTHER.PARTY>
    Y.OT.PARTY.ROLE = RET.COND<AA.CUS.ROLE>

    Y.VALS.PARTY = 'PRIMARY.OWN':'*':Y.PR.OWN
    FLG.PR = ''
    Y.PAR.CNT = DCOUNT(Y.OT.PARTY,@VM) ;*R22 AUTO CONVERSION
    LOOP
...
PART.CHG.PRO:
    ...
    Y.PCNT = DCOUNT(R.REDO.GET.AAA.NEW.CHG.PDIS,@FM) ; P.FLG = '' ;*R22 AUTO CONVERSION
    LOOP
    WHILE Y.PCNT GT 0 DO
        ...
        *Y.AAA.ACT = R.AAA<AA.ARR.ACT.ACTIVITY>
        ...
        Y.CHR.CNT = DCOUNT(Y.CHARGS,@VM) ; PP.DLF = '' ;*R22 AUTO CONVERSION
        LOOP
...
GET.BILLS.DE:
    ...
    BLS = CHANGE(BLS,@VM,@FM) ;*R22 AUTO CONVERSION START
    BLS = CHANGE(BLS,@SM,@FM)
    Y.BLS.CNT = DCOUNT(BLS,@FM) ; FL.BLS = '' ; FLG.2 = '' ;*R22 AUTO CONVERSION END
    LOOP
