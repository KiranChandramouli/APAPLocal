* @ValidationCode : MjoxNzA4ODY1NTQyOkNwMTI1MjoxNzA0OTc2NDM1OTE5OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 11 Jan 2024 18:03:55
...
$PACKAGE APAP.TAM
SUBROUTINE REDO.GET.DISB.PAYM.DETAILS
...
* 19-06-2012        S.MARIMUTHU     PACS00202947         Changed for deal slip printing
* 28-11-2012        S.MARIMUTHU     PACS00236823         Partial Disbursement
* 11-02-2013        S.MARIMUTHU     PACS00245100
*------------------------------------------------------------------------
*DATE               WHO                    REFERENCE
* 23-05-2023     Conversion Tool        R22 Auto Conversion - SM TO @SM AND VM TO @VM AND FM TO @FM AND ADDED IF CONDITION AND TNO TO C$T24.SESSION.NO
...
PROCESS:
    ...
    IF E EQ "EB-UNKNOWN.VARIABLE" THEN    ;* R22 Auto Conversion
        Y.TEMP.ID = ""
    END
    Y.ARR.AC = R.NEW(FT.DEBIT.ACCT.NO)
...
*    CALL F.READ(FN.REDO.AA.DISBURSE.METHOD,Y.AA.ID,R.REDO.AA.DISBURSE.METHOD,F.REDO.AA.DISBURSE.METHOD,AA.DIS.ERR)
    CALL F.READU(FN.REDO.AA.DISBURSE.METHOD,Y.AA.ID,R.REDO.AA.DISBURSE.METHOD,F.REDO.AA.DISBURSE.METHOD,AA.DIS.ERR,'');* R22 UTILITY AUTO CONVERSION
    IF R.REDO.AA.DISBURSE.METHOD THEN
        ...
        LOCATE Y.TEMP.ID IN R.REDO.AA.DISBURSE.METHOD<DIS.MET.FC.DISBURSE.ID,1> SETTING PS.FC THEN
            ...
            END ELSE
                ...
                IF R.NEW(FT.DEBIT.AMOUNT) NE '' THEN
                    R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT,PS.FC,Y.REF.ID.CNT+1> = R.NEW(FT.DEBIT.AMOUNT)
                END ELSE
                    R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT,PS.FC,Y.REF.ID.CNT+1> = R.NEW(FT.CREDIT.AMOUNT)
                END
                GOSUB TYPE.PAYMENT
        ...
        END ELSE
            ...
            IF R.NEW(FT.DEBIT.AMOUNT) NE '' THEN
                R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT,Y.REF.FC.CNT+1> = R.NEW(FT.DEBIT.AMOUNT)
            END ELSE
                R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT,Y.REF.FC.CNT+1> = R.NEW(FT.CREDIT.AMOUNT)
            END
            GOSUB TYPE.PAYMENT
...
AA.Framework.GetArrangementConditions(Y.AA.ID,'CUSTOMER','','',RET.PROP,RET.COND,RET.ERR);* R22 UTILITY AUTO CONVERSION
    RET.COND = RAISE(RET.COND)
*Y.PR.OWN = RET.COND<AA.CUS.PRIMARY.OWNER>
    Y.PR.OWN = RET.COND<AA.CUS.CUSTOMER>   ;*R22 Manual Conversion
*Y.OWN = RET.COND<AA.CUS.OWNER>
    Y.OWN = RET.COND<AA.CUS.CUSTOMER>     ;*R22 Manual Conversion
...
AA.Framework.GetArrangementConditions(Y.AA.ID,'TERM.AMOUNT','','',RET.PROP,RET.COND.TM,RET.ERR);* R22 UTILITY AUTO CONVERSION
    ...
    IF R.NEW(FT.DEBIT.AMOUNT) NE '' THEN
        R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT> = R.NEW(FT.DEBIT.AMOUNT)
    END ELSE
        R.REDO.AA.DISBURSE.METHOD<DIS.MET.DISBURSE.AMT> = R.NEW(FT.CREDIT.AMOUNT)
    END

    GOSUB TYPE.PAYMENT
    R.REDO.AA.DISBURSE.METHOD<DIS.MET.TYPE.PAYMENT> = Y.TYPE.PAY

    Y.CHG.AMT = R.NEW(FT.LOCAL.REF)<1,Y.LC.POS>
...
PART.CHG.PRO:
    ...
    WHILE Y.PCNT GT 0 DO
       ...
       * Y.AAA.ACT = R.AAA<AA.ARR.ACT.ACTIVITY>
          Y.AAA.ACT = R.AAA<AA.Framework.RrControl.ArrActActivity>  ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
...
WRITE.TEMPLATE:
    ...
    R.REDO.AA.DISBURSE.METHOD<DIS.MET.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR  ;*R22 Auto Conversion TNO TO C$T24.SESSION.NO
    R.REDO.AA.DISBURSE.METHOD<DIS.MET.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR ;*R22 Auto Conversion TNO C$T24.SESSION.NO
