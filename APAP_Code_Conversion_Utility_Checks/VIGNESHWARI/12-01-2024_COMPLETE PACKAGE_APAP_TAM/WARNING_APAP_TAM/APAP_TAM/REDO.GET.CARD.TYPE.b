* @ValidationCode : MjotMTAxNDk0Mzg1MDpDcDEyNTI6MTcwNDk3NTEyMjIzMjp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 11 Jan 2024 17:42:02
...
$PACKAGE APAP.TAM
SUBROUTINE REDO.GET.CARD.TYPE(Y.CARD.NO,Y.CARD.ACCT,Y.CARD.TYPE)
...
IDVAR.1 = 'SYSTEM' ;* R22 UTILITY AUTO CONVERSION
    CALL CACHE.READ(FN.SUN.PARAM,IDVAR.1,R.SUN.PARAM,PARAM.ERR);* R22 UTILITY AUTO CONVERSION
...
*Calling Sunnel interface
    ...
    Y.MESSAGE = FIELD(Y.OUT.DATA,SEC.MARK,1)
    IF Y.MESSAGE EQ 'SUCCESS' THEN
        Y.CARD.ACCT    = FIELD(Y.OUT.DATA,SEC.MARK,2)
        Y.CARD.MESSAGE = FIELD(Y.OUT.DATA,SEC.MARK,4)
    END
    ELSE
        Y.CARD.TYPE = ''
    END
    IF Y.CARD.MESSAGE THEN
        Y.GET.VISA.TYPE   = Y.CARD.MESSAGE[1,4]
        Y.GET.MASTER.TYPE = Y.CARD.MESSAGE[1,10]
        IF Y.GET.VISA.TYPE EQ 'VISA' THEN
            Y.CARD.TYPE = 'VISA'
        END
        ELSE
            IF Y.GET.MASTER.TYPE EQ 'MASTERCARD' THEN
