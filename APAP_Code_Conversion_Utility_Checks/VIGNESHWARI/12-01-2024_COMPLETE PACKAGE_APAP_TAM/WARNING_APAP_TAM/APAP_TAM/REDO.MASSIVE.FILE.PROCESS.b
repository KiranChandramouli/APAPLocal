* @ValidationCode : Mjo4NjU4NjEzMjU6VVRGLTg6MTcwNDk1NTIxMzc2ODpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 11 Jan 2024 12:10:13
...
*Modification History:
*DATE                 WHO                  REFERENCE                     DESCRIPTION
*12/04/2023      CONVERSION TOOL     AUTO R22 CODE CONVERSION           FM TO @FM, VM TO @VM, SM TO @SM,++ TO +=
*12/04/2023         SURESH           MANUAL R22 CODE CONVERSION         CALL routine format modified
* ----------------------------------------------------------------------------
...
* PACS00761324 END
    ...
    IN.ARR.ID = ''
*APAP.TAM.REDO.CONVERT.ACCOUNT(ACC.ID,IN.ARR.ID,OUT.ID,ERR.TEXT) ;*MANUAL R22 CODE CONVERSION
    APAP.TAM.redoConvertAccount(ACC.ID,IN.ARR.ID,OUT.ID,ERR.TEXT) ;*MANUAL R22 CODE CONVERSION
    ARR.ID = OUT.ID
...
*---------------------------------------------------
    ...
    OPTIONS = ''
*APAP.TAM.REDO.AA.BUILD.OFS(ARR.ID,Y.PRIN.RATE.ARRAY.OFS,Y.ACT.PROP,OFS.STRING.PRIN) ;*MANUAL R22 CODE CONVERSION
    APAP.AA.redoAaBuildOfs(ARR.ID,Y.PRIN.RATE.ARRAY.OFS,Y.ACT.PROP,OFS.STRING.PRIN) ;*MANUAL R22 CODE CONVERSION
...
*    CALL OFS.CALL.BULK.MANAGER(OFS.SRC, OFS.STRING.PRIN, OFS.RESP, TXN.COMMIT)
    EB.Interface.OfsCallBulkManager(OFS.SRC, OFS.STRING.PRIN, OFS.RESP, TXN.COMMIT);* R22 UTILITY AUTO CONVERSION
...
*OFS.STRING.FINAL = ''
* APAP.AA.REDO.AA.BUILD.OFS(ARR.ID,Y.PENAL.RATE.ARRAY.OFS,Y.ACT.PROP,OFS.STRING.PENAL) ;*MANUAL R22 CODE CONVERSION
    APAP.AA.redoAaBuildOfs(ARR.ID,Y.PENAL.RATE.ARRAY.OFS,Y.ACT.PROP,OFS.STRING.PENAL) ;*MANUAL R22 CODE CONVERSION
...
*    CALL OFS.CALL.BULK.MANAGER(OFS.SRC, OFS.STRING.PENAL, OFS.RESP, TXN.COMMIT)
    EB.Interface.OfsCallBulkManager(OFS.SRC, OFS.STRING.PENAL, OFS.RESP, TXN.COMMIT);* R22 UTILITY AUTO CONVERSION
    CALL OCOMO(" END MESSAGE- ":ARR.ID:" - END RESP - ":OFS.RESP)
...
*APAP.AA.REDO.CRR.GET.CONDITIONS(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.PRINCIPAL.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
    APAP.AA.redoCrrGetConditions(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.PRINCIPAL.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
...
*---------------------------------------------------
    ...
    ERR.MSG = ''
    R.PENALTY.PROPERTY = ''
*APAP.AA.REDO.CRR.GET.CONDITIONS(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.PENALTY.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
    APAP.AA.redoCrrGetConditions(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.PENALTY.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
...
*APAP.AA.REDO.CRR.GET.CONDITIONS(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.TERM.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
    APAP.AA.redoCrrGetConditions(Y.ARRG.ID,EFF.DATE,PROPERTY.CLASS,PROPERTY,R.TERM.PROPERTY,ERR.MSG) ;*MANUAL R22 CODE CONVERSION
