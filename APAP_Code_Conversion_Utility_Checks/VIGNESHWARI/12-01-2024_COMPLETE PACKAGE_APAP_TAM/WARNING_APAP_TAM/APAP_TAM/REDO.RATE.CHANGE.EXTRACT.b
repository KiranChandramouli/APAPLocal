* @ValidationCode : MjoxNTI2OTYzOTUzOkNwMTI1MjoxNzA0ODg2ODAwOTUwOjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 10 Jan 2024 17:10:00
...
*    CALL CACHE.READ(FN.REDO.MASSIVE.FILE.PATH,'SYSTEM',R.FILE.DETAILS,F.REDO.MASSIVE.FILE.PATH)
    ...
    CALL CACHE.READ(FN.REDO.MASSIVE.FILE.PATH,IDVAR.1,R.FILE.DETAILS,F.REDO.MASSIVE.FILE.PATH);* R22 UTILITY AUTO CONVERSION
...
*        CALL F.READ(FN.REDO.RATE.CHANGE,Y.RATE.CHANGE.ID,R.REDO.RATE.CHANGE,F.REDO.RATE.CHANGE,RATE.ERR)
        CALL F.READU(FN.REDO.RATE.CHANGE,Y.RATE.CHANGE.ID,R.REDO.RATE.CHANGE,F.REDO.RATE.CHANGE,RATE.ERR,'');* R22 UTILITY AUTO CONVERSION
...
** R22 Manual conversion
            APAP.TAM.redoCheckMarginIds(Y.AA.IDS.LIST,Y.RATE.CHANGE.ID,Y.RETURN.IDS)
        END
    END

    IF Y.RETURN.IDS THEN
*CALL REDO.CHECK.AA.IDS(Y.RETURN.IDS,RETURN.AA.IDS);* R22 Manual conversion
        APAP.AA.redoCheckAaIds(Y.RETURN.IDS,RETURN.AA.IDS) ;* R22 Manual conversion - CALL method format changed
    END
...
** R22 Manual conversion
    APAP.TAM.redoGetInterestProperty(ARR.ID,PROP.NAME,PRIN.PROP,ERR)
    IF PRIN.PROP THEN
        EFF.DATE = TODAY
        PROP.CLASS='INTEREST'
        PROPERTY = PRIN.PROP
...
** R22 Manual conversion
        APAP.AA.redoCrrGetConditions(ARR.ID,EFF.DATE,PROP.CLASS,PROPERTY,R.CONDITION.INTEREST,ERR.MSG)
    END
