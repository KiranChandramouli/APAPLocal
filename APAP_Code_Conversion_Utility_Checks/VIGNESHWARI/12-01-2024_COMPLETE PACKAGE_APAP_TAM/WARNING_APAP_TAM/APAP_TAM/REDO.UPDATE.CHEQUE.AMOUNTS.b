* @ValidationCode : MjotMTYzMzI1NjAyMzpVVEYtODoxNzA0OTcwMTc0OTQ0OkFkbWluOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 11 Jan 2024 16:19:34
...
*13/07/2023	               CONVERSION TOOL      AUTO R22 CODE CONVERSION		 VM TO @VM,FM TO @FM,SM TO @SM,++ TO +=1
...
*    CALL F.READ(FN.REDO.LOAN.FT.TT.TXN,Y.ID.NEW,R.TXN,F.REDO.LOAN.FT.TT.TXN,TXN.ERR)
    CALL F.READU(FN.REDO.LOAN.FT.TT.TXN,Y.ID.NEW,R.TXN,F.REDO.LOAN.FT.TT.TXN,TXN.ERR,'');* R22 UTILITY AUTO CONVERSION
...
*        CALL F.READ(FN.REDO.CONCAT.CHEQUE.NOS,Y.UPDATE.ID,R.CHQ.NOS,F.REDO.CONCAT.CHEQUE.NOS,CHQ.ERR)
        CALL F.READU(FN.REDO.CONCAT.CHEQUE.NOS,Y.UPDATE.ID,R.CHQ.NOS,F.REDO.CONCAT.CHEQUE.NOS,CHQ.ERR,'');* R22 UTILITY AUTO CONVERSION
        IF R.CHQ.NOS THEN
...
* R.TXN<LN.FT.TT.TRANSACTION.ID,Y.VAR2> = Y.CHEQUE.TRANS.ID
        R.TXN<LN.FT.TT.FT.TRANSACTION.ID,Y.VAR2> = Y.CHEQUE.TRANS.ID ;*MANUAL R22 CODE CONVERSION
        Y.VAR2 += 1
    REPEAT
    Y.ID.NEW = ID.NEW
