* @ValidationCode : MjozNDAwODM5MDQ6VVRGLTg6MTcwNDk2ODM2NjA3ODpBZG1pbjotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 11 Jan 2024 15:49:26
...
*10-08-2023    VICTORIA S          R22 MANUAL CONVERSION   FM TO @FM, VM TO @VM, SM TO @SM
*----------------------------------------------------------------------------------------
...
*----------------------------------------------------------------------------------
    ...
    Y.CURRENCY =  CHANGE(Y.CURRENCY,@SM,@VM) ;*R22 MANUAL CONVERSION
    Y.CURRENCY =  CHANGE(Y.CURRENCY,@FM,@VM) ;*R22 MANUAL CONVERSION
...
*            IF Y.CURRENCY<1,1> = 'DOP' THEN
            IF Y.CURRENCY<1,1> EQ 'DOP' THEN;* R22 UTILITY AUTO CONVERSION
...
*            IF Y.CURRENCY<1,2> = 'DOP' THEN
            IF Y.CURRENCY<1,2> EQ 'DOP' THEN;* R22 UTILITY AUTO CONVERSION
...
*-------------------------------------------------------------------------
    ...
    BEGIN CASE
        ...
        CASE Y.FIELD.NAME EQ 'Y.STMT.NO'
            ...
            Y.STMT.NO= CHANGE(Y.STMT.NO,@SM,@VM) ;*R22 MANUAL CONVERSION
            Y.STMT.NO= CHANGE(Y.STMT.NO,@FM,@VM) ;*R22 MANUAL CONVERSION
...
*-----------------------------------------------
    ...
    Y.OVERRIDE.2 = CHANGE(Y.OVERRIDE.2,@SM,@VM) ;*R22 MANUAL CONVERSION START
    Y.OVERRIDE.2 = CHANGE(Y.OVERRIDE.2,@FM,@VM)
    Y.OVERRIDE.2 = Y.OVERRIDE.2<1,2>
    Y.OVERRIDE.2 = CHANGE(Y.OVERRIDE.2,'{',@FM)
    Y.OVERRIDE.2 = CHANGE(Y.OVERRIDE.2,'}',@VM) ;*R22 MANUAL CONVERSION END
...
*-----------------------------------------------
    ...
    Y.OVERRIDE.1 = CHANGE(Y.OVERRIDE.1,@SM,@VM) ;*R22 MANUAL CONVERSION START
    Y.OVERRIDE.1 = CHANGE(Y.OVERRIDE.1,@FM,@VM)
    Y.OVERRIDE.1 = Y.OVERRIDE.1<1,1>
    Y.OVERRIDE.1 = CHANGE(Y.OVERRIDE.1,'{',@FM)
    Y.OVERRIDE.1 = CHANGE(Y.OVERRIDE.1,'}',@VM) ;*R22 MANUAL CONVERSION END
