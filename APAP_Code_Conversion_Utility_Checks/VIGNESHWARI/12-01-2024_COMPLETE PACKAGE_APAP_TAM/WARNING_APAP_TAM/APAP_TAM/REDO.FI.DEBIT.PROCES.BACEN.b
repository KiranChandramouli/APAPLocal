* @ValidationCode : MjotMTU3NTE1MzY4MjpDcDEyNTI6MTcwNDg4Nzc0ODYyNjp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 10 Jan 2024 17:25:48
...
*28-08-2023    CONVERSION TOOL     R22 AUTO CONVERSION	   FM TO @FM, INSERT FILE MODIFIED,VARIABLE NAME MODIFIED
*28-08-2023    VICTORIA S          R22 MANUAL CONVERSION   DYN.TO.OFS to OFS.BUILD.RECORD,CALL ROUTINE MODIFIED
*10-01-2024    VIGNESHWARI S       R22 MANUAL CONVERSION    CHANGE F.READ TO F.CACHE.READ
*----------------------------------------------------------------------------------------
...
*R22 MANUAL CONVERSION
        APAP.REDOENQ.redoNominaAcctStatusCheck(IN.ACCT.ID,IN.ACCT.TYPE,OUT.ACCT.STATUS)
...
*
    ...
    IF Y.ERR.MSG NE "" THEN
        WERROR.MSG = 'Entry.Param.NotExists.&':@FM:WCONTROL.ID ;*R22 AUTO CONVERSION
        RETURN
    END
...
*VALIDATE THE AMOUNTS
    IF F.TOT.AMOUNT GT Y.AMOUNT.DEPOSITED THEN
        WERROR.MSG = "EB-Not.enough.BALANCE.in.&.transaction":@FM:WCONTROL.ID ;*R22 AUTO CONVERSION
        RETURN
    END
...
*
    ...
    IF YER.CTA THEN
        WERROR.MSG = "AC-AC.REC.MISS":@FM:COMM.CUST.ACCT ;*R22 AUTO CONVERSION
    END ELSE
...
*Y.OFS.STR = DYN.TO.OFS(R.FUNDS.TRANSFER,'FUNDS.TRANSFER',ADDNL.INFO)
    ...
    OFS.FUNCTION='I'
    OFS.PROCESS='PROCESS'
    OFS.VERSION=APP.NAME:',':COMM.VERSION   ;*R22 MANUAL CONVERSION
    Y.GTSMODE=''
    NO.OF.AUTH=''
    TRANSACTION.ID=''
...
EB.Interface.OfsCallBulkManager(OFS.SOURCE.ID, Y.OFS.STR, OFS.RESP, TXN.COMMIT);* R22 UTILITY AUTO CONVERSION
...
*
    OUT.ERR<2> = "ERROR"
    Y.VAR = FIELD(OFS.RESP,'-1/NO,',2) ;*R22 AUTO CONVERSION START
    Z = INDEX(Y.VAR,",",1) - 1
    IF Z EQ '-1' THEN
        Z = 50
    END
    OUT.ERR<3> ="ERROR"
    IF Y.VAR NE '' THEN
        OUT.ERR<4>=Y.VAR ;*R22 AUTO CONVERSION END
    END
*
RETURN
*
...
*  CALL F.READ(FN.REDO.INTERFACE.PARAM, Y.INTERFACE, R.REDO.INTERFACE.PARAM, F.REDO.INTERFACE.PARAM, Y.ERR)
                CALL F.CACHE.READ(FN.REDO.INTERFACE.PARAM, Y.INTERFACE, R.REDO.INTERFACE.PARAM, Y.ERR)  ;*R22 MANUAL CODE CONVERSION-CHANGE F.READ TO F.CACHE.READ
                IF Y.ERR THEN
                    PROCESS.GOAHEAD = 0
...
*
    ...
    Y.POS.AV.BAL           = ""
    LREF.POS = ''
    LREF.APP = 'FUNDS.TRANSFER':@FM:'ACCOUNT' ;*R22 AUTO CONVERSION
    LREF.FIELD = 'L.COMMENTS':@FM:'L.AC.AV.BAL' ;*R22 AUTO CONVERSION
