* @ValidationCode : MjotOTgzMzIxOTU0OkNwMTI1MjoxNzAyODM2ODIyMjQ4OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 17 Dec 2023 23:43:42
...
*------------------------------------------------------------------------
* 06-06-2023      Conversion Tool       R22 Auto Conversion - FM TO @FM AND VM TO @VM AND F.READ TO CACHE.READ
* 06-06-2023      ANIL KUMAR B          R22 Manual Conversion - ADDING the veriable Y.MAIL.MSG<-1> = R.REDO.TEMP.FI.CONTROL<FI.TEMP.MAIL.MSG> AND adding packag APAP.TAM and alles changed for call routine
*15-12-2023         VIGNESHWARI            R22 Manual Conversion - OPF IS ADDED, VARIABLE IS ASSIGN
*------------------------------------------------------------------------
*
* =============================================================================
*
    ...
    WHILE LOOP.CNT LE MAX.LOOPS AND PROCESS.GOAHEAD  DO
        BEGIN CASE
            CASE LOOP.CNT EQ 1        ;*           CONVERT FORMAT XML TO TXT
                APAP.REDOFCFI.redoFiConvertXmlToTxt(I.XML.MSG, R.TXT.MSG, O.ERROR.MSG)   ;* R22 MANUAL CONVERSION
                IF O.ERROR.MSG THEN
                    PROCESS.GOAHEAD = 0
                END
            ...
            CASE LOOP.CNT EQ 2        ;*           VALIDATE HEADER AND DETAIL
                W.TXT.MSG = R.TXT.MSG
                APAP.REDOFCFI.redoFiValidateHeadAndData(R.TXT.MSG, R.POS.VALUES, R.VALUES, O.ERROR.MSG)   ;* R22 MANUAL CONVERSION
                IF O.ERROR.MSG THEN
                    PROCESS.GOAHEAD = 0
...
*   Opens and Writes TXT message in corresponding dir.
    ...
    FOR II = 2 TO Y.COUNT
        ...
        APAP.REDOFCFI.redoFiMsgFormat(FI.INTERFACE,R.TXT.MSG<II>,DATO.OUT)   ;* R22 MANUAL CONVERSION

        IF Y.ERR.MSG NE "" THEN
            W.STATUS = "04"
            W.ERROR.MSG = Y.ERR.MSG
            Y.MAIL.MSG<-1> = R.REDO.TEMP.FI.CONTROL<FI.TEMP.MAIL.MSG>  ;*R22 MANUAL CONVERSION adding the veribal Y.MAIL.MSG<-1> = R.REDO.TEMP.FI.CONTROL<FI.TEMP.MAIL.MSG>
        END
...
*   Paragraph that record the error message and move the file to the rejected directory renaming the file
    OUT.RESP = ''
    APAP.TAM.redoFiDebitProcesBacen(R.PARAM, OUT.REF, OUT.RESP)   ;*R22 MANUAL CONVERSION 
...
*
    ...
    CALL OPF(FN.REDO.APAP.PARAM.EMAIL,F.REDO.APAP.PARAM.EMAIL)  ;*R22 Manual Conversion - OPF IS ADDED
    ID.VAR='SYSTEM' ;*R22 Manual Conversion - VARIABLE IS ASSIGN
    CALL CACHE.READ(FN.REDO.APAP.PARAM.EMAIL,ID.VAR,R.EMAIL,MAIL.ERR)
