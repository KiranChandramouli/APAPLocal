* @ValidationCode : MjoxOTA5MzQ0MDMwOkNwMTI1MjoxNzAzNTg2ODQwNjg5OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 26 Dec 2023 16:04:00
...
$PACKAGE APAP.AA
SUBROUTINE REDO.APAP.FT.REV.CHGS.AA
***********************************************************
*----------------------------------------------------------
*
* COMPANY NAME    : APAP
...
* 29-MAR-2023    Conversion Tool     R22 Auto conversion       F.READ to CACHE.READ, FM TO @FM, VM to @VM, SM to @SM
* 29-MAR-2023    Harishvikram C      R22 Manual conversion     No changes
* 26-12-2023      VIGNESHWARI S     R22 MANUAL CONVERSTION   CALL RTN MODIFIED, VARIABLE IS HARDCODED
*-----------------------------------------------------------------------------------
...
*GOSUB PROCESS ;* Process of reversing the tax and commission has been removed since core is reversing the local entries(PACS00324212).
*GOSUB UPD.REVERSE.DET
        GOSUB RAISE.ACCOUNTING.WT
    END
*
RETURN  ;* END
...
UPD.REVERSE.DET:
* ============
    Y.REV.FT.ID  = R.NEW(FT.LOCAL.REF)<1,Y.L.FT.ADD.INFO.POS>
    Y.REV.FT.ID  = FIELD(Y.REV.FT.ID,'-',2)
    R.REDO.FT.REVERSE.LIST = ID.NEW
...
* Taxes
    ...
    WHILE Y.TAX.CTR LE Y.TAX.CODE.CNT
        ...
        IF Y.WV.TAX.YES.NO<Y.TAX.CTR> EQ "YES" AND Y.WAIVE.CATEG THEN
            ...
            Y.SIGN        = -1
            Y.TXN.CODE    = Y.TR.CODE.DR
            Y.DATE        = Y.DR.VALUE.DATE
            NARRATIVE     = 'TAX ENTRY - WAIVED'
            W.TAX.ACCOUNT = Y.TAX.ACCOUNT
            Y.CRF.TYPE    = 'DEBIT'
            GOSUB POPULATE.STMT.ENTRY
            Y.SIGN        = 1
            Y.TXN.CODE    = Y.TR.CODE.CR
            Y.DATE        = Y.CR.VALUE.DATE
            Y.CRF.TYPE    = 'CREDIT'
            Y.PL.CATEG    = Y.WAIVE.CATEG
...
*
    ...
    CALL CACHE.READ(FN.FTCT, Y.FTCT.ID, R.FTCT, FTCT.ERR) ;*R22 Auto Conversion
    IF R.FTCT THEN
...
*
    ...
    R.CATEG.ENT<AC.CAT.CRF.TYPE>         = Y.CRF.TYPE
...
*
    ...
    R.CATEG.ENT<AC.CAT.VALUE.DATE>       = Y.DATE
    R.CATEG.ENT<AC.CAT.AMOUNT.LCY>       = Y.TAX.AMT.LCY *  Y.SIGN
    R.CATEG.ENT<AC.CAT.TRANSACTION.CODE> = Y.TXN.CODE
...
*
    ...
    END ELSE
        R.CATEG.ENT<AC.CAT.AMOUNT.FCY>    = Y.TAX.AMT.FCY *  Y.SIGN
...
*
    ...
    R.STMT.ARR<AC.STE.CRF.TYPE>          = Y.CRF.TYPE
...
*
    IF Y.TXN.CCY EQ LCCY THEN
        R.STMT.ARR<AC.STE.AMOUNT.LCY>         = Y.TAX.AMT.LCY *  Y.SIGN   ;* '-1'
        R.STMT.ARR<AC.STE.EXCHANGE.RATE> = ''
        R.STMT.ARR<AC.STE.AMOUNT.FCY>    = ''
    END ELSE
        R.STMT.ARR<AC.STE.AMOUNT.FCY>         = Y.TAX.AMT.FCY * Y.SIGN    ;* '-1'
...
*
   ...
   * CALL EB.ACCOUNTING("AC","SAO",Y.COMMON.ARRAY,'');*R22 MANUAL CODE CONVERSION -CALL RTN MODIFIED
AC.API.EbAccounting("AC","SAO",Y.COMMON.ARRAY,'')
...
* =========
   ...
   * CALL CACHE.READ(FN.REDO.H.AA.DIS.CHG,'SYSTEM',R.REDO.H.AA.DIS.CHG,Y.ERR)
   VAR.VALUE = 'SYSTEM'
   CALL CACHE.READ(FN.REDO.H.AA.DIS.CHG,VAR.VALUE,R.REDO.H.AA.DIS.CHG,Y.ERR) ;*R22 MANUAL CODE CONVERSION-VARIABLE IS HARDCODED
    Y.INT.ACC = R.REDO.H.AA.DIS.CHG<REDO.DIS.CHG.REVERSE.INT.ACC>
...
*------------------------------------------------------------
    ...
    CALL F.READ(FN.ACCOUNT,Y.DR.ACC,R.ACCOUNT.DR,F.ACCOUNT,ERR.ACC)
    IF R.ACCOUNT.DR THEN
        Y.CUST.ID       = R.ACCOUNT.DR<AC.CUSTOMER>
        Y.ACCT.OFFICER  = R.ACCOUNT.DR<AC.ACCOUNT.OFFICER>
        Y.ACCT.CATEGORY = R.ACCOUNT.DR<AC.CATEGORY>
    END

    IF Y.CUST.ID OR Y.INT.ACC EQ '' THEN
        RETURN
    END

    Y.AMOUNT     = WTRAN.AMOUNT
    Y.CR.CODE    = 82
    Y.DR.CODE    = 201
...
*
    ...
    R.STMT.ARR<AC.STE.POSITION.TYPE>     = 'TR'
*
    R.STMT.ARR<AC.STE.ACCOUNT.NUMBER>    = R.NEW(FT.DEBIT.ACCT.NO)
    R.STMT.ARR<AC.STE.COMPANY.CODE>      = ID.COMPANY
    R.STMT.ARR<AC.STE.TRANSACTION.CODE>  = Y.DR.CODE
    R.STMT.ARR<AC.STE.ACCOUNT.OFFICER>   = R.ACCOUNT.DR<AC.ACCOUNT.OFFICER>
    R.STMT.ARR<AC.STE.PRODUCT.CATEGORY>  = R.ACCOUNT.DR<AC.CATEGORY>
    R.STMT.ARR<AC.STE.VALUE.DATE>        = Y.DR.VALUE.DATE
    R.STMT.ARR<AC.STE.CURRENCY>          = Y.TXN.CCY
*
    R.STMT.ARR<AC.STE.CUSTOMER.ID>       = R.ACCOUNT.DR<AC.CUSTOMER>

*
    IF Y.TXN.CCY EQ LCCY THEN
        R.STMT.ARR<AC.STE.AMOUNT.LCY>         = WTRAN.AMOUNT * -1         ;* '-1'
        R.STMT.ARR<AC.STE.EXCHANGE.RATE> = ''
        R.STMT.ARR<AC.STE.AMOUNT.FCY>    = ''
    END ELSE
        R.STMT.ARR<AC.STE.AMOUNT.FCY>         = WTRAN.AMOUNT * -1         ;* '-1'
        R.STMT.ARR<AC.STE.EXCHANGE.RATE> = ''
    END
*
...
*
    ...
    Y.CR.ACC = Y.INT.ACC
    CALL F.READ(FN.ACCOUNT,Y.CR.ACC,R.ACCOUNT.CR,F.ACCOUNT,ERR.ACC)
...
*
    ...
    R.STMT.ARR<AC.STE.POSITION.TYPE>     = 'TR'
*
    R.STMT.ARR<AC.STE.ACCOUNT.NUMBER>    = Y.CR.ACC
    R.STMT.ARR<AC.STE.COMPANY.CODE>      = ID.COMPANY
    R.STMT.ARR<AC.STE.TRANSACTION.CODE>  = Y.CR.CODE
    R.STMT.ARR<AC.STE.ACCOUNT.OFFICER>   = R.ACCOUNT.CR<AC.ACCOUNT.OFFICER>
    R.STMT.ARR<AC.STE.PRODUCT.CATEGORY>  = R.ACCOUNT.CR<AC.CATEGORY>
    R.STMT.ARR<AC.STE.VALUE.DATE>        = Y.CR.VALUE.DATE
    R.STMT.ARR<AC.STE.CURRENCY>          = Y.TXN.CCY
*
    R.STMT.ARR<AC.STE.CUSTOMER.ID>       = R.ACCOUNT.CR<AC.CUSTOMER>

*
    IF Y.TXN.CCY EQ LCCY THEN
        R.STMT.ARR<AC.STE.AMOUNT.LCY>         = WTRAN.AMOUNT    ;* '-1'
        R.STMT.ARR<AC.STE.EXCHANGE.RATE> = ''
        R.STMT.ARR<AC.STE.AMOUNT.FCY>    = ''
    END ELSE
        R.STMT.ARR<AC.STE.AMOUNT.FCY>         = WTRAN.AMOUNT    ;* '-1'
        R.STMT.ARR<AC.STE.EXCHANGE.RATE> = ''
    END
    Y.COMMON.ARRAY<-1> = LOWER(R.STMT.ARR)
    IF Y.COMMON.ARRAY THEN
