* @ValidationCode : MjoxMzM2MTk5NTA3OkNwMTI1MjoxNzAzODQyNzA2MTE5OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 29 Dec 2023 15:08:26
...
* PACS00329804 - S
  ...
  *  CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.ERR)
   R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read(Y.AA.ID, Y.ERR);*r22 manual code conversion -aa.framework is changed f.read
* PACS00329804 - E
    IF Y.ERR THEN
        OUT.LIM.AA = 'ERROR'
        RETURN
    ...
    *Y.LOAN.CUSTOMER = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
     Y.LOAN.CUSTOMER = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer>;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
*AA Changes 20161013
...
*
  ...
  *  CALL AA.GET.ARRANGEMENT.PROPERTIES(ARR.INFO, Y.ARR.DATE, R.ARRANGEMENT, PROP.LIST)
   AA.Framework.GetArrangementProperties(ARR.INFO, Y.ARR.DATE, R.ARRANGEMENT, PROP.LIST) ;*R22 MANUAL CODE CONVERSION-CALL RTN MODIFIED
 ...
 *   CALL AA.GET.ARRANGEMENT.CONDITIONS(ARR.INFO,PROPERTY.CLASS,AA.PROPERTY,Y.EFFEC.DATE,returnIds,returnConditions,returnError)
  AA.Framework.GetArrangementConditions(ARR.INFO,PROPERTY.CLASS,AA.PROPERTY,Y.EFFEC.DATE,returnIds,returnConditions,returnError) ;*R22 MANUAL CONVERSTION -CALL RTN MODIFIED
...
*===============
   ...
   *     CALL F.READ(FN.AA.ARRANGEMENT,LIM.AA.ID<2>,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.ERR)
        R.AA.ARRANGEMENT = AA.Framework.Arrangement.Read("LIM.AA.ID", Y.ERR);*r22 manual code conversion -aa.framework is changed f.read
        IF Y.ERR THEN
            OUT.LIM.AA = 'ERROR'
            RETURN
        END
