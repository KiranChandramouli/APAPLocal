* @ValidationCode : MjotMTM5OTU4OTczMTpDcDEyNTI6MTcwMzY3OTA2MjE2NTp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 27 Dec 2023 17:41:02
...
* 27-12-2023       VIGNESHWARI S      R22 MANUAL CONVERSTION       call rtn modified,variable is hardcoded
*------------------------------------------------------
* -----------------------------------------------------------------------------------------------------
...
*CALL CACHE.READ(FN.CREDIT.LIMIT.MAINTENANCE,'SYSTEM',R.CRD.LIM.MAIN,CRD.ERR)
    VAR.VAL = 'SYSTEM'   ;*R22 MANUAL CONVERSTION-variable is hardcoded
    CALL CACHE.READ(FN.CREDIT.LIMIT.MAINTENANCE,VAR.VAL,R.CRD.LIM.MAIN,CRD.ERR)
    ...
    *CALL GET.LOC.REF(LOC.REF.APPLICATION,LOC.REF.FIELDS,LOC.REF.POS)
    EB.LocalReferences.GetLocRef(LOC.REF.APPLICATION,LOC.REF.FIELDS,LOC.REF.POS) ;*R22 MANUAL CONVERSTION-  call rtn modified
...
*CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,'SYSTEM',R.CRD.LIM.MAIN)
        VAR.VALUE = 'SYSTEM' ;*R22 MANUAL CONVERSTION-variable is hardcoded
        CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,VAR.VALUE,R.CRD.LIM.MAIN)
        RETURN
    END ELSE
*Check relation customer of the main customer is employee of the bank or not
        ...
        WHILE REL.CUS.ID:REL.POS
            ...
            IF VAR.REL.SECTOR EQ 1002 THEN
                ...
                VAL.ID.1='SYSTEM'
* CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,'SYSTEM',R.CRD.LIM.MAIN)
                 CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,VAL.ID.1,R.CRD.LIM.MAIN) ;*R22 MANUAL CONVERSTION-variable is hardcoded
                    RETURN
                END
            END
        REPEAT
    ...
    IF VAR1 EQ '' THEN
        ...
        *CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,'SYSTEM',R.CRD.LIM.MAIN)
        VAL.ID='SYSTEM'
        CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,VAL.ID,R.CRD.LIM.MAIN)  ;*R22 MANUAL CONVERSTION-variable is hardcoded
    END
RETURN
*-------------------------------------------------------------------------------------------------------------------------------
CHECK.RISK:
...
* Check if the customer having any risk group
    ...
    VAR.VAL.ID='SYSTEM'
    *CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,'SYSTEM',R.CRD.LIM.MAIN)
    CALL F.WRITE(FN.CREDIT.LIMIT.MAINTENANCE,VAR.VAL.ID,R.CRD.LIM.MAIN)
RETURN
*-----------------------------------------------------------------------------------------------------------------------------------
CHECK.APPLICATION:
*--------------------------------------------------------------------------------------------------------------------------------------
