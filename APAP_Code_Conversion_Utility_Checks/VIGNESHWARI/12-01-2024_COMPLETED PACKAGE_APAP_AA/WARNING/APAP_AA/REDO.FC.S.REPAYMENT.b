* @ValidationCode : MjotNjYxMDAwNDQwOkNwMTI1MjoxNzA0MjY0MjkyOTA2OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 03 Jan 2024 12:14:52
...
*02-01-2024     VIGNESHWARI S      R22 MANUAL CONVERSION                AA.FRAMEWORK IS CHANGED
*-----------------------------------------------------------------------------------
*
...
IF AA.Framework.getC_aalocactivitystatus() EQ 'AUTH' OR AA.Framework.getC_aalocactivitystatus() EQ 'AUTH-REV' OR AA.Framework.getC_aalocactivitystatus() EQ 'REVERSE' THEN ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
*IF c_aalocActivityStatus EQ 'AUTH-REV' OR c_aalocActivityStatus EQ 'REVERSE' THEN
    IF AA.Framework.getC_aalocactivitystatus() EQ 'AUTH-REV' OR AA.Framework.getC_aalocactivitystatus() EQ 'REVERSE' THEN ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
CALL F.READ(FN.REDO.FC.CL.BALANCE,AA.Framework.getC_aalocarrid(),R.REDO.FC.CL.BALANCE,F.REDO.FC.CL.BALANCE,YERR);* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
IF R.REDO.FC.CL.BALANCE THEN
*APAP.AA.redoFcClPaymentAa(TOTAL.ACC.AMT, c_aalocArrId);* R22 Manual conversion
APAP.AA.redoFcClPaymentAa(TOTAL.ACC.AMT, AA.Framework.getC_aalocarrid());* R22 Manual conversion ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
APAP.AA.redoFcClPaymentAa(TOTAL.ACC.AMT, AA.Framework.getC_aalocarrid());* PACS00350509 - S/E, ;* R22 Manual conversion ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
END
*
END ELSE
*CALL F.READ(FN.REDO.FC.CL.BALANCE,c_aalocArrId,R.REDO.FC.CL.BALANCE,F.REDO.FC.CL.BALANCE,YERR)
...
VAR.ARR.ID =  AA.Framework.getC_aalocarrid() ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
Y.ARR.ID = AA.Framework.getC_aalocarrid() ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
AA.Framework.GetPeriodBalances(Y.ACCOUNT.ID, Y.TOT, DATE.OPTIONS, EFFECTIVE.DATE, "", "", TOT.BAL.DETAILS, "");* R22 AUTO CONVERSION
...
AA.Framework.GetPeriodBalances(Y.ACCOUNT.ID, Y.CUR, DATE.OPTIONS, EFFECTIVE.DATE, "", "", CUR.BAL.DETAILS, "");* R22 AUTO CONVERSION
...
R.ACCOUNT.DETAILS = AA.Framework.getC_aalocaccountdetails() ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
*IF Y.REPAY.REF.ID EQ c_aalocArrActivityId AND Y.REPAY.DATE EQ TODAY THEN
    IF Y.REPAY.REF.ID EQ AA.Framework.getC_aalocarractivityid() AND Y.REPAY.DATE EQ TODAY THEN ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
...
R.AA.ACTIVITY  = AA.Framework.getC_aalocarractivityrec() ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
RETURN
*----------------------------------------------------------------------
BILL.DETAILS:
*----------------------------------------------------------------------
...
IF VAR.REPAY.REF.ID EQ AA.Framework.getC_aalocarractivityid() THEN ;* R22 MANUAL CONVERSION-AA.FRAMEWORK IS CHANGED
