* @ValidationCode : MjoxNjU5MzM0MDQ3OkNwMTI1MjoxNzA0Njk0NTI2Mzk1OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 08 Jan 2024 11:45:26
...
** 01-01-2024 Vigneshwari R22 Manual Conversion - CALL method format changed,AA.FRAMEWORK IS CHANGED
*------------------------------------------------------------------------------------------------------------------------------
*
...
ACCOUNT.DETAILS = AA.Framework.getC_aalocaccountdetails()
ACTIVITY.ID =  AA.Framework.getC_aaloccurractivity()
EFF.DATE = AA.Framework.getC_aalocactivityeffdate()
ARR.CURRENCY = AA.Framework.getC_aalocarrcurrency()
ARR.ACCOUNT.ID = AA.Framework.getC_aaloclinkedaccount()
RET.ERR = ''
ARR.EFF.DATE = AA.Framework.getC_aalocaccountdetails()<AA.AD.START.DATE>
...
Y.CURR.ACT.REC = AA.Framework.getC_aalocarractivityrec()    ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
Y.LOCAL.REF = AA.Framework.getC_aalocarractivityrec()<AA.Framework.RrControl.ArrActFieldName>   ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
...
*Y.LL.S = c_aalocArrActivityRec<AA.ARR.ACT.FIELD.VALUE>        ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED

Y.LL.S = AA.Framework.getC_aalocarractivityrec()<AA.Framework.SimulationCapture.ArrActFieldValue>   ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
Y.LL.S = AA.Framework.getC_aalocarractivityrec()<AA.Framework.SimulationCapture.ArrActFieldValue>  ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
...
AA.Framework.GetArrangementConditions(ARR.ID,'ACCOUNT', '','',RET.IDS, ACC.COND, RET.ERR);* R22 AUTO CONVERSION
...
Y.ACCT.MAS = AA.Framework.getC_aalocmasteractivity()<1,2>;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
...
CALL F.READU(FN.REDO.AA.NAB.HISTORY, ARR.ID, R.NAB.HISTORY.REC, F.REDO.AA.NAB.HISTORY, RET.ERR,'');* R22 AUTO CONVERSION
...
Y.NAB.PAY.DATE = AA.Framework.getC_aalocactivityeffdate()    ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
END
...
EFFEC.DATE    = AA.Framework.getC_aalocactivityeffdate()    ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
PROP.CLASS  = 'OVERDUE'
PROPERTY    = ''
ERR.MSG     = ''
R.OVERDUE.COND   = ''
...
ACCOUNT.ID = AA.Framework.getC_aaloclinkedaccount()       ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
START.DATE = AA.Framework.getC_aalocactivityeffdate() ; END.DATE = '' ; SYSTEM.DATE = ''        ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
REQUEST.TYPE    = ''
REQUEST.TYPE<4>  = 'ECB'
BAL.DETAILS = ''
*CALL AA.GET.PERIOD.BALANCES(ACCOUNT.ID, BAL.TYPES, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, ERROR.MESSAGE)
AA.Framework.GetPeriodBalances(ACCOUNT.ID, BAL.TYPES, REQUEST.TYPE, START.DATE, END.DATE, SYSTEM.DATE, BAL.DETAILS, ERROR.MESSAGE);* R22 AUTO CONVERSION
...
FINDSTR 'LENDING-MAKEDUE-REPAYMENT.SCHEDULE' IN R.AA.SCH.ACT<AA.Framework.ScheduledActivity.SchActivityName,1> SETTING POS.NEX THEN   ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
Y.NEX.BL.DATE = R.AA.SCH.ACT<AA.Framework.ScheduledActivity.SchNextDate,POS.NEX>  ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
END ELSE
...
PROPERTY = ''
*EFF.DATE = c_aalocActivityEffDate
EFF.DATE = AA.Framework.getC_aalocactivityeffdate()    ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWORK IS CHANGED
ERR.MSG = ''
R.PAY.SCH.COND = ''
