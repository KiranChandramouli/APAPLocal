* @ValidationCode : MjotMTEzOTcyNTY2NTpDcDEyNTI6MTcwMzIwNDUyMzI0MTp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Dec 2023 05:52:03
...
*21-12-2023             VIGNESHWARI                     MANUAL R22 CODE CONVERSION      CALL RTN MODIFIED , changed  EB.Service
*-----------------------------------------------------------------------------
...
BUILD.CONTROL.LIST:
*******************
;*MANUAL R22 CODE CONVERSION-call rtn modified-start
*    CALL EB.CLEAR.FILE(FN.DR.REG.RIEN5.REP1,F.DR.REG.RIEN5.REP1)
CALL EB.Service.ClearFile(FN.DR.REG.RIEN5.REP1,F.DR.REG.RIEN5.REP1)
 *   CALL EB.CLEAR.FILE(FN.DR.REG.RIEN5.REP2,F.DR.REG.RIEN5.REP2)
 CALL EB.Service.ClearFile(FN.DR.REG.RIEN5.REP2,F.DR.REG.RIEN5.REP2)
  *  CALL EB.CLEAR.FILE(FN.DR.REG.RIEN5.REP3,F.DR.REG.RIEN5.REP3)
  CALL EB.Service.ClearFile(FN.DR.REG.RIEN5.REP3,F.DR.REG.RIEN5.REP3)
;*MANUAL R22 CODE CONVERSION-call rtn modified-end
CONTROL.LIST<-1> = "REP1"
...
*        LIST.PARAMETER<3> := " AND ((ARR.STATUS EQ ":"CURRENT":") OR (ARR.STATUS EQ ":"EXPIRED":"))"
           * CALL BATCH.BUILD.LIST(LIST.PARAMETER, "")
            CALL EB.Service.BatchBuildList(LIST.PARAMETER, "")  ;*MANUAL R22 CODE CONVERSION-call rtn modified
        CASE 1
            DUMMY.LIST = ""
            *CALL BATCH.BUILD.LIST("",DUMM.LIST)
            CALL EB.Service.BatchBuildList("",DUMM.LIST)    ;*MANUAL R22 CODE CONVERSION-call rtn modified
    END CASE
