* @ValidationCode : MjoxNDU3MDQyMzc6Q3AxMjUyOjE3MDMyNTMzNDQ3NjI6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 22 Dec 2023 19:25:44
...
*22-12-2023         VIGNESHWARI                         R22 MANUAL CODE CONVERSION    call rtn modified
...
*--------------------------------------------------------------------------------------------------------
   * CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,Y.AA.ERRR)
    R.AA.ARRANGEMENT=AA.Framework.Arrangement.Read(Y.AA.ID, Y.AA.ERRR);*changed f.read
...
*    Y.LOAN.NUMBER = Y.AA.ID
   ;*R22 MANUAL CODE CONVERSION-CHANGED AA.FRAMEWORK-START 
    Y.LOAN.PORTFOLIO.TYPE=R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProductGroup>
    Y.PRODUCT.TYPE=R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrProduct>
    Y.LOAN.ORGIN.AGENCY=R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode>
    Y.CURRENCY=R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency>
    Y.LOAN.OVERALL.STATUS=R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrArrStatus>
...
*TUS END
   ...
   * Y.ACCT.NO = R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
      Y.ACCT.NO = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId>;*R22 MANUAL CODE CONVERSION
 ...
 *   CALL AA.GET.PERIOD.BALANCES(Y.ACCT.NO, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "")
     AA.Framework.GetPeriodBalances(Y.ACCT.NO, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "");*R22 MANUAL CODE CONVERSION -CALL RTN MODIFIED
    IF BAL.DETAILS THEN
...
*----------------------------------------------------------------------------------------------------------
   ...
   * Y.EFF.DATE = R.AA.ARRANGEMENT<AA.ARR.PROD.EFF.DATE>
    Y.EFF.DATE = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrProdEffDate>   ;*R22 MANUAL CODE CONVERSION-CHANGED AA.FRAMEWORK
    ARR.INFO<1> =Y.AA.ID
    R.ARRANGEMENT=''
...
*-----------------------------------------------------------------------------
   ...
   * ID.COMPONENT<AA.IDC.ARR.NO> = Y.AA.ID
    ID.COMPONENT< AA.Framework.IdcArrNo> = Y.AA.ID
    IF idPropertyClass EQ "INTEREST" THEN
        *ID.COMPONENT<AA.IDC.PROPERTY> = Y.FIRST.PROP
        ID.COMPONENT<AA.Framework.IdcProperty> = Y.FIRST.PROP
    END ELSE
    
      *  ID.COMPONENT<AA.IDC.PROPERTY> = PS.PROPERTY
        ID.COMPONENT<AA.Framework.IdcProperty> = PS.PROPERTY
    END
   * ID.COMPONENT<AA.IDC.EFF.DATE> = Y.EFFECTIVE.DATE
    ID.COMPONENT<AA.Framework.IdcEffDate> = Y.EFFECTIVE.DATE
...
*-----------------------------------------------------------------------------
  ...
  *  CALL F.READ(FN.AA.ACTIVITY.HISTORY,Y.AA.ID,R.AA.ACTIVITY.HISTORY,F.AA.ACTIVITY.HISTORY,Y.ERR.ACT.HIS)       ;*Process Arrangement activity records with LENDING-CHANGE-PRINCIPALINT
  R.AA.ACTIVITY.HISTORY=AA.Framework.ActivityHistory.Read(Y.AA.ID, Y.ERR.ACT.HIS);*r22 manual code conversion-f.read is modified
    IF R.AA.ACTIVITY.HISTORY THEN
        *Y.EFFECTIVE.DATE.HIS=R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE>  ;* getting the all effective date
        Y.EFFECTIVE.DATE.HIS=R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate>
    ...
    WHILE Y.COUNT LE Y.EFFECTIVE.DATE.COUNT
            * Y.ACTIVITY.HIS = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.COUNT>  ;* getting the activity for the first effective date
            Y.ACTIVITY.HIS = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity,Y.COUNT>  ;* getting the activity for the first effective date
        ...
        WHILE Y.COUNT1 LE Y.ACT.COUNT
           ...
           *  Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY,Y.COUNT,Y.COUNT1>
            *    Y.EFF.DATE =   R.AA.ACTIVITY.HISTORY<AA.AH.EFFECTIVE.DATE,Y.COUNT>
                Y.ACTIVITY = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhActivity,Y.COUNT,Y.COUNT1>
                Y.EFF.DATE =   R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate,Y.COUNT>
                ...
                IF Y.ACTIVITY EQ Y.ACT.CHECK AND Y.FLAG NE '1' THEN
                   ...
                   * Y.AAA.ID = R.AA.ACTIVITY.HISTORY<AA.AH.ACTIVITY.REF,Y.COUNT,Y.COUNT1> ;* activity reference for the 'LENDING-CHANGE-PRINCIPALINT' activity
                    ...
                    Y.EFFECTIVE.DATE=R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistoryHist.AhEffectiveDate,Y.COUNT>
                    Y.AAA.ID = R.AA.ACTIVITY.HISTORY<AA.Framework.ActivityHistory.AhActivityRef,Y.COUNT,Y.COUNT1> ;* activity reference for the 'LENDING-CHANGE-PRINCIPALINT' activity
...
*-----------------------------------------------------------------------------
   * CALL F.READ(FN.AA.ARRANGEMENT.ACTIVITY,Y.AAA.ID,R.AA.ARRANGEMENT.ACTIVITY,F.AA.ARRANGEMENT.ACTIVITY,Y.ERR.AAA)
    R.AA.ARRANGEMENT.ACTIVITY=AA.Framework.ArrangementActivity.Read(Y.AAA.ID, Y.ERR.AAA);*r22 manual code conversion-f.read is modified
    IF R.AA.ARRANGEMENT.ACTIVITY NE '' THEN
        IF Y.FLAG NE '1' THEN
            * Y.RATE.USER = R.AA.ARRANGEMENT.ACTIVITY<AA.ARR.ACT.INPUTTER> 
            Y.RATE.USER = R.AA.ARRANGEMENT.ACTIVITY<AA.Framework.ArrangementActivity.ArrActInputter>    ;*getting the value of inputter
