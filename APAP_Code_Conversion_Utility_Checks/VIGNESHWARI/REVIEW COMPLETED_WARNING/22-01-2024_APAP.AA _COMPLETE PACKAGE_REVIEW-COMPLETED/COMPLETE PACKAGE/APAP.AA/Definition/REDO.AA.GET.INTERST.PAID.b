* @ValidationCode : MjotMTM1NzY5MzczMjpDcDEyNTI6MTcwMzI1NTI4MTI4Njp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Dec 2023 19:58:01
...
*22-12-2023         VIGNESHWARI            R22 MANUAL CODE CONVERSION-CHANGED F.READ TO F.READU
*----------------------------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_Table
...
*---------------------------------------------------------------------------------------
   ...
   * CALL F.READ(FN.FUNDS.TRANSFER,Y.TXN.ID,R.FUNDS.TRANSFER,F.FUNDS.TRANSFER,FT.ERR)
      CALL F.READU(FN.FUNDS.TRANSFER,Y.TXN.ID,R.FUNDS.TRANSFER,F.FUNDS.TRANSFER,FT.ERR,"");*r22 manual code conversion-changed F.READ TO F.READU
...
*Here we will not handle the unmapped NCF, since API will return the NCF for sure.
    ...
    *CALL F.READ(FN.REDO.NCF.ISSUED,Y.NCF.ID,R.NCF.ISSUED,F.REDO.NCF.ISSUED,NCF.ERR)
      CALL F.READU(FN.REDO.NCF.ISSUED,Y.NCF.ID,R.NCF.ISSUED,F.REDO.NCF.ISSUED,NCF.ERR,"");*r22 manual code conversion-changed F.READ TO F.READU
   * CALL F.READ(FN.REDO.L.NCF.STATUS,Y.NCF.ID,R.NCF.STATUS,F.REDO.L.NCF.STATUS,STATUS.ERR)
    CALL F.READU(FN.REDO.L.NCF.STATUS,Y.NCF.ID,R.NCF.STATUS,F.REDO.L.NCF.STATUS,STATUS.ERR,"");*r22 manual code conversion-changed F.READ TO F.READU
