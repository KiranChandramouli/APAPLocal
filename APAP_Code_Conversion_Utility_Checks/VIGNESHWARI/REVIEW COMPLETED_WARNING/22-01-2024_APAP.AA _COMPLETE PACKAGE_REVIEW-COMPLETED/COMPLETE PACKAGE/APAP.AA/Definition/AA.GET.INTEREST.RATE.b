* @ValidationCode : MjozODI4MTkzODQ6Q3AxMjUyOjE3MDMxODEzNjY3NTk6dmlnbmVzaHdhcmk6LTE6LTE6MDowOmZhbHNlOk4vQTpSMjFfQU1SLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 21 Dec 2023 23:26:06
...
*19/12/2023         Arun               R22 Manual Conversion             Changed to AA.Interest.GetInterestNominalRate
*21/12/2023     VIGNESHWARI     R22 Manual Conversion               CHANGED AA.INTEREST
*----------------------------------------------------------------------------
...
*---------------------------------------------------------------------------------
    *Y.INT.PAY.TYPE=R.NEW(AA.INT.LOCAL.REF)<1,POS.L.AA.REV.RT.TY>
    Y.INT.PAY.TYPE=R.NEW(AA.Interest.Interest.IntLocalRef)<1,POS.L.AA.REV.RT.TY>    ;*R22 Manual Conversion-CHANGED AA.INTEREST
...
*CALL F.READ(FN.GROUP.DATE,Y.COND.AND.CURR,R.GROUP.DATE,F.GROUP.DATE,GROUP.ERR)
    ...
    IF Y.TOTAL.INT.RATE THEN
        ...
        *Y.FIXED.RATE.CNT = DCOUNT(R.NEW(AA.INT.FIXED.RATE),@VM)
        Y.FIXED.RATE.CNT = DCOUNT(R.NEW(AA.Interest.Interest.IntFixedRate),@VM) ;*AUTO R22 CODE CONVERSION  ;*R22 Manual Conversion-CHANGED AA.INTEREST
        Y.VAR2 = 0
        LOOP
        WHILE Y.VAR2 LE Y.FIXED.RATE.CNT
            IF Y.FIXED.RATE.CNT EQ 0 THEN
                *R.NEW(AA.INT.FIXED.RATE)<1,Y.VAR2+1> = Y.FINAL.RATE
                R.NEW(AA.Interest.Interest.IntFixedRate)<1,Y.VAR2+1> = Y.FINAL.RATE ;*R22 Manual Conversion-CHANGED AA.INTEREST
                *R.NEW(AA.INT.FLOATING.INDEX)<1,Y.VAR2+1> = ''
                R.NEW( AA.Interest.Interest.IntFloatingIndex)<1,Y.VAR2+1> = ''  ;*R22 Manual Conversion-CHANGED AA.INTEREST
            END ELSE
                IF Y.VAR2 NE 0 THEN
                    *R.NEW(AA.INT.FIXED.RATE)<1,Y.VAR2> = Y.FINAL.RATE
                    R.NEW(AA.Interest.Interest.IntFixedRate)<1,Y.VAR2> = Y.FINAL.RATE   ;*R22 Manual Conversion-CHANGED AA.INTEREST
                    *R.NEW(AA.INT.FLOATING.INDEX)<1,Y.VAR2> = ''
R.NEW(AA.Interest.Interest.IntFloatingIndex)<1,Y.VAR2> = '' ;*R22 Manual Conversion-CHANGED AA.INTEREST
...
*----------------------------------------------------------------------------
   * TOT.FIXED = DCOUNT(R.NEW(AA.INT.FIXED.RATE),@VM) ;*AUTO R22 CODE CONVERSION
   TOT.FIXED = DCOUNT(R.NEW(AA.Interest.Interest.IntFixedRate),@VM) ;*AUTO R22 CODE CONVERSION  ;*R22 Manual Conversion-CHANGED AA.INTEREST
    TIER.I = 1

    LOOP
    WHILE TOT.FIXED GE TIER.I
*IF R.NEW(AA.INT.FIXED.RATE)<1,TIER.I> NE '' THEN
        IF R.NEW(AA.Interest.Interest.IntFixedRate)<1,TIER.I> NE '' THEN    ;*R22 Manual Conversion-CHANGED AA.INTEREST
*   CURRENT.RATE = R.NEW(AA.INT.FIXED.RATE)<1,TIER.I>
        CURRENT.RATE = R.NEW(AA.Interest.Interest.IntFixedRate)<1,TIER.I>   ;*R22 Manual Conversion-CHANGED AA.INTEREST
*  MARGIN.RATES = R.NEW(AA.INT.MARGIN.RATE)<1,TIER.I>
        MARGIN.RATES = R.NEW(AA.Interest.Interest.IntMarginRate)<1,TIER.I>  ;*R22 Manual Conversion-CHANGED AA.INTEREST
*  MARGIN.OPERANDS = R.NEW(AA.INT.MARGIN.OPER)<1,TIER.I>
        MARGIN.OPERANDS = R.NEW(AA.Interest.Interest.IntMarginOper)<1,TIER.I>   ;*R22 Manual Conversion-CHANGED AA.INTEREST
*  MAX.TIER.CAP = R.NEW(AA.INT.TIER.MAX.RATE)<1,TIER.I>
        MAX.TIER.CAP = R.NEW(AA.Interest.Interest.IntTierMaxRate)<1,TIER.I> ;*R22 Manual Conversion-CHANGED AA.INTEREST
*  MIN.TIER.CAP = R.NEW(AA.INT.TIER.MIN.RATE)<1,TIER.I>
        MIN.TIER.CAP = R.NEW(AA.Interest.Interest.IntTierMinRate)<1,TIER.I> ;*R22 Manual Conversion-CHANGED AA.INTEREST
*TUS START
* PRICING.OPERAND = R.NEW(AA.INT.RELATIONSHIP.OPERAND)<1,TIER.I>
         PRICING.OPERAND = R.NEW(AA.Interest.Interest.IntRelationshipOperand)<1,TIER.I>
*PRICING.MARGIN = R.NEW(AA.INT.RELATIONSHIP.MARGIN)<1,TIER.I>
            PRICING.MARGIN = R.NEW(AA.Interest.Interest.IntRelationshipMargin)<1,TIER.I>
*CALL AA.GET.INTEREST.NOMINAL.RATE(CURRENT.RATE, NEGATIVE.RATE, MARGIN.RATES, MARGIN.OPERANDS, MAX.TIER.CAP, MIN.TIER.CAP, PRICING.OPERAND, PRICING.MARGIN, NOMINAL.RATE)
            AA.Interest.GetInterestNominalRate(CURRENT.RATE, NEGATIVE.RATE, MARGIN.RATES, MARGIN.OPERANDS, MAX.TIER.CAP, MIN.TIER.CAP, PRICING.OPERAND, PRICING.MARGIN, NOMINAL.RATE);*MANUAL R22 CODE CONVERSION
*TUS END
* R.NEW(AA.INT.EFFECTIVE.RATE)<1,TIER.I> = NOMINAL.RATE
        R.NEW(AA.Interest.Interest.IntEffectiveRate)<1,TIER.I> = NOMINAL.RATE
        END
