...
*14-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     F.READ to CACHE.READ,VM to@ VM,++ to +=
*14-07-2023    AJITHKUMAR S        R22 MANUAL CONVERSION   CALL RTN FORMAT CHANGED
*---------------------------------------------------------------------------------------	-
...
* End of Fix
    ...
    LRF.FIELD='L.TR.AML.CHECK':@FM:'L.CU.AMLBUY.RT':@FM:'L.RTE.FORM':@VM:'L.NEXT.VERSION'
    LRF.POS=''
...
*-----------------------------------------------------------
    ...
    R.TRANSACTION=''
    TRANS.ERR=''
    CALL CACHE.READ(FN.FTTC, Y.FTTC.ID, R.FTTC, ERR.FTTC) ;*R22 AUTO CODE CONVERSION
    FT.TXN.CODE.ID=R.FTTC<FT6.TXN.CODE.CR>

    CALL CACHE.READ(FN.TRANSACTION, FT.TXN.CODE.ID, R.TRANSACTION, TRANS.ERR)
...
* CALL REDO.V.AUT.RTE.REPRINT(Y.HID)
                APAP.REDOVER.redoVAutRteReprint(Y.HID)
...
*                OFS$DEAL.SLIP.PRINTING = 1 ;* Commented not to produce RTE during Input
                ...
                Y.HID = C$LAST.HOLD.ID
* CALL REDO.V.AUT.RTE.REPRINT(Y.HID)
                APAP.REDOVER.redoVAutRteReprint(Y.HID) ;*R22 MANUAL CODE CONVERSION
                PRT.ADVICED.PRODUCED = ""
            END
        END
    END
