* @ValidationCode : MjotMjAxNTQzMDk2NjpDcDEyNTI6MTcwMzY3OTcxMDExMzp2aWduZXNod2FyaTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 27 Dec 2023 17:51:50
...
*14-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     FM,VMto @VM,@FM,++ to +=
*14-07-2023    AJITHKUMAR S        R22 MANUAL CONVERSION   VARIABLE NAME MODIFIED
* 27-12-2023   VIGNESHWARI S      R22 MANUAL CONVERSTION    CHANGED F.READ TO F.READU
*---------------------------------------------------------------------------------------	-
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
CALL F.READU(FN.REDO.TRANSACTION.CHAIN,Y.RTC.ID,R.RTC,F.REDO.TRANSACTION.CHAIN,RTC.ERR,"") ;*R22 MANUAL CODE CONVERSION- CHANGED F.READ TO F.READU
    IF R.RTC ELSE
...
*        CALL F.READ(FN.REDO.LOAN.FT.TT.TXN,Y.CNCT.ID,R.TXN,F.REDO.TRANSACTION.CHAIN,TXN.ERR)
 CALL F.READU(FN.REDO.LOAN.FT.TT.TXN,Y.CNCT.ID,R.TXN,F.REDO.TRANSACTION.CHAIN,TXN.ERR,"") ;*R22 MANUAL CODE CONVERSION-CHANGED F.READ TO F.READU
        IF R.TXN THEN
...
*R.TXN<LN.FT.TT.REQ.TXN.AMT,Y.VAR3> = Y.CHEQUE.AMT<POS1>
                R.TXN<LN.FT.TT.TRANS.AMOUNT,Y.VAR3> = Y.CHEQUE.AMT<POS1> ;*R22 MANUAL CONVERSION
                Y.CHEQUE.AMT<POS1> = 0
            END ELSE
                Y.CHEQUE.AMT<POS1> = Y.CHEQUE.AMT<POS1> - Y.TOT.TXN.AMT
*R.TXN<LN.FT.TT.REQ.TXN.AMT,Y.VAR3> = Y.TOT.TXN.AMT
                R.TXN<LN.FT.TT.TRANS.AMOUNT,Y.VAR3> = Y.TOT.TXN.AMT ;*R22 MANUAL CONVERSION
                Y.TOT.TXN.AMT = 0
            END
        END
    END ELSE
*R.TXN<LN.FT.TT.REQ.TXN.AMT,Y.VAR3> = 0
        R.TXN<LN.FT.TT.TRANS.AMOUNT,Y.VAR3> = 0 ;*R22 MANUAL CONVERSION
    END
RETURN
*---------------------------------------
PROCESS.AUTH:
*---------------------------------------
    ...
    Y.FLG = ''
    Y.TT.TXN.IDS = R.TXN<LN.FT.TT.FT.TRANSACTION.ID>
  ...
  *      CALL F.READ(FN.REDO.TEMP.WORK,Y.TT.ID,R.REDO.TEMP.WORK,F.REDO.TEMP.WORK,TEMP.ERR)
  CALL F.READU(FN.REDO.TEMP.WORK,Y.TT.ID,R.REDO.TEMP.WORK,F.REDO.TEMP.WORK,TEMP.ERR,"") ;*R22 MANUAL CODE CONVERSION-CHANGED F.READ TO F.READU
