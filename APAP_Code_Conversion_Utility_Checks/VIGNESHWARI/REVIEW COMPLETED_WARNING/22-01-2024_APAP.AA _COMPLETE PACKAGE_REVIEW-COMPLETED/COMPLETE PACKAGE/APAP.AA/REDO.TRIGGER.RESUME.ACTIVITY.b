* @ValidationCode : MjoyMDYzNzEwODkzOkNwMTI1MjoxNzA0NDQxNTM2ODk4OnZpZ25lc2h3YXJpOi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 05 Jan 2024 13:28:56
...
*05-01-2024     VIGNESHWARI                 R22 MANUAL CODE CONVERSION  AA.FRAMEWOR IS MODIFIED
*--------------------------------------------------------------------------------------------------------
...
IF (AA.Framework.getC_aalocactivitystatus() EQ 'AUTH') THEN ;*R22 MANUAL CODE CONVERSION-AA.FRAMEWOR IS MODIFIED
GOSUB INITIALISE
GOSUB CHK.REQD.INFO
...
;*R22 MANUAL CODE CONVERSION-AA.FRAMEWOR IS MODIFIED-START
ARR.ID =  AA.Framework.getC_aalocarrid()
R.ACC.DETAILS = AA.Framework.getC_aalocaccountdetails()
PRODUCT.ID = AA.Framework.getC_aalocarrproductid()
;*R22 MANUAL CODE CONVERSION-AA.FRAMEWOR IS MODIFIED-END
RETURN
*-----------------------------------------------------------------------------------------------------------------------
...
AA.Framework.GetArrangementConditions(ARR.ID, 'OVERDUE','','', RET.IDS, OD.COND, RET.ERR);* R22 AUTO CONVERSION
...
;*R22 MANUAL CODE CONVERSION-AA.FRAMEWOR IS MODIFIED-START
R.APP.RECORD<AA.Framework.ArrangementActivity.ArrActArrangement> = ARR.ID
R.APP.RECORD<AA.Framework.RrControl.ArrActActivity> = 'LENDING-RESUME-ARRANGEMENT'
R.APP.RECORD<AA.Framework.SimulationCapture.ArrActEffectiveDate> = TODAY
R.APP.RECORD<AA.Framework.RrControl.ArrActProduct> = PRODUCT.ID
;*R22 MANUAL CODE CONVERSION-AA.FRAMEWOR IS MODIFIED-END
...
EB.Interface.OfsCallBulkManager(OFS.SOURCE.ID, OFS.MESSAGE, OFS.RESPONSE, TXN.COMMIT)        ;*Process the Activity :);* R22 AUTO CONVERSION
