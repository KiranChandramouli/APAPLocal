* @ValidationCode : MjotMjY1NDUxODgwOkNwMTI1MjoxNzAzMTM0NzM5ODk1OjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIyX1NQNS4wOi0xOi0x
* @ValidationInfo : Timestamp         : 21 Dec 2023 10:28:59
...
*-----------------------------------------------------------------------------
*-----------------------------------------------------------------------------
SUBROUTINE REDO.B.ACI.UPD(Y.ACCT.NO)
****************************************************************
*-------------------------------------------------------------------------
* Company Name  : ASOCIACION POPULAR DE AHORROS Y PRESTAMOS
* Developed By  : SUDHARSANAN S
* Program Name  : REDO.B.ACI.UPD
* ODR Number    : ODR-2009-10-0317
*-------------------------------------------------------------------------

* Description : This batch routine is used update ACCOUNT.CREDIT.INT when the field
...
* WHO                DATE              REFERENCE      DESCRIPTION
* SUDHARSANAN S     14-09-2011      PACS00101166      Restrict to generate the ACI record when the first cob after creating account
* 04-APR-2023      Harishvikram C   R22 Auto conversion       No changes
* 04-APR-2023      Harishvikram C   Manual R22 conversion     CNT1 to BAS2.CNT, CNT to BAS2.CNT, VM to @VM
...
*   CALL F.READ(FN.ACCOUNT,Y.ACCT.NO,R.ACCOUNT,F.ACCOUNT,ACC.ERR)
    CALL F.READU(FN.ACCOUNT,Y.ACCT.NO,R.ACCOUNT,F.ACCOUNT,ACC.ERR,'') ;*R22 Manual Conversion
    VAR.OPENING.DATE = R.ACCOUNT<AC.OPENING.DATE>
    Y.STATUS=R.ACCOUNT<AC.LOCAL.REF,POS.L.AC.STATUS1>
*IF (Y.STATUS EQ 'ACTIVE' OR Y.STATUS EQ '') OR (VAR.OPENING.DATE EQ VAR.LAST.WORK.DAY) THEN
    IF (Y.STATUS EQ '') OR (VAR.OPENING.DATE EQ VAR.LAST.WORK.DAY) THEN
        RETURN
    END ELSE
        GOSUB PROCESS
    END
...
*---------------------------------------------------------------------------------
    ...
    BEGIN CASE
        CASE R.REDO.ACC.CR.INT<ACI.CR.BASIC.RATE> NE ''
            CNT = DCOUNT(R.REDO.ACC.CR.INT<ACI.CR.BASIC.RATE>,@VM) ;*Manual R22 conversion
        CASE R.REDO.ACC.CR.INT<ACI.CR.INT.RATE> NE ''
            CNT = DCOUNT(R.REDO.ACC.CR.INT<ACI.CR.INT.RATE>,@VM) ;*Manual R22 conversion
    END CASE
    ...
    NEXT BAS.CNT ;*Manual R22 conversion
    ...
    BEGIN CASE
        CASE R.REDO.ACC.CR.INT<ACI.CR2.BASIC.RATE> NE ''
            CNT1 = DCOUNT(R.REDO.ACC.CR.INT<ACI.CR2.BASIC.RATE>,@VM) ;*Manual R22 conversion
        CASE R.REDO.ACC.CR.INT<ACI.CR2.INT.RATE> NE ''
            CNT1 = DCOUNT(R.REDO.ACC.CR.INT<ACI.CR2.INT.RATE>,@VM) ;*Manual R22 conversion
    END CASE
    ...
    IF CNT1 NE '' THEN
        ...
        NEXT BAS2.CNT ;*Manual R22 conversion
    END
