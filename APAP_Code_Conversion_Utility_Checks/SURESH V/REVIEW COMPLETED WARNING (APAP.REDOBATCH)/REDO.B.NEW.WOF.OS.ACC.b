* @ValidationCode : MjoxOTU5Nzc1ODU3OkNwMTI1MjoxNzA0MTkxNDc1NzAxOjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 02 Jan 2024 16:01:15
...
*18/12/2023        Suresh                R22 Manual Conversion  -CALL routine modified, Change F.READ to F.READU
*-----------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
*  $INSERT I_F.COMPANY ;*R22 Manual Conversion
    $INSERT I_F.ACCOUNT
...
*    $INSERT I_F.AA.ARRANGEMENT  ;*R22 Manual Conversion
    ...
    $USING APAP.TAM
    $USING EB.Interface ;*R22 Manual Conversion - Start
    $USING AC.API
    $USING AA.Framework
    $USING ST.CompanyCreation ;*R22 Manual Conversion - End
*-----------------------------------------------------------------
MAIN:
*-----------------------------------------------------------------
...
*CALL F.READ(FN.REDO.ACCT.MRKWOF.HIST,Y.ID,R.REDO.ACCT.MRKWOF.HIST,F.REDO.ACCT.MRKWOF.HIST,HIST.ERR)
    CALL F.READU(FN.REDO.ACCT.MRKWOF.HIST,Y.ID,R.REDO.ACCT.MRKWOF.HIST,F.REDO.ACCT.MRKWOF.HIST,HIST.ERR,'');* R22 AUTO CONVERSION
*CALL F.READ(FN.AA.ARRANGEMENT,Y.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,AA.AR.ERR)
    R.AA.ARRANGEMENT =AA.Framework.Arrangement.Read(Y.ID,AA.AR.ERR)

*Y.LOAN.COMP = R.AA.ARRANGEMENT<AA.ARR.CO.CODE>
    Y.LOAN.COMP = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCoCode> ;*R22 Manual Conversion

*    Y.LOAN.CUR = R.AA.ARRANGEMENT<AA.ARR.CURRENCY>
    Y.LOAN.CUR = R.AA.ARRANGEMENT<AA.Framework.Arrangement.ArrCurrency> ;*R22 Manual Conversion
    
*    Y.LOAN.AC = R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
    Y.LOAN.AC = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId> ;*R22 Manual Conversion
...
*CALL F.READ(FN.REDO.WOF.CONCAT.FILE,Y.ID,R.REDO.WOF.CONCAT.FILE,F.REDO.WOF.CONCAT.FILE,CNCT.ERR)
    CALL F.READU(FN.REDO.WOF.CONCAT.FILE,Y.ID,R.REDO.WOF.CONCAT.FILE,F.REDO.WOF.CONCAT.FILE,CNCT.ERR,'');* R22 AUTO CONVERSION
...
*CALL F.READ(FN.REDO.CONCAT.ACC.WOF,Y.LOAN.AC,R.REDO.CONCAT.ACC.WOF,F.REDO.CONCAT.ACC.WOF,CON.ERR)
    CALL F.READU(FN.REDO.CONCAT.ACC.WOF,Y.LOAN.AC,R.REDO.CONCAT.ACC.WOF,F.REDO.CONCAT.ACC.WOF,CON.ERR,'');* R22 AUTO CONVERSION
...
*  CALL OFS.CALL.BULK.MANAGER(CALL.INFO,OFS.RECORD,THE.RESPONSE,TXN.CMM)
    EB.Interface.OfsCallBulkManager(CALL.INFO,OFS.RECORD,THE.RESPONSE,TXN.CMM) ;*R22 Manual Conversion
...
*    CALL CACHE.READ(FN.COMPANY,Y.LOAN.COMP,R.COMP.REC,COM.ERR)
    R.COMP.REC=ST.CompanyCreation.Company.CacheRead(Y.LOAN.COMP, COM.ERR) ;*R22 Manual Conversion
*    Y.COMP.DIV.CODE = R.COMP.REC<EB.COM.SUB.DIVISION.CODE>
    Y.COMP.DIV.CODE = R.COMP.REC<ST.CompanyCreation.Company.EbComSubDivisionCode> ;*R22 Manual Conversion
...
*    R.CATEG.ENT<AC.CAT.CUSTOMER.ID>      = R.AA.ARRANGEMENT<AA.ARR.CUSTOMER>
    R.CATEG.ENT<AC.CAT.CUSTOMER.ID>      = R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrCustomer>;*R22 Manual Conversion
...
*CALL LOAD.COMPANY(Y.LOAN.COMP)
        ST.CompanyCreation.LoadCompany(Y.LOAN.COMP);* R22 AUTO CONVERSION
    END
    ACC.PRODUCT = 'AC'
    ACC.TYPE = 'SAO'
*CALL EB.ACCOUNTING(ACC.PRODUCT,ACC.TYPE,R.STMT.ENTRY,'')
    AC.API.EbAccounting(ACC.PRODUCT,ACC.TYPE,R.STMT.ENTRY,'');* R22 AUTO CONVERSION
...
*CALL AA.GET.PERIOD.BALANCES(Y.LOAN.AC, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "")
        AA.Framework.GetPeriodBalances(Y.LOAN.AC, BALANCE.TO.CHECK, DATE.OPTIONS, EFFECTIVE.DATE, "", "", BAL.DETAILS, "");* R22 AUTO CONVERSION
