* @ValidationCode : MjotMTk5MzY4OTU3NDpDcDEyNTI6MTcwMzIzMzUxNjE4NjozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Dec 2023 13:55:16
...
*13-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     TNO TO C$T24.SESSION.NO,FM TO @FM,VM TO @VM,++ TO +=1,CONVERT TO CHANGE
...
*------------------------------------------------------------------------
    ...
    $USING ST.CompanyCreation ;*R22 Manual Conversion
    Y.TODAY =  TODAY
    R.OUT.CLEAR.FILE = ''
    R.REDO.BATCH.FILE = ''
...
*    CALL F.READ(FN.OUT.CLEAR.FILE,Y.TODAY,R.OUT.CLEAR.FILE,F.OUT.CLEAR.FILE,OUT.ERR)
    CALL F.READU(FN.OUT.CLEAR.FILE,Y.TODAY,R.OUT.CLEAR.FILE,F.OUT.CLEAR.FILE,OUT.ERR,"") ;*R22 Manual Conversion
...
*    CALL F.READ(FN.LOCKING,Y.LOCKING.ID,R.LOCKING,F.LOCKING.VAR,LOCKING.ERR)
    CALL F.READU(FN.LOCKING,Y.LOCKING.ID,R.LOCKING,F.LOCKING.VAR,LOCKING.ERR,"") ;*R22 Manual Conversion
    IF R.LOCKING THEN
...
*R22 MANUAL CONVERSION
        APAP.REDOAPAP.redoApapAchMirrorAcctEnt(Y.UNIV.CLEAR.ACC,AMOUNT.VAL,VAR.CURRENCY, Y.ENT.REF.ID)
    END ELSE
        Y.ID = DRAWER.ACCOUNT.VAL:'-':VAL.CHEQUE.NO
        TOTAL.CHQ = DEBIT.VAL
        REDO.CLEARING.OUTWARD.ID = Y.ID
...
*R22 MANUAL CONVERSION
        APAP.TAM.redoGetAccCoCode(PRIMARY.ACCOUNT,Y.RET.CO.CODE)
...
*    CALL F.READ(FN.REDO.INTRANSIT.CHQ,ACCOUNT.VAL,R.REDO.INTRANSIT.CHQ,F.REDO.INTRANSIT.CHQ,CHQ.ERR)
    CALL F.READU(FN.REDO.INTRANSIT.CHQ,ACCOUNT.VAL,R.REDO.INTRANSIT.CHQ,F.REDO.INTRANSIT.CHQ,CHQ.ERR,"") ;*R22 Manual Conversion
...
*-----------------------------------------------------------------------------------
    ...
    IF DEPOSIT.FLAG EQ 1 THEN ;*R22 AUTO CONVERSION
    ...
    CHANGE ':' TO '' IN TEMPTIME ;*R22 AUTO CONVERSION
    CHECK.DATE = DATE()
    ...
    R.REDO.CLEARING.OUTWARD<CLEAR.OUT.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR ;*R22 AUTO CONVERSION
    R.REDO.CLEARING.OUTWARD<CLEAR.OUT.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR ;*R22 AUTO CONVERSION
...
*R22 MANUAL CONVERSION
        APAP.REDOAPAP.redoApapAchMirrorAcctEnt(Y.UNIV.CLEAR.ACC,AMOUNT.VAL,VAR.CURRENCY,TFS.REFERENCE.ID)
    END
...
*        CALL LOAD.COMPANY(CO.CODE)
        ST.CompanyCreation.LoadCompany(CO.CODE) ;*R22 Manual Conversion
...
*-----------------------------------------------------------------------------
    ...
    CHANGE @VM TO @FM IN BANK.CATEGORY ;*R22 AUTO CONVERSION
...
*-----------------------------------------------------------------------------
    ...
    FETCH.CUST.TYPE = ''
    CHANGE @VM TO @FM IN LIST.CURRENCY ;*R22 AUTO CONVERSION
    ...
    LOCATE VAR.CURRENCY IN LIST.CURRENCY SETTING CURR.POS THEN
        ...
        CHANGE @SM TO @FM IN FETCH.CUST.TYPE ;*R22 AUTO CONVERSION
        CHANGE @SM TO @FM IN FETCCH.TRANSIT.DAYS ;*R22 AUTO CONVERSION
...
*    CALL EB.ACCOUNTING("AC","SAO",Y.CLEARING.ARR,'')
    AC.API.EbAccounting("AC","SAO",Y.CLEARING.ARR,'') ;*R22 Manual Conversion
RETURN
*------------------------------------------------------------------------------------------
GET.STMT.DETAILS:
*****************
...
* This has been added since we had problem in Processing OFS. since we posted diff ofs message for same ALE
    Y.LOOP.CNT = 1
    Y.TOTAL.CNT = DCOUNT(Y.FINAL.ALE.ID,@FM) ;*R22 AUTO CONVERSION
    LOOP
