* @ValidationCode : MjoxMTYwMDcyNDc5OkNwMTI1MjoxNzAzMjM4MzIxMjkxOjMzM3N1Oi0xOi0xOjA6MDpmYWxzZTpOL0E6UjIxX0FNUi4wOi0xOi0x
* @ValidationInfo : Timestamp         : 22 Dec 2023 15:15:21
...
*13-07-2023    CONVERSION TOOL     R22 AUTO CONVERSION     FM TO @FM,CONVERT TO CHANGE,++ TO +=1,TNO TO C$T24.SESSION.NO
*13-07-2023    VICTORIA S          R22 MANUAL CONVERSION   CALL ROUTINE MODIFIED
*21/12/2023         Suresh         R22 Manual Conversion   IDVAR Variable Changed, Change F.READ to F.READU
*------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_F.ACCOUNT
    ...
    $USING APAP.TAM
    $USING APAP.REDOAPAP
    IDVAR='OUT.RETURN' ;*R22 Manual Conversion
*    CALL F.READ(FN.REDO.MAPPING.TABLE,'OUT.RETURN',R.REDO.MAPPING.TABLE,F.REDO.MAPPING.TABLE,MAP.ERR)
    CALL F.READ(FN.REDO.MAPPING.TABLE,IDVAR,R.REDO.MAPPING.TABLE,F.REDO.MAPPING.TABLE,MAP.ERR) ;*R22 Manual Conversion
...
*  CALL CACHE.READ(FN.REDO.APAP.CLEAR.PARAM,'SYSTEM',R.REDO.APAP.CLEAR.PARAM,PARAM.ERR)
    ...
    TEMP.COUNT = DCOUNT(R.APERTA,@FM) ;*R22 AUTO CONVERSION
    LOOP
...
*    DRAW.ACCT = R.TFS<TFS.PRIMARY.ACCOUNT>
    ...
    ACCOUNT.VAL += 1 ;*R22 AUTO CONVERSION
    ACCOUNT.VAL -= 1 ;*R22 AUTO CONVERSION
...
*R22 MANUAL CONVERSION
    APAP.TAM.redoGetRejectReason(CREDIT.VAL,Y.REDO.REJECT.REASON)
...
*    CALL F.READ(FN.REDO.OUTWARD.RETURN,REDO.OUTWARD.RETURN.ID,R.REDO.OUTWARD.RETURN,F.REDO.OUTWARD.RETURN,REDO.CLEAR.ERR)
    CALL F.READU(FN.REDO.OUTWARD.RETURN,REDO.OUTWARD.RETURN.ID,R.REDO.OUTWARD.RETURN,F.REDO.OUTWARD.RETURN,REDO.CLEAR.ERR,"") ;*R22 Manual Conversion
    VAR.TODAY = TODAY
...
*IF ROUTE.VAL EQ '999907000' THEN
    ...
    R.REDO.OUTWARD.RETURN<CLEAR.RETURN.AUTHORISER> = C$T24.SESSION.NO:'_':OPERATOR ;*R22 AUTO CONVERSION
    R.REDO.OUTWARD.RETURN<CLEAR.RETURN.INPUTTER> = C$T24.SESSION.NO:'_':OPERATOR ;*R22 AUTO CONVERSION
    R.REDO.OUTWARD.RETURN<CLEAR.RETURN.CURR.NO> = 1
    Y.RET.CO.CODE = ''
*CALL REDO.GET.ACC.CO.CODE(ACCOUNT.VAL,Y.RET.CO.CODE)
*R22 MANUAL CONVERSION
    APAP.TAM.redoGetAccCoCode(ACCOUNT.VAL,Y.RET.CO.CODE)
...
*R22 MANUAL CONVERSION
        APAP.TAM.redoUpdateOutwardReturnChq(ACCOUNT.VAL,REDO.OUTWARD.RETURN.ID)
        
*CALL REDO.APAP.ACH.MIRROR.RET.ACCT.ENT(AMOUNT.VAL,VAR.CURRENCY,REDO.OUTWARD.RETURN.ID)
        
        APAP.REDOAPAP.redoApapAchMirrorRetAcctEnt(AMOUNT.VAL,VAR.CURRENCY,REDO.OUTWARD.RETURN.ID) ;*R22 MANUAL CONVERSION
    END
RETURN
END
