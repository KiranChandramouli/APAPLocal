* @ValidationCode : MjotMTEzNjcyOTkzMjpDcDEyNTI6MTcwNDcxNTIwMDY5MjozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMl9TUDUuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 08 Jan 2024 17:30:00
...
*    CALL F.READ(FN.PARAM,Y.POLIZA,R.PARAM,F.PARAM,Y.ERR)
    CALL CACHE.READ(FN.PARAM,Y.POLIZA,R.PARAM,Y.ERR) ;*R22 Manual Conversion
...
*        CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRAN,F.AA.ARRANGEMENT,AA.ARR.ER)
        R.AA.ARRAN=AA.Framework.Arrangement.Read(Y.AA.ID, AA.ARR.ER) ;*R22 Manual Conversion
        IF R.AA.ARRAN THEN
*            IF R.AA.ARRAN<AA.ARR.ARR.STATUS> EQ 'CURRENT' THEN
            IF R.AA.ARRAN<AA.Framework.ArrangementSim.ArrArrStatus> EQ 'CURRENT' THEN ;*R22 Manual Conversion
...
*        CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRAN,F.AA.ARRANGEMENT,AA.ARR.ER)
        ...
        IF R.AA.ARRAN THEN
*            IF R.AA.ARRAN<AA.ARR.ARR.STATUS> EQ 'CURRENT' THEN
            IF R.AA.ARRAN<AA.Framework.ArrangementSim.ArrArrStatus> EQ 'CURRENT' THEN
                CALL F.READ(FN.REDO.ACCT.MRKWOF.HIST,Y.AA.ID,R.REDO.ACCT.MRKWOF.HIST,F.REDO.ACCT.MRKWOF.HIST,WOF.ERR)
                IF R.REDO.ACCT.MRKWOF.HIST EQ '' THEN
                    Y.CHR.TYPE = R.INSURANCE<INS.DET.CHARGE>
                    Y.CUSTOMER = R.INSURANCE<INS.DET.CUSTOMER>
...
*    CALL F.READ(FN.AA.ARRANGEMENT,Y.AA.ID,R.AA.ARRANGEMENT,F.AA.ARRANGEMENT,YERR)
    R.AA.ARRANGEMENT=AA.Framework.Arrangement.Read(Y.AA.ID, YERR) ;*R22 Manual Conversion
*    Y.ACCOUNT.ID  =  R.AA.ARRANGEMENT<AA.ARR.LINKED.APPL.ID>
    Y.ACCOUNT.ID  =  R.AA.ARRANGEMENT<AA.Framework.ArrangementSim.ArrLinkedApplId> ;*R22 Manual Conversion
...
*        CALL AA.GET.PERIOD.BALANCES(Y.ACCOUNT.ID, Y.BALANCE.TO.CHECK, Y.DATE.OPTIONS, Y.PROCESS.DATE, Y.PROCESS.DATE, '', Y.BAL.DETAILS, "")
        AA.Framework.GetPeriodBalances(Y.ACCOUNT.ID, Y.BALANCE.TO.CHECK, Y.DATE.OPTIONS, Y.PROCESS.DATE, Y.PROCESS.DATE, '', Y.BAL.DETAILS, "");* R22 UTILITY AUTO CONVERSION
