* @ValidationCode : MjotMTY1MTkzMzU2MDpDcDEyNTI6MTcwMzIyNDE2MTkwNjozMzNzdTotMTotMTowOjA6ZmFsc2U6Ti9BOlIyMV9BTVIuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 22 Dec 2023 11:19:21
...
*    CALL F.READ(FN.REDO.TRANSIT.LOCK,ACCT.ID,R.REDO.TRANSIT.LOCK,F.REDO.TRANSIT.LOCK,ERR)
    CALL F.READU(FN.REDO.TRANSIT.LOCK,ACCT.ID,R.REDO.TRANSIT.LOCK,F.REDO.TRANSIT.LOCK,ERR,"") ;*R22 Manual Conversion
    IF R.REDO.TRANSIT.LOCK THEN
        GOSUB REMOVE.TRANSIT.LOCK

        TOTAL.CNTR = DCOUNT(R.REDO.TRANSIT.LOCK,@FM) ;*R22 AUTO CONVERSION
...
*PACS00149089  ------START------------------------------------------------
    ...
    Y.CUSTOMER.TOT=DCOUNT(Y.CUSTOMER.LIST,@VM) ;*R22 AUTO CONVERSION
    Y.CUSTOMER.CNT=1
    LOOP
    WHILE Y.CUSTOMER.CNT LE Y.CUSTOMER.TOT
        ...
        END
        Y.CUSTOMER.CNT += 1 ;*R22 AUTO CONVERSION
    REPEAT

    CHANGE @VM TO @FM IN Y.RGO.ID.LIST ;*R22 AUTO CONVERSION
...
*    CALL F.READ(FN.REDO.INTRANSIT.CHQ,ACCT.ID,R.REDO.INTRANSIT.CHQ,F.REDO.INTRANSIT.CHQ,CHQ.TR.ERR)
    CALL F.READU(FN.REDO.INTRANSIT.CHQ,ACCT.ID,R.REDO.INTRANSIT.CHQ,F.REDO.INTRANSIT.CHQ,CHQ.TR.ERR,"") ;*R22 Manual Conversion
    IF R.REDO.INTRANSIT.CHQ THEN
        GET.CLEAR.IDS = R.REDO.INTRANSIT.CHQ<TRAN.CHQ.CLEAR.OUT.ID>
    END

    CHANGE @VM TO @FM IN GET.CLEAR.IDS ;*R22 AUTO CONVERSION
...
REMOVE.TRANSIT.LOCK:
    ...
    LOOP.CNTR = 1
    LOOP.TOTAL.CNTR = DCOUNT(R.REDO.TRANSIT.LOCK,@FM) ;*R22 AUTO CONVERSION
    ...
    WHILE LOOP.CNTR LE LOOP.TOTAL.CNTR
        ...
        LOOP.CNTR += 1 ;*R22 AUTO CONVERSION
...
*       CALL F.READ(FN.AZ.ACCOUNT,VAR.AZ.ACCOUNT,R.AZ.ACCOUNT,F.AZ.ACCOUNT,AZ.ERR)
        CALL F.READU(FN.AZ.ACCOUNT,VAR.AZ.ACCOUNT,R.AZ.ACCOUNT,F.AZ.ACCOUNT,AZ.ERR,"") ;*R22 Manual Conversion
